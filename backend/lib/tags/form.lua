...
-- require: name;
-- link: ![admin]%(rdir-domain)/form.action?agnCTOKEN=%(company-token)&agnFN=%(name)&agnUID=##AGNUID## Formular %(name);
...
function form (ctx, parm, cust)
	...
	if ctx._result == nil or ctx._result[parm] == nil then
		...
		map['param.name'] = parm.name
		ctx._result[parm] = agn.strmap ('%(rdir_domain)/form.action?agnCTOKEN=%(company_token)&agnFN=%(param.name)&agnUID=##AGNUID##', map)
	end
	return ctx._result[parm]
end
