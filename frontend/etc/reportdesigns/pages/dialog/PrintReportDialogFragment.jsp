    ...
    <TR>
        <TD>
            <DIV ID="printFormatSetting">
                <DIV><label for="printAsHTML"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.print.format", locale )%></label></DIV>
                <br/>
                <DIV>
                    <INPUT TYPE="radio" ID="printAsHTML" name="printFormat" CHECKED />
                    <label for="printAsHTML"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.print.format.html", locale )%></label>
                </DIV>
                <DIV>
                    <INPUT TYPE="radio" ID="printAsPDF" name="printFormat" />
                    <label for="printAsPDF"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.print.format.pdf", locale )%></label>
                    &nbsp;&nbsp;
                    <SELECT	ID="printFitSetting" CLASS="birtviewer_printreport_dialog_select" DISABLED="true">
                        <option value="0" selected><%=EmmBirtResources.getMessage( "birt.viewer.dialog.export.pdf.fittoauto", locale )%></option>
                        <option value="1"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.export.pdf.fittoactual", locale )%></option>
                        <option value="2"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.export.pdf.fittowhole", locale )%></option>
                    </SELECT>
                </DIV>
            </DIV>
        </TD>
                ...
                <TABLE>
                    ...
                    <TR>
                        <TD>
                            ...
                            <label for="printPageAll"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.page.all", locale )%></label>
                        </TD>
                        <TD STYLE="padding-left:5px">
                            <INPUT TYPE="radio" ID="printPageCurrent" NAME="printPages"/>
                            <label for="printPageCurrent"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.page.current", locale )%></label>
                        </TD>
                        <TD STYLE="padding-left:5px">
                            <INPUT TYPE="radio" ID="printPageRange" NAME="printPages"/>
                            <label for="printPageRange"><%=EmmBirtResources.getMessage( "birt.viewer.dialog.page.range", locale )%></label>
