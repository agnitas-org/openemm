...
echo "DROP DATABASE IF EXISTS ${dbname};" | MYSQL_PWD=${password} mariadb -h ${hostname} -P 3306 -u ${username} --default-character-set=utf8

echo "Creating database ${dbname}"
echo "CREATE DATABASE ${dbname} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" | MYSQL_PWD=${password} mariadb -h ${hostname} -P 3306 -u ${username} --default-character-set=utf8
...
for sqlfilename in ${sqlScriptsToDo}; do
	...
	MYSQL_PWD=${password} mariadb -h ${hostname} -P 3306 -u ${username} --database=${dbname} --default-character-set=utf8 < ${sqlfilename}
...
if [ -e "${scriptDir}/emm-mysql-test-aso.sql" ]; then {
	sqlScriptsToDo+=" ${scriptDir}/emm-mysql-test-aso.sql"
} fi
