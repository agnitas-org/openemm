...
public class BC {
	 ...
	 */
	private List<Reference> sortedReferences = null;
	 ...
	 */
	public boolean prepareQueryParts() throws CancelException {
		boolean rc = false;
		...
		if (!data.targetExpression.resolveByDatabase().isEmpty()) {
			partFrom +=
	...
	public String createSelect (boolean fullExpression) {
		if (fullExpression && !data.targetExpression.resolveByDatabase().isEmpty()) {
			return
				"WITH\n    " + data.targetExpression.resolveByDatabase ().stream ()
	...
	private void getExtensions(List<String> collect) {
		if (Str.atob(data.company.info("use-extended-usertypes"), true)) {
			...
			if (data.maildropStatus.isWorldMailing()) {
				...
				if ((! data.maildropStatus.selectedTestRecipients ()) || data.enforceTestVIP ()) {
					userType = "t";
				}
			}
			if (userType != null) {
	...
	private boolean collectRecipientsToTemporaryTable (String partFromPrepare) throws CancelException {
		...
		if (data.bounceBleed ()) {
			...
			try (DBase.With with = data.dbase.with ()) {
				...
				if (query != null) {
					...
					for (Map <String, Object> row : data.dbase.query (with.cursor (),
											  query,
		...
		if (data.company.limitMailsPerDay() && (data.maildropStatus.isWorldMailing() || data.maildropStatus.isRuleMailing() || data.maildropStatus.isOnDemandMailing())) {
			...
			try {
				...
				for (State state : State.values ()) {
					...
					if (limit > 0) {
						upd = "DELETE FROM " + table + " WHERE customer_id IN (" + switch (state) {
							case Local -> "SELECT customer_id FROM " + limitTable + " " +
								"WHERE company_id = " + companyID +
								" AND senddate = " + timestamp +
								" AND status_field = '" + data.maildropStatus.statusField () + "'" +
								" AND mailcount > " + limit;
							case Global -> "SELECT customer_id FROM " + limitTable +" r1 " +
								"WHERE company_id = " + companyID +
								" AND senddate = " + timestamp +
								" AND (" +
	...
	private boolean removeReceiversReachedDailyLimit(String ctable) {
		...
		if ((data.maildropStatus.limitReceivers() > 0) && (data.maildropStatus.isWorldMailing() || data.maildropStatus.isRuleMailing() || data.maildropStatus.isOnDemandMailing())) {
			long cnt;
	...
	private boolean removePrioritizedReceiver() {
		...
		if (data.isPriorityMailing) {
			...
			try {
				...
				data.dbase.flush();
				long remain = countTable(table, "distinct customer_id", "user_type = 'W'");
	...
	private boolean fillMissingVoucherCodes() throws CancelException {
		boolean ok = true;
	...
	private boolean setMissingVoucherCode(long customerID) throws SQLException, CancelException {
	...
	private void processReactivation (String partFromPrepare) {
		if (partReactivate != null && !partReactivate.isEmpty() && hasReactivationCandidates()) {
			String	query =
	...
	private void vouchersMandatory (Reference r) throws CancelException {
		boolean	isMandatory = data.vouchersMandatory ();
		
		data.logging (isMandatory ? Log.WARNING : Log.INFO, "bc", r.name () + ": insufficient voucher codes, mailing generation " + (isMandatory ? "aborted" : "continues"));
		if (isMandatory && (! data.maildropStatus.isPreviewMailing ()) && (! data.maildropStatus.isVerificationMailing ())) {
			if (data.maildropStatus.isTestMailing () || data.maildropStatus.isRuleMailing () || data.maildropStatus.isOnDemandMailing () || data.maildropStatus.isWorldMailing ()) {
				Map <String, String>	options = new HashMap <> ();
				
				options.put ("voucher", r.voucherName ());
				data.trigger ("mandatory-voucher", options);
			}
			throw new CancelException (r.name () + ": insufficient voucher codes", MailingStatus.INSUFFICIENT_VOUCHERS);
		}
	}
}
