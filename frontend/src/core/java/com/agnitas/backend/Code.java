...
import com.agnitas.backend.exceptions.EMMTagCodeException;
...
public class Code {
	 ...
	 */
	public void requestFields(Set<String> predef, Map<String, String> parameter) throws EMMTagCodeException {
	...
	private void resolveDatabaseColumns(Set<String> predef, Map<String, String> parameter) throws EMMTagCodeException {
						...
						if ((c.length() > 2) && c.startsWith("/") && c.endsWith("/")) {
							...
							if (o != -1) {
								...
								if (!ref.isEmpty()) {
									...
									if (rmatch == null) {
										throw new EMMTagCodeException("Unknown reference table in \"" + c + "\" requested.");
									}
								} else {
									ref = null;
								}
							...
							try {
								...
								throw new EMMTagCodeException("Invalid pattern for requested columns \"" + c + "\"", e);
							}
							...
							if ((!optional) && (count == 0)) {
								throw new EMMTagCodeException("Referenced column wildcard \"" + c + "\" in \"" + column + "\" not found in database");
							}
						} else if (!c.isEmpty()) {
							if (data.columnByName(c) == null) {
								throw new EMMTagCodeException("Referenced column \"" + c + "\" in \"" + column + "\" not found in database");
							}
	...
	private void resolveVouchers(Set<String> predef, Map<String, String> parameter) throws EMMTagCodeException {
				...
				if (vouchers != null) {
					for (String v : vouchers) {
						...
						if (!v.isEmpty()) {
							...
							if (!mt.matches()) {
								throw new EMMTagCodeException("Voucher \"" + v + "\" is invalid");
							}
							String column = "vc_" + v + ".voucher_code";
							if (data.columnByName(column) == null) {
								throw new EMMTagCodeException("Voucher \"" + v + "\" not found");
							}
	...
	private void resolveLinks(Map<String, String> parameter) throws EMMTagCodeException {
		if (valid && (requestedLinks != null)) {
			...
			if (requestedLinksCharset != null) {
				...
				if (requestedLinksCharset.startsWith(">")) {
					...
					if (requestedLinksCharset == null) {
						throw new EMMTagCodeException("Missing parameter link character set: " + parm);
					}
			...
			for (String requestedLink : requestedLinks) {
				...
				for (int state = 0; state < 2; ++state) {
					...
					switch (state) {
						...
						case 1:
							...
							break;
						default:
							throw new EMMTagCodeException("Invalid state");
					}
				...
				if (useLink) {
					...
					if (link.startsWith(">")) {
						...
						if (link == null) {
							throw new EMMTagCodeException("Missing parameter \"" + parm + "\" (expected URL)");
						}
						link = data.substituteString(link, urlparameter, missing);
						showMissing("indirect link", link);
					}
					if (data.requestURL(link, data.substituteString(comment, parameter), isAdminLink) == null) {
						throw new EMMTagCodeException("Required link \"" + link + "\" cannot be used");
					}
				}
			}
		}
	...
	private void validateRequiredParameter(Map<String, String> parameter) throws EMMTagCodeException {
		if (requiredParameter != null) {
			for (String require : requiredParameter) {
				if (!parameter.containsKey(require)) {
					throw new EMMTagCodeException("Missing required parameter \"" + require + "\"");
				}
			}
		}
	}
	...
	private void resolveReferenceParameter(Map<String, String> parameter) throws EMMTagCodeException {
		if (requestedReferences != null) {
			for (String ref : requestedReferences) {
				...
				if (!data.addReference(sref, false)) {
					throw new EMMTagCodeException("Invalid reference requested: " + sref + " (coming from " + ref + ")");
				}
			}
		}
	}
	...
	private String[] resolveParameter(String element, Map<String, String> parameter, String ident, boolean optional) throws EMMTagCodeException {
		String[] elements;
		...
		if (element.startsWith(">")) {
			...
			if (element == null) {
				if (!optional) {
					throw new EMMTagCodeException(Str.fill("Missing parameter \"${param}\" (expected ${ident})", "param", param, "ident", ident));
				}
				elements = null;
			} else {
	...
	private void showMissing(String... what) throws EMMTagCodeException {
		if (!missing.isEmpty()) {
			throw new EMMTagCodeException("Missing parameter" + (missing.size() > 1 ? "s " : " ") + missing.stream().map(e -> "\"" + e + "\"").reduce((s, e) -> s + ", " + e).orElse("") + " in " + Arrays.stream(what).reduce((s, e) -> s + " " + e).orElse("unspecified"));
		}
	}
}
