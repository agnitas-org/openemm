...
import com.agnitas.backend.dao.CompanyDAO;
import com.agnitas.backend.exceptions.InactiveCompanyException;
import com.agnitas.backend.exceptions.MissingCompanyException;
...
public class Company {
	 ...
	 */
	public void retrieveInformation() throws SQLException, MissingCompanyException, InactiveCompanyException {
		CompanyDAO company = new CompanyDAO(data.dbase, id);

		if (company.companyID() == 0L) {
			throw new MissingCompanyException(id, "no database entry for companyID found");
		}
		String status = company.status();
		if ((status == null) || (!status.equals("active"))) {
			throw new InactiveCompanyException(id, status, "abort creating mailing for inactive company");
		}
