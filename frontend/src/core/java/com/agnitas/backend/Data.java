...
import com.agnitas.backend.exceptions.BackendRuntimeException;
...
public class Data implements Closeable {
	 ...
	 */
	private static final int LICENCE_UNSPEC = 0;
	/**
	 ...
	 */
	public static final int OPL_TOP = 1;
	/**
	 * Constant for onepixellog: insertion at bottom
	 */
	public static final int OPL_BOTTOM = 2;
	/**
	...
	public static Locale dateLocale = new Locale.Builder ().setLanguage ("en").build ();
	/**
	...
	public void close () {
		try {
			...
			logging (Log.ERROR, "close", "failed to cleanup: " + e.toString ());
		}
	}
	...
	public void setup (String generation_spec, Map <String, Object> opts) throws Exception {
		...
		try {
			...
			logging(Log.ERROR, "init", "Failed setup: " + e.getMessage ());
			throw e;
		}
	}
	 ...
	 */
	protected final void setupLogging(String program) {
	...
	protected void configure() throws Exception {
		...
		if (((sampleEmails = cfg.cget("sample_emails", sampleEmails)) != null) && (sampleEmails.isEmpty() || sampleEmails.equals("-"))) {
			sampleEmails = null;
		}
		if (((deliveryCheckEmails = cfg.cget ("delivery_check_emails", deliveryCheckEmails)) != null) && (deliveryCheckEmails.isEmpty() || deliveryCheckEmails.equals ("-"))) {
	...
	private void getMailingInformations(String generation_spec) throws Exception {
		try {
			...
			logging(Log.ERROR, "init", "Error in quering initial data: " + e.toString(), e, true);
				...
				if (mediaConfig != null) {
					...
					for (Map.Entry <String, String> es : mediaConfig.entrySet ()) {
						...
						if ((keyParts.length == 2) &&
							...
							String	value = company.info (key);
							
							if (value == null) {
								value = es.getValue ();
							}
							tmp.setParameter (keyParts[1], value);
						}
					}
				}
			}
	...
	public void checkMailingData() throws Exception {
		...
		if (onePixelURL == null || onePixelURL.isEmpty()) {
	...
	public Locale getLocale(String language, String country) {
		...
		if (language != null) {
			String[]	temp = language.split ("_", 2);
			
			if (temp.length == 2) {
				language = temp[0];
				fallbackCountry = temp[1];
			}
		}
		if (country != null) {
			String[]	temp = country.split ("_", 2);
			
			if (temp.length == 2) {
				country = temp[1];
				fallbackLanguage = temp[0];
			}
		}
		if (language == null) {
		...
		if (language != null) {
			try {
				...
				logging (Log.ERROR, "locale", "Invalid locale country=\"" + country + "\", language=\"" + language + "\" parameter: " + e.toString ());
			}
		}
		return null;
	}
	 ...
	 */
	public void updateGenerationState(int newstatus, boolean forceSetProcessedBy) {
		if (maildropStatus.isAdminMailing() || maildropStatus.isTestMailing() || maildropStatus.isWorldMailing() || maildropStatus.isRuleMailing() || maildropStatus.isOnDemandMailing()) {
			try {
				...
				maildropStatus.setGenerationStatus(0, newstatus, forceSetProcessedBy);
			} catch (Exception e) {
	...
	public void updateGenerationState() {
		updateGenerationState(0, false);
	}
	
	public void updateGenerateionStateForCancel () {
		updateGenerationState(4, true);
	}
	...
	private long[] obj2longarray(Object o, String what) throws Exception {
		if (o instanceof String) {
			return Stream.of (((String) o).split (", *")).mapToLong (Long::parseLong).toArray ();
		}
	...
	public void options(Map<String, Object> opts, int state) throws Exception {
		...
		case 1:
			...
			if ((tmp = opts.get("preview-linelength")) != null) {
				lineLength = obj2int (tmp, "preview-line-length");
				logging(Log.DEBUG, "options1", "--> lineLength = " + lineLength);
			}
			break;
		case 2:
	 ...
	 * @param column the column to fetch plain text instead creating an image link
	 * @param use    marks if this request comes for an actual usage of the image
	 * @return the created link
	 */
	public String defaultImageLink(String name, String source, String column, boolean use) {
		boolean nocache = maildropStatus.isPreviewMailing() && (!previewCacheImages);
		return defaultImageLink(name, source, column, use, nocache);
	}

	protected String defaultImageLink(String name, String source, String column, boolean use, boolean nocache) {
		if (name == null) {
			return name;
		}
		setupImagepool ();
		...
		if (column == null) {
			...
			return imagepool.findMediapoolPlainText (name, column);
		}
		return name;
	}
	
	public List <Image> getImageDefinitions () {
		if (imagepool != null) {
			return imagepool.getImageDefinitions ();
		}
		return null;
	}
	 ...
	 * @param th     optional exception to log stacktrace for
	 * @param unwind unwind consequent stacktrace
	 */
	public void logging(int loglvl, String mid, String msg, Throwable th, int limit, boolean unwind) {
		if (log != null) {
			if (lid != null) {
				if (mid != null) {
					mid = mid + "/" + lid;
			...
			if (msg != null) {
				log.out(loglvl, mid, msg, th, limit, unwind);
			} else if (th != null) {
				log.out (loglvl, mid, "empty message", th, limit, unwind);
			}
		}
	}
	public void logging(int loglvl, String mid, String msg, Throwable th, boolean unwind) {
		logging (loglvl, mid, msg, th, -1, unwind);
	}
	public void logging(int loglvl, String mid, String msg, Throwable th, int limit) {
		logging (loglvl, mid, msg, th, limit, false);
	}
	public void logging(int loglvl, String mid, String msg, Throwable th) {
		logging(loglvl, mid, msg, th, 8, false);
	}
	 ...
	 */
	private void setStandardFields(Set<String> predef, Map<TagID, EMMTag> tagNames) throws ItemException {
		...
		for (EMMTag tag : tagNames.values()) {
			try {
				...
				logging(Log.ERROR, "tag", "Failed to get required fields for tag " + tag.mTagFullname + ": " + e.getMessage ());
			}
		}
		...
		if (URLlist != null) {
			for (URL url : URLlist) {
				if (url.getStaticValue ()) {
					...
					hashtags.stream ().filter (Hashtag::isColumn).forEach (h -> {
	 ...
	 */
	public void setUsedFieldsInLayout(BlockCollection allBlocks, Map<TagID, EMMTag> tagNames) throws ItemException {
		int sanity = 0;
		Set<String> predef;
		...
		setStandardFields(predef, tagNames);
		for (Column c : layout) {
	...
	public Map <Long, String> gridContent (String name, int type) {
		return grid.getValues (name, type);
	}
	
	public String gridDivIdToDynName (long divId) {
		return grid.divIdToDynName (divId);
	}
	
	public long gridDynNameToDivId (String name) {
		return grid.dynNameToDivId (name);
	}
	...
	public String RFCDate(Date ts) {
		SimpleDateFormat fmt = new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss z", dateLocale);
	...
	public void getControlColumns(StringBuffer selectString) {
		...
		for (Target t : targetExpression.resolveByDatabase()) {
			if (t.hasEvaluatedValue()) {
				selectString.append(", ").append(t.evaluatedValue() ? "1" : "0").append(" AS agn_tg_").append(t.getID());
			} else {
				selectString.append(", agn_tg_").append(t.getID()).append(".value AS agn_tg_").append(t.getID());
			}
		}
	}
			...
			for (String extra : new String[]{"exact", mailingTypeName, maildropStatus.statusField (), ""}) {
				if (extra != null) {
					...
					if ((expr != null) && (! seen.contains (expr))) {
						seen.add (expr);
						logging (Log.INFO, "target", "Using fixed target clause " + (!extra.isEmpty() ? "(using " + extra + ")" : "") + ": \"" + expr + "\"");
						if (! expr.equals ("-")) {
							rc.add (expr);
						}
					}
