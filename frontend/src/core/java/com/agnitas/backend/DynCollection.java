...
import com.agnitas.backend.dao.ContentDAO;
...
public class DynCollection {
	...
	public void collectParts() throws SQLException {
		...
		for (DynName tmp : dynNames) {
			long	divId = data.gridDynNameToDivId (tmp.name);
			
			if ((tmp.getDivId () == 0) && (divId != 0)) {
				tmp.setDivId (divId);
				data.logging (Log.WARNING, "dyn", tmp.name + " (" + tmp.id + "): set missing div_child_id from heuristic evaluation to " + tmp.getDivId ());
			}
