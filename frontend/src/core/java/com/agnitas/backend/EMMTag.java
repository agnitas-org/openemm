...
import com.agnitas.backend.tags.Tag;
...
public class EMMTag {
	 ...
	 * @dname tagID the unique ID for this instance
	 * @param tagSpecification  the tag itself
	 */
	public EMMTag(Data data, String tagSpecification) throws EMMTagException, SQLException {
		this (data, null, tagSpecification);
	}
	public EMMTag(Data data, TagID tagID, String tagSpecification) throws EMMTagException, SQLException {
		tag = null;
		this.tagID = tagID != null ? tagID : new TagID ("<auto generated>", tagSpecification, null);
		...
		if ((parsed == null) || ((pcnt = parsed.size()) == 0)) {
			throw new EMMTagException(data, tagSpecification, "failed in parsing (empty?) tag", "error.agnTag.parsing");
		}
		...
		if (check_tags(data) == TAG_DBASE) {
			...
			if (entry != null) {
				...
				if (mSelectString == null) {
					throw new EMMTagException(data, tagSpecification, "unknown empty tag", "error.agnTag.unknownEmpty");
				}
				interpretTagType(data);
			} else if (!dynamicTag(data)) {
				throw new EMMTagException(data, tagSpecification, "unknown tag", "error.agnTag.unknown");
			}
				...
				if (isComplex) {
					for (String e : mTagParameters.keySet()) {
						...
						if (mSelectString.indexOf(alias) == -1) {
							throw new EMMTagException(data, tagSpecification, "parameter '" + alias + "' not found in tag entry", "error.agnTag.param.notFound", alias);
						}
					...
					if (mSelectString.indexOf("{") != -1) {
						...
						throw new EMMTagException(data, tagSpecification, "missing required parameter '" + param + "'", "error.agnTag.param.required", param);
					}
				} else if (mTagParameters.size() > 0) {
					throw new EMMTagException(data, tagSpecification, "no parameters supported in simple tag", "error.agnTag.simple.params.notSupported");
				}
		...
		switch (tagSpec) {
			...
			case TI_IMAGE:
				if ((imagePatternName != null) && (imagePatternColumn != null) && (! imagePatternColumn.isnull ())) {
					mTagValue = data.defaultImageLink(imagePatternName.replace("*", imagePatternColumn.get()), imageSource, imageColumn, false);
				}
				break;
			case TI_EMAIL:
				switch (emailCode) {
			...
			case TI_TITLEFIRST: {
				...
				if (title != null) {
					...
					if (!mTagValue.isEmpty() && (titlePrefix != null || titlePostfix != null)) {
	...
	protected final String clearify(String tagString) {
		return StringOps.htmlUnescape(tagString).replaceAll("[„“‚‘“”‘’«»‹›]", "\"").trim();
	}
	...
	private final List<String> splitTag(Data data) {
		int tlen;
		String tagString;
	...
	private String substitute(Data data, String id) {
		switch (id) {
			...
			case "rdir-domain-for-images":	return data.rdirDomainForImages != null ? data.rdirDomainForImages : data.rdirDomain;
			default:			return null;
		}
	}
	 ...
	 */
	protected final void interpretTagType(Data data) {
	 ...
	 */
	protected final boolean dynamicTag(Data data) {
		boolean rc = false;
	...
	protected void initializeInternalTag(Data data, boolean strict) throws EMMTagException {
		switch (tagSpec) {
			...
			case TI_IMAGE: {
				...
				imageColumn = mTagParameters.get ("column");
				...
				if (imagePatternName != null) {
					...
					mTagValue = data.defaultImageLink(name, imageSource, imageColumn, true);
					globalValue = true;
				}
			}
			break;
			...
			case TI_IMGLINK: {
				...
				ilPostfix = null;
				if (name != null) {
					ilURL = data.defaultImageLink(name, source, null, true);
					ilPrefix = "<a href=\"";
