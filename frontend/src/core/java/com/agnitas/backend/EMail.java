...
public class EMail {
		...
		if ((inet == null) || (inet.length == 0)) {
			...
			if (inet.length == 1) {
				...
				for (int n = 0; n < inet.length; ++n) {
					...
					if (email == null && e != null && !e.isEmpty()) {
						email = e;
					}
				}
			}
	 ...
	 */
	public final boolean valid(boolean relaxed) {
	 ...
	 */
	private final String extractPureAddress(String str) {
		int n, m;
	 ...
	 */
	private final void makePunyCoded() {
		pure = null;
		full_puny = null;
		pure_puny = null;
		if (full != null) {
