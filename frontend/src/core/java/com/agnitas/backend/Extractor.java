...
public class Extractor implements ResultSetExtractor<Object> {
	...
	private Map<TagID, EMMTag> tagNames = null;
	...
	public Extractor(Map<TagID, EMMTag> tagNames, MediaMap mmap, Data nData, Custinfo nCinfo, Blocklist nBlist, MailWriter nMailer, boolean nNeedSamples, Set<Long> nSeen, boolean nMulti, boolean nHasOverwriteData, boolean nHasVirtualData, List<EMMTag> nEmailTags, int nEmailCount) {
				...
				for (Media m : data.media()) {
					...
					if (check != null) {
						...
						if (bl != null) {
							...
							blist.writeBounce (data.licenceID (), data.mailing.id (), cid, m);
							isBlocklisted = true;
						}
					}
				}
