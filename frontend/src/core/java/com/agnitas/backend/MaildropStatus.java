...
import com.agnitas.backend.dao.MaildropStatusDAO;
...
public class MaildropStatus {
	...
	public void id(long nId) throws SQLException, CancelException {
		if (id != nId) {
			id = nId;
			statusField = null;
			sendDate = null;
			...
			Validator.maildrop (data, this);
		}
	}

	public long companyID () {
		return exists () ? maildrop.companyID () : 0;
	}
	...
	public void sendDate(Date nSendDate) {
		sendDate = nSendDate;
	}
	...
	public Date genericSendDate(Date defaultSendDate) {
		Date	rc;
		
		try {
			rc = (exists() ? maildrop : new MaildropStatusDAO(data.dbase, 0, data.mailing.id())).realSendDate();
		} catch (SQLException e) {
			data.logging(Log.ERROR, "senddate", "Failed to query real send date: " + e.toString());
			rc = null;
		}
		return rc != null ? rc : (defaultSendDate != null ? defaultSendDate : new Date ());
	}
		...
		if (maildrop.dependsOnAutoImportID () && (! maildrop.autoImportOK ())) {
			...
			if ((! isCampaignMailing ()) && (! isPreviewMailing ()) && (! isVerificationMailing ())) {
				try {
					...
					data.trigger ("import-dependency");
				} catch (Exception e) {
