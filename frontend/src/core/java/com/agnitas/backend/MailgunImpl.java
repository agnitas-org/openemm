...
public class MailgunImpl implements Mailgun {
	 ...
	 */
	protected Map<TagID, EMMTag> tagNames = null;
	...
	public void close () {
		try {
			...
			Log.error ("mailgun", "close: " + e.toString ());
		}
	}
	@Override
	...
	public String fire(String status_id, Map<String, Object> opts) throws Exception {
		...
		try {
			...
			boolean unexpectedException = ! (e instanceof BackendException);
			
			if (data != null) {
				if (unexpectedException) {
					data.logging(Log.ERROR, "mailout", "Creation failed: " + e.toString(), e, true);
				}
	...
	protected void doExecute(Map<String, Object> opts) throws Exception {
		...
		try {
			...
			data.updateGenerationState(4, true);
			data.suspend();
	 ...
	 */
	protected String getSelectvalue(Map<TagID, EMMTag> tagNames, boolean hint) {
		...
		if (tagNames != null) {
			...
			for (EMMTag current_tag : tagNames.values()) {
				if ((!current_tag.globalValue) && (current_tag.tagType == EMMTag.TAG_DBASE)) {
					selectString.append(", ").append(current_tag.mSelectString);
				}
			}
			if (data.lusecount > 0) {
	...
	public static void main(String[] args) throws Exception {
		...
		try (MailgunImpl mailout = new MailgunImpl()) {
