...
public class Target {
	...
	private static final Pattern CHECKER_PATTERN = Pattern.compile("[a-z][a-z0-9_]*\\.[a-z][a-z0-9_]*", Pattern.CASE_INSENSITIVE);
	...
	public void requestFields(Set<String> use) {
		if ((sql != null) && (!databaseOnly)) {
			String[] parts = CLEANER_PATTERN.split(sql);

			for (int n = 0; n < parts.length; ++n) {
				Matcher m = CHECKER_PATTERN.matcher(parts[n]);
