...
public class TargetExpression {
	...
	public void retrieveInformation() throws InvalidTargetException {
		...
		if (data.maildropStatus.isAdminMailing () || data.maildropStatus.isTestMailing ()) {
			...
			if (altgRestriction != null && !altgRestriction.isEmpty()) {
			...
			for (int n = 0; n < elen; ++n) {
				...
				if ((ch == '(') || (ch == ')')) {
					...
					if ((temp != null) && temp.valid()) {
						buf.append(" (").append(temp.getSQL(true)).append(")");
					}
				}
			}
	...
	public void setEvaluatedValues(String targetInformation) {
		...
		for (int n = 0; n < targetsToEvaluate.length; ++n) {
			if (!targetsToEvaluate[n].isEmpty()) {
				try {
		...
		resolveByDatabase().forEach(t -> t.setEvaluateValue(targetIDs.contains(t.getID())));
	}

	public void clearEvaluatedValues() {
		resolveByDatabase().forEach(Target::clearEvaluateValue);
	}
}
