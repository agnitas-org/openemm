			...
			for (int n = 0; n < rq.size (); ++n) {
				...
				switch (tmp.comptype) {
					case 0:
						...
						} else if (tmp.cid.equals(Const.Component.NAME_RCS)) {
							tmp.type = BlockData.RCS;
							tmp.media = Media.TYPE_RCS;
						} else if (tmp.cid.equals(Const.Component.NAME_RCSTITLE)) {
							tmp.type = BlockData.RCSTITLE;
							tmp.media = Media.TYPE_RCS;
						} else if (tmp.cid.equals(Const.Component.NAME_RCSSMS)) {
							tmp.type = BlockData.RCSSMS;
							tmp.media = Media.TYPE_RCS;
						} else {
				...
				if (tmp != null) {
					...
					tmp.binary = binary;
					if (tmp.binary != null) {
						if (!tmp.isParseable && tmp.content != null && tmp.content.isEmpty()) {
							tmp.content = null;
						}
					}
