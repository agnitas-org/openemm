...
public class MaildropStatusDAO {
	...
	public MaildropStatusDAO (DBase dbase, long forStatusID, long forMailingID) throws SQLException {
		...
		try (DBase.With with = dbase.with ()) {
			...
			if (row != null) {
				...
				sendDate = dbase.asDate (row.get ("senddate"));
	...
	private void determinateRealSendDate (DBase dbase) throws SQLException {
		...
		try (DBase.With with = dbase.with ()) {
			...
			for (int n = 0; n < rq.size (); ++n) {
				...
				if (checkStatusField != null && !checkStatusField.isEmpty()) {
	...
	private static long findStatusIDForWorldMailing (DBase dbase, long mailingID, String direction) throws SQLException {
		List <Map <String, Object>>	rq;
	 ...
	 */
	public static long findLargestStatusIDForWorldMailing (DBase dbase, long mailingID) throws SQLException {
		return findStatusIDForWorldMailing (dbase, mailingID, "DESC");
	}
	/**
	 * find the smallest status id for a world mailing
	 */
	public static long findSmallestStatusIDForWorldMailing (DBase dbase, long mailingID) throws SQLException {
	...
	public boolean updateGenStatus (DBase dbase, int fromStatus, int toStatus, boolean forceSetProcessedBy) throws SQLException {
		try (DBase.With with = dbase.with ()) {
			...
			DBase.Retry <Integer>	r = dbase.new Retry <> ("genstatus", dbase, with.cursor ()) {
				private int getCurrentGenstatus () throws SQLException {
					int			rc = -1;
					Map <String, Object>	rq = dbase.querys (cursor,
										   "SELECT genstatus " +
										   "FROM maildrop_status_tbl " +
										   "WHERE status_id = :statusID",
										   "statusID", statusID);
					if (rq == null) {
						dbase.logging (Log.DEBUG, "genstatus", "no entry for status_id " + statusID + " found");
					} else {
						Object	obj = rq.get ("genstatus");
						if (obj == null) {
							dbase.logging (Log.DEBUG, "genstatus", "genstatus is NULL for status_id " + statusID);
						} else {
							rc = dbase.asInt (obj);
						}
					}
				...
				@Override
				public void execute () throws SQLException {
					if (getCurrentGenstatus () == toStatus) {
						dbase.logging (Log.INFO, "genstatus", "genstatus is already " + toStatus + ", no change required"); 
						priv = 1;
					} else {
