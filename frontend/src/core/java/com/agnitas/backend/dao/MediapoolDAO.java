	...
	public MediapoolDAO(DBase dbase, long companyID) throws SQLException {
		try (DBase.With with = dbase.with()) {
			...
			rq = dbase.query (with.cursor (),
					  "SELECT mediapool_element_id, filename, mime_type, source, alt_text, height, width " +
			...
			for (int n = 0; n < rq.size (); ++n) {
				...
				String			description = dbase.asString (row.get ("source"));
				String			altText = dbase.asString (row.get ("alt_text"));
				int			height = dbase.asInt (row.get ("height"));
				int			width = dbase.asInt (row.get ("width"));
				String			ext;
				int			pos;
				...
				images.put(filename, new Image(id, filename, String.format("%d.%s", id, ext), mime, null, description, altText, height, width, true));
			}
		}
	}
