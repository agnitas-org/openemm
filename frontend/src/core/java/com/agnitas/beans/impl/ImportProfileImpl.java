...
public class ImportProfileImpl implements ImportProfile {
	...
	private Locale reportLocale = Locale.of("en", "US");
	...
	public Integer getGenderValueByFieldValue(String fieldValue) {
        if (isParsableToInt(fieldValue)) {
            ...
            return Objects.requireNonNullElseGet(gender, () -> Integer.valueOf(fieldValue));
        } else {
	...
	public String toString() {
		...
		output.append("Charset: ").append(Charset.getCharsetById(charset).getCharsetName()).append("\n");

		output.append("EncryptedZip: ").append(zipPassword != null).append("\n");
		output.append("AutoMapping: ").append(autoMapping).append("\n");
		
		output.append("Data type: ").append(datatype).append("\n");
		ImportDataType importDataType = ImportDataType.getImportDataTypeForName(datatype);

		if (importDataType != ImportDataType.JSON) {
			output.append("NoHeaders: ").append(noHeaders).append("\n");
		}
		if (importDataType == ImportDataType.CSV) {
			output.append("Separator: ").append(Separator.getSeparatorById(separator).getValueChar()).append("\n");

			try {
				output.append("TextRecognitionChar: ").append(TextRecognitionChar.getTextRecognitionCharById(textRecognitionChar).name()).append("\n");
			} catch (Exception e) {
				output.append("TextRecognitionChar: Invalid (\"").append(e.getMessage()).append("\")\n");
			}
			output.append("DecimalSeparator: ").append(decimalSeparator).append("\n");
		}
		
		try {
			output.append("DateFormat: ").append(DateFormat.getDateFormatById(dateFormat).getValue()).append("\n");
		} catch (Exception e) {
			output.append("DateFormat: Invalid (\"").append(e.getMessage()).append("\")\n");
		}

		output.append("ImportMode: ").append(ImportMode.getFromInt(importMode).getMessageKey()).append("\n");
		output.append("CheckForDuplicates: ").append(CheckForDuplicates.getFromInt(checkForDuplicates).name()).append("\n");

		output.append("NullValuesAction: ").append(NullValuesAction.getFromInt(nullValuesAction).name()).append("\n");

		output.append("DefaultMailType: ").append(MailType.getFromInt(defaultMailType).name()).append("\n");
		output.append("UpdateAllDuplicates: ").append(updateAllDuplicates).append("\n");
		output.append("ImportProcessActionID: ").append(importProcessActionID).append("\n");
		output.append("ActionForNewRecipients: ").append(actionForNewRecipients).append("\n");
		output.append("MailForReport: ").append(mailForReport).append("\n");
		output.append("MailForError: ").append(mailForError).append("\n");
		
		output.append("GenderMapping: ");
		if (genderMapping != null && !genderMapping.isEmpty()) {
			output.append("\n\t").append(AgnUtils.mapToString(genderMapping).replace("\n", "\n\t")).append("\n");
		} else {
			output.append("NONE\n");
		}
		...
		for (ColumnMapping mapping : columnMapping) {
			output.append("\t").append(mapping.getDatabaseColumn()).append(" = ").append(mapping.getFileColumn()).append("\n");
		}
		
		output.append("KeyColumns: ").append(StringUtils.join(keyColumns, ", "));
