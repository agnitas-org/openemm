...
public class RecipientImpl implements Recipient {
	...
	@Override
	public int insertNewCustWithException() {
	...
	public int getCustomerID() {
		if (customerID == 0) {
			...
			if (value == null || value instanceof String) {
				...
				throw new IllegalArgumentException("Invalid data type for customerID: " + value.getClass());
			}
		}
	...
	public Timestamp getTimestamp() {
		...
		try {
			return new Timestamp(RecipientDaoImpl.DATE_FORMAT.parse(dateStr).getTime());
	...
	public void setCustParameters(String key, String custParameter) {
		...
		if (getCustDBStructure().containsKey(trimmedkey)) {
			...
			if (!Strings.CS.equals(custParameter, value)) {
				changeFlag = true;
	...
	public boolean copyDateFromRequest(Map<String, Object> req, String name, String suffix) {
		String[] field = { RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY, RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH, RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR, RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR, RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE, RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND };
		String s = null;
	...
	public boolean isAllowedName(String name) {
        ...
        return !name.equals("timestamp") && !name.equals("creation_date");
    }
				...
				if (StringUtils.isNotBlank((String) caseInsensitiveParameters.get(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_FORMAT))) {
					...
					if (StringUtils.isNotBlank(value)) {
						try {
							...
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY, Integer.toString(date.get(Calendar.DAY_OF_MONTH)));
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH, Integer.toString(date.get(Calendar.MONTH) + 1));
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR, Integer.toString(date.get(Calendar.YEAR)));
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR, Integer.toString(date.get(Calendar.HOUR_OF_DAY)));
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE, Integer.toString(date.get(Calendar.MINUTE)));
							setCustParameters(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND, Integer.toString(date.get(Calendar.SECOND)));
						} catch (ParseException e) {
