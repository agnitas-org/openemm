...
public class TargetImpl extends TargetLightImpl implements Target {
	
	private static final Logger logger = LogManager.getLogger(TargetImpl.class);
    		...
    		if (sql.length() > targetSQL.length()) {
    			...
    			if (tmp.equals( sql)) {
    				try {
    					...
    					logger.error( "possible error with parenthesis detected! target ID: {}, company ID: {}, targetName: '{}', old SQL: '{}', newSQL: '{}'",
								id, companyID, targetName, targetSQL, sql, e);
    				}
    			}
    		}
    	}
