...
public class CompanyDaoImpl extends PaginatedBaseDaoImpl implements CompanyDao {
	...
	public Company getCompany(int companyID) {
		...
		String query = """
				...
				WHERE company_id = ?
				""";
		return selectObjectDefaultNull(query, new CompanyRowMapper(), companyID);
	}
		...
		try {
			if (companyId == 1) {
				return select("""
						...
						""", new CompanyRowMapper(), CompanyStatus.ACTIVE.getDbValue());
			} else {
				return select("""
						SELECT company_id,
						       creator_company_id,
						...
						WHERE (creator_company_id = ? OR company_id = ?)
						  AND status = ?
						ORDER BY LOWER(shortname)
						""", new CompanyRowMapper(), companyId, companyId, CompanyStatus.ACTIVE.getDbValue());
			}
		} catch (Exception e) {
	...
	private void createIntervalTrackTable(int companyId) {
		...
		executeWithTableSpace(query, TABLESPACE_DATA_SUCCESS);
		executeWithTableSpace("CREATE INDEX intervtrack$%dmid$idx ON %s (mailing_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX intervtrack$%d$cid$idx ON %s (customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX intervtrack$%d$sendd$idx ON %s (send_date)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
	}
	...
	private void createMailTrackTable(int companyId) {
		...
		executeWithTableSpace("CREATE INDEX mailtr%d$cid$idx ON %s (customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX mailtr%d$mdrstatid$idx ON %s (maildrop_status_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX mailtr%d$mid$idx ON %s (mailing_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX mailtr%d$ts$idx ON %s (timestamp)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
	}
	...
	private void createOnePixelLogTable(int companyId) {
		...
		executeWithTableSpace(query, TABLESPACE_DATA_CUSTOMER_TABLE);

		executeWithTableSpace("CREATE INDEX onepix%d$mlid_cuid$idx ON %s (mailing_id, customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX onpx%d$mid$idx ON %s (mailing_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX onpx%d$coid_mlid_cuid$idx ON %s (company_id, mailing_id, customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
	}
	...
	private void createOnePixelLogDeviceTable(int companyId) {
		...
		String query = """
				...
				""".formatted(tableName);
		executeWithTableSpace(query, TABLESPACE_DATA_WAREHOUSE);
		executeWithTableSpace("CREATE INDEX onedev%d$creat$idx ON %s (creation)".formatted(companyId, tableName), TABLESPACE_DATA_WAREHOUSE_INDEX);
		executeWithTableSpace("CREATE INDEX onepixdev%d$mlid_cuid$idx ON %s (mailing_id, customer_id)".formatted(companyId, tableName), TABLESPACE_DATA_WAREHOUSE_INDEX);
		executeWithTableSpace("CREATE INDEX onedev%d$ciddevclidmlid$idx ON %s (customer_id, device_class_id, mailing_id)".formatted(companyId, tableName), TABLESPACE_DATA_WAREHOUSE_INDEX);
	}
	...
	private void createRdirLogTable(int companyId) {
		...
		executeWithTableSpace("CREATE INDEX rlog%d$mlid_urlid_cuid$idx ON %s (mailing_id, url_id, customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX rlog%d$ciddevclidmlid$idx ON %s (customer_id, device_class_id, mailing_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		executeWithTableSpace("CREATE INDEX rlog%d$tmst$idx ON %s (timestamp)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
	}
	...
	private void createRdirLogUserFormTable(int companyId) {
		...
		String query = """
				...
				""".formatted(tableName);
		executeWithTableSpace(query, TABLESPACE_DATA_CUSTOMER_TABLE);
		executeWithTableSpace("CREATE INDEX rlogform%d$fid_urlid$idx ON %s (form_id, url_id)".formatted(companyId, tableName), TABLESPACE_INDEX_CUSTOMER);
	}
	...
	private void createSuccessTable(int companyId) {
		...
		executeWithTableSpace("CREATE INDEX suc%d$mid$idx ON %s (mailing_id)".formatted(companyId, tableName), TABLESPACE_INDEX_SUCCESS);
		executeWithTableSpace("CREATE INDEX suc%d$cid$idx ON %s (customer_id)".formatted(companyId, tableName), TABLESPACE_INDEX_SUCCESS);
		executeWithTableSpace("CREATE INDEX suc%d$tmst$idx ON %s (timestamp)".formatted(companyId, tableName), TABLESPACE_INDEX_SUCCESS);
	}
	...
	public void createRdirValNumTable(int newCompanyId) {
		...
		if (isOracleDB()) {
			...
			String sql = """
					...
					""".formatted(tableName);
			executeWithTableSpace(sql, TABLESPACE_DATA_SUCCESS);
			...
			executeWithTableSpace("CREATE INDEX rvalnum%d$cod_cid_mid$idx ON %s (company_id, customer_id, mailing_id)".formatted(newCompanyId, tableName), TABLESPACE_INDEX_CUSTOMER);
			executeWithTableSpace("CREATE INDEX rvalnum%d$mid_pagetag$idx ON %s (mailing_id, page_tag)".formatted(newCompanyId, tableName), TABLESPACE_INDEX_CUSTOMER);
		} else {
			String sql;
			// Create reveue tracking table for this new company
	...
	private void createCustomerTable(int companyId, boolean createPreventConstraint) {
		...
		executeWithTableSpace("CREATE INDEX cust%d$email$idx ON %s (email)".formatted(companyId, tableName), indexTableSpace);
		executeWithTableSpace("CREATE INDEX cust%d$lowemail$idx ON %s (LOWER(email))".formatted(companyId, tableName), indexTableSpace);
	}
	...
	private void createCustomerBindingTable(int companyId, boolean createPreventConstraint) {
		...
		executeWithTableSpace("CREATE INDEX cust%db$tmst$idx ON %s (timestamp)".formatted(companyId, tableName), indexTableSpace);
		executeWithTableSpace("CREATE INDEX cust%db$cuid_utype_mlid$idx ON %s (customer_id, user_type, mailinglist_id)".formatted(companyId, tableName), indexTableSpace);
		executeWithTableSpace("CREATE INDEX cust%db$cuid_ustat_mlid$idx ON %s (customer_id, user_status, mailinglist_id)".formatted(companyId, tableName), indexTableSpace);
	}
	...
	public List<Company> getAllActiveCompaniesWithoutMasterCompany() {
		String query = """
				...
				""";
		try {
			return select(query, new CompanyRowMapper(), CompanyStatus.ACTIVE.getDbValue(), CompanyStatus.LOCKED.getDbValue());
		} catch (Exception e) {
		...
		if (!DbUtilities.checkIfTableExists(getDataSource(), tableName)) {
			...
			try {
				if (isOracleDB()) {
					...
					executeWithTableSpace(sql, TABLESPACE_CUSTOMER_HISTORY);

					executeWithTableSpace("CREATE INDEX hstcb%d$email$idx ON %s (email)".formatted(companyID, tableName), TABLESPACE_CUSTOMER_HISTORY_INDEX);
					executeWithTableSpace("CREATE INDEX hstcb%d$mlidcidl$idx ON %s (mailinglist_id, customer_id)".formatted(companyID, tableName), TABLESPACE_CUSTOMER_HISTORY_INDEX);
					executeWithTableSpace("CREATE INDEX hstcb%d$tsch$idx ON %s (timestamp_change)".formatted(companyID, tableName), TABLESPACE_CUSTOMER_HISTORY_INDEX);
	...
	public List<Company> getAllActiveCompanies() {
		String query = """
				...
				""";
		try {
			return select(query, new CompanyRowMapper(), CompanyStatus.ACTIVE.getDbValue());
		} catch (Exception e) {
			// return an empty list, for no further actions
			return new ArrayList<>();
		}
	...
	public void setupPremiumFeaturePermissions(Set<String> allowedPremiumFeatures, Set<String> unAllowedPremiumFeatures, String comment, int companyID) {
		...
		if (unAllowedPremiumFeatures != null && !unAllowedPremiumFeatures.isEmpty()) {
			...
			List<String> foundUnAllowedPremiumFeatures = select("SELECT permission_name FROM company_permission_tbl WHERE company_id = " + companyID + " AND permission_name IN (" + AgnUtils.repeatString("?", unAllowedPremiumFeatures.size(), ", ") + ")", StringRowMapper.INSTANCE, parameters);
			...
			if (touchedLines1 > 0 && logger.isWarnEnabled()) {
				logger.warn("Deleted unallowed premium features for company {}: {}", companyID, touchedLines1);
				logger.warn(StringUtils.join(foundUnAllowedPremiumFeatures, ", "));
			}
		}
	}
	...
	public boolean createFrequencyFields(int companyID) {
		...
		try {
			if (isOracleDB()) {
				...
				executeWithTableSpace("CREATE INDEX cust%d$freq$idx ON %s (freq_count_day, freq_count_week, freq_count_month )".formatted(companyID, tableName), TABLESPACE_INDEX_CUSTOMER);
			} else {
	...
	public Company getCompanyByName(String companyName) {
		...
		List<Company> list = select(sql, new CompanyRowMapper(), companyName);
		if (!list.isEmpty()) {
			return list.get(0);
		}
	...
	public String getSpamCheckAddress(int companyID) {
		String newTestVipEmailAddress = configService.getValue(ConfigValue.DeliveryCheckAddress);
	...
	private void executeWithTableSpace(String statement, String tableSpace) {
		String tablespaceClause = "";
