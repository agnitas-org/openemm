...
public class DatasourceDescriptionDaoImpl extends PaginatedBaseDaoImpl implements DatasourceDescriptionDao {
	...
	private static Map<String, Integer> sourceGroupTypeIdCache;

	private static final RowMapper<DataSourceDto> DATA_SOURCE_ROW_MAPPER = (rs, i) -> new DataSourceDto(
            rs.getBigDecimal("datasource_id").intValue(),
            rs.getInt("recipients_count"),
            rs.getString("description"),
            rs.getTimestamp("timestamp"),
            SourceGroupType.getUserSourceGroupType(rs.getString("sourcegroup_type")),
    ...
    public DatasourceDescription getByDescription(SourceGroupType sourceGroupType, int companyID, String description) {
    	...
    	if (sourceGroupId <= 0) {
	        ...
	        return selectObjectDefaultNull(sql, new DatasourceDescriptionRowMapper(), sourceGroupId, companyID, description);
    	}
    }
    ...
    public List<DataSourceDto> getDataSources(int companyId) {
        return select("""
                        SELECT
                          ...
                          d.timestamp,
                          s.sourcegroup_type,
                          CASE WHEN s.sourcegroup_type = ? THEN wsu.username END AS extra_data,
                          COUNT(c.customer_id) AS recipients_count
                        FROM datasource_description_tbl d JOIN sourcegroup_tbl s ON d.sourcegroup_id = s.sourcegroup_id
                        LEFT JOIN webservice_user_tbl wsu ON wsu.default_data_source_id = d.datasource_id
                        LEFT JOIN customer_%d_tbl c ON c.latest_datasource_id = d.datasource_id
                        WHERE d.company_id IN (0, ?)
                        GROUP BY d.datasource_id, d.description, d.timestamp, s.sourcegroup_type, wsu.username
                        """.formatted(companyId),
				DATA_SOURCE_ROW_MAPPER, SourceGroupType.SoapWebservices.getStorageString(), companyId);
    }

    private class DatasourceDescriptionRowMapper implements RowMapper<DatasourceDescription> {
		@Override
	...
	public DatasourceDescription getDatasourceDescription(int datasourceId) {
		...
		return selectObjectDefaultNull(query, new DatasourceDescriptionRowMapper(), datasourceId);
	}
	...
	public DatasourceDescription getDatasourceDescription(int datasourceId, int companyId) {
		...
		return selectObjectDefaultNull(query, new DatasourceDescriptionRowMapper(), datasourceId, companyId);
    }
