...
public class EmmActionDaoImpl  extends PaginatedBaseDaoImpl implements EmmActionDao {
	...
	public EmmAction getEmmAction(int actionID, int companyID) {
		...
		return selectObjectDefaultNull(sql, new EmmActionRowMapper(), actionID, companyID);
	}

	@Override
	public List<Integer> findTriggerIdsByName(String name, int companyId) {
		return select(
				"SELECT action_id FROM rdir_action_tbl WHERE shortname = ? AND company_id = ? ORDER BY action_id",
	...
	public List<EmmAction> getEmmActions(int companyID, boolean includeDeleted) {
		...
		return select(sql, new EmmActionRowMapper(), companyID);
	}

	@Override
	public List<Integer> getTriggerIds(int companyId) {
		String sql = "SELECT action_id FROM rdir_action_tbl WHERE company_id = ? AND deleted = 0";
		return select(sql, IntegerRowMapper.INSTANCE, companyId);
	}
	...
	public List<EmmAction> getActiveEmmActionsByOperationType(int companyID, ActionOperationType... actionTypes) {
		...
		try {
			...
			return select(sql, new EmmActionRowMapper(), params.toArray());
		} catch (Exception e) {
			return Collections.emptyList();
		}
	}
	...
	public List<EmmAction> getEmmActionsByName(int companyID, String shortName) {
		...
		return select(query, new EmmActionRowMapper(), companyID, shortName);
	}
	...
	public List<EmmAction> getEmmNotFormActions(int companyID, boolean includeInactive) {
		...
		return select(sqlGetActions, new EmmActionRowMapper(), companyID, EmmAction.TYPE_FORM);
	}
	...
	public List<EmmAction> getEmmNotLinkActions(int companyID, boolean includeInactive) {
		...
		return select(sqlGetActions, new EmmActionRowMapper(), companyID, EmmAction.TYPE_LINK);
	}
	...
	public List<EmmAction> getActionListBySendMailingId(int companyId, int mailingId) {
		...
		return select(sqlGetActionsBySendMailingId, new EmmActionRowMapper(), mailingId, companyId);
	}
	...
	private static class EmmActionRowMapper implements RowMapper<EmmAction> {
		@Override
