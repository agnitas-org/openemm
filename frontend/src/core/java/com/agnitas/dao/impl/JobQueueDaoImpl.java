...
public class JobQueueDaoImpl extends BaseDaoImpl implements JobQueueDao {
	...
	public List<JobDto> getJobsWithLostNextStart() {
		return select(isOracleDB() || isPostgreSQL() ? """
			...
			WHERE deleted <= 0
			  ...
			  AND nextStart IS NULL
			  AND hostname = ?
			""", new JobRowMapper(), AgnUtils.getHostName());
	}

	@Override
	public JobDto getJob(int id) {
		return selectObjectDefaultNull("SELECT * FROM job_queue_tbl WHERE id = ?", new JobRowMapper(), id);
	}
	...
	public JobDto getJob(String description) {
		...
		return selectObjectDefaultNull("SELECT * FROM job_queue_tbl WHERE description = ?", new JobRowMapper(), description);
	}
	...
	private void fixErroneousJobs() {
		...
		List<JobDto> jobs = select("""
            ...
            WHERE hostname = ?
              ...
              AND lastresult != 'OK'""", new JobRowMapper(), hostName);
		for (JobDto job : jobs) {
	...
	private void fixKilledJobs() {
		...
		List<JobDto> jobs = select("""
            ...
            FROM job_queue_tbl
            WHERE hostname = ?
              AND running = 1""", new JobRowMapper(), hostName);
		for (JobDto job : jobs) {
	...
	private class JobRowMapper implements RowMapper<JobDto> {
		@Override
			...
			return select("""
					...
					WHERE deleted <= 0
					  AND ((lastResult IS NULL OR lastResult != 'OK') OR (nextStart IS NULL OR nextStart < %s) OR
					       interval IS NULL OR job_name IS NULL)
	...
	public List<JobDto> getAllActiveJobs() {
		return select("SELECT * FROM job_queue_tbl WHERE deleted <= 0 ORDER BY id", new JobRowMapper());
	}

	@Override
	public List<JobDto> getForOverview() {
        return select("SELECT * FROM job_queue_tbl WHERE deleted <= 0 ORDER BY id DESC", new JobRowMapper());
	}
	 ...
	 * Do not update the fields "description", "created", "lastStart", "deleted", "nextstart", "interval", "hostname", "job_name", "runonlyonhosts", "startAfterError" and "emailonerror",
