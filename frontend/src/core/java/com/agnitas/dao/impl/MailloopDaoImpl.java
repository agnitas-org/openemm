...
public class MailloopDaoImpl extends PaginatedBaseDaoImpl implements MailloopDao {
		...
		if (mailloopId == 0 || companyId == 0) {
			...
			return selectObjectDefaultNull("""
				SELECT rid,
				       ...
				       mailinglist_id,
				       form_id,
				       autoresponder_mailing_id,
				       security_token
				FROM mailloop_tbl
				WHERE rid = ?
				  AND company_id = ?
				""", new MailloopRowMapper(), mailloopId, companyId);
		}
	}
    ...
    @Override
    public boolean deleteExpiredLogs(int daysToHold, int companyId) {
        final Date expireDate = DateUtilities.getDateOfDaysAgo(daysToHold);
        update("DELETE FROM mailloop_log_tbl WHERE timestamp < ? AND company_id = ?", expireDate, companyId);
        return selectInt("SELECT COUNT(*) FROM mailloop_log_tbl WHERE timestamp < ? AND company_id = ?", expireDate, companyId) == 0;
    }

	private static class MailloopEntryRowMapper implements RowMapper<MailloopEntry> {
		@Override
	...
	private static class MailloopRowMapper implements RowMapper<Mailloop> {
		@Override
    ...
    public List<MailloopEntry> getDependentBounceFiltersWithActiveAutoResponder(int companyId, int mailingId) {
		...
		return select(query, new MailloopEntryRowMapper(), companyId, mailingId);
	}
