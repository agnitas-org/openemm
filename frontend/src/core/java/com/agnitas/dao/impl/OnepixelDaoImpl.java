...
public class OnepixelDaoImpl extends BaseDaoImpl implements OnepixelDao {
	...
	private void updateCustomerForOpen(int companyID, int customerID) {
		if(customerID != 0) {
			try {
				...
				update(updateLastOpenStatement, customerID);
			} catch (Exception e) {
				logger.info("Cannot update last open date for customer {} of company {}", customerID, companyID, e);
			}
		}
	}
    ...
    public boolean writePixel(int companyID, int recipientID, int mailingID, String remoteAddr, DeviceClass deviceClass, int deviceID, int clientID) {
        try {
    		...
    		int touchedLines = update(getSqlUpdateString(companyID), mobileCountDelta, mailingID, recipientID);
    		
        	if (touchedLines == 0) {
				// Insert new entry
        		update(getSqlInsertString(companyID), companyID, mailingID, recipientID, remoteAddr, 1, mobileCountDelta);
				...
				List<Map<String, Object>> result = select("SELECT " + FIELD_OPEN_COUNT + ", " + FIELD_MOBILE_COUNT + " FROM " + getOnepixellogTableName(companyID) + " WHERE mailing_id = ? AND customer_id = ?", mailingID, recipientID);
				if (result.size() > 1) {
					...
					update("DELETE FROM " + getOnepixellogTableName(companyID) + " WHERE mailing_id = ? AND customer_id = ?", mailingID, recipientID);
					// Insert summed up entry
					update(getSqlInsertString(companyID), companyID, mailingID, recipientID, remoteAddr, countSum - correctionValue, mobileSum - correctionValue);
				}
			}
        	
        	// write additional device entry
			update(getSqlDeviceInsertString(companyID), companyID, mailingID, recipientID, deviceClass.getId(), deviceID, clientID);
    ...
    public void deleteAdminAndTestOpenings(int mailingID, int companyID) {
		...
		update(sqlOpen, mailingID, mailingID);
		...
		String sqlOpenDevice = "DELETE FROM onepixellog_device_" + companyID + "_tbl"
			+ " WHERE mailing_id = ?"
				+ " AND customer_id IN (SELECT customer_id FROM customer_" + companyID + "_binding_tbl"
					+ " WHERE user_type IN ('" + UserType.Admin.getTypeCode() + "', '" + UserType.TestUser.getTypeCode() + "', '" + UserType.TestVIP.getTypeCode() + "') AND mailinglist_id = (SELECT mailinglist_id FROM mailing_tbl WHERE mailing_id = ?))";
		update(sqlOpenDevice, mailingID, mailingID);
    }
}
