...
import com.agnitas.emm.core.profilefields.exception.TooManyRowsForDefaultValueChangeException;
...
public class ProfileFieldDaoImpl extends BaseDaoImpl implements ProfileFieldDao {
	 ...
	 */
	private static final TimeoutLRUMap<Integer, CaseInsensitiveMap<String, ProfileField>> PROFILESTRUCTURE_CACHE = new TimeoutLRUMap<>(1000, 60 * 1000L);
	...
	public ProfileField getProfileField(int companyID, String columnName) {
		if (companyID <= 0) {
			...
			if (columnType == null) {
				...
				ProfileField profileField = selectObjectDefaultNull(SELECT_PROFILEFIELD_BY_COMPANYID_AND_COLUMNNAME, new ProfileFieldRowMapper(), companyID, columnName);
				if (profileField == null) {
					...
					profileField.setCreationDate(profileField.getCreationDate());
					profileField.setChangeDate(profileField.getChangeDate());
    ...
    public ProfileField getProfileField(int companyID, String columnName, int adminID) {
    	if (companyID <= 0) {
            ...
            if (profileField == null) {
				...
				ProfileFieldPermission permission = selectObjectDefaultNull(SELECT_PROFILEFIELDPERMISSION, new ProfileFieldPermissionRowMapper(), companyID, profileField.getColumn().toLowerCase(), adminID);
            	if (permission == null) {
    				return profileField;
    			}

				profileField.setAdminID(adminID);
				profileField.setModeEdit(permission.getModeEdit());
				return profileField;
            }
		}
    }
	...
	public ProfileField getProfileFieldByShortname(int companyID, String shortName) {
		if (companyID <= 0) {
			...
			ProfileField profileField = selectObjectDefaultNull(SELECT_PROFILEFIELD_BY_COMPANYID_AND_SHORTNAME, new ProfileFieldRowMapper(), companyID, shortName);
			if (profileField == null) {
				return null;
			}
	...
	public List<ProfileField> getProfileFields(int companyID) {
		if (companyID <= 0) {
            ...
            return new ArrayList<>(profileFieldList);
		}
	}
	...
	private CaseInsensitiveMap<String, ProfileField> getComProfileFieldsMap(int companyID, @Deprecated boolean determineDefaultValues, boolean excludeNonHistorized) {
		...
		if (companyID <= 0) {
			...
			if (cachedValue != null) {
	            ...
	            for (ProfileField field : select(sqlSelectFields, new ProfileFieldRowMapper(), companyID)) {
	...
	private CaseInsensitiveMap<String, LightProfileField> getLightProfileFieldsMap(int companyId, boolean excludeNonHistorized) {
		...
		for (LightProfileField field : select(sqlSelectFields, new LightProfileFieldRowMapper(), companyId)) {
			customFieldsMap.put(field.getColumn(), field);
		}
    ...
    public CaseInsensitiveMap<String, ProfileField> getComProfileFieldsMap(int companyID, int adminID, boolean noNotNullConstraintCheck) {
        if (companyID <= 0) {
			...
			for (ProfileField comProfileField : comProfileFieldMap.values()) {
				ProfileFieldPermission permission = selectObjectDefaultNull(SELECT_PROFILEFIELDPERMISSION, new ProfileFieldPermissionRowMapper(), companyID, comProfileField.getColumn().toLowerCase(), adminID);
            	if (permission != null) {
    				comProfileField.setAdminID(adminID);
    				comProfileField.setModeEdit(permission.getModeEdit());
    ...
    public List<ProfileField> getProfileFieldsWithInterest(int companyID, int adminID) {
		if (companyID <= 0) {
			...
			List<ProfileField> comProfileFieldList = select(SELECT_PROFILEFIELDS_BY_COMPANYID_HAVINGINTEREST, new ProfileFieldRowMapper(), companyID);
			CaseInsensitiveMap<String, DbColumnType> dbDataTypes = DbUtilities.getColumnDataTypes(getDataSource(), "customer_" + companyID + "_tbl");
			...
			for (ProfileField comProfileField : comProfileFieldList) {
				...
				if (found) {
					...
					ProfileFieldPermission permission = selectObjectDefaultNull(SELECT_PROFILEFIELDPERMISSION, new ProfileFieldPermissionRowMapper(), companyID, comProfileField.getColumn(), adminID);
	            	if (permission != null) {
	    				comProfileField.setAdminID(adminID);
	    				comProfileField.setModeEdit(permission.getModeEdit());
    ...
    public boolean saveProfileField(ProfileField field, Admin admin) throws Exception {
		...
		if (previousProfileField == null) {
			...
			if (getProfileFieldByShortname(field.getCompanyID(), field.getShortname()) != null) {
				throw new IllegalArgumentException("New shortname for customerprofilefield already exists");
			}
			...
			if (!createdDbField) {
				throw new IllegalStateException("DB-field could not be created");
			}
			...
			update(statementString, field.getCompanyID(), field.getColumn(), field.getAdminID(), field.getShortname().trim(), field.getDescription(), field.getModeEdit().getStorageCode(), field.getLine(), field.getSort(), field.getInterest(), field.getHistorize() ? 1 : 0, allowedValuesJson);
		} else {
			// Check if new shortname already exists before a new column is added to dbtable
			...
			if (field.getDataType() != null) {
    			...
    			if (!alteredDbField) {
    				throw new IllegalStateException("DB-field could not be changed");
    			}
			}
			...
			if (selectInt("SELECT COUNT(*) FROM " + TABLE + " WHERE " + FIELD_COMPANY_ID + " = ? AND LOWER(" + FIELD_COLUMN_NAME + ") = LOWER(?)", field.getCompanyID(), field.getColumn()) < 1) {
    			...
    			update("UPDATE " + TABLE + " SET " + FIELD_SHORTNAME + " = ?, " + FIELD_DESCRIPTION + " = ?, " + FIELD_MODE_EDIT + " = ?, " + FIELD_LINE + " = ?, " + FIELD_SORT + " = ?, " + FIELD_ISINTEREST + " = ?, " + FIELD_CHANGE_DATE + " = CURRENT_TIMESTAMP, " + FIELD_HISTORIZE + " = ?, " + FIELD_ALLOWED_VALUES + " = ? WHERE " + FIELD_COMPANY_ID + " = ? AND UPPER(" + FIELD_COLUMN_NAME + ") = UPPER(?)",
   					field.getShortname().trim(), field.getDescription(), field.getModeEdit().getStorageCode(), field.getLine(), field.getSort(), field.getInterest(), field.getHistorize() ? 1 : 0, allowedValuesJson, field.getCompanyID(), field.getColumn());
			}
		}
	...
	public boolean addColumnToDbTable(int companyID, String fieldname, String fieldType, long length, String fieldDefault, SimpleDateFormat fieldDefaultDateFormat, boolean notNull) throws Exception {
		if (companyID <= 0 || StringUtils.isBlank(fieldname) || StringUtils.isBlank(fieldType)) {
    		return false;
    	}

		if (DbUtilities.containsColumnName(getDataSource(), "customer_" + companyID + "_tbl", fieldname)) {
			return false;
		}

		if (!checkAllowedDefaultValue(companyID, fieldname, fieldDefault)) {
			throw new TooManyRowsForDefaultValueChangeException(configService.getIntegerValue(ConfigValue.MaximumNumberOfEntriesForDefaultValueChange, companyID));
		}

		if (("FLOAT".equalsIgnoreCase(fieldType) || "DOUBLE".equalsIgnoreCase(fieldType) || "NUMBER".equalsIgnoreCase(fieldType) || "INTEGER".equalsIgnoreCase(fieldType)) && StringUtils.isNotBlank(fieldDefault) && !AgnUtils.isDouble(fieldDefault)) {
			// check for valid numerical default value failed
			throw new IllegalArgumentException("Invalid non-numerical default value");
		}
    ...
    private void sortCustomComProfileList(List<ProfileField> listToSort) {
		Collections.sort(listToSort, (profileField1, profileField2) -> {
			if (profileField1.getSort() < MAX_SORT_INDEX && profileField2.getSort() < MAX_SORT_INDEX) {
				if (profileField1.getSort() < profileField2.getSort()) {
					return -1;
				}
				...
				if (profileField1.getSort() == profileField2.getSort()) {
					...
					return shortname1 != null ? 1 : -1;
				}

				return 1;
			}

			if (profileField1.getSort() < MAX_SORT_INDEX) {
				return -1;
			}

			if (profileField2.getSort() < MAX_SORT_INDEX) {
				return 1;
			}
    ...
    private class ProfileFieldRowMapper implements RowMapper<ProfileField> {
		@Override
		public ProfileField mapRow(ResultSet resultSet, int row) throws SQLException {
			...
			if (allowedValuesJson != null) {
				try {
					...
					logger.error("Error occurred while parsing allowed values for profile field: '{}' JSON: {}", readProfileField.getShortname(), allowedValuesJson, e);
				}
			}
    ...
    private static class LightProfileFieldRowMapper implements RowMapper<LightProfileField> {
		@Override
    ...
    private static class ProfileFieldPermissionRowMapper implements RowMapper<ProfileFieldPermission> {
		@Override
	...
	private boolean checkAllowedDefaultValue(int companyID, String fieldname, String fieldDefault) {
	...
	private void doPostProcessing(int companyID) throws RecipientProfileHistoryException {
		if (configService.isRecipientProfileHistoryEnabled(companyID)) {
			logger.info("Profile field history is enabled form company {} - starting post-processing of profile field structure modification", companyID);
