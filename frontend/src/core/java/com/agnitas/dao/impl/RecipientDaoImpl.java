...
import com.agnitas.util.SqlPreparedUpdateStatementManager;
...
public class RecipientDaoImpl extends PaginatedBaseDaoImpl implements RecipientDao {

	public static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	private static final SimpleDateFormat ISO_DATE_TIME_FORMAT = new SimpleDateFormat(DateUtilities.ISO_8601_DATETIME_FORMAT);

	private static final String[] MAILINGLIST_VALUE_FIELDS = {
		RecipientHistory.USER_TYPE,
		RecipientHistory.USER_STATUS,
		RecipientHistory.USER_REMARK,
		RecipientHistory.EXIT_MAILING_ID,
		RecipientHistory.EMAIL
	};

	private static final List<String> SUPPLEMENTAL_DATE_COLUMN_SUFFIXES = Arrays.asList(
			RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY,
	...
	public List<RecipientLiteImpl> getAdminAndTestRecipients(int companyID, int mailinglistID) {
		if (companyID > 0) {
			...
			return select(builder, new RecipientLiteRowMapper(), mailinglistID);
		} else {
	...
	public List<RecipientSalutationDto> getAdminAndTestRecipientsSalutation(int companyId, int adminId) {
		...
		String sql = "SELECT cust.customer_id, cust.email, cust.gender, cust.title, cust.firstname, cust.lastname"
		...
		return select(sql, (rs, i) -> new RecipientSalutationDto(
	...
	@Override
	public List<RecipientLiteImpl> listAdminAndTestRecipientsByAdmin(int companyID, int adminID) {
		if (companyID > 0) {
			...
			return select(sql.toString(), new RecipientLiteRowMapper(), params.toArray());
		}
	...
	public List<Integer> getAdminAndTestRecipientIds(int companyID, int mailinglistID) {
		if (companyID > 0) {
			...
			return select(sql, IntegerRowMapper.INSTANCE, UserType.Admin.getTypeCode(), UserType.TestUser.getTypeCode(), UserType.TestVIP.getTypeCode(), UserStatus.Active.getStatusCode(), mailinglistID);
		} else {
			return new ArrayList<>();
		}
	}
	...
	public int getAdminOrTestRecipientId(int companyID, int adminId) {
		if (companyID > 0) {
			...
			return selectInt(sql, params.toArray());
		} else {
			return 0;
		}
	}
	...
	public boolean updateDataSource(Recipient cust) {
		try {
			if (cust.hasCustParameter("DATASOURCE_ID")) {
				...
				int oldDatasourceID = selectIntWithDefaultValue(sql, 0, cust.getCustomerID());
				...
				if (oldDatasourceID == 0 && StringUtils.isNotEmpty(newDatasourceIDString)) {
					...
					update(sql, newDatasourceID, cust.getCustomerID());
	...
	public PaginatedList<Map<String, Object>> getPaginatedRecipientsData(int companyID, Set<String> columns, String statement, Object[] parameters, String sortColumn, boolean sortedAscending, int pageNumber, int pageSize) {
		...
		if (isRecipientsNumberExceedsLimit(totalRows, companyID)) {
			...
			if (isOracleDB()) {
				...
				modifiedSqlStatementForData = modifiedSqlStatementForData + " " + sortClause + " LIMIT ? OFFSET ?";
				parameters = AgnUtils.extendObjectArray(parameters, pageSize, (offset - pageSize));
			}
		}
	...
	protected List<Recipient> getRecipientList(int companyID, String statement, Object[] parameters) {
		...
		return select(statement, rowMapper, parameters);
	}
	...
	protected List<Map<String, Object>> getRecipientDataList(String statement, Object[] parameters) throws SQLException {
		try (Connection connection = getDataSource().getConnection()) {
			try {
				...
				logSqlStatement(statement, parameters);
				...
				logSqlError(e, statement, parameters);
				throw e;
			}
		}
	}
	...
	public int getNumberOfRecipients(int companyId, boolean ignoreBounceLoadValue) {
		...
		if (DbUtilities.checkIfTableExists(getDataSource(), tableName)) {
			if (ignoreBounceLoadValue) {
				return selectInt(String.format("SELECT COUNT(*) FROM %s", tableName));
			} else {
				return selectInt(String.format("SELECT COUNT(*) FROM %s WHERE %s = 0", tableName, RecipientStandardField.Bounceload.getColumnName()));
			}
		}
	...
	public int getNumberOfRecipients(int companyID, String statement, Object[] parameters) {
		...
		if (Strings.CI.startsWith(statement, "SELECT * FROM")) {
			selectTotalRows = Strings.CS.replaceOnce(statement, "SELECT * FROM", "SELECT COUNT(*) FROM");
		} else {
			selectTotalRows = "SELECT COUNT(*) FROM " + statement;
		}

		try {
			return selectInt(selectTotalRows, parameters);
		} catch (Exception e) {
	...
	@Override
	public int getNumberOfRecipients(int companyId, int mailingListId, String sqlConditions, Object... sqlConditionParameters) {
		return getNumberOfRecipients(companyId, mailingListId, null, sqlConditions, sqlConditionParameters);
	}

	@Override
	public int getNumberOfRecipients(int companyId, int mailingListId, List<MediaTypes> mediaTypes, String sqlConditions, Object... sqlConditionParameters) {
	...
	@Override
	public CaseInsensitiveMap<String, Object> getCustomerDataFromDb(int companyID, int customerID, DateFormat dateFormat) {
		return getCustomerDataFromDb(companyID, customerID, Collections.emptySet(), false, dateFormat);
	}
	...
	public CaseInsensitiveMap<String, Object> getCustomerDataFromDb(int companyID, int customerID, Collection<String> columns) {
		return getCustomerDataFromDb(companyID, customerID, columns, false, ISO_DATE_TIME_FORMAT);
	}

	@Override
	public List<CaseInsensitiveMap<String, Object>> getCustomersDataFromDb(int companyID, int lastId, int count, Collection<String> columns) {
		String query = "SELECT " + (CollectionUtils.isEmpty(columns) ? "*" : StringUtils.join(columns, ", ")) +
				" FROM " + getCustomerTableName(companyID) +
				" WHERE customer_id > ? AND " + RecipientStandardField.Bounceload.getColumnName() + " = 0" +
				" ORDER BY customer_id";

		return select(addRowLimit(query, count), lastId)
				.stream()
				.map(data -> processRecipientData(data, companyID, ISO_DATE_TIME_FORMAT))
				.toList();
	}
	...
	public CaseInsensitiveMap<String, Object> getCustomerDataFromDb(int companyID, int customerID, boolean respectHideSignIfSet) {
		return getCustomerDataFromDb(companyID, customerID, Collections.emptySet(), respectHideSignIfSet, ISO_DATE_TIME_FORMAT);
	}

	private CaseInsensitiveMap<String, Object> getCustomerDataFromDb(int companyID, int customerID, Collection<String> columns, boolean respectHideSignIfSet, DateFormat dateFormat) {
		String additionalWhereClause = " AND " + RecipientStandardField.Bounceload.getColumnName() + " = 0";
		if (respectHideSignIfSet && configService.getBooleanValue(ConfigValue.RespectHideDataSign, companyID)) {
		...
		try {
			List<Map<String, Object>> result = select(sql, customerID);

			if (!result.isEmpty()) {
				return processRecipientData(result.getFirst(), companyID, dateFormat);
			}
		} catch (Exception e) {
	...
	private CaseInsensitiveMap<String, Object> processRecipientData(Map<String, Object> data, int companyId, DateFormat dateFormat) {
		...
		CaseInsensitiveMap<String, RecipientFieldDescription> recipientFieldsMap = new CaseInsensitiveMap<>(
				recipientFields.stream().collect(Collectors.toMap(RecipientFieldDescription::getColumnName, Function.identity()))
		);
		...
		for (Map.Entry<String, Object> e : data.entrySet()) {
			...
			if (dateColumns.contains(columnName) || dateTimeColumns.contains(columnName)) {
				if (value == null) {
					...
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY, Integer.toString(calendar.get(Calendar.DAY_OF_MONTH)));
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH, Integer.toString(calendar.get(Calendar.MONTH) + 1));
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR, Integer.toString(calendar.get(Calendar.YEAR)));
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR, Integer.toString(calendar.get(Calendar.HOUR_OF_DAY)));
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE, Integer.toString(calendar.get(Calendar.MINUTE)));
					customerParameters.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND, Integer.toString(calendar.get(Calendar.SECOND)));
    ...
    public Map<String, Object> getRecipientData(int companyId, int recipientId, boolean respectHideSignIfSet) {
		...
		List<Map<String, Object>> result = select(sql, recipientId);
		if (!result.isEmpty()) {
			recipientData.putAll(result.getFirst());
		}
	...
	public int findByKeyColumn(Recipient customer, String keyColumn, String value) {
		try {
			...
			if (keyColumnDescription != null) {
				...
				if (keyColumnDescription.getSimpleDataType() == SimpleDataType.Characters) {
					if ("email".equalsIgnoreCase(keyColumn) && configService.getBooleanValue(ConfigValue.AllowUnnormalizedEmails, customer.getCompanyID())) {
						custList = select("SELECT customer_id FROM " + getCustomerTableName(customer.getCompanyID()) + " cust WHERE cust." + SafeString.getSafeDbColumnName(keyColumn) + " = ? AND " + RecipientStandardField.Bounceload.getColumnName() + " = 0", value);
					} else {
						custList = select("SELECT customer_id FROM " + getCustomerTableName(customer.getCompanyID()) + " cust WHERE LOWER(cust." + SafeString.getSafeDbColumnName(keyColumn) + ") = ? AND " + RecipientStandardField.Bounceload.getColumnName() + " = 0", value == null ? null : value.toLowerCase());
					}
				} else {
			...
			logger.error("Error in RecipientDaoImpl.findByKeyColumn()", e);
			customer.setCustomerID(0);
		}
	 ...
	 */
	@Override
	public int findByUserPassword(int companyID, String keyColumn, Object keyColumnValue, String passwordColumn, String passwordColumnValue) {
		if (Strings.CI.equals(keyColumn, "email")) {
			keyColumnValue = keyColumnValue.toString().toLowerCase();
		}
		...
		try {
			return selectInt(sql, keyColumnValue, passwordColumnValue);
		} catch (Exception e) {
			return 0;
		}
	}
	...
	public boolean isMailtrackingEnabled(int companyID) {
		...
		int mailtracking = selectInt(query, companyID);
		return mailtracking != 0;
	}

    @Override
    public List<RecipientMailing> getMailingsDeliveredToRecipient(int customerID, int companyID) {
    	if(!isMailtrackingEnabled(companyID)) {
       	...
       	final String sql = String.format(" SELECT "
            ...
            return select(sql, RecipientMailingRowMapper.INSTANCE, customerID, customerID, customerID, customerID);
    }
    ...
    public List<Map<String, Object>> getRecipientClicksHistory(int recipientId, int mailingId, int companyId) {
        ...
        return select(sql, recipientId, mailingId);
    }

	private static class RecipientMailingRowMapper implements RowMapper<RecipientMailing> {
		
		public static final RecipientMailingRowMapper INSTANCE = new RecipientMailingRowMapper();
		
		@Override
		public RecipientMailing mapRow(ResultSet resultSet, int i) throws SQLException {
			RecipientMailing recipientMailing = new RecipientMailingImpl();
	...
	public List<RecipientHistory> getRecipientProfileHistory(int recipientID, int companyID) {
		try {
			...
			logger.error("Error reading profile field history for company {}, recipient {}", companyID, recipientID, e);
			return new ArrayList<>();
		}
	}

	@Override
	public List<RecipientReaction> getRecipientReactionsHistory(int recipientID, int companyID) {
	 ...
	 */
	@Override
	public List<RecipientHistory> getRecipientBindingHistory(int recipientID, int companyID) {
		boolean companyHasBindingHistory = DbUtilities.checkIfTableExists(getDataSource(), "hst_customer_" + companyID + "_binding_tbl");
		List<RecipientHistory> recipientHistories = new ArrayList<>();
		
		// Create history entries for existing binding entries
		List<Map<String, Object>> allCurrentBindings = select("SELECT bind.mailinglist_id, bind.mediatype, bind.user_type, bind.user_status, bind.user_remark, bind.exit_mailing_id, bind.mediatype, customer.email, bind.creation_date"
		...
		for (Map<String, Object> bindingEntry : allCurrentBindings) {
			...
			List<Map<String, Object>> mailingListData = select("SELECT shortname, change_date, deleted FROM mailinglist_tbl WHERE mailinglist_id = ?", mailinglistID);
			...
			if (bindingChanges == null || bindingChanges.isEmpty()) {
				...
				recipientHistories.addAll(createChangeHistoryEntries(null, bindingEntry, mailinglistName, mediatype));
			} else {
				...
				while (iterator.hasNext()) {
					...
					recipientHistories.addAll(createChangeHistoryEntries(previous, next, mailinglistName, mediatype));
					previous = next;
				}
				recipientHistories.addAll(createChangeHistoryEntries(previous, bindingEntry, mailinglistName, mediatype));
			}
			
			if (mailinglistIsDeleted) {
				RecipientHistory recipientHistoryDeletedMailinglist = new RecipientHistoryImpl();
				recipientHistoryDeletedMailinglist.setChangeDate(mailinglistChangeDate);
				recipientHistoryDeletedMailinglist.setFieldName(RecipientHistory.MAILINGLIST_DELETED);
				recipientHistoryDeletedMailinglist.setMailingList(mailinglistName);
				recipientHistories.add(recipientHistoryDeletedMailinglist);
			}
		}
		
		// Create history entries for existing changes of already deleted binding entries
		List<Map<String, Object>> deletedBindingsData = select("SELECT DISTINCT hst.mailinglist_id, hst.mediatype"
		...
		for (Map<String, Object> deletedBinding : deletedBindingsData) {
			...
			List<Map<String, Object>> bindingChanges = select("SELECT *"
					+ " FROM hst_customer_" + companyID + "_binding_tbl"
					+ " WHERE customer_id = ? AND mailinglist_id = ? AND mediatype = ?"
					+ " ORDER BY timestamp_change ASC",
					recipientID, mailinglistID, mediatype);
			...
			RecipientHistory recipientHistoryDeletedBinding = new RecipientHistoryImpl();
			recipientHistoryDeletedBinding.setChangeDate((Date) previous.get("timestamp_change"));
			recipientHistoryDeletedBinding.setFieldName(RecipientHistory.CUSTOMER_BINDING_DELETED);
			recipientHistoryDeletedBinding.setMailingList(mailinglistName);
			recipientHistories.add(recipientHistoryDeletedBinding);
	...
	private List<RecipientHistory> createChangeHistoryEntries(Map<String, Object> previous, Map<String, Object> next, String mailinglistName, int mediatype) {
		ArrayList<RecipientHistory> recipientHistories = new ArrayList<>();
		
		if (previous == null) {
			Date changeDate = (Date) next.get("creation_date");
			
			RecipientHistory recipientHistoryUserType = new RecipientHistoryImpl();
			recipientHistoryUserType.setChangeDate(changeDate);
			recipientHistoryUserType.setFieldName(RecipientHistory.USER_TYPE);
			...
			recipientHistoryUserType.setNewValue(next.get(RecipientHistory.USER_TYPE));
			recipientHistories.add(recipientHistoryUserType);

			RecipientHistory recipientHistoryUserStatus = new RecipientHistoryImpl();
			recipientHistoryUserStatus.setChangeDate(changeDate);
			recipientHistoryUserStatus.setFieldName(RecipientHistory.USER_STATUS);
			...
			recipientHistoryUserStatus.setNewValue(next.get(RecipientHistory.USER_STATUS));
			recipientHistories.add(recipientHistoryUserStatus);

			RecipientHistory recipientHistoryRemark = new RecipientHistoryImpl();
			recipientHistoryRemark.setChangeDate(changeDate);
			recipientHistoryRemark.setFieldName(RecipientHistory.USER_REMARK);
			...
			recipientHistoryRemark.setNewValue(next.get(RecipientHistory.USER_REMARK));
			recipientHistories.add(recipientHistoryRemark);
		} else {
			...
			for (String mailinglistValueField : MAILINGLIST_VALUE_FIELDS) {
				...
				if (!RecipientHistory.EMAIL.equals(mailinglistValueField)) {
					...
					if ((previousValue == null && nextValue != null) || (previousValue != null && !previousValue.equals(nextValue))) {
						RecipientHistory recipientHistory = new RecipientHistoryImpl();
						...
						recipientHistories.add(recipientHistory);
					}
				}
			}
		}
	...
	protected String buildCustomerTimestamp(Recipient customer, String fieldName) {
		try {
			...
			int year = Integer.parseInt(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR));
			int hour = NumberUtils.toInt(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR));
			int minute = NumberUtils.toInt(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE));
			int second = NumberUtils.toInt(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND));
	...
	private static boolean hasTripleDateParameter(Recipient customer, String fieldName) {
		return customer.hasCustParameter(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY)
				&& customer.hasCustParameter(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH)
				&& customer.hasCustParameter(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR);
	}

	private SqlPreparedInsertStatementManager prepareInsertStatement(CaseInsensitiveMap<String, RecipientFieldDescription> customerTableStructure, Recipient customer) {
		...
		for (Entry<String, RecipientFieldDescription> entry : customerTableStructure.entrySet()) {
			...
			if (fieldName.equalsIgnoreCase("customer_id")) {
				...
				if (hasTripleDateParameter(customer, fieldName)) {
					...
					if (StringUtils.isNotBlank(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY))) {
                    ...
                    if (dateObj == null || StringUtils.isBlank(dateObj.toString())) {
						// Simple date field
						// Only default values are filled, other values must be set as split timestamp (fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY ...)
					...
					} else if (StringUtils.isNotBlank(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_FORMAT))) {
						Date date;
						try {
							SimpleDateFormat format = new SimpleDateFormat(customer.getCustParametersNotNull(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_FORMAT));
	...
	public List<Object> insertCustomers(int companyID, List<Recipient> recipients, List<Boolean> doubleCheck, List<Boolean> overwrite, List<String> keyFields) {
		try {
			...
			for (int i = 0; i < recipients.size(); i++) {
				...
				if (Boolean.FALSE.equals(doubleCheck.get(i))) {
					// Insert without duplicate check
					SqlPreparedInsertStatementManager insertStatementManager = prepareInsertStatement(recipientFieldsMap, customer);
					if (isOracleDB()) {
						int customerID = selectInt("SELECT customer_" + customer.getCompanyID() + "_tbl_seq.NEXTVAL FROM DUAL");
					...
					if (StringUtils.isEmpty(keyValue)) {
						...
						} else if (Boolean.TRUE.equals(overwrite.get(i))) {
							int customerID = customerIds.getFirst();
			...
			if (!currentSqlParameters.isEmpty()) {
				if (currentSqlStatement == null) {
					throw new IllegalStateException("currentSqlStatement was null");
				} else if (isOracleDB() || currentSqlStatement.toLowerCase().startsWith("update")) {
					batchupdate(currentSqlStatement, currentSqlParameters);
	...
	public void lockCustomers(int companyId, List<Integer> ids) {
		...
		if(!nonZeroIds.isEmpty()) {
			getJdbcTemplate().batchUpdate(insertCustomerId, new BatchPreparedStatementSetter() {
				...
				public void setValues(PreparedStatement ps, int i) throws SQLException {
					logger.fatal("i={}, uuid='{}', customer-id={}", i, uuid, nonZeroIds.get(i));
		...
		select(selForUpd, uuid.toString());
		update(deleteTemp, uuid.toString());
	}
		...
		try {
			...
			for (Recipient customer : recipients) {
				...
				if (customer.getCustomerID() == 0) {
					...
					SqlPreparedInsertStatementManager insertStatementManager = prepareInsertStatement(recipientFieldsMap, customer);
					if (isOracleDB()) {
						int customerID = selectInt("SELECT customer_" + companyID + "_tbl_seq.NEXTVAL FROM DUAL");
						insertStatementManager.addValue("customer_id", customerID);
					}
					nextSqlStatement = insertStatementManager.getPreparedSqlString();
					nextParameters = insertStatementManager.getPreparedSqlParameters();
				...
				if (currentSqlParameters.size() < 1000 && (currentSqlStatement == null || currentSqlStatement.equals(nextSqlStatement))){
					...
					for (int touchedLines : touchedLinesArray) {
						if(!(touchedLines > 0 || touchedLines == Statement.SUCCESS_NO_INFO)) {
							logger.warn("SQL batch update returned code {}\nStatement: {}\nParameters:{}",
									touchedLines, currentSqlStatement, currentSqlParameters);
						}
	...
	private void checkHtmlTags(Map<String, Object> recipientFields, int companyId) {
		boolean allowHtmlTags = configService.allowHtmlInReferenceAndProfileFields(companyId);
		for (Entry<String, Object> entry : recipientFields.entrySet()) {
			if (entry.getValue() instanceof String value) {
				HtmlChecker.checkTags(entry.getKey(), value, allowHtmlTags);
			}
		}
	}
	...
	@Override
	@DaoUpdateReturnValueCheck
	public int insertNewCustWithException(Recipient customer) {
		...
		final SqlPreparedInsertStatementManager insertStatementManager = prepareInsertStatement(recipientFieldsMap, customer);

		int customerID = 0;
		// Execute insert
		if (isOracleDB()) {
			// Set customerID for Oracle only, MySql gets it as an statement return value
			customerID = selectInt("SELECT customer_" + companyID + "_tbl_seq.NEXTVAL FROM DUAL");
			insertStatementManager.addValue("customer_id", customerID);
	...
	@Override
	public boolean updateInDbWithException(Recipient recipient) {
		return updateInDbWithException(recipient, true);
	}
	
	@Override
	public boolean updateInDbWithException(Recipient customer, boolean missingFieldsToNull) {
		checkRecipientDataValidity(customer);
	...
	private void checkRecipientDataValidity(Recipient customer) {
		...
		} else if (email instanceof String emailStr && StringUtils.isNotBlank(emailStr) && !AgnUtils.isEmailValid(emailStr)) {
	...
	public int getSizeOfCustomerDataFromDbList(int companyId, boolean matchAll, List<CriteriaEquals> criteriaEquals) {
		...
		try {
			...
			recipientResults = selectInt(sql, whereParameters.toArray());
	...
	@Override
	public int getSizeOfCustomerDataFromDbList(int companyId, String condition) {
		return 0;
	}
	...
	public List<Integer> getCustomerDataFromDb(int companyId, boolean matchAll, List<CriteriaEquals> criteriaEquals) {
		...
		try {
			...
			recipientResults = select(sql, IntegerRowMapper.INSTANCE, whereParameters.toArray());
		} catch (IllegalArgumentException e) {
			logger.error("getCustomerDataFromDb: {}", sql, e);
			throw e;
		} catch (Exception e) {
	...
	@Override
	public List<Integer> getCustomerDataFromDb(int companyId, String condition) {
		throw new UnsupportedOperationException("Method not available in OpenEMM");
	}
	...
	private StringBuilder initWhereParametersAndCreateSqlWhereCriteria(int companyId, boolean matchAll, List<CriteriaEquals> criteriaEquals, List<Object> whereParameters) {
		...
		for (CriteriaEquals criteriaEqualsElement : criteriaEquals) {
			...
			if (criteriaEqualsElement.getDateformat() != null && isPostgreSQL()) {
				sqlWhereCriteria.append("date_trunc('second', ").append(criteriaEqualsElement.getProfilefield()).append(") = ? ");
			} else {
				sqlWhereCriteria.append(criteriaEqualsElement.getProfilefield()).append(" = ? ");
			}
	...
	public void deleteCustomerDataFromDb(int companyID, int customerID) {
		update("DELETE FROM " + getCustomerBindingTableName(companyID) + " WHERE customer_id = ?", customerID);
		update("DELETE FROM " + getCustomerTableName(companyID) + " WHERE customer_id = ?", customerID);
	}
	...
	public Map<Integer, Map<Integer, BindingEntry>> loadAllListBindings(int companyID, int customerID) {
		...
		try {
			...
			List<Map<String, Object>> list = select(sqlGetLists);
	...
	public String getField(String selectVal, int recipientID, int companyID) {
		if (StringUtils.isNotBlank(selectVal) && recipientID > 0 && companyID > 0) {
			...
			try {
				List<Map<String, Object>> list = select(sql, recipientID);
	
				if (!list.isEmpty()) {
                    Object temp = list.getFirst().get("value");
					if (temp != null) {
	...
	public boolean deleteRecipients(int companyID, String target) {
		try {
			...
			if (isOracleDB() || isPostgreSQL()) {
				...
				execute("CREATE TABLE drop_cust_" + companyID + "_tbl" + tablespacePart + " AS SELECT customer_id FROM " + getCustomerTableName(companyID) + " cust WHERE " + target);
			} else {
				execute("CREATE TABLE drop_cust_" + companyID + "_tbl ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci AS SELECT customer_id FROM " + getCustomerTableName(companyID) + " cust WHERE " + target);
			}
			execute("CREATE INDEX dropCust" + companyID + "$cid$idx ON drop_cust_" + companyID + "_tbl (customer_id)");

			int stepsize = 200000;
			int touchedLines = 0;
			do {
				if (isOracleDB()) {
					touchedLines = update("DELETE FROM " + getCustomerBindingTableName(companyID) + " WHERE customer_id in (SELECT customer_id FROM drop_cust_" + companyID + "_tbl) AND ROWNUM <= ?", stepsize);
				} else if (isPostgreSQL()) {
					touchedLines = update("DELETE FROM " + getCustomerBindingTableName(companyID) + " WHERE customer_id IN (SELECT customer_id FROM drop_cust_" + companyID + "_tbl LIMIT " + stepsize + ")");
				} else {
					touchedLines = update("DELETE FROM " + getCustomerBindingTableName(companyID) + " WHERE customer_id in (SELECT customer_id FROM drop_cust_" + companyID + "_tbl) LIMIT " + stepsize);
				}
			...
			do {
				if (isOracleDB()) {
					touchedLines = update("DELETE FROM " + getCustomerTableName(companyID) + " WHERE customer_id in (SELECT customer_id FROM drop_cust_" + companyID + "_tbl) AND ROWNUM <= ?", stepsize);
				} else if (isPostgreSQL()) {
					touchedLines = update("DELETE FROM " + getCustomerTableName(companyID) + " WHERE customer_id IN (SELECT customer_id FROM drop_cust_" + companyID + "_tbl LIMIT " + stepsize + ")");
				} else {
					touchedLines = update("DELETE FROM " + getCustomerTableName(companyID) + " WHERE customer_id in (SELECT customer_id FROM drop_cust_" + companyID + "_tbl) LIMIT " + stepsize);
				}
			} while (touchedLines == stepsize);
			...
			try {
				...
				logger.error("Cannot drop temporary table drop_cust_{}_tbl", companyID, e);
			}
		}
	}
	...
	public void deleteAllNoBindings(int companyID, String toBeDeletedTable) {
		...
		String subQuery = "SELECT customer_id FROM " + getCustomerTableName(companyID)
		...
		String query = "DELETE FROM %s WHERE customer_id IN (%s)".formatted(getCustomerTableName(companyID), subQuery);
		if (isMariaDB()) {
			query += " LIMIT ?";
		}
		do {
			touchedLines = update(query, stepsize);
		} while (touchedLines == stepsize);
	...
	public String createTmpTableByMailinglistID(int companyID, int mailinglistID) {
		...
		update(String.format(sql, mailinglistID));
	...
	public void deleteRecipientsBindings(int mailinglistID, int companyID, boolean activeOnly, boolean notAdminsAndTests) {
		...
		StringBuilder suqQuery = new StringBuilder("SELECT customer_id FROM " + getCustomerBindingTableName(companyID) + " WHERE mailinglist_id = ?");

		if (activeOnly) {
			suqQuery.append(" ").append(String.format("AND user_status = %d", UserStatus.Active.getStatusCode()));
		}
	
		if (notAdminsAndTests) {
			suqQuery.append(" ").append(String.format("AND user_type <> '%s' AND user_type <> '%s' AND user_type <> '%s'", UserType.Admin.getTypeCode(), UserType.TestUser.getTypeCode(), UserType.TestVIP.getTypeCode()));
		}

		if (isOracleDB()) {
			suqQuery.append(" AND ROWNUM <= ?");
		} else if (isPostgreSQL()) {
			suqQuery.append(" LIMIT ?");
		}

		String query = "DELETE FROM %s WHERE customer_id IN (%s)".formatted(getCustomerBindingTableName(companyID), suqQuery.toString());
		...
		int touchedLines = 0;
		do {
			touchedLines = update(query, mailinglistID, stepSize);
		} while (touchedLines == stepSize);
	}
	...
	public Map<Integer, String> getAdminAndTestRecipientsDescription(int companyId, int mailingId) {
        List<Map<String, Object>> tmpList = select(getMailingAdminAndTestRecipientsSql(companyId), mailingId);
    ...
    @Override
    public List<RecipientLiteImpl> getMailingAdminAndTestRecipients(int mailingId, int companyId) {
        String sql = getMailingAdminAndTestRecipientsSql(companyId);
        return select(sql, new RecipientLiteRowMapper(), mailingId);
    }
	...
	public int getCustomerIdWithEmailInMailingList(int companyId, int mailingId, String email) {
		...
		try {
			List<Map<String, Object>> tmpList = select(sql, AgnUtils.normalizeEmail(email), mailingId);
			if (!tmpList.isEmpty()) {
	...
	public int deleteRecipients(int companyID, List<Integer> list) {
		if (list == null || list.isEmpty()) {
			throw new IllegalArgumentException("Invalid customerID list size");
		}

		String where = " WHERE " + makeBulkInClauseForInteger("customer_id", list);

		update("DELETE FROM " + getCustomerBindingTableName(companyID) + where);
		return update("DELETE FROM " + getCustomerTableName(companyID) + where);
	}

	@Override
	public boolean exist(int customerId, int companyId) {
		return selectInt("SELECT COUNT(*) FROM " + getCustomerTableName(companyId) + " WHERE customer_id = ?", customerId) > 0;
	}
			...
			for (RecipientFieldDescription recipientFieldDescription : recipientFields) {
				...
				if (SimpleDataType.Date == columnType || SimpleDataType.DateTime == columnType) {
					if (value == null) {
						...
						calendar.setTime((Date) value);
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY, Integer.toString(calendar.get(Calendar.DAY_OF_MONTH)));
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH, Integer.toString(calendar.get(Calendar.MONTH) + 1));
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR, Integer.toString(calendar.get(Calendar.YEAR)));
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR, Integer.toString(calendar.get(Calendar.HOUR_OF_DAY)));
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE, Integer.toString(calendar.get(Calendar.MINUTE)));
						params.put(columnName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND, Integer.toString(calendar.get(Calendar.SECOND)));
	...
	@Override
	public CaseInsensitiveMap<String, RecipientFieldDescription> getAvailableProfileFields(int companyID) {
		List<RecipientFieldDescription> recipientFields = recipientFieldService.getRecipientFields(companyID);
        return new CaseInsensitiveMap<>(recipientFields.stream().collect(Collectors.toMap(RecipientFieldDescription::getColumnName, Function.identity())));
	}
	...
	public int getDefaultDatasourceID(String username, int companyID) {
		...
		return selectInt(sql, username, companyID);
	}
	...
	public void updateForActionOperationUpdateCustomer(int companyID, String columnName, int updateType, String updateValue, int customerID) throws Exception {
		...
		StringBuilder updateStatement = new StringBuilder("UPDATE " + getCustomerTableName(companyID) + " SET timestamp = CURRENT_TIMESTAMP, " + columnName + " = ");
		Object value = null;
		if (columnType == SimpleDataType.Numeric || columnType == SimpleDataType.Float) {
			...
			if (updateType == ActionOperationUpdateCustomerParameters.TYPE_INCREMENT_BY) {
				if (isOracleDB()) {
					...
					updateStatement.append(columnName).append(" + make_interval(secs => ? * 86400)");
				} else {
				...
				if (matcher.matches()) {
					...
					if (matcher.group(2) != null) {
						...
						if (isOracleDB()) {
							...
							updateStatement.append(" make_interval(secs => ? * 86400)");
						} else {
							// 86400 seconds per day, this factor is used to adopt MySQL date calucation to Oracle date calculation
		...
		if (value == null) {
			update(updateStatement.toString(), customerID);
		} else {
			update(updateStatement.toString(), value, customerID);
		}
	}

	@Override
	public String selectCustomerValue(String selectValue, int companyID, int customerId) {
		return select("SELECT " + selectValue + " FROM " + getCustomerTableName(companyID) + " cust WHERE cust.customer_id = ?", String.class, customerId);
	}

	@Override
	public void batchUpdateData(List<Integer> customerIds, List<Map<String, ? extends Object>> rowsData, List<String> columns, int companyId) {
		...
		String sql = """
            UPDATE %s
            SET timestamp = CURRENT_TIMESTAMP, %s
            WHERE customer_id = ?
        """.formatted(getCustomerTableName(companyId), setClause);

    	List<Object[]> batchArgs = new ArrayList<>(customerIds.size());

		for (int i = 0; i < customerIds.size(); i++) {
			Map<String, ? extends Object> row = rowsData.get(i);
			List<Object> params = new ArrayList<>(columns.size() + 1);
			columns.forEach(col -> params.add(row.get(col)));
	...
	public int bulkUpdateEachRecipientsFields(int companyId, int adminId, int mailingListId, String sqlTargetExpression, Map<String, Object> updateValues) throws Exception {
		...
		update("INSERT INTO import_temporary_tables (session_id, temporary_table_name, import_table_name, host, description) VALUES(?, ?, ?, ?, ?)", "", "", "customer_" + companyId + "_tbl", AgnUtils.getHostName(), "Bulk recipient update");
		...
		try {
			...
			for(Map.Entry<String, Object> fieldData : updateValues.entrySet()) {
				...
				sqlUpdateStatement.append(isPostgreSQL() ? ", " : ", cust.").append(fieldName).append(" = ?");
				...
				try {
					int updated = update(sqlUpdateStatement.toString(), fieldParams.toArray());
			...
			update("DELETE FROM import_temporary_tables WHERE import_table_name = ? AND description = ?", "customer_" + companyId + "_tbl", "Bulk recipient update");
		}
	}
	...
	public int getAffectedRecipientsForBulkUpdateFields(int companyId, int adminId, int mailingListId, String targetExpression, Map<String, Object> updateValues) throws Exception {
		...
		for (Map.Entry<String, Object> fieldData : updateValues.entrySet()) {
			...
			try {
                uniqueIds.addAll(select(query.toString(), IntegerRowMapper.INSTANCE, params.toArray()));
			} catch (Exception e) {
				if (e.getCause() instanceof SQLDataException) {
					throw new ProfileFieldBulkUpdateException(fieldName, fieldValue, companyId, e.getCause());
				}
	...
	public int getRecipientsAmountForTargetGroup(int companyId, int adminId, int mailingListId, String sqlTargetExpression) {
		...
		return selectInt(wholeAmountStatement, parameters.toArray());
	}
	...
	protected String getWhereSubStatement(List<Object> parameters, int companyId, int adminId, int mailingListId, String sqlTargetExpression, boolean checkDisabledMailingLists) {
	...
	public RecipientDates getRecipientDates(int companyId, int recipientId) {
		...
		return selectObjectDefaultNull(sqlGetDates, new RecipientDatesRowMapper(), recipientId);
	}

	@Override
	public List<Integer> insertTestRecipients(int companyId, List<String> addresses) {
		...
		query(sqlGetCustomers, new EmailMapCallback(addressMap), addresses.toArray());
	...
	public String getEmail(int companyId, int customerId) {
		...
		return selectWithDefaultValue(query, String.class, null, customerId);
	}
	...
	@Override
	public void writeEmailAddressChangeRequest(int companyID, int customerID, String newEmailAddress, String confirmationCode) {
		...
		update(sql, companyID, customerID, newEmailAddress, confirmationCode);
	}
	
	@Override
	public String readEmailAddressForPendingChangeRequest(int companyID, int customerID, String confirmationCode) {
		...
		final List<String> list = select(sql, StringRowMapper.INSTANCE, companyID, customerID, confirmationCode);
	...
	@Override
	public void deletePendingEmailAddressChangeRequest(int companyID, int customerID, String confirmationCode) {
		final String sql = "DELETE FROM pending_email_change_tbl WHERE company_ref = ? AND customer_ref = ? AND confirmation_code = ?";
		
		update(sql, companyID, customerID, confirmationCode);
	}
	...
	private static class RecipientLiteRowMapper implements RowMapper<RecipientLiteImpl> {

		private final String columnPrefix;

		public RecipientLiteRowMapper(){
			columnPrefix = StringUtils.EMPTY;
		}

		@Override
		public RecipientLiteImpl mapRow(ResultSet resultSet, int row) throws SQLException {
			RecipientLiteImpl recipientLite = new RecipientLiteImpl();

			recipientLite.setId(resultSet.getInt(columnPrefix + "customer_id"));
			recipientLite.setEmail(resultSet.getString(columnPrefix + "email"));
			recipientLite.setFirstname(resultSet.getString(columnPrefix + "firstname"));
			recipientLite.setLastname(resultSet.getString(columnPrefix + "lastname"));

			return recipientLite;
		}
	}

	public static class RecipientReactionRowMapper implements RowMapper<RecipientReaction> {

		@Override
		public RecipientReaction mapRow(ResultSet rs, int rowNum) throws SQLException {
			RecipientReaction entry = new RecipientReactionImpl();
			...
			entry.setReactionType(RecipientReaction.ReactionType.getById(rs.getInt("action_type")));
	...
	@Override
	public List<CaseInsensitiveMap<String, Object>> getMailinglistRecipients(int companyID, int mailinglistID, MediaTypes mediaTypes, String targetSql, List<String> profileFieldsList, List<UserStatus> userstatusList, TimeZone timeZone) {
		...
		List<Map<String, Object>> result = select(sql, parameter.toArray(new Object[0]));
	...
	@Override
	public DbColumnType getColumnDataType(int companyId, String columnName) {
        return DbUtilities.getColumnDataType(getDataSource(), "customer_" + companyId + "_tbl", columnName);
	}

	@Override
	public List<Integer> getRecipientIDs(int companyID, String keyColumn, Object keyValue) {
		return select("SELECT customer_id FROM customer_" + companyID + "_tbl WHERE " + SafeString.getSafeDbColumnName(keyColumn) + " = ? AND " + RecipientStandardField.Bounceload.getColumnName() + " = 0", IntegerRowMapper.INSTANCE, keyValue);
	}
	...
	public boolean isRecipientMatchTarget(int companyId, String targetExpression, int customerId) {
		return selectInt("SELECT COUNT(cust.customer_id) FROM customer_" + companyId + "_tbl cust WHERE cust.customer_id = ? AND (" + targetExpression + ")", customerId) > 0;
	}

	@Override
	public boolean isNotSavedRecipientDataMatchTarget(int companyId, int recipientId, String targetExpression, Map<String, Object> entry) {
		if (companyId == 0) {
			return false;
		}
		...
		String sql = "SELECT COUNT(cust.customer_id) FROM (SELECT "
				+ StringUtils.join(columns, ", ")
				+ (isPostgreSQL() ? "" : " FROM DUAL") + ") cust WHERE "
				+ targetExpression;

		return selectInt(sql, sqlParameters.toArray()) > 0;
    }
	...
	public int saveRecipient(int companyId, int recipientId, Map<String, Object> recipientValues) {
		...
		if (recipientId > 0 && exist(recipientId, companyId)) {
			...
			checkHtmlTags(recipientValues, companyId);
	...
	@Override
	public List<Integer> listRecipientIdsByTargetGroup(int companyId, Target target) {
		final String sql = String.format("SELECT customer_id FROM customer_%d_tbl cust WHERE %s", companyId, target.getTargetSQL());
		
		return select(sql, IntegerRowMapper.INSTANCE);
	}

	@Override
	public List<Recipient> findByData(int companyID, Map<String, Object> searchDataMap) {
	...
	public List<Integer> findIdsByEmail(String email, int companyId) {
		return select("SELECT customer_id FROM " + getCustomerTableName(companyId) + " WHERE email = ?", IntegerRowMapper.INSTANCE, email);
	}

	@Override
	public void updateEmail(String email, int id, int companyId) {
		update("UPDATE " + getCustomerTableName(companyId) + " SET email = ? WHERE customer_id = ?", email, id);
	}
	...
	@Override
	/**
	 * TODO: check if the same!!! {@link BindingEntryDaoImpl#get(int, int, int, int)}.
	 * */
	public BindingEntry getMailinglistBinding(int companyID, int customerID, int mailinglistId, int mediaType) {
		List<Map<String, Object>> list = select("SELECT user_type, user_status, user_remark, timestamp, mediatype, exit_mailing_id FROM " + getCustomerBindingTableName(companyID) + " WHERE customer_id = ? AND mailinglist_id = ? AND mediatype = ?", customerID, mailinglistId, mediaType);
		if (list == null || list.isEmpty()) {
			return null;
		}

		if (list.size() != 1) {
			throw new IllegalStateException("Invalid number of binding entries (" + list.size() + ") found for customer: " + companyID + "/" + customerID);
		}
	...
	public int getMinimumCustomerId(int companyID) {
		return selectIntWithDefaultValue("SELECT MIN(customer_id) FROM customer_" + companyID + "_tbl", 0);
	}
	...
	public boolean isRecipientTrackingAllowed(int companyID, int recipientID) {
		...
		final int doNotTrack = selectIntWithDefaultValue(sql, 0, recipientID);

		return doNotTrack == 0;
	}

	@Override
	public List<CaseInsensitiveMap<String, Object>> getTargetRecipients(int companyID, String targetSql, List<String> profileFieldsList, TimeZone timeZone) {
		String fieldsPart = "customer_id";
		if (profileFieldsList != null && !profileFieldsList.isEmpty()) {
			fieldsPart = StringUtils.join(profileFieldsList, ", ");
		}
	...
	public List<Integer> getFilteredRecipientIDs(int companyID, CaseInsensitiveMap<String, RecipientFieldDescription> recipientFieldsMap, Map<String, String> recipientFilters) {
		...
		for (Entry<String, String> parameter : recipientFilters.entrySet()) {
			if (recipientFieldsMap.get(parameter.getKey()).getSimpleDataType() == SimpleDataType.Date) {
				if (isOracleDB() || isPostgreSQL()) {
					...
					sql += " AND DATE_FORMAT(" + SafeString.getSafeDbColumnName(parameter.getKey()) + ", '%Y-%m-%d') LIKE ?";
				}
		...
		return select(sql, IntegerRowMapper.INSTANCE, parameters.toArray(new Object[0]));
	}
	...
	public int countFilteredRecipientIDs(int companyID, CaseInsensitiveMap<String, RecipientFieldDescription> recipientFieldsMap, Map<String, String> recipientFilters) {
		...
		return selectInt(sql, parameters.toArray(new Object[0]));
	}

	@Override
	public List<String> getReadableNames(Set<Integer> ids, int companyID) {
		...
		return select(query, StringRowMapper.INSTANCE, ids.toArray());
	}
