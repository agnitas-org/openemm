...
public class ServerStatusDaoImpl extends BaseDaoImpl implements ServerStatusDao {
	...
	public ServerStatusDaoImpl(@Qualifier("dataSource") DataSource dataSource, JavaMailService javaMailService) {
	...
	public boolean checkDatabaseConnection() {
		try {
			int result = isPostgreSQL() ? selectInt("SELECT 1") : selectInt(CHECK_DB_CONNECTION);
			return result == 1;
	...
	public boolean checkDatabaseVersion(int majorVersion, int minorVersion, int microVersion, int hotfixVersion) {
		...
		try {
			...
			if (hotfixVersion > 0) {
				results = select((isOracleDB() || isPostgreSQL() ? SELECT_DB_VERSION_ORACLE_HOTFIX : SELECT_DB_VERSION_MYSQL_HOTFIX), majorVersion, minorVersion, microVersion, hotfixVersion);
			} else {
				results = select((isOracleDB() || isPostgreSQL() ? SELECT_DB_VERSION_ORACLE : SELECT_DB_VERSION_MYSQL), majorVersion, minorVersion, microVersion);
			}
	...
	public String getJobWorkerStatus(String jobWorkerName) {
		...
		if (row.get("laststart") == null) {
			return "Never ran";
		}
		if (((Date) row.get("laststart")).before(DateUtilities.getDateOfHoursAgo(1))) {
			return "Did not run within last hour";
		}
		if (!"OK".equals(row.get("lastresult"))) {
			return "Error";
		}
		return "OK";
	}
    ...
    public String getDbVendor() {
		...
		if (DbUtilities.checkDbVendorIsOracle(dataSource)) {
			...
			return "MySQL";
		} else if (DbUtilities.checkDbVendorIsPostgreSQL(dataSource)) {
			return "PostgreSQL";
		} else {
			return "N/A";
		}
    ...
    @Override
    public Map<String, String> getDbInformation() {
		DataSource dataSource = getDataSource();
		if (DbUtilities.checkDbVendorIsOracle(dataSource)) {
			String version = select("SELECT banner FROM v$version WHERE rownum = 1", String.class);
			return Map.of("db.version", "Oracle " + version);
			
		}
		if (isPostgreSQL()) {
			return Map.of("db.version", "PostgreSQL " + select("SELECT version()", String.class));
		}
	...
	public List<String> getErroneousImports(int maxUserImportDurationMinutes) {
		return select(isOracleDB()
			? """
			SELECT description || ' (Table ' || import_table_name || ')'
			FROM import_temporary_tables
			WHERE creation_date < ?"""
			: """
			SELECT CONCAT(description, ' (Table ', import_table_name, ')')
			FROM import_temporary_tables
			WHERE creation_date < ?""",
			StringRowMapper.INSTANCE,
			DateUtilities.getDateOfMinutesAgo(maxUserImportDurationMinutes));
	}
