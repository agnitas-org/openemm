...
public class TitleDaoImpl extends PaginatedBaseDaoImpl implements TitleDao {
	...
	public Title getTitle(int titleID, int companyID) {
		return selectObjectDefaultNull("SELECT company_id, title_id, description FROM title_tbl WHERE title_id = ? AND (company_id = ? OR company_id = 0)", new TitleRowMapper(), titleID, companyID);
	}
	...
	public List<Title> getTitles(int companyID, boolean includeGenders) {
		...
		return select(query, includeGenders ? new TitleRowMapper() : new TitleLight_RowMapper(), companyID);
	}
    ...
    private class TitleRowMapper implements RowMapper<Title> {
		@Override
		public Title mapRow(ResultSet resultSet, int row) throws SQLException {
			Title title = new TitleImpl();
			title.setId(resultSet.getInt("title_id"));
			...
			query(
					"SELECT gender, title FROM title_gender_tbl WHERE title_id = ?",
					rs -> genderMap.put(rs.getInt("gender"), rs.getString("title")),
					title.getId()
			);
