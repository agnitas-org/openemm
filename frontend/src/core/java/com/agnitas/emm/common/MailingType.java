...
public enum MailingType {
	...
	public static class NameDeserializer extends JsonDeserializer<MailingType> {
		@Override
		public MailingType deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
		...
		private static String getAllowedValuesStr() {
			return Arrays.stream(MailingType.values())
					.map(MailingType::name)
