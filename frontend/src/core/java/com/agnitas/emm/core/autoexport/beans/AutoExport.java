...
public class AutoExport {
    ...
    public static final String DEFAULT_EXPORT_FILENAME_PATTERN = "export_[CID]_[YYYY][MM][DD]-[HH][MI].csv";
	...
	public enum AutoExportType {
		 ...
		 */
		public static AutoExportType getAutoExportTypeFromStorageString(String storageString) {
			for (AutoExportType type : AutoExportType.values()) {
				if (type.getStorageString().equalsIgnoreCase(storageString)) {
					return type;
				}
			}
			throw new IllegalArgumentException("Invalid storage string '" + storageString + "' for AutoExportType");
		}
