...
@Component
public class BirtReportDtoToBirtReportConverter implements Converter<BirtReportDto, BirtReport> {
    
    @Autowired
    private BirtReportFactory birtReportSettingsFactory;

    @Override
    public BirtReport convert(BirtReportDto source) {
        BirtReport report = birtReportSettingsFactory.createReport();
    ...
    private Map<ReportSettingsType, BirtReportSettings> convertReportSettings(Map<ReportSettingsType, Map<String, Object>> settingsMap) {
        Map<ReportSettingsType, BirtReportSettings> map = new EnumMap<>(ReportSettingsType.class);
        for (ReportSettingsType type : ReportSettingsType.values()) {
             BirtReportSettings settings = birtReportSettingsFactory.createReportSettings(type);
             if (settings != null) {
