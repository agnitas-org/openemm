...
public class BirtReportServiceImpl implements BirtReportService {
        ...
        if (mailingGeneralType == MAILING_NORMAL) {
            if (mailingType == BirtReportMailingSettings.MAILING_PREDEFINED) {
                ...
                if (filterType == FilterType.FILTER_ARCHIVE.getKey() || filterType == FilterType.FILTER_MAILINGLIST.getKey() || filterType == FilterType.FILTER_TARGET.getKey()) {
                    filterValue = reportMailingSettings.getReportSettingAsInt(BirtReportSettings.PREDEFINED_ID_KEY);
                }
    ...
    protected void updateSelectedComparisonIds(BirtReportComparisonSettings reportComparisonSettings, DateTimeFormatter formatter, int companyId, int targetId, Set<Integer> adminAltgIds) {
        ...
        if (mailingType == BirtReportSettings.MAILINGS_PREDEFINED) {
            ...
            int filterValue = 0;
            if ((filterType == FilterType.FILTER_ARCHIVE.getKey() || filterType == FilterType.FILTER_MAILINGLIST.getKey() || filterType == FilterType.FILTER_TARGET.getKey())) {
                filterValue = reportComparisonSettings.getReportSettingAsInt(BirtReportSettings.PREDEFINED_ID_KEY);
            }
            if (numOfMailings >= 0) {
                    ...
                    if (dateTimeFormatter != null) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                throw new IllegalArgumentException("Unknown start date format", e2);
                            }
                        }
                        ...
                        try {
                            ...
                            try {
                                ...
                                throw new IllegalArgumentException("Unknown stop date format", e2);
                            }
                        }
                    }
                } else {
    ...
    public List<Campaign> getCampaignList(int companyId) {
        return campaignDao.getCampaignList(companyId, "upper( shortname )", 1);
    }
    ...
    @Override
    public Map<MailingType, List<MailingBase>> getFilteredMailings(Admin admin, Set<MailingType> mailingTypes) {
        Set<Integer> adminAltgIds = null;
        int altgId = 0;

        if (configService.isExtendedAltgEnabled(admin.getCompanyID())) {
            adminAltgIds = admin.getAltgIds();
        } else {
            altgId = getAltgId(admin);
        }

        Map<MailingType, List<MailingBase>> resultMap = mailingDao.getPredefinedMailingsForReports(
                admin.getCompanyID(),
                mailingTypes,
                altgId,
                adminAltgIds,
        ...
        mailingTypes.forEach(type -> resultMap.putIfAbsent(type, Collections.emptyList()));
