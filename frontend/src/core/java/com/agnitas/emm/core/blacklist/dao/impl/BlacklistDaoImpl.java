...
public class BlacklistDaoImpl extends BaseDaoImpl implements BlacklistDao {
	...
	public PaginatedList<BlackListEntry> getBlacklistedRecipients(BlacklistOverviewFilter filter, int companyID) {
		...
		List<BlackListEntry> blacklistElements = select(blackListQuery, new BlackListEntryRowMapper(), paramsCopy.toArray());
		...
		if (blacklistElements.isEmpty()) {
			...
			blacklistElements = select(blackListQuery, new BlackListEntryRowMapper(), params.toArray());
		}
	...
	public List<String> getBlacklist(int companyID) {
		try {
            ...
            logger.error("Error reading blacklist entries for company: {}", companyID);
    ...
    public List<BlackListEntry> getBlacklistedRecipients( int companyID) {
        ...
        return select(blackListQuery, new BlackListEntryRowMapper());
    }
	...
	protected static class BlackListEntryRowMapper implements RowMapper<BlackListEntry> {
		@Override
	...
	public List<BlackListEntry> getBlacklistCheckEntries(int companyID, String email) {
		...
		return select(sql, new BlackListEntryRowMapper(), AgnUtils.normalizeEmail(email));
	}
}
