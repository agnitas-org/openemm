...
public class CalendarCommentDaoImpl extends ReminderBaseDaoImpl implements CalendarCommentDao {
    ...
    public List<Reminder> getReminders(Date date) {
        ...
        if (isOracleDB()) {
            ...
            for (Map<String, Object> row : rows) {
                ...
                String adminTimezone = (String) row.get("admin_timezone");

                // When a timezone is the same as a server's one
                if ("SYSTEM".equalsIgnoreCase(adminTimezone)) {
                    adminTimezone = TimeZone.getDefault().getID();
                }

                int adminOffsetHours = TimeZone.getTimeZone(adminTimezone).getRawOffset() / 3600000;
                Calendar adminCalendar = Calendar.getInstance();
                adminCalendar.setTime(date);
                adminCalendar.setTimeZone(TimeZone.getTimeZone(adminTimezone));
    ...
    private Reminder createReminder(Map<String, Object> row) {
    ...
    private String getRemindersQuery(Date date) {
        ...
        if (isOracleDB()) {
            ...
            return sqlGetReminders + """
                 AND planned_send_date BETWEEN
                   TIMESTAMP '%s' - INTERVAL '1' DAY
                   AND
                   TIMESTAMP '%s' + INTERVAL '1' DAY
                """.formatted(dateForQuery, dateForQuery);
        } else {
            SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String dateForQuery = format.format(date);
