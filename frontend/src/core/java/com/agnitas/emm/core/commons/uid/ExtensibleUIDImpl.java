...
final class ExtensibleUIDImpl implements ExtensibleUID {
	 ...
	 */
 	...
 	private static int warnIfZero(final int value, final String warning) {
 		if(value == 0) {
 			logger.warn("Warning when creating new UID: {}", warning, new Exception(warning));
 		}
 		return value;
 	}
	...
	public ExtensibleUIDImpl(final ExtensibleUID uid) {
		...
		this.divId = uid.getDivId ();
		this.statusField = uid.getStatusField ();
		this.payload = uid.getPayload ();
	}
	...
	@Override
	public final ExtensibleUID setMediaID (int mediaID) {
	...
	@Override
	public ExtensibleUID setDivId (long divId) {
		this.divId = divId;
		return this;
	}
	...
	@Override
	public ExtensibleUID setPayload (byte[] payload) {
	...
	public String toString() {
		return "ExtensibleUIDImpl{" +
				...
				", payload=" + ((payload != null) && (payload.length > 0) ? payload.length + " bytes" : "unset") +
				'}';
	}
}
