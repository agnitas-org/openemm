...
public final class ExtensibleUIDStringBuilderChain implements ExtensibleUIDStringBuilder {
	...
	@Override
	public String buildUIDString(final ExtensibleUID extensibleUID) throws UIDStringBuilderException {
		...
		logger.info("Company {} has UID version {} enabled", extensibleUID.getCompanyID(), enabledUidversion);
		...
		for(final ExtensibleUIDStringBuilder stringBuilder : this.stringBuilderList) {
			...
			try {
				...
				if( uidString != null) {
					...
					logger.info("string builder {} didn't return a UID string", stringBuilder.getClass().getCanonicalName());
				}
			} catch( RequiredInformationMissingException e) {
				if( logger.isDebugEnabled()) {
					logger.debug("string builder {} was not able to build UID string", stringBuilder.getClass().getCanonicalName(), e);
				} else if( logger.isInfoEnabled()) {
					logger.info("string builder {} was not able to build UID string", stringBuilder.getClass().getCanonicalName());
				} 
			}
			/*
