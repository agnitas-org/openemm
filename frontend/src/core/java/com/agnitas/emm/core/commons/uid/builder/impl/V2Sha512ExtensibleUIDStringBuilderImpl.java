...
public class V2Sha512ExtensibleUIDStringBuilderImpl implements ExtensibleUIDStringBuilder {
	...
	private static final Logger logger = LogManager.getLogger(V2Sha512ExtensibleUIDStringBuilderImpl.class);
	...
	public V2Sha512ExtensibleUIDStringBuilderImpl() {
	...
	private ExtensibleUID workaroundMissingMailingId(final ExtensibleUID uid) {
		...
		if (getWorkaroundCompanyMailingCache().get(uid.getCompanyID()) != null) {
			...
			logger.info("No mailing ID was previously cached for company ID {} - reading from DB", uid.getCompanyID());
        ...
        return UIDFactory.copyWithNewMailingID(uid, mailingId);
	}
	
	private String makeBaseUID(final ExtensibleUID uid, final long timestamp) {
