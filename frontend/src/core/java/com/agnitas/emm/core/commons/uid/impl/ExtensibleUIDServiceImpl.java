...
import com.agnitas.emm.core.commons.uid.builder.impl.exception.BuildUIDException;
...
public class ExtensibleUIDServiceImpl implements ExtensibleUIDService {
    ...
    @Override
    public String buildUID(ExtensibleUID extensibleUID) {
        try {
            return buildUIDString(extensibleUID);
        } catch (Exception e) {
            throw new BuildUIDException("Error when building UID for: " + extensibleUID, e);
        }
    }
    ...
    private ExtensibleUID tryParseUid(String uid) {
        try {
            ...
            logger.error("Deprecated UID version of UID: {}", uid, e);
        } catch (UIDParseException e) {
            logger.error("Error parsing UID: {}", uid, e);
        } catch (InvalidUIDException e) {
            logger.error("Invalid UID: {}", uid, e);
        }
        return null;
    }
