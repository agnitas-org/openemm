 ...
 */
public final class ExtensibleUIDParserChain implements ExtensibleUIDParser {

    private static final Logger logger = LogManager.getLogger(ExtensibleUIDParserChain.class);

    /** List of parsers. */
    private List<ExtensibleUIDParser> parserList;

    public ExtensibleUIDParserChain() {
        parserList = new Vector<>();
    }

    @Override
    public final ExtensibleUID parse(final String uidString) throws UIDParseException, DeprecatedUIDVersionException {
        ...
        logger.info("Chain parser is trying to parse UID '{}'", uidString);

        for (final ExtensibleUIDParser parser : filteredParsers) {
            final String parserName = parser.getClass().getCanonicalName();
            
            logger.info("Trying to decode the uid by parser. uid: {}, parserName: {}", uidString, parserName);

            try {
                final ExtensibleUID uid = parser.parse(uidString);
                if (Objects.nonNull(uid)) {
                    logger.info("UID parsed successfully. uid: {}, parserName: {}", uidString, parserName);
                    return uid;
                }
            } catch (final UIDParseException e) {
                logger.info("UID '{}' could not be parsed by parser '{}'", uidString, parserName, e);
            	// Try next parser in chain
            } catch (final InvalidUIDException e) {
                logger.info("UID '{}' is invalid (parser '{}')", uidString, parserName, e);
            	// Try next parser in chain
            } catch (final DeprecatedUIDVersionException e) {
                logger.info("Version of UID '{}' is deprepcated (parser '{}')", uidString, parserName, e);
