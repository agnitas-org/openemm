...
public class CompanyInfoDao extends BaseDaoImpl {
	...
	public List<Integer> getAllAffectedCompaniesForConfigValue(String value){
		String sql = "SELECT company_id FROM company_info_tbl WHERE cname = ?";
		List<Map<String, Object>> results = select(sql, value);
		return results.stream()
				.map(r -> ((Number) r.get("company_id")).intValue())
