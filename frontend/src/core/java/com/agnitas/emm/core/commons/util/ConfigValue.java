...
public class ConfigValue {
	...
	public static final ConfigValue SystemMessageMaximumRetryCount = new ConfigValue("system_message.retries.maxCount", "10");

	/** Number of seconds between this send attempt and next retry. */
	public static final ConfigValue SystemMessageRetryDelaySeconds = new ConfigValue("system_message.retries.delaySeconds", "600");
...
// 1. System (system.url, ApplicationVersion, Saltfile, Cache, ...)
	...
	/** Do not read this value by configservice, so it is not cached (hostname = 1: host is active, 0: host is not active) **/
	...
	public static final class Honeypot {
		...
		/** defines if intermediate page will be shown at all **/
		public static final ConfigValue EnableIntermediatePage = new ConfigValue("honeypot.intermediatePage.enabled", "false");
		/** arbitrary name and value for link parameter to prevent intermediate page to be shown in endless loop **/
		...
		/** time period for which the treshold is relevant **/
		public static final ConfigValue TrackingPeriodDays = new ConfigValue("honeypot.tracking.periodDays", "90");
		/** number of occurrences until IP or network address is marked as bot address **/
		public static final ConfigValue TaggingThreshold = new ConfigValue("honeypot.tagging.threshold", "3");
	}
	...
	public static final class Voucher {
		private Voucher() {}
		public static final ConfigValue Vouchers_Mandatory = new ConfigValue("vouchers-mandatory", "false");
		public static final ConfigValue ReportEnable = new ConfigValue("report:enable[MandatoryVoucher]", "false");
		public static final ConfigValue ReportCc = new ConfigValue("report:cc[MandatoryVoucher]", "");
		public static final ConfigValue ReportBcc = new ConfigValue("report:bcc[MandatoryVoucher]", "");
	}

	/**
	 * If set to true: Use (random) DKIM key defined for company 0, if DKIM key for sender domain is missing
	 */
	public static final ConfigValue DkimGlobalActivation = new ConfigValue("dkim-global-key", "false");

	/**
	 * If set to true: Use (random) DKIM key of specific company, if DKIM key for sender domain is missing
	 */
	...
	/** Stores the name of attribute to look for in the SAML response to determine the email address of the user to be authenticated */
	public static final ConfigValue SsoLoginSamlEmailAttribute = new ConfigValue("system.SsoLoginSamlEmailAttribute");
	public static final ConfigValue SsoSamlIdpUrl = new ConfigValue("system.sso.saml.idp.url");
	public static final ConfigValue SsoExtendedLogs = new ConfigValue("system.sso.extended.logs", "false");
	...
	public static final ConfigValue UpsellingShow = new ConfigValue("company.upselling.show", "true");
	...
	public static final ConfigValue TextGeneratorModel = new ConfigValue("TextGeneratorModel", "gpt-5-mini");

	public static final ConfigValue AiSupportChatBotApiKey = new ConfigValue("system.AiSupportChatBotApiKey");
	public static final ConfigValue AiSupportChatBotPromptId = new ConfigValue("system.AiSupportChatBotPromptId");
	public static final ConfigValue AiSupportChatBotPromptVersion = new ConfigValue("system.AiSupportChatBotPromptVersion");
	/**
	 * Maximum number of messages to fetch from OpenAI when loading chat history.
	 * Note: OpenAI API allows a maximum of 100 messages per request.
	 * See: https://platform.openai.com/docs/api-reference/messages/listMessages
	 */
	public static final ConfigValue AiSupportChatMessagesHistoryLimit = new ConfigValue("ai.support.chat.messages.history.limit", "20");
	...
	public static final ConfigValue ImageGeneratorModel = new ConfigValue("ImageGeneratorModel", "gpt-image-1");
	...
	public static final ConfigValue DeliveryCheckAddress = new ConfigValue("system.DeliveryCheckAddress");

	public static final ConfigValue IntervalMailingStatisticsExpirationDays = new ConfigValue("mailing.interval.statistics.expiration", "30");

	/** if set, recipient overview in GUI shows only active recipients by default */
	public static final ConfigValue FilterRecipientsOverviewForActiveRecipients = new ConfigValue("recipients.overview.filterForActive", "false");

	public static final ConfigValue CleanAdminAndTestRecipientsActivities = new ConfigValue("recipients.adminAndTest.cleanActivity", "false");

	public static final ConfigValue IndividualLinkTrackingForAllMailings = new ConfigValue("mailings.individualLinkTracking", "false");
...
// 2. License
	...
	public static final ConfigValue System_License_AllowedPremiumConfigValues = new ConfigValue("allowedPremiumConfigValues");
...
// 4. General client settings (e.g. retention periods)
	...
	/** Default value for age of entries in mailloop_log_tbl **/
	public static final ConfigValue ExpireMailloopLog = new ConfigValue("expire.mailloopLog", "90");
...
// 5. Login & Security
	...
	/** Use iframe URL for login screen, even if check of URL is not successful (used by clients whose frontend does not have access to the Internet) **/
	public static final ConfigValue SkipLogonIframeUrlCheck = new ConfigValue("logon.iframe.url.skipCheck", "false");
	/** Flag decides if additional (splash) content is shown on login page or just the login mask **/
...
//   1. Mailing
		...
		public static final ConfigValue PreviewUrl = new ConfigValue("preview.url", "http://localhost:8080");
		...
		public static final ConfigValue UseLatestJoditEditor = new ConfigValue("useLatestJoditEditor", "true");
...
//   1. Links
		...
		public static final ConfigValue LinkChecker_UserAgent = new ConfigValue("linkchecker.userAgent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0");
...
//   4. Forms & Actions
		...
		ConfigValue(String name) {
			...
			for (ConfigValue existingValue : LIST_OF_ALL_CONFIGVALUES) {
				if (existingValue.name.equals(name)) {
					throw new IllegalArgumentException("Duplicate creation of configuration value: " + name);
				}
			}
		...
		public static ConfigValue getConfigValueByName(String name) {
			...
			throw new IllegalArgumentException("Unknown config value name: " + name);
		}
		...
		public final boolean equals(final Object obj) {
			if (obj instanceof ConfigValue configValue) {
				return name.equals(configValue.name);
			} else {
				return false;
			}
		}
