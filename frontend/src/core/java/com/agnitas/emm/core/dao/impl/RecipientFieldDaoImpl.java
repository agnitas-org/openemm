...
public class RecipientFieldDaoImpl extends BaseDaoImpl implements RecipientFieldDao {
	...
	public List<RecipientFieldDescription> getRecipientFields(int companyID) {
		...
		select("SELECT col_name, shortname, description, mode_edit, field_sort, line, isinterest, historize, allowed_values, creation_date, change_date"
			...
			new RecipientFieldRowMapper(recipientFieldsMap), companyID);
		...
		select("SELECT column_name, admin_id, mode_edit"
			...
			+ " WHERE company_id = ?",
			new RecipientFieldPermissionRowMapper(recipientFieldsMap), companyID);
		
		return recipientFieldsMap
			.values()
			.stream()
			.sorted(Comparator.comparing(RecipientFieldDescription::isStandardField).reversed()
				.thenComparing(RecipientFieldDescription::getSortOrder)
				.thenComparing(e -> e.getShortName() == null ? "" : e.getShortName())
				.thenComparing(RecipientFieldDescription::getColumnName))
			.collect(Collectors.toList());
	}

    private class RecipientFieldRowMapper implements RowMapper<RecipientFieldDescription> {

    	private final Map<String, RecipientFieldDescription> recipientFieldsMap;
    	
    	public RecipientFieldRowMapper(Map<String, RecipientFieldDescription> recipientFieldsMap) {
    ...
    private static class RecipientFieldPermissionRowMapper implements RowMapper<RecipientFieldDescription> {

    	private final Map<String, RecipientFieldDescription> recipientFieldsMap;
    	
    	public RecipientFieldPermissionRowMapper(Map<String, RecipientFieldDescription> recipientFieldsMap) {
    		this.recipientFieldsMap = recipientFieldsMap;
    	}
	...
	@Override
	public void saveRecipientField(int companyID, RecipientFieldDescription recipientFieldDescription) {
		...
		if (dbColumnType == null) {
			...
			if (!success) {
				throw new IllegalStateException("Creation of new database profile field failed");
			}

			insertRecipientField(recipientFieldDescription, columnName, companyID);
		} else {
			...
			if (defaultValue != null && !defaultValue.equals(recipientFieldDescription.getDefaultValue())
			...
			if (hasDescription) {
				...
				update("UPDATE customer_field_tbl SET shortname = ?, description = ?,"
					...
					recipientFieldDescription.isHistorized() ? 1 : 0,
	...
	private void insertRecipientField(RecipientFieldDescription fieldDescription, String columnName, int companyID) {
		ProfileFieldMode fallbackProfileFieldMode = (fieldDescription.getPermissions() == null || fieldDescription.getPermissions().get(0) == null) ? ProfileFieldMode.Editable : fieldDescription.getPermissions().get(0);
		update("INSERT INTO customer_field_tbl (company_id, col_name, shortname, description, mode_edit, field_sort, line, isinterest, historize, allowed_values, creation_date, change_date)"
						+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				companyID,
				fieldDescription.getColumnName().toLowerCase(),
				fieldDescription.getShortName(),
				fieldDescription.getDescription(),
				fallbackProfileFieldMode.getStorageCode(),
				fieldDescription.getSortOrder(),
				fieldDescription.getLine(),
				fieldDescription.getInterest(),
				fieldDescription.isHistorized() ? 1 : 0,
				getSerializedAllowedValues(fieldDescription),
				new Date(),
				new Date()
		);

		// Create or update recipient field admin permissions
		updateRecipientFieldPermissions(companyID, fieldDescription, columnName, fallbackProfileFieldMode);
	}
	...
	private void updateRecipientFieldPermissions(int companyID, RecipientFieldDescription recipientFieldDescription, String columnName, ProfileFieldMode fallbackProfileFieldMode) {
		...
		if (recipientFieldDescription.getPermissions() != null) {
			// Some old cases of profile field col_name may be stored in uppercase and cannot be changed because of FK relationships. Therefore use the stored name's letter case.
	...
	public boolean isReservedKeyWord(String fieldname) {
		if (isOracleDB()) {
			...
			List<String> keywords = select("SELECT word FROM pg_get_keywords()", StringRowMapper.INSTANCE);
			return new CaseInsensitiveSet(keywords).contains(fieldname);
		} else {
	...
	public boolean hasRecipients(int companyID) {
		return countCustomerEntries(companyID) > 0;
	}
	...
	@Override
	public int countCustomerEntries(int companyID) {
