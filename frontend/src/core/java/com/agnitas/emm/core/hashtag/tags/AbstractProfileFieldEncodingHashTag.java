...
public abstract class AbstractProfileFieldEncodingHashTag extends AbstractColonHashTag {
	...
	private static final Logger logger = LogManager.getLogger(AbstractProfileFieldEncodingHashTag.class);
	...
	protected AbstractProfileFieldEncodingHashTag() {
		this.encoderMap = initializeEncoderMap();
	}
	
	private static Map<String, ByteArrayEncoder> initializeEncoderMap() {
	...
	public final String handleInternal(final HashTagContext context, final String tagName, final String appendix) {
		...
		if(parts.length == 0) {
			...
			try {
				return postProcessResult(encodeProfileField(context, profileFieldName, encodingNames), tagName);
	...
	protected String postProcessResult(String string, String tagName) {
		return string;
	}
	
	private String encodeProfileField(HashTagContext context, String profileFieldName, String[] encodingNames) throws EncodingException, HashTagException {
		final String profileFieldValue = this.support.evaluateExpression(context, profileFieldName);
		
		byte[] data = profileFieldValue.getBytes(StandardCharsets.UTF_8);
		data = encodeData(data, encodingNames);
		
		return getByteArrayToStringEncoder().encodeToString(data, StandardCharsets.UTF_8);
	}
	
	private byte[] encodeData(byte[] data, String[] encodingNames) throws EncodingException {
	...
	private ByteArrayEncoder getByteArrayEncoder(String name) throws UnknownEncodingException {
	...
	public void setProfileFieldHashTagSupport(ProfileFieldHashTagSupport support) {
