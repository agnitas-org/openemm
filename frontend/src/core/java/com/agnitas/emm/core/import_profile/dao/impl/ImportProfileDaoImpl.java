...
public class ImportProfileDaoImpl extends BaseDaoImpl implements ImportProfileDao {
        ...
        public ImportProfile mapRow(ResultSet resultSet, int row) throws SQLException {
        	try {
	            ...
	            if (StringUtils.isNotBlank(resultSet.getString("report_locale_lang"))) {
	            	profile.setReportLocale(Locale.of(resultSet.getString("report_locale_lang"), resultSet.getString("report_locale_country")));
	            }
	...
	@Override
	public CaseInsensitiveMap<String, DbColumnType> getBlacklistColumnTypes(int companyId) {
		String tableName = "cust%d_ban_tbl".formatted(companyId);
		if (!DbUtilities.checkIfTableExists(dataSource,  tableName)) {
			return new CaseInsensitiveMap<>();
		}

		return DbUtilities.getColumnDataTypes(dataSource, tableName);
	}
}
