...
public class ProfileImportReporter {
	...
	public File writeProfileImportResultFile(ProfileImportWorker profileImportWorker, int companyID, Admin admin) {
		...
		try {
			...
			FileUtils.writeStringToFile(resultFile, resultFileContent, StandardCharsets.UTF_8);
			return resultFile;
		} catch (Exception e) {
	...
	public int writeProfileImportReport(ProfileImportWorker profileImportWorker, int companyID, Admin admin, Locale reportLocale, String reportTimezone, DateTimeFormatter reportDateTimeFormatter, boolean isError) {
		/*
	 ...
	 */
	public void sendProfileImportReportMail(ProfileImportWorker profileImportWorker, int companyID, Locale reportLocale, String reportTimezone, DateTimeFormatter reportDateTimeFormatter) {
		...
		if (!emailRecipients.isEmpty()) {
			...
			String subject = I18nString.getLocaleString("import.recipients.report", reportLocale) + " \"" + profileImportWorker.getImportProfile().getName() + "\" (" + I18nString.getLocaleString("Company", reportLocale) + ": " + company.getShortname() + ")";
...
//			}
	...
	private String generateLocalizedImportTextReport(ProfileImportWorker profileImportWorker, Locale reportLocale, String reportTimezone, DateTimeFormatter reportDateTimeFormatter, boolean showVerboseProfileData) {
		...
		if (showVerboseProfileData) {
			...
			if (MapUtils.isNotEmpty(profileImportWorker.getImportProfile().getGenderMapping())) {
		...
		if (MapUtils.isNotEmpty(mailinglistAssignStatistics)) {
			...
			for (MediaTypes mediaType : mailinglistAssignStatistics.keySet()) {
				if (mailinglistAssignStatistics.size() > 1) {
	...
	public String generateLocalizedImportHtmlReport(ProfileImportWorker importWorker, Locale reportLocale, String reportTimezone, DateTimeFormatter reportDateTimeFormatter, boolean showVerboseProfileData, ImportStatus status) {
		...
		if (importWorker.getStatus().getFatalError() != null) {
			...
			String errorMessage;
			if (importWorker.getError() instanceof ImportException importException) {
		...
		if (importWorker.getError() == null) {
			...
			if (!importWorker.getStatus().getErrorColumns().isEmpty()) {
			...
			if (MapUtils.isNotEmpty(mailinglistAssignStatistics)) {
				...
				for (MediaTypes mediaType : mailinglistAssignStatistics.keySet()) {
					if (mailinglistAssignStatistics.size() > 1) {
						htmlContent.append(mediaType.name()).append(":\n");
					}
					for (Entry<Integer, Integer> entry : mailinglistAssignStatistics.get(mediaType).entrySet()) {
		...
		if (showVerboseProfileData) {
			...
			if (MapUtils.isNotEmpty(importWorker.getImportProfile().getGenderMapping())) {
	...
	public List<ImportReportEntry> generateImportStatusEntries(ProfileImportWorker importWorker, boolean noHeaders) {
		...
		if (!customerImportStatus.getErrorColumns().isEmpty()) {
	...
	public void sendProfileImportErrorMail(ProfileImportWorker profileImportWorker, int companyID, Locale reportLocale, String reportTimezone, DateTimeFormatter reportDateTimeFormatter) {
		Set<String> emailRecipients = new HashSet<>();
