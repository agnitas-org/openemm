...
/**
 * Implementation of {@link LogonService}.
 */
public class LogonServiceImpl implements LogonService {

	private static final Logger logger = LogManager.getLogger(LogonServiceImpl.class);
	...
	private Admin doRegularLogin(String username, String password, String hostIpAddress) throws LogonServiceException {
		...
		if (adminOptional.isEmpty()) {
			logger.info("Login for user '{}' failed - invalid combination of username and password", username);
		...
		logger.info("Login for user {} successful", username);

		loginTrackService.trackLoginSuccessful(hostIpAddress, admin.getUsername());
	...
	protected void checkIPBlockState( int companyID, String loginName, String hostIpAddress) throws LogonFailedException {
		if (loginTrackService.isIpAddressLocked(hostIpAddress, companyID)) {
			logger.info("Login for user {} failed - IP address blocked", loginName);
	...
	public SimpleServiceResult requestPasswordReset(String username, String email, String clientIp, String linkPattern) {
		...
		if (passwordResetDao.getPasswordResetTokenHash(adminId) != null) {
			logger.info("Multiple password reset request for {}. Reset is already pending.", admin.getUsername());
	...
	public ServiceResult<Admin> resetPassword(String username, String token, String password, String clientIp) {
		...
		if(adminOptional.isEmpty()) {
			logger.info("Unknown username '{}'", username);
			loginTrackService.trackLoginFailed(clientIp, username);
	...
	private String getPasswordResetLink(String linkPattern, String username, String token) {
		...
		if (Strings.CI.startsWith(link, "http://") || Strings.CI.startsWith(link, "https://")) {
			return link;
		} else {
			return Strings.CS.removeEnd(baseUrl, "/") + "/" + Strings.CS.removeStart(link, "/");
		}
	}
	...
	private Admin getAdmin(String username, String email) {
		...
		if (Strings.CI.equals(admin.getEmail(), email)) {
			return admin;
		} else {
			return null;
		}
	...
	private Message asMessage(LicenseError e) {
		if (Strings.CS.equals(e.getErrorKey(), "error.license.outdated")) {
	...
	public void riseErrorCount(String username) {
        adminService.getAdminByName(username)
				.ifPresent(admin -> passwordResetDao.riseErrorCount(admin.getAdminID()));
	}
	...
	@Override
	public String getLoginUsername(Admin admin) {
