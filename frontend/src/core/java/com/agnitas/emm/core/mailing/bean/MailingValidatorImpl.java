...
public class MailingValidatorImpl implements MailingValidator {
    ...
    public MailingValidatorImpl(AgnDynTagGroupResolverFactory agnDynTagGroupResolverFactory,
                                ApplicationContext applicationContext,
                                TAGCheckFactory tagCheckFactory,
                                AgnTagService agnTagService,
                                LinkService linkService,
                                ConfigService configService,
                                MailingSendService mailingSendService, DkimDao dkimDao) {
    ...
    private void validateMailingComponents(Mailing mailing, Locale locale, Popups popups, List<String[]> errorReports) throws Exception {
        ...
        if (validateDeprecatedTags(mailing.getCompanyID(), components, popups) && mailing.getId() > 0) {
            ...
            try (TAGCheck tagCheck = tagCheckFactory.createTAGCheck(mailing.getCompanyID(), mailing.getId(), mailing.getMailinglistID(), locale)) {
     ...
     */
    @Override
    public void validateMailingAfterSave(Admin admin, Mailing mailing, Popups popups) {
        validateMailingModules(mailing, popups);
        validateNeedDkimKey(admin, mailing, popups);
    }
    ...
    @Override
    public boolean validateNeedDkimKey(Admin admin, Mailing mailing, Popups popups) {
        ...
        if (!configService.getBooleanValue(ConfigValue.DkimGlobalActivation, companyID)
            || dkimDao.existsDkimKeyForDomain(companyID, senderDomain)) {
            return true;
        }
