...
public class MailingParameterDaoImpl extends PaginatedBaseDaoImpl implements MailingParameterDao {
	...
	public List<MailingParameter> getAllParameters(int companyID) {
		return select("SELECT mailing_info_id, mailing_id, company_id, name, value, description, change_date, change_admin_id, creation_date, creation_admin_id FROM mailing_info_tbl WHERE company_id = ? ORDER BY creation_date", new MailingParameterRowMapper(), companyID);
	}
	
	@Override
	public List<MailingParameter> getMailingParameters(int companyID, int mailingID) {
		return select("SELECT mailing_info_id, mailing_id, company_id, name, value, description, change_date, change_admin_id, creation_date, creation_admin_id FROM mailing_info_tbl WHERE company_id = ? AND mailing_id = ? ORDER BY creation_date", new MailingParameterRowMapper(), companyID, mailingID);
	}
	...
	public PaginatedList<MailingParameter> getParameters(MailingParamOverviewFilter filter, int companyID) {
		...
		PaginatedList<MailingParameter> list = selectPaginatedList(query.toString(), "mailing_info_tbl", filter.getSortOrDefault("creation_date"),
				filter.ascending(), filter.getPage(), filter.getNumberOfRows(), new MailingParameterRowMapper(), params.toArray());
	...
	public MailingParameter getParameter(int mailingInfoID) {
		return selectObjectDefaultNull("SELECT mailing_info_id, mailing_id, company_id, name, value, description, change_date, change_admin_id, creation_date, creation_admin_id FROM mailing_info_tbl WHERE mailing_info_id = ?", new MailingParameterRowMapper(), mailingInfoID);
	}
	...
	private class MailingParameterRowMapper implements RowMapper<MailingParameter> {
		@Override
	...
	@Override
	public MailingParameter getParameterByName(String parameterName, int mailingId, int companyId) {
        return getParameterByNameDefaultNull(parameterName, mailingId, companyId);
	}
	...
	private MailingParameter getParameterByNameDefaultNull(String parameterName, int mailingId, int companyId){
		List<MailingParameter> list = select(SELECT_BY_NAME, new MailingParameterRowMapper(), companyId, mailingId, parameterName);

		if (list.isEmpty()) {
			list = select(SELECT_BY_NAME, new MailingParameterRowMapper(), companyId, 0, parameterName);
		}
