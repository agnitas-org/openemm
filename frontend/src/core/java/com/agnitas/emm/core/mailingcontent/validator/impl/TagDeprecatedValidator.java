...
public class TagDeprecatedValidator implements DynTagValidator {
    ...
    public boolean validate(DynTagDto dynTagDto, Popups popups, Admin admin) {
        dynTagDto.getContentBlocks()
                .stream()
                .flatMap(block -> agnTagService.parseDeprecatedTagNamesFromString(block.getContent(), admin.getCompanyID()).stream())
                .distinct()
                .forEach(tagName -> popups.warning("warning.mailing.agntag.deprecated", tagName));
