...
public class MailloopServiceImpl implements MailloopService {
	...
	private static final Logger logger = LogManager.getLogger(MailloopServiceImpl.class);
	...
	@Override
	public void sendAutoresponderMail(int mailloopID, int companyID, int customerID, String securityToken) {
		logger.info("Called sendAutoresponderMail() - mailloop {}, company {}, customer {}", mailloopID, companyID, customerID);

		final Mailloop mailloop = this.mailloopDao.getMailloop(mailloopID, companyID);

		if(mailloop == null || mailloop.getId() == 0) {
			logger.info("Unknown mailloop {} for company {}", mailloopID, companyID);
	 ...
	 */
	private void sendAutoresponderMail(Mailloop mailloop, int customerID) {
	 ...
	 * @throws UnableToSendAutoresponderMailingException on errors sending auto-responder mailing
	 */
	private void sendAutoresponderAsActionbasedMailing(Mailloop mailloop, int customerID) {
