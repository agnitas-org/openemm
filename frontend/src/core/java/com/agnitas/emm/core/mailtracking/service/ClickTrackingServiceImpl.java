...
public final class ClickTrackingServiceImpl implements ClickTrackingService {
	...
	private static final Logger logger = LogManager.getLogger(ClickTrackingServiceImpl.class);
		...
		if(uid == null) {
			...
			if(link != null) {
				...
				if(trackingLevel == TrackingLevel.ANONYMOUS) {
					logger.info("Recipient {} disagreed tracking", uid.getCustomerID());
				}
				...
				if(!result) {
					logger.warn("Could not track click on link {} for customer {} (company ID {})", uid.getUrlID(), uid.getCustomerID(), uid.getCompanyID());
				}

				notifyOnLinkClickedHandlers(uid.getCompanyID(), uid.getMailingID(), trackedRecipientID, uid.getUrlID());
			} else {
				logger.warn("Trackable link {} for company {} not found", uid.getUrlID(), uid.getCompanyID(), new Exception("Trackable link not found"));
			}
		}
	}
	...
	private final void notifyOnLinkClickedHandlers(final int companyID, final int mailingID, final int customerID, final int linkID) {
		for(final OnLinkClickedHandler handler : this.linkClickedHandlerList) {
			try {
				...
				logger.warn("OnLinkClickedHandler of type '{}' threw an exception (company ID {}, mailing ID {}, customer ID {}, link ID {})",
						handler.getClass().getCanonicalName(), companyID, mailingID, customerID, linkID, e);
			}
		}
	}
