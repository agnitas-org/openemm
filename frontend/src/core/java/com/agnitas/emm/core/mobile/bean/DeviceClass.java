...
public enum DeviceClass {
    ...
    DeviceClass(int id) {
        this.id = id;
        this.isUnknown = this.getName().startsWith("UNKNOWN_");
    }
    ...
    public static DeviceClass fromIdWithDefault(int id, DeviceClass defaultClass) {
        if (id > 0) {
            for (DeviceClass deviceClass : DeviceClass.values()) {
    ...
    public String getMessageKey() {
        return isUnknown
                ? "Unknown"
                : "report.device.%s".formatted(this.name().toLowerCase());
    }
