...
final class HyperlinkHelper {
	...
	public static String toHyperlink(ObjectUsage usage, Locale locale) {
        return switch (usage.getType()) {
            case TARGET_GROUP -> targetGroupHyperlink(usage, locale);
            case WORKFLOW -> workflowHyperlink(usage, locale);
            case TRIGGER -> triggerHyperlink(usage, locale);
            case MAILING -> mailingHyperLink(usage, locale);
            case MAILINGLIST -> mailinglistHyperlink(usage, locale);
            case EXPORT_PROFILE -> exportProfileHyperlink(usage, locale);
            case CLASSIC_MAILING_CONTENT -> classicMailingContentHyperLink(usage, locale);
            case EMC_CONTENT -> emcContentHyperLink(usage, locale);
            case BIRT_REPORT -> birtReportHyperlink(usage, locale);
        };
	}
