...
import com.agnitas.post.PostalDeField;
...
public final class ProfileFieldServiceImpl implements ProfileFieldService {
    ...
    @Override
    public void createPostalDeFieldsIfNotExist(Admin admin) {
        for (PostalDeField postalField : PostalDeField.values()) {
            if (!exists(admin.getCompanyID(), postalField.getName())) {
                ProfileField profileField = new ProfileFieldImpl();

                profileField.setCompanyID(admin.getCompanyID());
                profileField.setColumn(postalField.getName());
                profileField.setDataType("VARCHAR");
                profileField.setDataTypeLength(postalField.getLength());

                createNewField(profileField, admin);
            }
        }
    }
