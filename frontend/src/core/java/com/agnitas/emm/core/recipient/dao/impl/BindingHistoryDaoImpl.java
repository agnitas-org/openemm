	...
	public void recreateBindingHistoryTrigger(int companyID) {
		if (isOracleDB()) {
			...
			String query = """
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            PERFORM emm_log_db_errors(SQLERRM, %d, 'hst_customer_%d_bind_delete_trigger');
                            RETURN OLD;
                    END;
                    $$ LANGUAGE plpgsql;
                    ...
                    """.formatted(companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID);

			execute(query);

			query = """
                    DROP TRIGGER IF EXISTS hst_customer_%d_bind_update_trigger
                        ON customer_%d_binding_tbl;
                    DROP FUNCTION IF EXISTS hst_customer_%d_bind_update_func();
                
                    CREATE OR REPLACE FUNCTION hst_customer_%d_bind_update_func()
                        RETURNS TRIGGER AS $$
                    DECLARE
                        v_hist  text;
                        v_email text;
                    BEGIN
                        ...
                        IF (OLD.user_type IS DISTINCT FROM NEW.user_type)
                           OR (OLD.user_status IS DISTINCT FROM NEW.user_status)
                           OR (OLD.mediatype IS DISTINCT FROM NEW.mediatype) THEN
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            PERFORM emm_log_db_errors(SQLERRM, %d, 'hst_customer_%d_bind_update_trigger');
                            RETURN NEW;
                    END;
                    $$ LANGUAGE plpgsql;
                
                    CREATE TRIGGER hst_customer_%d_bind_update_trigger
                        BEFORE UPDATE ON customer_%d_binding_tbl
                        FOR EACH ROW
                        EXECUTE FUNCTION hst_customer_%d_bind_update_func();
                    """.formatted(companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID, companyID);
