...
public interface RecipientService {
	...
	void checkColumnsAvailable(RecipientModel model);
    ...
    Recipient getRecipient(RecipientModel model);
	
    Recipient getRecipient(int companyID, int customerID);

	List<Recipient> getRecipients(int count, int lastId, int companyId, Set<String> columnNames);
	...
	/**
	 * Performs a batch update of recipient data.
	 *
	 * <p><b>Important:</b> All entries in the {@code data} map must contain
	 * the <b>same set of columns</b>. The column set and its order are used
	 * to build a single SQL {@code UPDATE} statement for batch execution.
	 *
	 * <p>If a value for a column cannot be computed for a specific recipient,
	 * the column must still be present in the map with a {@code null} value.
	 *
	 * @param data
	 *        map where the key is {@code customer_id} and the value is a map
	 *        of column names to values to be updated; all inner maps must
	 *        have identical keys
	 * @param companyId
	 *        company identifier used to resolve the customer table
	 */
    void batchUpdateData(Map<Integer, Map<String, ? extends Object>> data, int companyId);

	void updateRecipientWithEmailChangeConfiguration(Recipient recipient, int mailingID, String profileFieldForConfirmationCode) throws SendActionbasedMailingException;

	void confirmEmailAddressChange(ExtensibleUID uid, String confirmationCode);
	...
	PaginatedList<RecipientDto> getPaginatedRecipientList(Admin admin, RecipientSearchParamsDto searchParams, String sort, String order, int page, int rownums, Map<String, String> fields) throws Exception;

	PaginatedList<RecipientDto> getPaginatedDuplicateList(Admin admin, String searchFieldName, boolean caseSensitive, String sort, String order, int page, int rownums, Map<String, String> fields);
	
	List<Integer> listRecipientIdsByTargetGroup(int targetId, int companyId);
	...
	void updateBindingsFromRequest(Recipient recipient, Map<String, Object> params, boolean doubleOptIn, String remoteAddr, String referrer);
	...
	boolean updateRecipientInDB(Recipient recipient);

	void deleteCustomerDataFromDb(int companyId, int customerId);

	int saveNewCustomer(Recipient recipient);
	...
	List<String> getReadableNames(Set<Integer> bulkIds, int companyID);
    ...
    CaseInsensitiveMap<String, Object> getFields(int recipientId, int companyId);

    List<CaseInsensitiveMap<String, Object>> getFields(List<Integer> recipientIds, int companyId);

    List<Integer> getFilteredRecipientIDs(
			int companyId,
