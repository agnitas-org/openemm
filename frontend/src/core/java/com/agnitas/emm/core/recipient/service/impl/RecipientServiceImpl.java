...
import com.agnitas.emm.core.recipient.exception.RecipientUpdateException;
...
public class RecipientServiceImpl implements RecipientService {
	...
	@Override
	public List<Recipient> getRecipients(int count, int lastId, int companyId, Set<String> columnNames) {
		if (!recipientDao.tableExists(companyId)) {
			return Collections.emptyList();
		}

		return recipientDao.getCustomersDataFromDb(companyId, lastId, count, columnNames)
				.stream()
				.map(data -> {
					Recipient recipient = recipientFactory.newRecipient(companyId);
					recipient.setCustParameters(data);
	...
	public List<Integer> getSubscribers(RecipientsModel model) {
		if (model.getEql() == null) {
			return recipientDao.getCustomerDataFromDb(model.getCompanyId(), model.isMatchAll(), model.getCriteriaEquals());
		}

		try {
			SqlCode sqlCode = eqlFacade.convertEqlToSql(model.getEql(), model.getCompanyId());
			return recipientDao.getCustomerDataFromDb(model.getCompanyId(), sqlCode.getSql());
		} catch (EqlParserException | CodeGeneratorException | ReferenceTableResolveException | ProfileFieldResolveException e) {
			logger.warn("Error getting recipients (eql={})", model.getEql(), e);
			return Collections.emptyList();
		}
	}
	...
	public int getSubscribersSize(RecipientsModel model) {
		...
		try {
			SqlCode sqlCode = eqlFacade.convertEqlToSql(model.getEql(), model.getCompanyId());
			return recipientDao.getSizeOfCustomerDataFromDbList(model.getCompanyId(), sqlCode.getSql());
		} catch (EqlParserException | CodeGeneratorException | ReferenceTableResolveException| ProfileFieldResolveException e) {
			logger.warn("Error counting recipients (eql={})", model.getEql(), e);
			return 0;
		}
	}
	...
	public boolean update(Recipient recipient) {
		return recipientDao.updateInDbWithException(recipient, false);
	}

	@Override
	public void batchUpdateData(Map<Integer, Map<String, ? extends Object>> data, int companyId) {
		if (data.isEmpty()) {
			return;
		}

		List<String> columns = List.copyOf(data.values().iterator().next().keySet());
		List<Integer> customerIds = new ArrayList<>(data.size());
		List<Map<String, ? extends Object>> rowsData = new ArrayList<>(data.size());

		data.forEach((customerId, rowData) -> {
			customerIds.add(customerId);
			rowsData.add(rowData);
		});

		recipientDao.batchUpdateData(customerIds, rowsData, columns, companyId);
	}

	@Override
	public void updateRecipientWithEmailChangeConfiguration(Recipient recipient, int mailingID, String profileFieldForConfirmationCode) throws SendActionbasedMailingException {
		...
		final boolean emailChanged = !Strings.CS.equals(currentData.get("email").toString(), recipient.getCustParametersNotNull("email"));
		...
		if (emailChanged) {
			...
			if (!this.recipientDao.updateInDbWithException(recipient)) {
				throw new RecipientUpdateException("Error saving recipient %d (company ID %d)".formatted(recipient.getCustomerID(), recipient.getCompanyID()));
			}
	...
	@Transactional
	@Override
	public void confirmEmailAddressChange(ExtensibleUID uid, String confirmationCode) {
		final String newEmailAddress = this.recipientDao.readEmailAddressForPendingChangeRequest(uid.getCompanyID(), uid.getCustomerID(), confirmationCode);
		if (newEmailAddress == null) {
			throw new IllegalStateException(String.format("No pending change request for email address of customer %d (company %d, confirmation code '%s')", uid.getCustomerID(), uid.getCompanyID(), confirmationCode));
		}
		...
		if (!recipientDao.updateInDbWithException(recipient)) {
			throw new RecipientUpdateException(
					"Cannot change email address for pending change requence (customer %d, company %d, confirmation code '%s')"
							.formatted(uid.getCustomerID(), uid.getCompanyID(), confirmationCode)
			);
		}
	...
	private void supplySourceID(Recipient recipient, final String username, final int companyID) {
		...
		if (!recipient.hasCustParameter("DATASOURCE_ID")) {
			logger.trace("Set default datasource_id = {} for recipient with email {}", defaultId, recipient.getEmail());
			recipient.getCustParameters().put("DATASOURCE_ID", String.valueOf(defaultId));
		} else if (logger.isTraceEnabled()) {
			logger.trace("Set datasource_id = {} from parameter for recipient with email {}", recipient.getCustParametersNotNull("DATASOURCE_ID"), recipient.getEmail());
		}
	...
	private SqlPreparedStatementManager prepareBaseOverviewStatement(RecipientSqlOptions options, Admin admin, boolean useCountQuery) {
				...
				if (StringUtils.isNotBlank((String) caseInsensitiveParameters.get(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_FORMAT))) {
					...
					if (StringUtils.isNotBlank(value)) {
						try {
							...
							logger.error("Invalid value for customer field '{}' with expected format '{}'",
									entry.getKey(), caseInsensitiveParameters.get(entry.getKey() + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_FORMAT));
						}
					}
				} else {
...
//	 * @param tafWriteBack
	 ...
	 */
	@Override
	public void updateBindingsFromRequest(Recipient recipient, Map<String, Object> params, boolean doubleOptIn, String remoteAddr, String referrer) {
		...
		for (Entry<String, Object> entry : requestParameters.entrySet()) {
			if (Strings.CI.startsWith(entry.getKey(), "agnSUBSCRIBE")) {
				String postfix = "";
				...
				if (StringUtils.isBlank(agnSubscribeString)) {
					throw new IllegalArgumentException("Mandatory subscribeStatus (form-param: agnSUBSCRIBE) is missing: " + getRequestParameterString(params));
				}

				try {
					subscribeStatus = Integer.parseInt(agnSubscribeString);
				} catch (Exception e) {
					throw new IllegalArgumentException("Mandatory subscribeStatus (form-param: agnSUBSCRIBE) is missing or invalid: " + getRequestParameterString(params));
				}
				if (subscribeStatus != 0 && subscribeStatus != 1) {
					throw new IllegalArgumentException("Mandatory subscribeStatus (form-param: agnSUBSCRIBE) is invalid: " + getRequestParameterString(params));
				}
				...
				if (StringUtils.isBlank(agnMailinglistString)) {
					throw new IllegalArgumentException("Mandatory mailinglistID (form-param: agnMAILINGLIST) is missing: " + getRequestParameterString(params));
				}
				...
				try {
					...
					throw new IllegalArgumentException("Mandatory mailinglistID (form-param: agnMAILINGLIST) is missing or invalid: " + getRequestParameterString(params));
				}
				if (mailinglistID <= 0) {
					throw new IllegalArgumentException("Mandatory mailinglistID (form-param: agnMAILINGLIST) is invalid: " + getRequestParameterString(params));
				}
    ...
    private boolean isBindingChanged(BindingEntry bindingToUpdate, RecipientBindingDto bindingDto,
        ...
        return bindingToUpdate.getExitMailingID() != bindingDto.getExitMailingId()
                ...
                || !Strings.CS.equals(oldUserType, newUserType)
	...
	@Override
	public List<String> getReadableNames(Set<Integer> bulkIds, int companyID) {
		if (CollectionUtils.isEmpty(bulkIds)) {
			return Collections.emptyList();
		}

		return recipientDao.getReadableNames(bulkIds, companyID);
	}
	...
	@Override
	public CaseInsensitiveMap<String, Object> getFields(int recipientId, int companyId) {
		return recipientDao.getCustomerData(companyId, recipientId);
	}

	@Override
	public List<CaseInsensitiveMap<String, Object>> getFields(List<Integer> recipientIds, int companyId) {
		return recipientDao.getCustomersData(recipientIds, companyId);
	}

	@Override
	public List<Integer> getFilteredRecipientIDs(
			int companyId,
			CaseInsensitiveMap<String, RecipientFieldDescription> recipientFieldsMap,
			Map<String, String> recipientFilters
	) {
		return recipientDao.getFilteredRecipientIDs(companyId, recipientFieldsMap, recipientFilters);
	}
