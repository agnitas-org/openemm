...
public final class RebuildBindingHistoryTriggersOnStartupListener implements ServletContextListener {
	...
	private static final Logger logger = LogManager.getLogger(RebuildBindingHistoryTriggersOnStartupListener.class);
	...
	public final void updateMarkedCompanies() {
		try {
			...
			if (!markedCompanies.isEmpty()) {
				logger.warn("Found {} companies to rebuild binding history triggers", markedCompanies.size());
			}

			for (final int companyID : markedCompanies) {
				if (CompanyStatus.ACTIVE == getCompanyDao().getCompany(companyID).getStatus()) {
					try {
						logger.warn("Rebuilding binding history triggers for company {}", companyID);
						...
						logger.error("Could not rebuild binding history trigger for company {}", companyID, e);
					}
				}
			}
		} catch (Exception e) {
			logger.error("Could not rebuild binding history triggers: {}", e.getMessage(), e);
		}
	}
