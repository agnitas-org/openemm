...
public class ServerStatusServiceImpl implements ServerStatusService {
	...
	private static final int VERSION_YEAR_EPOCH = 2000;
	...
	private Map<String, Object> getStatusProperties(ServletContext servletContext) {
        ...
        // Python Version
        status.put("python.version", Objects.toString(AgnUtils.getPythonVersion(), ERROR));
        status.put("python.ssl", Objects.toString(AgnUtils.getPythonSSL(), ERROR));
		...
		// Database info
		try {
			status.putAll(serverStatusDao.getDbInformation());
		} catch (Exception e) {
		...
		// OS name and version
        status.put("os.name", Objects.toString(System.getProperty("os.name"), ERROR));
        status.put("os.version", Objects.toString(System.getProperty("os.version"), ERROR));
		try {
            ...
            status.put("os.distribution", Objects.toString(osDistributionName + " " + osDistributionVersion, ERROR));
	...
	public ServerStatus getServerStatus(ServletContext servletContext, Admin admin) {
		return getServerStatusBuilder(servletContext, admin).build();
	}

	protected ServerStatus.StatusBuilder getServerStatusBuilder(ServletContext servletContext, Admin admin) {
		...
		boolean importStatusOK = getStallingImportsAmount(configService.getIntegerValue(ConfigValue.MaxUserImportDurationMinutes)) == 0 && (stallingAutoImports == null || stallingAutoImports.isEmpty());
		...
		return ServerStatus.builder(version, installPath, admin.getLocale(), configService)
			.database(serverStatusDao.getDbVendor(), getDbUrl(), getDbVersion(), checkDatabaseConnection(), getNetStatDbConnectionsCount())
	...
	private Integer getNetStatDbConnectionsCount() {
		try {
			String dbUrl = serverStatusDao.getDbUrl();
			int port = DbUtilities.extractPort(dbUrl);

			ProcessBuilder pb = new ProcessBuilder("bash", "-c", "netstat -ant | grep %d | wc -l".formatted(port));
			...
			logger.error("Server status: netstat command not successful {}", exitCode);
		} catch (Exception e) {
			Thread.currentThread().interrupt();
			logger.error("Server status: Can't run netstat command: {}", e.getMessage(), e);
		}
		return null;
	}
	...
	public SimpleServiceResult sendTestMail(Admin admin, String testMailAddress) {
		...
		try {
			...
			logger.error("Cannot sent test email to {}, cause: {}", testMailAddress, e.getMessage(), e);
	...
	@Override
    public boolean saveServerConfig(int companyId, String configName, String configValue, String description, Admin admin) {
        if (!configService.getListValue(ConfigValue.EditableConfigValues, admin.getCompanyID()).contains(configName)) {
			return false;
		}
	...
	@Override
	public String getEOLDate(Admin admin) {
		return configService
				.getApplicationVersion()
				.filter(Version::isLTS)
				.map(this::calculateEOLDate)
				.map(d -> admin.getDateFormatter().format(d))
				.orElse(null);
	}

	private LocalDate calculateEOLDate(Version version) {
		int year = VERSION_YEAR_EPOCH + version.getMajorVersion() + 1;

		return version.getMinorVersion() == 5
				? LocalDate.of(year, Month.MAY, 31)
				: LocalDate.of(year, Month.NOVEMBER, 30);
    }

	@Override
	public boolean isVersionUpdateExists() {
        return configService
				.getApplicationVersion()
				.filter(Version::isLTS)
				.map(this::isVersionUpdateExists)
				.orElse(false);
    }

	private boolean isVersionUpdateExists(Version version) {
		LocalDate now = LocalDate.now();
		LocalDate nextAvailable = nextLTSAvailabilityDate(version);

		return !now.isBefore(nextAvailable);
	}

	private LocalDate nextLTSAvailabilityDate(Version version) {
		int baseYear = VERSION_YEAR_EPOCH + version.getMajorVersion();

		if (version.getMinorVersion() == 5) {
			return LocalDate.of(baseYear, Month.NOVEMBER, 15);
		}

		return LocalDate.of(baseYear + 1, Month.MAY, 15);
	}
	...
	public boolean isEOLReached() {
        return configService
				.getApplicationVersion()
				.filter(Version::isLTS)
				.map(this::isEOLReached)
				.orElse(false);
    }

	private boolean isEOLReached(Version version) {
		LocalDate eol = calculateEOLDate(version);
		return !LocalDate.now().isBefore(eol);
	}
