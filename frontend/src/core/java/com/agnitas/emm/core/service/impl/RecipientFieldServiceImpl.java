...
public class RecipientFieldServiceImpl implements RecipientFieldService {
	...
	private static final Map<String, Comparator<RecipientFieldDescription>> SORTABLE_COLUMNS = Map.of(
		"shortname", Comparator.comparing(pf -> pf.getShortName() == null ? "" : pf.getShortName().toLowerCase()),
		"column", Comparator.comparing(pf -> pf.getColumnName().toLowerCase()),
		"dataType", Comparator.comparing(pf -> pf.getDatabaseDataType().toLowerCase()),
		"dataTypeLength", Comparator.comparing(RecipientFieldDescription::getCharacterLength),
		"defaultValue", Comparator.comparing(pf -> pf.getDefaultValue() == null ? "" : pf.getDefaultValue().toLowerCase()),
		"modeEdit", Comparator.comparing(pf -> pf.getDefaultPermission().getStorageCode()),
		"description", Comparator.comparing(pf -> pf.getDescription() == null ? "" : pf.getDescription().toLowerCase()),
		"creationDate", Comparator.comparing(RecipientFieldDescription::getCreationDate, Comparator.nullsLast(Comparator.naturalOrder())),
		"changeDate", Comparator.comparing(RecipientFieldDescription::getChangeDate, Comparator.nullsLast(Comparator.naturalOrder())),
		"historized", Comparator.comparing(RecipientStandardField::isHistorized)
	);
	...
	@Override
	public PaginatedList<RecipientFieldDescription> getPaginatedList(List<RecipientFieldDescription> fields, ProfileFieldForm form, int companyId) {
		if (StringUtils.isNotBlank(form.getSort())) {
			if (SORTABLE_COLUMNS.containsKey(form.getSort())) {
				Comparator<RecipientFieldDescription> comparator = SORTABLE_COLUMNS.get(form.getSort());
				if ("asc".equalsIgnoreCase(form.getDir()) && !"ascending".equalsIgnoreCase(form.getDir())) {
					comparator = comparator.reversed();
				}
				fields.sort(comparator);
			} else {
				logger.error("Profile field comparator was not found for property '{}'!", form.getSort());
			}
		}

		int startIndex = Math.min(fields.size(), (form.getPage() - 1) * form.getNumberOfRows());
		int endIndex = Math.min(fields.size(), startIndex + form.getNumberOfRows());
		List<RecipientFieldDescription> partialFieldsList = fields.subList(startIndex, endIndex);

		PaginatedList<RecipientFieldDescription> paginatedList = new PaginatedList<>(partialFieldsList, fields.size(), form.getNumberOfRows(), form.getPage(), form.getSort(), form.getDir());
		if (form.isUiFiltersSet()) {
			paginatedList.setNotFilteredFullListSize(getCountForOverview(companyId));
		}
	...
	public boolean mayAddNewRecipientField(int companyID) {
		if (companyID <= 0) {
			...
			return currentFieldCount < maxFields + configService.getIntegerValue(ConfigValue.System_License_MaximumNumberOfProfileFields_Graceful);
    	}
	}
	...
	@Override
	public CaseInsensitiveMap<String, RecipientFieldDescription> getVisibleFields(int companyId, int adminId) {
		return new CaseInsensitiveMap<>(getRecipientFields(companyId).stream()
			.filter(field -> (field.getAdminPermission(adminId) != ProfileFieldMode.NotVisible))
			.collect(Collectors.toMap(RecipientFieldDescription::getColumnName, Function.identity())));
	}

	@Override
	public List<RecipientFieldDescription> listVisibleFields(int companyId, int adminId) {
		return List.copyOf(getVisibleFields(companyId, adminId).values());
	}

	@Override
	public Set<String> getNamesOfChangeableFields(int companyId, int adminId) {
		return getRecipientFields(companyId).stream()
			.filter(field -> (field.getAdminPermission(adminId) == ProfileFieldMode.Editable))
			.map(RecipientFieldDescription::getColumnName)
			.collect(Collectors.toSet());
	}
