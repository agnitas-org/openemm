...
public class SystemMailMessageServiceImpl implements SystemMailMessageService {
    ...
    @Override
    public void sendTestMailingsFailedMail(Set<Integer> mailingIds, int companyId) {
    ...
    protected boolean sendMails(MailSettings settings, Collection<String> emails, int companyId) {
        if (emails.isEmpty()) {
            return false;
        }

        String emailsStr = emails.stream()
                .distinct()
                .collect(Collectors.joining(","));

        return sendMail(settings, emailsStr, companyId);
    }
