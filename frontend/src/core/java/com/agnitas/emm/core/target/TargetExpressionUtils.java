...
public class TargetExpressionUtils {
    ...
    public static String makeTargetExpressionWithAltgs(Collection<Integer> altgIds, Collection<Integer> targetIds, boolean conjunctionForTargets) {
    	if (targetIds != null && altgIds != null && !altgIds.isEmpty()) {
    ...
    private static boolean targetExpressionContainsAltgPart(String expression, TargetService targetService) {
        if (!Strings.CS.startsWith(expression, "(") || !isTargetExpressionContainsAnyAltg(expression, targetService)) {
            return false;
        }
    ...
    public static String makeTargetExpression(Collection<Integer> targetGroupIds, WorkflowTargetOption option) {
        ...
        return switch (option) {
            case ALL_TARGETS_REQUIRED -> makeTargetExpression(targetGroupIds, true);
            case NOT_IN_TARGETS -> negative(makeTargetExpression(targetGroupIds, true));
            case ONE_TARGET_REQUIRED -> makeTargetExpression(targetGroupIds, false);
        };
    }
