...
public class NotStartsWithRuleConverter implements RuleConverter {
    @Override
    public String convert(QueryBuilderRuleNode ruleNode, int companyId) {
        String value = StringUtil.makeEqlStringConstant(QueryBuilderUtil.getRuleNodeValueAsString(ruleNode));
        return String.format("`%s` NOT STARTS WITH %s", ruleNode.getId(), value);
    }
}
