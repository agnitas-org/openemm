...
public abstract class GenericEqlNodeParser<T> implements EqlNodeParser<T> {
    ...
    protected void processNodeAfterParse(final QueryBuilderGroupNode groupNode) {
        ...
        QueryBuilderBaseNode lastNode = rules.getLast();
        if (lastNode instanceof QueryBuilderGroupNode qbGroupNode) {
            lastNode = refactorIfContainsIncludingEmptyRule(qbGroupNode);
    ...
    private QueryBuilderBaseNode refactorIfContainsIncludingEmptyRule(final QueryBuilderGroupNode node) {
        ...
        QueryBuilderBaseNode leftNode = rules.get(0);
        QueryBuilderBaseNode rightNode = rules.get(1);

        if (!(leftNode instanceof QueryBuilderRuleNode leftRule && rightNode instanceof QueryBuilderRuleNode rightRule)) {
            return node;
        }
