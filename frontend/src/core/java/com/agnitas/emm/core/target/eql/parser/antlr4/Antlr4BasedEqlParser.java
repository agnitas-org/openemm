...
public class Antlr4BasedEqlParser implements EqlParser {
	...
	private static final Logger logger = LogManager.getLogger(Antlr4BasedEqlParser.class);
			...
			try(final Reader reader = new StringReader(nonNullEql)) {
				...
				if(!syntaxErrorListener.hasErrors()) {
					...
					if(logger.isInfoEnabled()) {
						logger.info("Found {} errors in EQL", syntaxErrorListener.getErrors().size());
						
						for(EqlSyntaxError error : syntaxErrorListener.getErrors()) {
                            logger.info("  - at line {}, column {}: {}", error.getLine(), error.getColumn(), error.getSymbol());
						}
					}
