...
public final class BeanShellRecipientTargetGroupMatcher implements RecipientTargetGroupMatcher {
	...
	private static final Logger logger = LogManager.getLogger(BeanShellRecipientTargetGroupMatcher.class);
	...
	@Override
	public boolean isInTargetGroup(Target target) {
		if(this.recipientCompanyID == target.getCompanyID()) {
			try {
				...
				logger.error("Error checking if recipient is in target group via BeanShell (company ID: {}, target ID: {})",
						target.getCompanyID(), target.getId(), e);
				return false;
			}
		} else {
			logger.warn("Mismatching company ID of customer and target group (customer company ID: {}, target group company ID: {})",
					recipientCompanyID, target.getCompanyID());
			return false;
		}
	}
