...
import com.agnitas.emm.core.trackablelinks.dto.UserFormLinkStatsRow;
...
public class FormTrackableLinkServiceImpl implements FormTrackableLinkService {
    ...
    private LinkService.LinkScanResult validateLinks(ResultSettings settings, Admin admin, List<Message> errors, List<Message> warnings) {
		...
		if (CollectionUtils.isNotEmpty(notTrackableLinks)) {
			errors.add(new Message("warning.mailing.link.agntag",
					new Object[]{ type, StringEscapeUtils.escapeHtml4(notTrackableLinks.getFirst()) }));
		}

		List<LinkService.ErroneousLink> erroneousLinks = links.getErroneousLinks();
		if (CollectionUtils.isNotEmpty(erroneousLinks)) {
			LinkService.ErroneousLink firstErroneousLink = erroneousLinks.getFirst();
	...
	@Override
	public List<UserFormLinkStatsRow> getStats(UserformStatsForm form, Admin admin) {
		Locale locale = admin.getLocale();
		int companyId = admin.getCompanyID();
		List<UserFormLinkStats> stats = trackableLinkDao.getStats(form.getFormId(), companyId, getDateRange(form));

		List<UserFormLinkStatsRow> result = stats.stream()
			.flatMap(stat -> Stream.of(
				new UserFormLinkStatsRow(
					stat.url(),
					t("report.total", locale),
					stat.clicksGross() + stat.clicksAnonymous(),
					String.valueOf(stat.clicksNet())),
				new UserFormLinkStatsRow(
					stat.url(),
					t("report.form.measured", locale),
					stat.clicksGross(),
					String.valueOf(stat.clicksNet())),
				new UserFormLinkStatsRow(
					stat.url(),
					t("report.form.anonym", locale),
					stat.clicksAnonymous(),
					"0*")
			))
			.collect(Collectors.toList());

		result.add(new UserFormLinkStatsRow(
			t("GrandTotal", locale),
			"",
			stats.stream().mapToInt(s -> s.clicksGross() + s.clicksAnonymous()).sum(),
			String.valueOf(stats.stream().mapToInt(UserFormLinkStats::clicksNet).sum())
		));
		return result;
	}

	public static DateRange getDateRange(UserformStatsForm form) {
		if (form.getPeriodMode() == DateMode.SELECT_MONTH) {
			...
			return new DateRange(
				...
				Date.valueOf(yearMonth.plusMonths(1).atDay(1))
			);
		}
		if (form.getPeriodMode() == DateMode.SELECT_YEAR) {
			return new DateRange(
				Date.valueOf(Year.of(form.getYear()).atDay(1)),
				Date.valueOf(Year.of(form.getYear() + 1).atDay(1))
			);
		}
	...
	@Override
	public byte[] getCsv(UserformStatsForm form, Admin admin) throws Exception {
		Locale locale = admin.getLocale();
		int companyId = admin.getCompanyID();
		List<UserFormLinkStats> rows = trackableLinkDao.getStats(form.getFormId(), companyId, getDateRange(form));

		return CsvWriter.csv(ListUtils.union(
			List.of(getHeaderForCSv(locale)),
			getRowsForCsv(rows))
		);
	}
	...
	private static List<String> getHeaderForCSv(Locale locale) {
		return List.of(
			t("URL", locale),
			t("report.form.measuredGross", locale),
			t("report.form.measuredNet", locale),
			t("report.form.anonym", locale),
			t("report.form.totalGross", locale),
			t("report.form.totalNet", locale)
		);
	}

	private List<List<String>> getRowsForCsv(List<UserFormLinkStats> rows) {
		return rows.stream()
			.map(row -> Arrays.asList(
				row.url(),
				String.valueOf(row.clicksGross()),
				String.valueOf(row.clicksNet()),
				String.valueOf(row.clicksAnonymous()),
				String.valueOf(row.clicksGross() + row.clicksAnonymous()),
				String.valueOf(row.clicksNet())
			))
			.toList();
	}
