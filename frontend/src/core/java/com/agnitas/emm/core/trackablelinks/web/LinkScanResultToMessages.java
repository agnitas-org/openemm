...
/**
 * Utility class to convert a {@link LinkService.LinkScanResult} to {@link Message}.
 */
public final class LinkScanResultToMessages {

	private LinkScanResultToMessages() {
	 ...
	 */
	public static final void linkWarningsToActionMessages(final LinkScanResult result, final List<Message> warnings) {
    ...
    /**
  	...
  	private static void insecureLinksToPopups(final LinkScanResult result, Popups popups) {
  		...
  		if(!list.isEmpty()) {
  			...
  			if(numMoreLinks == 0) {
  				popups.warning(MESSAGE_KEY_INSECURE_LINKS, links);
  			} else {
  				popups.warning(MESSAGE_KEY_INSECURE_LINKS_WITH_MORE, links, numMoreLinks);
  			}
  		}
  	}
