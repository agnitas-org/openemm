...
final class OneClickUnsubscription {
	...
	private static final Logger LOGGER = LogManager.getLogger(OneClickUnsubscription.class);
	...
	public final void performOneClickUnsubscription(final ExtensibleUID uid, final String remark) {
	...
	private static final boolean isCorrectMultipartFormDataContent(final HttpServletRequest request) {
		try {
			...
			if(part == null) {
				LOGGER.info("Multipart message does not contain parameter '{}'", UNSUBSCRIBE_PARAMETER_NAME);
				return false;
			}
