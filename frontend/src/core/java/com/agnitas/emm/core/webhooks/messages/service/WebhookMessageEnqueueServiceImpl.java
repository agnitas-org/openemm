...
public final class WebhookMessageEnqueueServiceImpl implements WebhookMessageEnqueueService {
	 ...
	 */
	private boolean isWebhookActive(int companyID, WebhookEventType type) {
		// Check if webhooks interface is enabled
		if(!configService.isWebhookInterfaceEnabled(companyID)) {
			LOGGER.info("Webhooks disabled for company ID {}", companyID);
			return false;
		}
		
		// Check if webhook URL is configured for given event type
		if(!isWebhookUrlConfigured(companyID, type)) {
			LOGGER.info("WebhookURL configured for event type '{}' for company ID {}", type, companyID);
			return false;
		}
	 ...
	 */
	private boolean isWebhookUrlConfigured(int companyID, WebhookEventType type) {
		try {
