...
@Component("WebhookMessageSender")
public final class WebhookMessageSenderImpl implements WebhookMessageSender {
	...
	private final WebhookMessageDequeueService dequeueService;
	private final WebhookHttpClientFactory httpClientFactory;
	private final WebhookConfigService configService;

    public WebhookMessageSenderImpl(
			WebhookRegistrationService registrationService,
			WebhookMessageDequeueService dequeueService,
			WebhookHttpClientFactory httpClientFactory,
		...
		client.execute(request, new FutureCallback<>() {
            ...
            public void failed(Exception e) {
				...
				if (e instanceof SocketTimeoutException) {
					note = "Timeout while sending webhook message (event type: '%s', company ID %d): %s"
							.formatted(
					...
					note = String.format("Error sending webhook message (event type: '%s', company ID %d, error: '%s')",
							webhookRegistryEntry.getEventType().name(), webhookRegistryEntry.getCompanyId(), e.getMessage());
	...
	private void tryCloseHttpClient(WebhookHttpClient client) {
		try {
			client.close();
		} catch (Exception e) {
	...
	private static void checkMessage(WebhookMessage message, WebhookRegistryEntry webhookRegistryEntry) {
		...
		if (message.getCompanyId() != webhookRegistryEntry.getCompanyId()) {
            throw new IllegalArgumentException(String.format(
                    "Encountered webhook message of invalid company ID (found: %d, expected %d, message ID: %d)",
	...
	private JSONObject createMessageJson(WebhookMessage message, WebhookRegistryEntry webhookRegistryEntry) {
		...
		obj.put(EVENT_DATA_JSON_PROPERTY, new JSONObject(message.getPayload()));
