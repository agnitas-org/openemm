...
package com.agnitas.emm.core.workflow.beans.parameters;
...
public class WorkflowParametersHelper {
    ...
    private WorkflowParametersHelper() {

    }
    
    private static final Logger logger = LogManager.getLogger(WorkflowParametersHelper.class);
	...
	private static <T> T convertOrNull(Object value, Class<T> type, Callable<T> handleStringValue) {
        ...
        if (value instanceof String) {
            try {
                ...
                logger.error("Could not convert value to {} type", type.getName(), e);
                return null;
            }
        }
    ...
    public static boolean isEmptyParams(HttpServletRequest req) {
        return isEmpty(find(req));
    }
