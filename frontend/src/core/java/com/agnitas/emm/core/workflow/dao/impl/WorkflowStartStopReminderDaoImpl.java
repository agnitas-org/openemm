...
public class WorkflowStartStopReminderDaoImpl extends ReminderBaseDaoImpl implements WorkflowStartStopReminderDao {
    ...
    public List<Reminder> getReminders(Date date, CompaniesConstraints constraints) {
        ...
        return select(sqlGetReminders, reminderRowMapper, sqlParameters);
    }
    ...
    public void setNotified(List<Reminder> reminders) {
        ...
        if (!sqlParameters.isEmpty()) {
            ...
            batchupdate(sqlSetNotified, sqlParameters);
        }
    }
    ...
    private void insertReminder(int companyId, int workflowId, WorkflowReminder reminder) {
        ...
        if (isOracleDB()) {
            reminderId = selectInt("SELECT workflow_reminder_tbl_seq.nextval FROM dual");
            ...
            update(sqlInsert, reminderId, companyId, workflowId, reminder.getSenderAdminId(), reminder.getType().getId(), reminder.getMessage(), reminder.getDate());
        } else {
    ...
    private Map<Integer, String> resolveAdminAddresses(int companyId, List<Integer> ids) {
        ...
        query(sqlResolveAdmins, new AdminAddressResolutionCallback(map), sqlParameters.toArray());
    ...
    private Map<String, Integer> resolveAdminIds(int companyId, List<String> addresses) {
        ...
        query(sqlResolveAdmins, new AdminIdResolutionCallback(map), sqlParameters.toArray());

        return map;
    }

    private class ReminderRowMapper implements RowMapper<Reminder> {
        @Override
        public Reminder mapRow(ResultSet rs, int i) throws SQLException {
        ...
        private void generateContent(Reminder reminder, ReminderType reminderType, String workflowName, LocalDateTime startDate, LocalDateTime stopDate) {
            Locale locale = Locale.of(reminder.getLang());
