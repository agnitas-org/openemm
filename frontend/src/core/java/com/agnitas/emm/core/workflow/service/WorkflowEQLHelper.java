...
public class WorkflowEQLHelper {
    ...
    private String identifyColumnType(int companyId, String profileField) {
        ...
        try {
            ...
            type = Objects.toString(field.getDataType(), type);
        } catch (Exception e) {
            logger.error("Cannot find field type for companyId {} and column '{}'", companyId, profileField, e);
        }
        ...
        for (WorkflowRule rule : rules) {
            ...
            switch (StringUtils.trimToEmpty(DbColumnType.dbType2String(type, 0))) {
				case DbColumnType.GENERIC_TYPE_VARCHAR:
					...
					break;
				case DbColumnType.GENERIC_TYPE_INTEGER, DbColumnType.GENERIC_TYPE_FLOAT:
					expression.append(generateNumericEQL(resolvedFieldName, operator, value, disableThreeValuedLogic));
					break;
				case DbColumnType.GENERIC_TYPE_DATE, DbColumnType.GENERIC_TYPE_DATETIME:
