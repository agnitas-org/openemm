...
public class WorkflowUtils {
	...
	public static final List<Integer> MAILING_ICON_TYPE_IDS = MAILING_ICON_TYPES.stream()
			.map(WorkflowIconType::getId)
			.toList();


	public static Map<ConditionalOperator, String> getOperatorTypeSupportMap() {
		Map<ConditionalOperator, String> map = new EnumMap<>(ConditionalOperator.class);
	...
	public static Deadline asDeadline(WorkflowDeadline deadline, TimeZone timezone) {
        return switch (deadline.getDeadlineType()) {
            case TYPE_DELAY -> asDelayDeadline(deadline);
            case TYPE_FIXED_DEADLINE ->
                    asDeadline(deadline.getDate(), deadline.getHour(), deadline.getMinute(), timezone);
        };
	}
	...
	private static Deadline asDelayDeadline(WorkflowDeadline deadline) {
		...
		switch (deadline.getTimeUnit()) {
			...
			case TIME_UNIT_WEEK:
				return new Deadline(TimeUnit.DAYS.toMillis(value * 7L));
			case TIME_UNIT_MONTH:
				return new Deadline(TimeUnit.DAYS.toMillis(value * 30L));
			default:
    ...
    public static WorkflowIcon getEmptyIcon(WorkflowIconType type) {
        return switch (type) {
            case START -> new WorkflowStartImpl();
            case STOP -> new WorkflowStopImpl();
            case DECISION -> new WorkflowDecisionImpl();
            case DEADLINE -> new WorkflowDeadlineImpl();
            case PARAMETER -> new WorkflowParameterImpl();
            case RECIPIENT -> new WorkflowRecipientImpl();
            case ARCHIVE -> new WorkflowArchiveImpl();
            case FORM -> new WorkflowFormImpl();
            case MAILING -> new WorkflowMailingImpl();
            case ACTION_BASED_MAILING -> new WorkflowActionBasedMailingImpl();
            case MAILING_MEDIATYPE_SMS -> new WorkflowMailingMediaTypeSmsImpl();
            case MAILING_MEDIATYPE_POST -> new WorkflowMailingMediaTypePostImpl();
            case DATE_BASED_MAILING -> new WorkflowDateBasedMailingImpl();
            case FOLLOWUP_MAILING -> new WorkflowFollowupMailingImpl();
            case IMPORT -> new WorkflowImportImpl();
            case EXPORT -> new WorkflowExportImpl();
