...
public abstract class DataProvider implements Closeable {
	...
	protected DataProvider(File importFile, char[] zipPassword) {
	...
	protected InputStream getInputStream() throws Exception {
		if (!importFile.exists()) {
			throw new IllegalStateException("Import file does not exist: " + importFile.getAbsolutePath());
		} else if (importFile.isDirectory()) {
			throw new IllegalStateException("Import path is a directory: " + importFile.getAbsolutePath());
		} else if (importFile.length() == 0) {
			throw new IllegalStateException("Import file is empty: " + importFile.getAbsolutePath());
		} else {
			try {
				if (Strings.CI.endsWith(importFile.getAbsolutePath(), ".zip")) {
					if (zipPassword != null) {
				...
				} else if (Strings.CI.endsWith(importFile.getAbsolutePath(), ".tar.gz")) {
					if (TarGzUtilities.getFilesCount(importFile) != 1) {
						throw new TooManyFilesInZipToImportException("Compressed import file does not contain a single compressed file: " + importFile.getAbsolutePath());
					} else {
						inputStream = TarGzUtilities.openCompressedFile(importFile);
					}
				} else if (Strings.CI.endsWith(importFile.getAbsolutePath(), ".tgz")) {
					if (TarGzUtilities.getFilesCount(importFile) != 1) {
						throw new TooManyFilesInZipToImportException("Compressed import file does not contain a single compressed file: " + importFile.getAbsolutePath());
					} else {
						inputStream = TarGzUtilities.openCompressedFile(importFile);
					}
				} else if (Strings.CI.endsWith(importFile.getAbsolutePath(), ".gz")) {
