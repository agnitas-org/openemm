...
public final class SessionBindingRequestFilter implements Filter {
    ...
    private static final Pattern MEDIAPOOL_ELEMENT_PATTERN = Pattern.compile("^/mediapool_element/.*$");
    private static final Pattern SC_PATTERN = Pattern.compile("^/sc(/.*)?$");
    ...
    private boolean isUrlExcluded(HttpServletRequest req) {
        String urlPath = AgnUtils.getUrlPath(req);

        if (MEDIAPOOL_ELEMENT_PATTERN.matcher(urlPath).matches()) {
            return true;
        }

        return SC_PATTERN.matcher(urlPath).matches();
    }
    ...
    private boolean isClientExcluded(HttpServletRequest request) {
        try {
            ...
            LOGGER.warn("Unable to resolve host '{}'. Assuming client is not excluded.", request.getRemoteAddr());
            return false;
        }
    }
    ...
    private void checkAndUpdateSessionBinding(HttpServletRequest request, HttpServletResponse response) throws AccessDeniedException {
        ...
        if (!sessionBindingData.securityToken().equals(cookie.getValue())) {
        ...
        for (String excludedClientString : excludedClientsList.trim().split("\\s*[,;]\\s*")) {
            try {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Could not resolve host '{}'. Host not excluded from session binding.", excludedClientString, e);
                } else {
                    LOGGER.warn("Could not resolve host '{}'. Host not excluded from session binding.", excludedClientString);
                }
            }
        }
    }
