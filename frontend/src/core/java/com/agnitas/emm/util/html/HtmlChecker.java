...
public final class HtmlChecker {
	...
	/**
	 * These script type are always allowed
	 */
	private static final String[] ALWAYS_ALLOWED_SCRIPT_TYPES = { "application/ld+json" };
	...
	private static void checkStartTag(boolean allowSafeHtmlTags, final Tag tag, final Set<HtmlCheckerError> errors) {
		...
		if ("script".equalsIgnoreCase(tagName)) {
			...
			boolean allowedScript = false;

			if (CollectionUtils.isNotEmpty(attributes)) {
				allowedScript = attributes.stream()
						.anyMatch(attr ->
								"type".equalsIgnoreCase(attr.getName()) &&
										containsIgnoreCase(attr.getValue(), ALWAYS_ALLOWED_SCRIPT_TYPES)
						);
			}

			if (!allowedScript) {
				errors.add(new HtmlCheckerForbiddenTagError(tagName));
			}

			return;
		}

		if (containsIgnoreCase(tagName, ALWAYS_FORBIDDEN_HTML_TAGS)) {
			errors.add(new HtmlCheckerForbiddenTagError(tagName));
		} else if (containsIgnoreCase(tagName, ALWAYS_ALLOWED_HTML_TAGS)) {
			final Attributes attributes = tag.parseAttributes();
			if (!"!--".equals(tagName) && CollectionUtils.isNotEmpty(attributes) && !allowSafeHtmlTags) {
				attributes.stream().map(Attribute::getName).forEach(name -> errors.add(new HtmlCheckerForbiddenTagAttributeError(tagName, name)));
			}
		} else if (allowSafeHtmlTags) {
			final Attributes attributes = tag.parseAttributes();
			if (!"!--".equals(tagName) && CollectionUtils.isNotEmpty(attributes)) {
