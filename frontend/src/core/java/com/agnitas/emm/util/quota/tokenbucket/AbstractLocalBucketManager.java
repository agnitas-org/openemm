...
public abstract class AbstractLocalBucketManager implements BucketManager {
	 ...
	 */
	protected AbstractLocalBucketManager() {
	...
	private void updateBucketSettings(Bucket bucket, String username, int companyId) throws BucketManagerException {
		final List<Bandwidth> bandwidthListOrNull = readBandwidthOrThrow(username, companyId);
		
		// Simply replacing the configuration resets the bucket and will break the desired behaviour.
	...
	private Bucket createNewBucket(String username, int companyId) throws BucketManagerException {
		LOGGER.debug("Create new token bucket for webservice user '{}'", username);

        LocalBucketBuilder builder = Bucket4j.builder();
		readBandwidthOrThrow(username, companyId).forEach(builder::addLimit);
	 ...
	 * 
	 * @param username name of the EMM user
	 * 
	 * @return bandwidths or <code>null</code>
	 * @throws BucketManagerException on errors reading bandwidths
	 */
	private List<Bandwidth> readBandwidthOrThrow(String username, int companyId) throws BucketManagerException {
		LOGGER.debug("Reading bandwidths for webservice user '{}' of company {}", username, companyId);
	 ...
	 */
	private static List<Bandwidth> toBandwidthList(final String bandwidthSpec) throws BucketManagerException {
	 ...
	 */
	private static Bandwidth toBandwidth(final String spec) throws BucketManagerException {
	...
	private String readBandwidthSpec(String username, int companyId) {
		...
		return settings.orElseGet(() -> findDefaultBandwidthSettings(companyId));
	}
