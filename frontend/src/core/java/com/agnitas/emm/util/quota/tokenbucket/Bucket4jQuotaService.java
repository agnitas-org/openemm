...
public final class Bucket4jQuotaService implements QuotaService {
	...
	private static final Logger LOGGER = LogManager.getLogger(Bucket4jQuotaService.class);

	private BucketManager bucketManager;
	private ApiInvocationCosts invocationCosts;

	@Override
	public void checkAndTrack(final String username, final int companyID, final String apiServiceName) throws QuotaLimitExceededException, QuotaServiceException {
		try {
			final Bucket bucket = this.bucketManager.getOrCreateBucket(username, companyID);
			...
			if (!consumed) {
				LOGGER.error("API quota exceeded. User:[{}]", username);
				throw new QuotaLimitExceededException(username, companyID, apiServiceName);
			}
			
		} catch (BucketManagerException e) {
			throw new QuotaServiceException(e);
		}
	}

	public void setBucketManager(final BucketManager manager) {
		this.bucketManager = Objects.requireNonNull(manager, "BucketManager is null");
	}
	
	public void setApiInvocationCosts(final ApiInvocationCosts costs) {
