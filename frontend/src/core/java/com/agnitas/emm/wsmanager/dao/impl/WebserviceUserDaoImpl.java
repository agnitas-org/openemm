...
public class WebserviceUserDaoImpl extends PaginatedBaseDaoImpl implements WebserviceUserDao {
	...
	@Override
	public PaginatedList<WebserviceUserListItem> getWebserviceUserList(int companyID, String sortColumn, boolean sortDirectionAscending, int pageNumber, int pageSize) {
		if (!SORTABLE_COLUMNS.contains(sortColumn)) {
			sortColumn = DEFAULT_SORT_COLUMN;
		}

		return selectPaginatedList("SELECT username, company_id, default_data_source_id, active FROM webservice_user_tbl WHERE company_id = ?",
				WS_USER_TBL_NAME, sortColumn, sortDirectionAscending, pageNumber, pageSize, LIST_ITEM_ROWMAPPER, companyID);
	}

	@Override
	public PaginatedList<WebserviceUserListItem> getWebserviceUserList(WebserviceUserOverviewFilter filter) {
	...
	@Override
	public PaginatedList<WebserviceUserListItem> getWebserviceUserMasterList(String sortColumn, boolean sortDirectionAscending, int pageNumber, int pageSize) {
		if (!SORTABLE_COLUMNS.contains(sortColumn)) {
			sortColumn = DEFAULT_SORT_COLUMN;
		}

		return selectPaginatedList("SELECT username, company_id, default_data_source_id, active FROM webservice_user_tbl",
				WS_USER_TBL_NAME, sortColumn, sortDirectionAscending, pageNumber, pageSize, LIST_ITEM_ROWMAPPER);
	}

	@Override
	public WebserviceUser getWebserviceUser(String username) {
	...
	@Override
	public void updateUser(WebserviceUser user) {
		update("UPDATE webservice_user_tbl SET active = ?, contact_email = ? WHERE username = ?",
				user.isActive() ? 1 : 0, user.getContactEmail(), user.getUsername());
	}

	@Override
	public void updatePasswordHash(String username, String passwordHash) {
		update("UPDATE webservice_user_tbl SET password_encrypted = ? WHERE username = ?", passwordHash, username);
	}

	@Override
	public boolean webserviceUserExists(final String username) {
        return selectInt("SELECT COUNT(*) FROM webservice_user_tbl WHERE username = ?", username) > 0;
	}

	@Override
	public void createWebserviceUser(WebserviceUserCredential user, int dataSourceId, int bulkSizeLimit) {
        update("INSERT INTO webservice_user_tbl (username, company_id, default_data_source_id, bulk_size_limit, password_encrypted, active, contact_email) VALUES (?, ?, ?, ?, ?, 1, ?)",
                user.getUsername(), user.getCompanyID(), user.getDefaultDatasourceID(), bulkSizeLimit, user.getPasswordHash(), user.getContactEmail());
	}
	...
	@Override
	public void saveGrantedPermissionsAndGroups(WebserviceUser user) {
		saveGrantedPermissions(user);
		saveGrantedPermissionGroups(user);
	}
	
	private void saveGrantedPermissions(WebserviceUser user) {
