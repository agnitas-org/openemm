...
public class JsonReader extends BasicReader {
	...
	protected JsonToken readNextTokenInternal() throws Exception {
		...
		switch (currentChar) {
			case '{': // Open JsonObject
				if (!openJsonItems.isEmpty() && openJsonItems.peek() == JsonToken.JsonObject_PropertyKey) {
					openJsonItems.pop();
				}
	...
	public boolean readNextJsonNode() throws Exception {
		if (getReadCharacters() == 0) {
			throw new IllegalStateException("JsonReader position was not initialized for readNextJsonItem()");
		}
	...
	public JsonNode read() throws Exception {
		if (getReadCharacters() != 0) {
			throw new IllegalStateException("JsonReader position was already initialized for other read operation");
		}
	...
	private JsonObject readJsonObject() throws Exception {
		if (openJsonItems.peek() != JsonToken.JsonObject_Open) {
			throw new IllegalStateException("Invalid read position for JsonArray in line " + getReadLines() + " at overall index " + getReadCharacters());
		} else {
			JsonObject returnObject = new JsonObject();
			JsonToken nextToken = readNextToken();
			while (nextToken != JsonToken.JsonObject_Close) {
				if (nextToken == JsonToken.JsonObject_PropertyKey && currentObject instanceof String propertyKey) {
	...
	private Object readSimpleJsonValue(String valueString) {
		if (valueString.equalsIgnoreCase("null")) {
			...
			throw new IllegalArgumentException("Invalid json data in line " + getReadLines() + " at overall index " + getReadCharacters());
		}
	}
	...
	protected void updateJsonPath(JsonToken jsonToken) throws Exception {
		if (jsonToken != null) {
			switch(jsonToken) {
				case JsonArray_Open:
					if (!currentJsonPath.isEmpty() && currentJsonPath.peek().startsWith("[")) {
						riseArrayIndex();
					}
	...
	private void riseArrayIndex() {
		...
		int newArrayIndex = 0;
		if (!currentArrayIndexString.isEmpty()) {
