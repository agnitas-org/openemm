...
public class OneOfValidator extends BaseJsonSchemaValidator {
	...
	public OneOfValidator(final JsonSchemaDependencyResolver jsonSchemaDependencyResolver, final String jsonSchemaPath, final Object validatorData, final JsonNode jsonNode, final String jsonPath) throws JsonSchemaDefinitionError {
        ...
        if (validatorData instanceof JsonArray validatorDataArray) {
            ...
            for (Object subValidationData : validatorDataArray) {
                if (subValidationData instanceof JsonObject subValidationDataObj) {
                    subValidatorPackages.add(JsonSchema.createValidators(subValidationDataObj, jsonSchemaDependencyResolver, jsonSchemaPath, jsonNode, jsonPath));
                } else {
