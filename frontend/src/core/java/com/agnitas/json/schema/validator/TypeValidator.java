...
public class TypeValidator extends BaseJsonSchemaValidator {
	...
	public void validate() throws JsonSchemaDefinitionError, JsonSchemaDataValidationError {
		if (validatorData instanceof JsonArray validatorDataArray) {
			for (Object typeData : validatorDataArray) {
				if (typeData == null) {
			...
			if (jsonNode.getJsonDataType() != jsonDataType && !(jsonDataType == JsonDataType.NUMBER && jsonNode.getJsonDataType() == JsonDataType.INTEGER)) {
				throw new JsonSchemaDataValidationError("Expected data type is '" + validatorData + "' but was '" + jsonNode.getJsonDataType().getName() + "'", jsonPath);
			}
		}
	}
}
