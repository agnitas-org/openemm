    ...
    public int save(Optimization optimization) {
        ...
        if(optimization.getEvalType() == null) {
        	...
        	try	{
        		...
        		logger.warn("Evaluation type of auto optimization {} is null. Defaulting to {}",
						optimization.getId(), WorkflowAutoOptimizationCriteria.AO_CRITERIA_CLICKRATE, e);
        	}
        ...
        if (optimization.getId() == 0) { // does not exist ? -> insert
            if (isOracleDB()) {
                ...
                update(insertQuery,
                        newOptimizationId, optimization.getCompanyID(),
            ...
            int rowsAffected = update(updateQuery,
                    optimization.getCompanyID(), optimization.getMailinglistID(),
                    optimization.getShortname(), optimization.getDescription(),
					optimization.getEvalType() == null ? 0 : optimization.getEvalType().getId(),
