...
public class MailingPreviewServiceImpl implements MailingPreviewService {
	...
	public String renderSmsPreview(int mailingID, int customerID) {
		Page page = renderPreview(mailingID, customerID);
		return Optional.ofNullable(page.getRCS())
				.map(Page.RCS::sms)
				.orElse(page.getSMS());
	}

	@Override
	public String renderRcsPreview(int mailingID, int customerID) {
		return renderPreview(mailingID, customerID).getRCS().content();
	}
