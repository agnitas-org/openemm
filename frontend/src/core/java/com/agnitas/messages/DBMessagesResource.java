...
public class DBMessagesResource {
	...
	public boolean hasMessageForKey(String key) {
        return allMessages != null
               && !allMessages.isEmpty()
               && allMessages.containsKey(DEFAULT_LANGUAGE_MAPKEY)
               && allMessages.get(DEFAULT_LANGUAGE_MAPKEY).containsKey(key)
               && allMessages.get(DEFAULT_LANGUAGE_MAPKEY).get(key) != null;
	}
