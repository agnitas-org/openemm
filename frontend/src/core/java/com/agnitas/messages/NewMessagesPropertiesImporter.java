...
public class NewMessagesPropertiesImporter {
	...
	private NewMessagesPropertiesImporter() {

	}

	private static final Logger logger = LogManager.getLogger(NewMessagesPropertiesImporter.class);
	...
	public static void importNewMessagesProperties(MessageDao messageDao) {
        ...
        try (InputStream newMessagesPropertiesInputStream = NewMessagesPropertiesImporter.class.getClassLoader().getResourceAsStream(NEWMESSAGES_FILENAME)) {
            if (newMessagesPropertiesInputStream == null) {
            	logger.warn("File {} is missing and no messages were inserted, updated or deleted. Basepath: {}",
						NEWMESSAGES_FILENAME, NewMessagesPropertiesImporter.class.getClassLoader().getResource("."));
            } else {
