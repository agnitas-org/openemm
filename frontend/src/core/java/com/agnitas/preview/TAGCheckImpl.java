...
public class TAGCheckImpl implements TAGCheck {
	...
	public TAGCheckImpl(int companyId, int mailingId, int mailinglistId, Locale locale) throws Exception {
		...
		if ((companyId == 0) && (mailinglistId == 0)) {
			previewExpression = String.valueOf (mailingId);
		} else {
			previewExpression = "%d,%d,%d".formatted(mailingId, companyId, mailinglistId);
		}
		data = new Data("tagcheck", "silent");
		seen = new HashMap<>();
		this.locale = locale;
		try {
			data.setup ("preview:" + previewExpression, null);
		...
		if (s == null) {
			...
			try {
				...
				if (StringUtils.isBlank(e.getMessageKey())) {
					s.report = tag + ": " + StringUtils.defaultIfBlank(e.getMessage(), e.getMessage());
				} else {
	...
	public boolean checkContent(String content, StringBuffer report, List<String> failures) {
		...
		do {
			try {
				...
				data.logging(Log.ERROR, "tc", "Failed to parse \"" + content + "\": " + e, e);
				tag = null;
				rc = false;
				addToReport(report, "failed to parse next tag: " + e);
			}
