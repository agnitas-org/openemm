...
import com.agnitas.reporting.birt.external.exception.DateParseException;
...
public class BIRTDataSet extends LongRunningSelectResultCacheDao {
	...
	// mobile/PC clicks
	protected static final List<Integer> CLICKER_INDEXES = List.of(
			CommonKeys.CLICKER_PC_INDEX,
	...
	// mobile/PC openings
	protected static final List<Integer> OPENING_INDEXES = List.of(
	...
	protected int getCachedTempTable(String statisticsType, String parameterString) {
		try {
			if (!checkEmbeddedTableExists(TEMPTABLECACHE_TABLENAME)) {
				executeEmbedded("CREATE TABLE " + TEMPTABLECACHE_TABLENAME + " ("
					+ "type VARCHAR(50),"
			...
			List<Map<String, Object>> result = selectEmbedded("SELECT table_name FROM " + TEMPTABLECACHE_TABLENAME + " WHERE cleanup_date < ?", new Date());
			for (Map<String, Object> row : result) {
				String tableNameToDelete = (String) row.get("table_name");
				updateEmbedded("DELETE FROM " + TEMPTABLECACHE_TABLENAME + " WHERE table_name = ?", tableNameToDelete);
				try {
			...
			while (blockingEntryExists) {
				int tempTableID = selectEmbeddedIntWithDefault("SELECT MAX(temptableid) FROM " + TEMPTABLECACHE_TABLENAME + " WHERE type = ? AND parameter = ? AND validity_date IS NULL AND creation_date <= ?", 0, statisticsType, parameterString, DateUtilities.addMinutesToDate(new Date(), 60));
			...
			int tempTableID = selectEmbeddedIntWithDefault("SELECT MAX(temptableid) FROM " + TEMPTABLECACHE_TABLENAME + " WHERE type = ? AND parameter = ? AND validity_date >= ?", 0, statisticsType, parameterString, new Date());
			List<Map<String, Object>> resultTables = selectEmbedded("SELECT table_name FROM " + TEMPTABLECACHE_TABLENAME + " WHERE temptableid = ?", tempTableID);
			if (!resultTables.isEmpty()) {
				String tableName = (String) resultTables.get(0).get("table_name");
				if (checkEmbeddedTableExists(tableName)) {
					return tempTableID;
				} else {
					updateEmbedded("DELETE FROM " + TEMPTABLECACHE_TABLENAME + " WHERE table_name = ?", tableName);
					return 0;
				}
			} else {
				updateEmbedded("DELETE FROM " + TEMPTABLECACHE_TABLENAME + " WHERE temptableid = ?", tempTableID);
				return 0;
			}
		} catch (Exception e) {
			logger.error("Cannot getCachedTempTable: {}", e.getMessage(), e);
			return 0;
		}
	}

	protected void createBlockEntryInTempTableCache(String statisticsType, String parameterString) {
		try {
			updateEmbedded("INSERT INTO " + TEMPTABLECACHE_TABLENAME + " (type, parameter, creation_date, cleanup_date) VALUES (?, ?, ?, ?)", statisticsType, parameterString, new Date(), DateUtilities.addMinutesToDate(new Date(), 60));
		} catch (Exception e) {
			logger.error("Cannot createBlockEntryInTempTableCache: {}", e.getMessage(), e);
		}
	}

	protected void storeTempTableInCache(String statisticsType, String parameterString, int tempTableID, String tempTableName) {
		try {
			updateEmbedded("INSERT INTO " + TEMPTABLECACHE_TABLENAME + " (type, parameter, temptableid, table_name, creation_date, validity_date, cleanup_date) VALUES (?, ?, ?, ?, ?, ?, ?)", statisticsType, parameterString, tempTableID, tempTableName, new Date(), DateUtilities.addMinutesToDate(new Date(), 10), DateUtilities.addMinutesToDate(new Date(), 15));
			updateEmbedded("DELETE FROM " + TEMPTABLECACHE_TABLENAME + " WHERE type = ? AND parameter = ? AND validity_date IS NULL", statisticsType, parameterString);
		} catch (Exception e) {
			logger.error("Cannot storeTempTableInCache: {}", e.getMessage(), e);
		}
	}
		...
		if (datasource != null) {
			...
			try {
				...
				throw new IllegalStateException(
						"Datasource 'java:comp/env/jdbc/emm_db' is not available in JNDI",
						e
				);
			}
		}
	...
	protected DataSource getEmbeddedDatasource() {
		if (embeddedDataSource != null) {
			...
			try {
				...
				throw new IllegalStateException(
						"Embedded datasource 'java:comp/env/jdbc/embedded' is not available in JNDI",
						e
				);
			}
		}
	...
	private JdbcTemplate getEmbeddedJdbcTemplate() {
	...
	protected class DateFormats {
		...
		private void calcPeriod() {
			try {
				...
				if (hourScale) {
					period = 1 + BigDecimal.valueOf(TimeUnit.HOURS.convert(edt.getTime() - sdt.getTime(), TimeUnit.MILLISECONDS)).longValue();
				} else {
					period = 1 + ChronoUnit.DAYS.between(LocalDate.parse(startDate), LocalDate.parse(stopDate));
				}
	...
	public void resetTempDatabase() {
		try {
			...
			List<String> existingTables = selectEmbedded("SELECT t.tablename FROM sys.systables t, sys.sysschemas s WHERE t.schemaid = s.schemaid AND t.tabletype = 'T'", StringRowMapper.INSTANCE);
			for (String existingTable : existingTables) {
				executeEmbedded("DROP TABLE " + existingTable);
			}
			...
			executeEmbedded("CREATE SEQUENCE birt_report_tmp_tbl_seq AS INT START WITH 1 MAXVALUE 999999 CYCLE");
		} catch (Exception e) {
	...
	public Map<Integer, Boolean> getMailingBouncesExpire(int companyId, String mailingIds) {
		...
		List<Map<String, Object>> result = select(sql, companyId);
		for (Map<String, Object> row : result) {
			int mailingId = toInt(row.get("mailing_id"));
	...
	public List<Integer> getAutoOptimizationMailings(int optimizationID, int companyID) {
        ...
        if (!optimizationElements.isEmpty()) {
            ...
            for (int i = 1; i <= 5; i++ ){
                int groupId = toInt(map.get("group" + i + "_id"));
                result.add(groupId);
            }
            result.add(map.get("final_mailing_id") == null ? 0 : toInt(map.get("final_mailing_id")));
        }
        return result;
    }
    ...
    public int getAutoOptimizationWinnerId(int optimizationID, int companyID) {
        ...
        return selectInt(query, optimizationID, companyID);
    }
    ...
    protected int getNumberSentMailings(int companyID, int mailingID, String recipientsType, String targetSql, String startDateString, String endDateString) {
    	Date startDate = null;
    	Date endDate = null;
        ...
        if (mailingType == MailingType.INTERVAL) {
            ...
            return selectIntWithDefaultValue(queryBuilder.toString(), 0, parameters.toArray(new Object[0]));
        } else {
			if (!useTargetGroup) {
	...
	private Date parseStatisticDate(String dateString) {
		String format = Strings.CS.contains(dateString, ":") ? DATE_PARAMETER_FORMAT_WITH_HOUR : DATE_PARAMETER_FORMAT;
		return parseDate(dateString, format);
	}
	
	private Date parseStatisticEndDate(String dateString) {
		if (Strings.CS.contains(dateString, ":")) {
	...
	protected int selectNumberOfDeliveredMails(int companyID, int mailingID, String recipientsType, String targetSql, String startDateString, String endDateString) {
    ...
    public boolean successTableActivated(int companyId) {
    	return select("SELECT COALESCE(mailtracking, 0) FROM company_tbl WHERE company_id = ?", Integer.class, companyId) != 0;
    }

    public boolean hasSuccessTableData(int companyID, int mailingID) {
    	return select("SELECT COUNT(*) FROM success_" + companyID + "_tbl WHERE mailing_id = ?", Integer.class, mailingID) > 0;
	}
	...
	protected void logSqlError(Exception e, String statement, Object... parameter) {
    	...
    	if (parameter != null && parameter.length > 0) {
			if (logger.isErrorEnabled()) {
				logger.error("Error: {}\nSQL:{}\nParameter: {}", e.getMessage(), statement, getParameterStringList(parameter), e);
			}
    	} else {
            logger.error("Error: {}\nSQL:{}", e.getMessage(), statement, e);
    	}
	}
	 ...
	 */
	protected List<Map<String, Object>> selectEmbedded(String statement, Object... parameter) {
		try {
			logSqlStatement("EMBEDDED: " + statement, parameter);
	 ...
	 * Logs the statement and parameter in debug-level, executes select and logs error.
	 */
	protected <T> List<T> selectEmbedded(String statement, RowMapper<T> rowMapper, Object... parameter) {
		try {
			logSqlStatement("EMBEDDED: " + statement, parameter);
	 ...
	 * Logs the statement and parameter in debug-level, executes select and logs error.
	 */
	protected <T> T selectEmbedded(String statement, Class<T> requiredType, Object... parameter) {
		try {
			logSqlStatement("EMBEDDED: " + statement, parameter);
	 ...
	 * Logs the statement and parameter in debug-level, executes select and logs error.
	 */
	protected int selectEmbeddedIntWithDefault(String statement, int defaultValue, Object... parameter) {
		try {
			logSqlStatement("EMBEDDED: " + statement, parameter);
	...
	protected int selectEmbeddedInt(String statement, Object... parameter) {
		return selectEmbeddedIntWithDefault(statement, 0, parameter);
	}
	 ...
	 */
	protected int updateEmbedded(String statement, Object... parameter) {
		try {
			logSqlStatement("EMBEDDED: " + statement, parameter);
	 ...
	 */
	protected void executeEmbedded(String statement) {
		try {
			logSqlStatement("EMBEDDED: " + statement);
			getEmbeddedJdbcTemplate().execute(statement);
		} catch (RuntimeException e) {
			logSqlError(e, "EMBEDDED: " + statement);
			throw e;
		}
	}
	 ...
	 */
	public int[] batchupdateEmbedded(String statement, List<Object[]> values) {
		try {
			...
			logSqlError(e, "EMBEDDED: " + statement, "BatchUpdateParameterList(Size: " + values.size() + ")");
			throw e;
		}
	}
    
	public boolean isMailTrackingExpired(int companyID, int mailingID) {
        int periodicallySendEntries = selectInt("SELECT COUNT(mst.mailing_id) AS count FROM maildrop_status_tbl mst JOIN mailing_tbl mt ON mst.mailing_id = mt.mailing_id WHERE mst.mailing_id = ? AND mst.status_field IN ('C', 'E', 'R', 'D') AND mt.work_status = '" + MailingStatus.ACTIVE.getDbKey() + "' AND mst.senddate < CURRENT_TIMESTAMP", mailingID);
		...
		int countOfOnceSending = selectInt("SELECT COUNT(mst.mailing_id) AS count FROM maildrop_status_tbl mst WHERE mst.mailing_id = ? AND mst.status_field IN ('W') AND mst.senddate < CURRENT_TIMESTAMP AND mst.senddate >= ?", mailingID, DateUtilities.getDateOfDaysAgo(expirePeriod));
		return countOfOnceSending <= 0;
    }
	
	public boolean checkEmbeddedTableExists(String tableName) {
	...
	public String getPredefineMailingName(int mailingFilter, int predefineMailingId, int companyId) {
		FilterType filterType = FilterType.getFilterTypeByKey(mailingFilter);
		String sql = getPredefineMailingNameSql(filterType);
		return StringUtils.isEmpty(sql) ? "" : select(sql, String.class, predefineMailingId, companyId);
	}
	...
	private String getPredefineMailingNameSql(FilterType filterType) {
		switch (filterType) {
			case FILTER_ARCHIVE:
				return "SELECT shortname FROM campaign_tbl WHERE campaign_id = ? AND company_id = ?";
			case FILTER_MAILINGLIST:
				return "SELECT shortname FROM mailinglist_tbl WHERE mailinglist_id = ? AND company_id = ?";
			case FILTER_TARGET:
				return "SELECT target_shortname FROM dyn_target_tbl WHERE target_id = ? AND company_id = ?";
			default:
				return "";
		}
	}
	...
	protected static Date parseDate(String date, SimpleDateFormat format) {
		try {
			return format.parse(date);
		} catch (ParseException e) {
			throw new DateParseException(date, format.toPattern(), e);
		}
	}
