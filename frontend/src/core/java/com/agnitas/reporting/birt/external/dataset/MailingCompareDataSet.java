...
public class MailingCompareDataSet extends ComparisonBirtDataSet  {
    ...
    public Map<Integer, Integer> prepareReport(String mailingIdsCsv, int companyId) {
        if (!StringUtils.containsOnly(mailingIdsCsv, "1234567890,")) {
            logger.error("Wrong format of mailing-IDs string: {}", mailingIdsCsv);
            return Collections.emptyMap();
        }

        return Arrays.stream(StringUtils.split(mailingIdsCsv, ","))
    ...
    public Map<Integer, Integer> prepareReport(Set<Integer> mailingIds, int companyId) {
        return mailingIds.stream()
                .filter(v -> v > 0)
                .collect(Collectors.toMap(
                        Function.identity(),
                        mailingId -> tryCollectMailingData(companyId, mailingId))
    ...
    public List<CompareStatRow> getData(Map<Integer, Integer> tempTables) {
    ...
    public List<CompareStatCsvRow> getCsvSummaryData(Map<Integer, Integer> tempTables) {
    ...
    public void dropTempTables(Map<Integer, Integer> tempTables) {
