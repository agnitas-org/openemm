...
public class MailingDeliveryTimeBasedDataSet extends BIRTDataSet {
    ...
    public List<TimeBasedDeliveryStatRow> getData(int mailingID, int companyID, String language, String startDateString, String endDateString) {
        String query;
        ...
        DateFormat dateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, Locale.of(language));
        List<TimeBasedDeliveryStatRow> rows = select(query, new TimeBasedDeliveryRowMapper(dateFormat, isDateBasedMailing), startDate, endDate, mailingID, companyID);
    ...
    private static Date getDeliveryDate(String dateString) {
        Date startDate;
        Calendar startDateCalendar = new GregorianCalendar();
        if (dateString.contains(".") && StringUtils.countOccurrencesOf(dateString, ":") > 1) {
            startDate = parseDate(dateString, DATE_PARAMETER_FORMAT_WITH_SECOND);
            startDateCalendar.setTime(startDate);
        } else if (dateString.contains(":")) {
            startDate = parseDate(dateString, DATE_PARAMETER_FORMAT_WITH_HOUR);
            startDateCalendar.setTime(startDate);
        } else {
            startDate = parseDate(dateString, DATE_PARAMETER_FORMAT);
            startDateCalendar.setTime(startDate);
    ...
    private static class TimeBasedDeliveryRowMapper implements RowMapper<TimeBasedDeliveryStatRow> {
        
        private final DateFormat format;
        private final boolean isDateBasedMailing;
    
        public TimeBasedDeliveryRowMapper(DateFormat format, boolean isDateBasedMailing) {
            this.format = format;
