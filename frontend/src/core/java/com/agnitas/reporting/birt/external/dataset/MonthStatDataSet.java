...
public class MonthStatDataSet extends BIRTDataSet {
	...
	public List<MonthCounterStat> getMailingCounts(int companyID, int adminId, String startDateString, String endDateString) {
		SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT_PATTERN);
		Date startDate = parseDate(startDateString, dateFormat);
		Date endDate = DateUtilities.addDaysToDate(parseDate(endDateString, dateFormat), 1);
		String query;
	 ...
	 * This must include numbers of already deleted mailings and numbers of test mailings, because it will be compared to billing statistics by some users
	 */
	public List<MonthDetailStatRow> getMonthDetails(int companyID, int adminId, String startDateString, String endDateString, int top10Metric) {
		MonthlyStatType metric = MonthlyStatType.get(top10Metric);
		if (metric == null) {
			throw new IllegalArgumentException("Invalid top10Metric type: %d".formatted(top10Metric));
		}
		...
		StringBuilder queryBuilder = new StringBuilder("SELECT m.mailing_id, m.shortname, m.description, a.mailtype, SUM(a.no_of_mailings) mailing_count, SUM(a.no_of_bytes) / SUM(a.no_of_mailings) * 1.0 / 1024 kbPerMail");
	 ...
	 * This must include numbers of already deleted mailings and numbers of test mailings, because it will be compared to billing statistics by some users
	 */
	public List<MonthTotalStatRow> getMonthTotals(int companyID, int adminId, String startDateString, String endDateString, int top10Metric) {
		MonthlyStatType metric = MonthlyStatType.get(top10Metric);
		if (metric == null) {
			throw new IllegalArgumentException("Invalid top10Metric type: %d".formatted(top10Metric));
		}
