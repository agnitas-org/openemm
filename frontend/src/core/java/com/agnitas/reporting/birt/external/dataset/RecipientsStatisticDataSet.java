...
public class RecipientsStatisticDataSet extends RecipientsBasedDataSet {
    ...
    public static final String OPENERS_MULTIPLE_DEVICES = "report.opens.pc.and.mobile";
    ...
    public Map<String, Integer> initRecipientsStatistic(int companyId, String selectedMailingListsAsString,
                                                        String selectedTargets, String startDateString, String stopDateString,
                                                        String figuresOptions, String hiddenFilterTargetIdStr) {
     ...
     */
    public List<RecipientsStatisticCommonRow> getRecipientsStatistic(int tempTableId) {
        if (statList.isEmpty()) {
    ...
    public List<RecipientsStatisticRowByActivity> getRecipientStatisticByActivity(int tempTableId) {
    ...
    public List<RecipientsStatisticRowByCategoryIndex> getRecipientDoiStatistics(int tempTableId) {
    ...
    public List<RecipientsStatisticRowByCategoryIndex> getRecipientStatisticByMailtype(int tempTableId) {
        List<RecipientsStatisticCommonRow> statistics = getRecipientsStatistic(tempTableId);
        List<RecipientsStatisticRowByCategoryIndex> statsByMailtype = new ArrayList<>(statistics.size() * 2);
    ...
    public List<RecipientsStatisticRowByCategoryIndex> getRecipientStatisticByStatuses(int tempTableId) {
        List<RecipientsStatisticCommonRow> statistics = getRecipientsStatistic(tempTableId);
    ...
    public List<RecipientsStatisticRowByTargetGroup> getRecipientStatisticByTargetGroup(int tempTableId) {
        List<RecipientsStatisticCommonRow> statistics = getRecipientsStatistic(tempTableId);
    ...
    private int prepareReport(int companyId, String selectedTargetsAsString,
                              ...
                              int tempTableInfoInRowId, String hiddenFilterTargetStr) {
        ...
        if (StringUtils.isNotBlank(startDateString)) {
            startDate = parseDate(startDateString, DATE_PARAMETER_FORMAT);
        }

        Date endDate = null;
        if (StringUtils.isNotBlank(endDateString)) {
            endDate = parseDate(endDateString, DATE_PARAMETER_FORMAT);
        }
     ...
     * @return list
     */
    public List<RecipientCollectedStatisticRow> getData(int tempTableId) {
     ...
     * @return id of the create temporary table
     **/
    private int createRecipientsStatisticTempTable() {
        int tempTableId = getNextTmpID();
    ...
    private Map<DeviceClass, Integer> getUniqueDeviceClassClickers(int companyId, int mailingListId, Date startDate,
                                                                   Date endDate, String targetSql, boolean useTargetSql, final String filterTargetSql) {
        ...
        Map<DeviceClass, Integer> uniqueDeviceClassClickers = new EnumMap<>(DeviceClass.class);
    ...
    private int getOverallClickersCounter(int companyId, int mailingListId, Date startDate, Date endDate,
                                          String targetSql, boolean useTargetSql, final String filterTargetSql) {
     ...
     */
    private void updateAllRates(int tempTableId, int companyId, List<LightTarget> targetGroups, List<BirtReporUtils.BirtReportFigure> figures) {
    ...
    private void updateRates(int tempTableId, int companyId, List<LightTarget> targetGroups, Object[] mainIndexes, Integer[] indexes) {
    ...
    private void addRecipientStatDoi(int companyId, int tempTableId, List<Integer> mailingListIds, String startDateStr, String stopDateStr) {
        Date startDate = parseDate(startDateStr, "yyyy-MM-dd");
        Date endDate = DateUtils.addDays(parseDate(stopDateStr, "yyyy-MM-dd"), 1);
    ...
    private void addRecipientStatMailtype(int companyId, int tempTableId, List<Integer> mailingListIds,
                                          String selectedTargets, String startDate, String stopDate, String hiddenFilterTargetStr) {
        for (int mailinglistId : mailingListIds) {
            // All subscribers
    ...
    private void addRecipientStatMailtype(int companyId, int tempTableId, int mailinglistId,
                                          LightTarget target, String startDate, String stopDate, String hiddenFilterTargetStr) {
        target = getDefaultTarget(target);
        final String hiddenTargetSql = getHiddenTargetSql(companyId, target, hiddenFilterTargetStr);
        ...
        for (Map<String, Object> row : result) {
            ...
            switch (MailType.getFromInt(mailtype)) {
                case TEXT:
                    countTypeText = count;
                    break;
                case HTML:
                ...
                case HTML_OFFLINE:
                    ...
                    break;
                default:
                    throw new UnsupportedOperationException("Invalid MailType: " + mailtype);
            }
        }
    ...
    private void insertRecipientStatUserStatus(int companyId, int tempTableId, int mailinglistId, LightTarget target,
                                               String startDate, String stopDate, boolean isAsOf, String hiddenFilterTargetStr) {
        target = getDefaultTarget(target);
        final String hiddenTargetSql = getHiddenTargetSql(companyId, target, hiddenFilterTargetStr);
    ...
    private String getDateConstraint(String fieldName, String startDate, String stopDate, int constraintType) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        // validate date format
        switch (constraintType) {
            case DATE_CONSTRAINT_BETWEEN:
                parseDate(startDate, format);
                parseDate(stopDate, format);
                break;
            ...
            default:
                //should never happen
                throw new IllegalArgumentException("Can not parse startDate(" + startDate + ") or endDate(" + stopDate + ")! Constraint type: " + constraintType);
        }
        ...
        if (isOracleDB()) {
            switch (constraintType) {
                ...
                default:
                    throw new IllegalArgumentException("Can not create DateConstraint");
            }
            ...
            return switch (constraintType) {
                ...
                default -> throw new IllegalArgumentException("Can not create DateConstraint");
            };
        } else {
            switch (constraintType) {
                case DATE_CONSTRAINT_BETWEEN:
    ...
    private void insertEmptyRowsIntoTempTable(int companyId, int tempTableId, List<Integer> mailingListIds, String selectedTargets) {
    ...
    private void addActiveRecipients(int companyId, List<LightTarget> targetGroups, List<Integer> mailingListIds,
                                     ...
                                     boolean calculateRecipientDevelopmentNet, String hiddenFilterTargetStr) {
    ...
    private void insertRecipientStat(int companyId, int tempTableId, int mailinglistId,
                                     ...
                                     String endDateString, String hiddenFilterTargetStr) {
    ...
    private void updateNumberOfTargetGroups(int tempTableId, int numberOfTargetGroups) {
