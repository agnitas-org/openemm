...
public class TopDomainReportDataSet extends TopDomainsDataSet {
	...
	private List<Integer> getSentMailings(int companyId, List<Integer> mailingListIds, String startDateString, String stopDateString) {
        Date startDate = null;
		...
		if (StringUtils.isNotBlank(stopDateString)) {
			endDate = parseDate(stopDateString, DATE_PARAMETER_FORMAT);
		}
    ...
    private List<Integer> getSentMailings(int companyId, int mailingListId, String startDateString, String stopDateString) {
    ...
    public int prepareReport(int companyID, String mailinglistAsString, String targetsAsString, String startDateString, String stopDateString,
							 int domainsMax, String language, boolean topLevelDomains, String figuresOptions) {
	...
	private void updateRates(int tempTableID, List<LightTarget> targets, boolean mailtrackingDataAvailable) {
	...
	private void updateRates(int tempTableID, int totalCategoryIndex, int targetGroupIndex, int... categories) {
		updateRates(tempTableID, totalCategoryIndex, targetGroupIndex, true, categories);
	}

	private void updateRates(int tempTableID, int totalCategoryIndex, int targetGroupIndex, boolean mailTracking, int... categories) {
	...
	private void insertEmptyDomainData(int tempTableID, List<LightMailingList> mailingLists, List<LightTarget> targets, String figuresOptions) {
		boolean targetsAvailable = CollectionUtils.isNotEmpty(targets);
	...
	private void insertEmptyRowIfNotExists(int tempTableID, TempRow tempRow, int mlid, String mlName) {
	...
	private void insertStatistic(int tempTableID, int companyID,
								 ...
								 String language, int domainsMax, boolean topLevelDomains, String figuresOptions) {
    ...
    private void insertSoftBouncesIntoTempTable(int tempTableID, int companyID, int mlId, String mlName, List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains) {
    ...
    private void insertHardBouncesIntoTempTable(int tempTableID, int companyID, int mlId, String mlName, List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains) {
        insertBouncesIntoTempTable(tempTableID, companyID, mlId, mlName, mailingIds, targets, language, domainsMax, topLevelDomains,
                new TopDomainsDataSet.Rule("bounce.detail >= 510", CATEGORY_NAME_HARDBOUNCES, CATEGORY_HARDBOUNCES));
    }
    
    @DaoUpdateReturnValueCheck
    private void insertBouncesIntoTempTable(int tempTableID, int companyID, int mlId, String mlName, List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains, Rule rule) {
	...
	@DaoUpdateReturnValueCheck
    private void insertOpenersIntoTempTable(int tempTableID, int companyID, int mlId, String mlName, List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains) {
		Map<Integer, TempRow> otherDomainsMap = new HashMap<>();
        int otherDomainIndex = domainsMax + 1;
    ...
    @DaoUpdateReturnValueCheck
	private void insertClickersIntoTempTable(int tempTableID, int companyID, int mlId, String mlName, List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains) {
		Map<Integer, TempRow> otherDomainsMap = new HashMap<>();
        int otherDomainIndex = domainsMax + 1;
    ...
    private void insertSentMailingPerDomain(int tempTableID, List<DomainStatisticRow> sentMailingStatistic, List<LightTarget> targets,
											...
											int maxDomains, String language) {
        int domainIndex = 1;
    ...
    private void insertSentStatIntoTempTable(int tempTableID, int companyID, int mlId, String mlName,
											 List<Integer> mailingIds, List<LightTarget> targets, String language, int domainsMax, boolean topLevelDomains) {
    ...
    private List<DomainStatisticRow> getSentMailingsStatistic(int companyId, List<LightTarget> targets, List<Integer> mailingIds, boolean topLevelDomains) {
        List<Integer> targetIds = targets.stream().map(LightTarget::getId).toList();

        StringBuilder queryBuilder = new StringBuilder("SELECT COUNT(mailing_id) AS mails_per_domain, domain_name");

        for (LightTarget target : targets) {
            queryBuilder.append(", SUM(tg_").append(target.getId()).append("_send) AS tg_").append(target.getId());
        }
        ...
        for (LightTarget target : targets) {
            queryBuilder.append(", CASE WHEN (").append(target.getTargetSQL()).append(") THEN 1 ELSE 0 END AS tg_").append(target.getId()).append("_send");
        }
    ...
    private void insertTotals(int tempTableID, int total, Map<Integer, Integer> targetTotals, List<LightTarget> targets,
                              int mailinglistId, String mailinglist) {
	...
	public List<DomainSendStatRow> getStatisticPerDomain(int tempTableID) {
	...
	private void insertIntoTempTable(int tempTableID, TempRow row, int mailinglistId, String mailinglist) {
	...
	private void insertIntoTempTable(int tempTableID, List<TempRow> rows, int mailinglistId, String mailinglist) {
	 ...
	 */
	private int createTopDomainsTempTable() {
