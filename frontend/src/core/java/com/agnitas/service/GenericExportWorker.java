...
public class GenericExportWorker implements Callable<GenericExportWorker> {
    ...
    public GenericExportWorker() {
		...
		dateFormat = DateFormat.getDateInstance(DateFormat.SHORT, dateAndDecimalLocale);
	...
	public GenericExportWorker call() throws Exception {
		try {
			...
			if (compressionType == FileCompressionType.ZIP && StringUtils.isEmpty(zipPassword)) {
				if (!Strings.CI.endsWith(exportFile, ".zip")) {
					if (!Strings.CI.endsWith(exportFile, "." + "csv")) {
				...
				if (!Strings.CI.endsWith(exportFile, ".tar.gz")) {
					if (!Strings.CI.endsWith(exportFile, "." + "csv")) {
						exportFile = exportFile + "." + "csv";
					}
				...
				if (!Strings.CI.endsWith(exportFile, ".tgz")) {
					if (!Strings.CI.endsWith(exportFile,"." + "csv")) {
						exportFile = exportFile + "." + "csv";
					}
				...
				if (!Strings.CI.endsWith(exportFile, ".gz")) {
					if (!Strings.CI.endsWith(exportFile, "." + "csv")) {
						exportFile = exportFile + "." + "csv";
					}
			...
			} else if (!Strings.CI.endsWith(exportFile, "." + "csv")) {
				exportFile = exportFile + "." + "csv";
			}
			    	...
			    	try (PreparedStatement preparedStatement = connection.prepareStatement(selectStatement)) {
			    		...
			    		try (ResultSet resultSet = preparedStatement.executeQuery()) {
					    	...
					    	if (csvFileHeaders != null) {
					    		...
					    		} else if (!csvFileHeaders.isEmpty()) {
			...
			if (compressionType == FileCompressionType.ZIP && StringUtils.isNotEmpty(zipPassword)) {
				...
				if (StringUtils.isBlank(entryFileName)) {
					...
					if (Strings.CI.endsWith(entryFileName, "." + "zip")) {
						entryFileName = entryFileName.substring(0, entryFileName.length() - 4);
					}
					if (!Strings.CI.endsWith(entryFileName, "." + "csv")) {
				...
				if (StringUtils.isBlank(entryFileName)) {
					entryFileName = new File(exportFile).getName();
					if (Strings.CI.endsWith(entryFileName, "." + "tar.gz")) {
				...
				if (StringUtils.isBlank(entryFileName)) {
					entryFileName = new File(exportFile).getName();
					if (Strings.CI.endsWith(entryFileName, "." + "tgz")) {
						entryFileName = entryFileName.substring(0, entryFileName.length() - 4);
					}
					if (!Strings.CI.endsWith(entryFileName, "." + "csv")) {
						entryFileName = entryFileName + "." + "csv";
