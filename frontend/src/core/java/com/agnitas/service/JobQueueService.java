...
public class JobQueueService implements ApplicationContextAware {
	...
	private synchronized void checkAndStartNewWorkers() {
		if (checkActiveNode()) {
			...
			while (!queuedJobsTodo.isEmpty() && queuedJobWorkers.size() < configService.getIntegerValue(ConfigValue.MaximumParallelJobQueueJobs)) {
				JobDto jobToStart = queuedJobsTodo.getFirst();
	...
	private JobWorkerBase createJobWorker(JobDto jobToStart) throws InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		JobWorkerBase worker = jobWorkersRegistry.findWorker(jobToStart.getName())
	...
	public boolean isStatusOK() {
		return jobQueueDao.selectCriticalErroneousJobs().isEmpty();
	}
	...
	public List<JobDto> getForOverview() {
		return jobQueueDao.getForOverview();
	}
	...
	public boolean isReportOK() {
		return birtReportDao.selectErroneousReports().isEmpty();
	}
