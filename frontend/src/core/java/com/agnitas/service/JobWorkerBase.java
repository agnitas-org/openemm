 ...
 */
public abstract class JobWorkerBase implements Runnable {

	/**
	 * Logger of the JobWorker class. The implementing classes should have their own logger
	 */
	private static final Logger logger = LogManager.getLogger(JobWorkerBase.class);
	...
	public void run() {
		try {
			...
			try {
				...
				logger.info("Job worker {} ({}) received STOP signal", this.getClass().getName(), job.getId());
				resultText = "OK";
				job.setLastResult("OK");
			} catch (Exception e) {
				logger.error("Error in {}: {}", this.getClass().getName(), e.getMessage(), e);
	 ...
	 * This may be ignored by deriving classes, but it would be nice if they use it to control execution
	 */
	protected boolean continueJobWanted() {
		try {
			while (pause) {
				Thread.sleep(500);
			}
	...
	public final List<Integer> parameterAsIntegerListOrNull(final String parameterName) throws Exception {
		...
		if (StringUtils.isNotBlank(listAsString)) {
			try {
				return AgnUtils.splitAndTrimList(listAsString)
						.stream()
	...
	protected String getParam(String paramName) {
		return job.getParameters().get(paramName);
	}

	protected int getPositiveIntParam(String param) {
		String val = getParam(param);
		if (StringUtils.isBlank(val) || !NumberUtils.isCreatable(val) || NumberUtils.toInt(val) <= 0) {
			throw new JobWorkerConfigurationException("Invalid parameter '%s'".formatted(param));
		}
