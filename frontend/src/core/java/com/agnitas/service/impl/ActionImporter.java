...
public class ActionImporter extends BaseImporterExporter {
	...
	public int importAction(int companyID, JsonObject actionJsonObject, Map<Integer, Integer> mailingIdReplacements) {
		...
		for (EmmAction existingAction : actionDao.getEmmActionsByName(companyID, actionName)) {
			...
			if (Strings.CS.equals(actionDescription, existingAction.getDescription())
				...
				for (int i = 0; i < actionOperationsJsonArray.size(); i++) {
					...
					if (!actionOperationType.equals(existingActionOperation.getOperationType().getName())) {
						...
						if (ActionOperationType.ACTIVATE_DOUBLE_OPT_IN.getName().equals(actionOperationType)) {
							...
							if (!Strings.CS.equals(tagName, ((ActionOperationContentViewParameters) existingActionOperation).getTagName())) {
								isMatching = false;
								break;
							}
							...
							if (!Strings.CS.equals(script, ((ActionOperationExecuteScriptParameters) existingActionOperation).getScript())) {
								isMatching = false;
								break;
							}
							...
							if (!Strings.CS.equals(expirationString, actionOperationGetArchiveMailing.getExpireYear() + "-" + actionOperationGetArchiveMailing.getExpireMonth() + "-" + actionOperationGetArchiveMailing.getExpireDay())) {
								isMatching = false;
								break;
							}
							...
							if (!Strings.CS.equals(keyColumn, ((ActionOperationIdentifyCustomerParameters) existingActionOperation).getKeyColumn())
									|| !Strings.CS.equals(passColumn, ((ActionOperationIdentifyCustomerParameters) existingActionOperation).getPassColumn())) {
								isMatching = false;
								break;
							}
							...
							if (mailingID != ((ActionOperationSendMailingParameters) existingActionOperation).getMailingID()
									...
									|| !Strings.CS.equals(bccAddress, ((ActionOperationSendMailingParameters) existingActionOperation).getBcc())) {
								isMatching = false;
								break;
							}
							...
							if (mailtype != ((ActionOperationServiceMailParameters) existingActionOperation).getMailtype()
									|| !Strings.CS.equals(toAddress, ((ActionOperationServiceMailParameters) existingActionOperation).getToAddress())
									|| !Strings.CS.equals(fromAddress, ((ActionOperationServiceMailParameters) existingActionOperation).getFromAddress())
									|| !Strings.CS.equals(replyAddress, ((ActionOperationServiceMailParameters) existingActionOperation).getReplyAddress())
									|| !Strings.CS.equals(subject, ((ActionOperationServiceMailParameters) existingActionOperation).getSubjectLine())
									|| !Strings.CS.equals(textMail, ((ActionOperationServiceMailParameters) existingActionOperation).getTextMail())
									|| !Strings.CS.equals(htmlMail, ((ActionOperationServiceMailParameters) existingActionOperation).getHtmlMail())) {
								isMatching = false;
								break;
							}
							...
							if (!Strings.CS.equals(keyColumn, ((ActionOperationSubscribeCustomerParameters) existingActionOperation).getKeyColumn())
							...
							if (!Strings.CS.equals(columnName, ((ActionOperationUpdateCustomerParameters) existingActionOperation).getColumnName())
									|| updateType != ((ActionOperationUpdateCustomerParameters) existingActionOperation).getUpdateType()
									|| !Strings.CS.equals(updateValue, ((ActionOperationUpdateCustomerParameters) existingActionOperation).getUpdateValue())
		...
		if (foundExistingActionID == -1) {
			...
			for (int i = 0; i < actionOperationsJsonArray.size(); i++) {
				...
				if (ActionOperationType.ACTIVATE_DOUBLE_OPT_IN.getName().equals(actionOperationType)) {
					...
					actionOperationGetArchiveList.setLimitType((Integer) actionOperation.get("limit_type"));
					actionOperationGetArchiveList.setLimitValue((Integer) actionOperation.get("limit_value"));
