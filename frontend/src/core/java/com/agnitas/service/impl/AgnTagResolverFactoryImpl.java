...
public class AgnTagResolverFactoryImpl implements AgnTagResolverFactory {
    ...
    private class AgnTagResolverImpl implements AgnTagResolver {
        ...
        private String resolveAgnTitle(int titleId) {
            ...
            return switch (name) {
                case "agnTITLE" -> value + " " + lastName;
                case "agnTITLEFIRST" -> genderMap.get(gender) + " " + firstName;
                case "agnTITLEFULL" -> value + " " + firstName + " " + lastName;
                default -> value;
            };
        }
        ...
        private String resolveCustomAgnTag(TagDetails tag) {
            ...
            if (value == null) {
                logger.error("Error processing tag {} ({}). (mailing #{}, customer #{})", name, tag.getFullText(), mailingId, customerId);
            }
        ...
        private TagDefinition getTagDefinition() {
            return definitionCache.computeIfAbsent(name, nameParameter -> {
                ...
                if (definition == null) {
                    logger.error("Unknown tag {} (no definition available)", nameParameter);
                    return null;
                }
