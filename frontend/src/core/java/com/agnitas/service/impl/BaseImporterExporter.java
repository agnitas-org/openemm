...
public class BaseImporterExporter {
    ...
    protected void checkIsJsonObject(JsonNode jsonNode) {
        if (!jsonNode.isJsonObject()) {
            throw new IllegalArgumentException("Invalid JSON data: not a JSON-Object");
        }
    }
	
    protected void checkJsonVersion(String version) {
        if (version == null) {
            return; // If no version given, we assume it is the latest
        }

        if (!AVAILABLE_JSON_VERSIONS.contains(StringUtils.trimToEmpty(version))) {
            throw new IllegalArgumentException("Invalid JSON data version: " + version);
        }
	}
