...
public class ImportProfileServiceImpl implements ImportProfileService {
    ...
    @Override
    public Map<String, DbColumnType> getProfileFieldsInfo(ImportProfile profile, Admin admin) {
        ...
        return recipientFieldService.getRecipientFields(admin.getCompanyID())
                ...
                .collect(Collectors.toMap(
                        ...
                        ImportProfileServiceImpl::getDbColumnType
                ));
    }

    private static DbColumnType getDbColumnType(RecipientFieldDescription f) {
        return new DbColumnType(
                f.getDatabaseDataType(),
                f.getCharacterLength(),
                f.getNumericPrecision(),
                f.getNumericScale(),
                f.isNullable()
        );
    }
    ...
    @Override
    public Map<String, DbColumnType> getBlacklistDbFieldsInfo(int companyId) {
        return importProfileDao.getBlacklistColumnTypes(companyId)
                .entrySet()
                .stream()
                .filter(e -> e.getValue().getSimpleDataType() == SimpleDataType.Characters)
    ...
    public boolean isManageAllowed(ImportProfile profile, Admin admin) {
        ...
        if (isKeyColumnsHasCustomerId(profile) && !isCustomerIdImportAllowed(admin)) {
    ...
    private static boolean isKeyColumnsHasCustomerId(ImportProfile profile) {
        return CollectionUtils.emptyIfNull(profile.getKeyColumns())
                .stream()
                .anyMatch(keyCol -> Strings.CI.equals(keyCol, RecipientStandardField.CustomerID.getColumnName()));
    }
