...
public class ImportWizardHelperImpl implements ImportWizardHelper {
	 ...
	 */
	private Map<ImportErrorType, StringBuffer> errorData = new EnumMap<>(ImportErrorType.class);
	...
	private boolean checkDateHas4Digits(Date inDate) {
        return !inDate.before(borderDate.getTime());
    }
		...
		try {
            if (inputData.size() > csvAllColumns.size()) {
				...
				if (logger.isInfoEnabled()) {
					logger.info("Structure error, missing header info: {}", StringUtils.join(inputData, " "));
				}
				return null;
				...
				if (getColumnMapping().containsKey(aCsvInfo.getName())) {
					...
					if (status.getDoubleCheck() != ImportStatus.DOUBLECHECK_NONE
						...
						if (!uniqueValues.add(aValue)) {
							...
							if (logger.isInfoEnabled()) {
								logger.info("Duplicate email: {}", StringUtils.join(inputData, ";"));
							}
							return null;
						}
					}
					if (aInfo.getName().equalsIgnoreCase("email")) {
						if (aValue.isEmpty()) {
							...
							if (logger.isInfoEnabled()) {
								logger.info("Empty email: {}", StringUtils.join(inputData, " "));
							}
							return null;
						}
						if (aValue.indexOf('@') == -1) {
							...
							if (logger.isInfoEnabled()) {
								logger.info("No @ in email: {}", StringUtils.join(inputData, " "));
							}
							return null;
						}
					...
					if (aInfo != null && aInfo.isActive()) {
						if (aValue.isEmpty()) { // is null value
							valueList.add(null);
						} else {
			...
			logger.error("parseLine: {}", e.getMessage(), e);
			return null;
		}
