...
public class DateUtilities {
	...
	private static final long A_MINUTE_MILLISECONDS = 1000 * 60L;
	private static final long A_DAYS_MILLISECONDS = A_MINUTE_MILLISECONDS * 60 * 24;

	private static final Pattern MONTH_REGEX = Pattern.compile("m+",  Pattern.CASE_INSENSITIVE);
	private static final Pattern DAY_REGEX = Pattern.compile("d+",  Pattern.CASE_INSENSITIVE);
	private static final Pattern YEAR_REGEX = Pattern.compile("y+",  Pattern.CASE_INSENSITIVE);
	...
	public static Date getDateOfDaysAgo(int daysCount, ZoneId zoneId, TemporalUnit truncatedTo) {
	...
	public static SimpleDateFormat getDateTimeFormat(int dateStyle, int timeStyle, Locale locale, TimeZone timezone) {
		SimpleDateFormat format = (SimpleDateFormat) DateFormat.getDateTimeInstance(dateStyle, timeStyle, locale);
	...
	public static int getNumberOfWeekdayInMonth(int dayOfMonth) {
        ...
        return (int) Math.round(Math.ceil(ordinalFloat));
	}
	...
	public static LocalDateTime getLocalDateTimeForDate(Date date) {
		if (date == null) {
			...
			try {
                ...
                return LocalDateTime.ofEpochSecond(epochSeconds, nanoseconds, ZoneOffset.ofTotalSeconds(date.getTimezoneOffset() * -60));
