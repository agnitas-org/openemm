...
public class DbUtilities {
    ...
    public static int readoutInOutputStream(Connection connection, String statementString, List<Object> statementParameter, List<String> csvColumnNames, OutputStream outputStream, String encoding, char separator, Character stringQuote) throws Exception {
		try (PreparedStatement statement = connection.prepareStatement(statementString)) {
			if (statementParameter != null && !statementParameter.isEmpty()) {
	...
	public static boolean checkDbVendorIsPostgreSQL(DataSource dataSource) {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			throw new IllegalStateException("Cannot detect db vendor: " + e.getMessage(), e);
		}
	}
	...
	private static boolean checkDbVendorIsPostgreSQL(Connection connection) {
		if (connection == null) {
	...
	private static boolean checkDbVendorIsOracle(Connection connection) {
		if (connection == null) {
			throw new IllegalArgumentException("Cannot detect db vendor: connection is null");
		}
	...
	private static boolean checkDbVendorIsMariaDB(Connection connection) {
		if (connection == null) {
			throw new IllegalArgumentException("Cannot detect db vendor: connection is null");
		}
		...
		try {
			...
			return "maria".equalsIgnoreCase(productName) || "mariadb".equalsIgnoreCase(productName)
					|| dbMetaData.getURL().toLowerCase().startsWith("jdbc:mariadb:");
		} catch (SQLException e) {
			logger.error("Cannot detect db vendor: {}", e.getMessage(), e);
			throw new IllegalStateException("Cannot detect db vendor: " + e.getMessage(), e);
		}
	...
	private static boolean checkDbVendorIsMySQL(Connection connection) throws SQLException {
		if (connection == null) {
			throw new IllegalArgumentException("Cannot detect db vendor: connection is null");
		}
		...
		try {
			...
			String productName = dbMetaData.getDatabaseProductName();
			return "mysql".equalsIgnoreCase(productName) || "mysqldb".equalsIgnoreCase(productName)
					|| dbMetaData.getURL().toLowerCase().startsWith("jdbc:mysql:");
		} catch (SQLException e) {
			logger.error("Cannot detect db vendor: {}", e.getMessage(), e);
			throw new IllegalStateException("Cannot detect db vendor: " + e.getMessage(), e);
		}
	...
	public static String getDbUrl(DataSource dataSource) {
		try (Connection connection = dataSource.getConnection()) {
			DatabaseMetaData dbMetaData = connection.getMetaData();
			return dbMetaData == null ? null : dbMetaData.getURL();
		} catch (SQLException e) {
			return null;
        }
    }

	public static boolean checkTableAndColumnsExist(DataSource dataSource, String tableName, String... columns) {
		try (final Connection connection = dataSource.getConnection()) {
			return checkColumnsAndTableExist(connection, tableName, columns);
		} catch (Exception e) {
			return false;
		}
	}

	private static boolean checkColumnsAndTableExist(Connection connection, String tableName, String[] columns) throws SQLException {
		try (Statement statement = connection.createStatement()){
			try (ResultSet resultSet = statement.executeQuery("SELECT * FROM " + tableName + " WHERE 1 = 0")) {
				...
				for (String column : columns) {
					if (column != null && !dbTableColumns.contains(column.toUpperCase())) {
						throw new IllegalStateException("Column '" + column + "' does not exist in table '" + tableName + "'");
					}
				}
				return true;
			}
	...
	protected static void checkTableExists(DataSource dataSource, String tableName) throws Exception {
		try (final Connection connection = dataSource.getConnection()) {
			checkTableExists(connection, tableName);
		}
	}

	private static void checkTableExists(Connection connection, String tableName) throws Exception {
		if (connection == null) {
			throw new IllegalArgumentException("Connection for checkTableExists is null");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("TableName for checkTableExists is empty");
	...
	public static TextTableBuilder getResultAsTextTable(DataSource datasource, String selectString) {
		...
		if (!results.isEmpty()) {
			...
			for (String column : results.getFirst().keySet()) {
	...
	public static List<String> getColumnNames(DataSource dataSource, String tableName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getColumnNames");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getColumnNames");
		}

		try (Connection connection = dataSource.getConnection()) {
			try (Statement stmt = connection.createStatement()) {
				String query = "SELECT * FROM " + SafeString.getSafeDbTableName(tableName) + " WHERE 1 = 0";
			...
			throw new IllegalStateException("Cant get column names of table '%s'!".formatted(tableName), e);
        }
    }

	public static DbColumnType getColumnDataType(DataSource dataSource, String tableName, String columnName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getColumnDataType");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getColumnDataType");
		} else if (StringUtils.isBlank(columnName)) {
			throw new IllegalArgumentException("Invalid empty columnName for getColumnDataType");
		} else {
			try (final Connection connection = dataSource.getConnection()) {
	...
	public static CaseInsensitiveMap<String, DbColumnType> getColumnDataTypes(DataSource dataSource, String tableName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Cannot getColumnDataTypes: dataSource is null");
		}
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			throw new IllegalStateException("Cannot getColumnDataTypes: " + e.getMessage(), e);
		}
	}
	...
	public static CaseInsensitiveMap<String, DbColumnType> getColumnDataTypes(Connection connection, String tableName) throws Exception {
		if (connection == null) {
			throw new IllegalArgumentException("Invalid empty connection for getColumnDataTypes");
		}

		if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getColumnDataTypes");
		}
		...
		if (checkDbVendorIsOracle(connection)) {
			...
			try (PreparedStatement preparedStatementNullable = connection.prepareStatement(nullableSql)) {
				preparedStatementNullable.setNString(1, tableName.toUpperCase());
				try (ResultSet resultSetNullable = preparedStatementNullable.executeQuery()) {
					while (resultSetNullable.next()) {
						String searchCondition = resultSetNullable.getString("search_condition");
						searchCondition = searchCondition != null ? searchCondition.toLowerCase().replace("'", "").replace("\"", "") : "";
						if (searchCondition.endsWith(" is not null")) {
	...
	public static CaseInsensitiveMap<String, CaseInsensitiveMap<String, DbColumnType>> getColumnDataTypes(DataSource dataSource, Set<String> tableNames) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getColumnDataTypes");
		} else {
	...
	public static int getTableEntriesCount(DataSource dataSource, String tableName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getTableEntriesCount");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getTableEntriesCount");
		}

		try (Connection connection = dataSource.getConnection()) {
			try (Statement stmt = connection.createStatement()) {
				String query = "SELECT COUNT(*) FROM " + SafeString.getSafeDbTableName(tableName);
				try (ResultSet rs = stmt.executeQuery(query)) {
					return rs.next() ? rs.getInt(1) : 0;
				}
			}
		} catch (SQLException e) {
			throw new IllegalStateException("Error occurred when reading entries count from table: " + tableName, e);
        }
    }

	public static boolean containsColumnName(DataSource dataSource, String tableName, String columnName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for containsColumnName");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for containsColumnName");
		} else if (StringUtils.isBlank(columnName)) {
			throw new IllegalArgumentException("Invalid empty columnName for containsColumnName");
		} else {
			return getColumnNames(dataSource, tableName).contains(columnName.toLowerCase());
		}
	}

	public static String getColumnDefaultValue(DataSource dataSource, String tableName, String columnName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getDefaultValueOf");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getDefaultValueOf");
		} else if (StringUtils.isBlank(columnName)) {
			throw new IllegalArgumentException("Invalid empty columnName for getDefaultValueOf");
		} else {
			if (checkDbVendorIsOracle(dataSource)) {
				...
				if (Strings.CI.equals(value, "null")) {
					return null;
				}
				...
				String sql = """
                        SELECT CASE
                                     ...
                                     ELSE regexp_replace(column_default, '::[a-zA-Z_ ]+', '', 'g')
	...
	public static CaseInsensitiveMap<String, String> getColumnDefaultValues(DataSource dataSource, String tableName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getDefaultValueOf");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getDefaultValueOf");
			...
			if (checkDbVendorIsOracle(dataSource)) {
				...
				List<Map<String, Object>> result = new JdbcTemplate(dataSource).queryForList(query, tableName);
				for (Map<String, Object> row : result) {
					String columnName = (String) row.get("column_name");
	...
	public static boolean isNowKeyword(String value) {
        return switch (StringUtils.trimToEmpty(value).toUpperCase()) {
            case "NOW", "NOW()", "SYSDATE", "SYSDATE()", "CURRENT_TIMESTAMP", "CURRENT_TIMESTAMP()" -> true;
            default -> false;
        };
	}

	protected static String getDateDefaultValue(String fieldDefault, SimpleDateFormat fieldDefaultDateFormat, boolean isOracle) {
		if (isNowKeyword(fieldDefault)) {
			...
			if (isOracle) {
				 ...
				 */
				return "to_date('" + new SimpleDateFormat(DateUtilities.DD_MM_YYYY).format(parseDate(fieldDefaultDateFormat, fieldDefault)) + "', 'DD.MM.YYYY')";
			} else {
				return "'" + new SimpleDateFormat(DateUtilities.YYYY_MM_DD).format(parseDate(fieldDefaultDateFormat, fieldDefault)) + "'";
			}
		}
	}

	private static Date parseDate(SimpleDateFormat format, String value) {
        try {
            return format.parse(value);
        } catch (ParseException e) {
			throw new IllegalArgumentException("Error parsing date: value '%s' does not match the expected format %s".formatted(value, format.toPattern()), e);
        }
    }

	public static boolean addColumnToDbTable(DataSource dataSource, String tablename, String fieldname, String fieldType, long length, String fieldDefault, SimpleDateFormat fieldDefaultDateFormat, boolean notNull) {
		boolean isOracle = checkDbVendorIsOracle(dataSource);
		boolean isPostgreSQL = !isOracle && checkDbVendorIsPostgreSQL(dataSource);
	...
	public static boolean dropColumnFromDbTable(DataSource dataSource, String tablename, String fieldname) {
		if (StringUtils.isBlank(fieldname)) {
			return false;
		} else if (!tablename.equalsIgnoreCase(SafeString.getSafeDbTableName(tablename))) {
			logger.error("Cannot drop db column: Invalid table name: {}", tablename);
			return false;
		}  else if (!fieldname.equalsIgnoreCase(SafeString.getSafeDbTableName(fieldname))) {
			logger.error("Cannot drop db column: Invalid field name: {}", fieldname);
			return false;
		} else if (!containsColumnName(dataSource, tablename, fieldname)) {
			return false;
		} else {
	...
	public static boolean alterColumnDefaultValueInDbTable(DataSource dataSource, String tablename, String fieldname, String fieldDefault, SimpleDateFormat fieldDefaultDateFormat, boolean notNull) {
		return alterColumnTypeInDbTable(dataSource, tablename, fieldname, null, -1, -1, fieldDefault, fieldDefaultDateFormat, notNull);
	}

	public static boolean alterColumnTypeInDbTable(DataSource dataSource, String tablename, String fieldname, String fieldType, int length, int precision, String fieldDefault, SimpleDateFormat fieldDefaultDateFormat, boolean notNull) {
		boolean isOracle = checkDbVendorIsOracle(dataSource);
		boolean isPostgreSQL = !isOracle && checkDbVendorIsPostgreSQL(dataSource);
		...
		if (StringUtils.isBlank(fieldname)) {
			...
			String changeColumnStatementPart = isPostgreSQL ? "" : fieldname.toLowerCase();
			...
			if (StringUtils.isNotEmpty(fieldType)) {
				...
				if (isPostgreSQL) {
					changeColumnStatementPart += "ALTER COLUMN %s TYPE".formatted(fieldname.toLowerCase());
				}
			...
			if ((currentDefaultValue == null && fieldDefault != null) || currentDefaultValue != null && !currentDefaultValue.equals(fieldDefault)) {
				if (isPostgreSQL) {
					changeColumnStatementPart += "%s ALTER COLUMN %s SET ".formatted(dbChangeIsNeeded ? ", " : "", fieldname.toLowerCase());
				}
			...
			if (dbChangeIsNeeded) {
				...
				if (isOracle) {
					...
					changeColumnStatement = "ALTER TABLE " + tablename + " " + changeColumnStatementPart;
				} else {
	...
	public static String getToDateString_Oracle(int day, int month, int year, int hour, int minute, int second) {
        return "TO_DATE('" + TWO_DIGIT_FORMAT.format(day) +
                "." + TWO_DIGIT_FORMAT.format(month) +
                "." + FOUR_DIGIT_FORMAT.format(year) +
                " " + TWO_DIGIT_FORMAT.format(hour) +
	...
	public static String getToDateString_MySQL(int day, int month, int year, int hour, int minute, int second) {
        return "STR_TO_DATE('" + TWO_DIGIT_FORMAT.format(day) +
                "-" + TWO_DIGIT_FORMAT.format(month) + "-" +
                FOUR_DIGIT_FORMAT.format(year) + " " +
                TWO_DIGIT_FORMAT.format(hour) + ":" +
                TWO_DIGIT_FORMAT.format(minute) + ":" +
                TWO_DIGIT_FORMAT.format(second) + "', '%d-%m-%Y %H:%i:%s')";
	}
	...
	public static boolean renameTableField(DataSource dataSource, String tableName, String fieldNameOld, String fieldNameNew) {
		String changeColumnStatement;
		if (checkDbVendorIsOracle(dataSource) || checkDbVendorIsPostgreSQL(dataSource)) {
	...
	public static int getMysqlMaxAllowedPacketSize(DataSource dataSource) {
		...
		if (!result.isEmpty()) {
            Object value = result.getFirst().get("value");
			if (value != null) {
	...
	public static void checkDatasourceConnection(DataSource dataSource) throws Exception {
		if (dataSource == null) {
			throw new IllegalArgumentException("Cannot acquire datasource");
		} else {
			try (Connection connection = dataSource.getConnection()) {
	...
	public static int extractPort(String jdbcUrl) {
		try {
			String uriPart = jdbcUrl.substring(5); // remove "jdbc:" prefix so URI can parse it
			int port = URI.create(uriPart).getPort();
			...
			throw new IllegalArgumentException("Unknown JDBC URL: " + jdbcUrl);
		} catch (Exception e) {
			throw new IllegalArgumentException("Cannot extract port from JDBC URL: " + jdbcUrl, e);
		}
	}
	...
	public static String joinForIN(Collection<?> elements) {
	...
	public static List<String> getTableIndexNames(DataSource dataSource, String tableName) {
		...
		if (checkDbVendorIsPostgreSQL(dataSource)) {
			String query = """
					...
					""";
			return new JdbcTemplate(dataSource).queryForList(query, String.class, tableName);
		} else {
            ...
            for (Map<String, Object> row : result) {
            	returnList.add((String) row.get("Key_name"));
            }
            return returnList;
        }
    }
	...
	public static CaseInsensitiveSet getPrimaryKeyColumns(Connection connection, String tableName) {
		if (StringUtils.isBlank(tableName)) {
			...
			try {
				...
				throw new IllegalStateException("Cannot read primarykey columns for table " + tableName + ": " + e.getMessage(), e);
			}
		}
	}
	...
	public static String makeBulkInClauseWithParameters(
			boolean isOracle,
			String columnName,
			int valueCount,
			boolean isIn
		...
		if (valueCount <= 0) {
			throw new IllegalArgumentException("Value count must be greater than zero");
		}
		...
		if (isOracle) {
			entriesCountLimit = 1000;
			splittingRequired = valueCount > entriesCountLimit;
		} else {
			entriesCountLimit = valueCount;
			splittingRequired = false;
		}
		...
		if (splittingRequired) {
			...
			while (remaining > 0) {
				int chunk = Math.min(entriesCountLimit, remaining);
				chunkSizes.add(chunk);
				remaining -= chunk;
			}
		} else {
			chunkSizes.add(valueCount);
		}
	 ...
	 */
	public static boolean checkTableIsView(String tableName, DataSource dataSource) {
		if(checkDbVendorIsOracle(dataSource)) {
	...
	public static boolean dropSequenceIfExists(String sequenceName, DataSource dataSource) {
		if (checkDbVendorIsOracle(dataSource)) {
			final JdbcTemplate template = new JdbcTemplate(dataSource);
			try {
	...
	public static boolean dropTriggerIfExists(String name, String tableName, DataSource dataSource) {
		final JdbcTemplate template = new JdbcTemplate(dataSource);

		try {
			// FIXME SQL injection possible
			if (checkDbVendorIsPostgreSQL(dataSource)) {
				template.execute("DROP TRIGGER IF EXISTS %s ON %s".formatted(name, tableName));
			} else {
				template.execute("DROP TRIGGER %s".formatted(name));
			}
			return true;
		} catch (Exception e) {
			return false;
	...
	public static boolean isSelectStatement(String sqlStatement) {
		return Strings.CI.startsWith(sqlStatement, "select ")
				|| Strings.CI.startsWith(sqlStatement, "select\t")
				|| Strings.CI.startsWith(sqlStatement, "select\n")
				|| Strings.CI.startsWith(sqlStatement, "select\r")
				|| Strings.CI.startsWith(sqlStatement, "with ")
				|| Strings.CI.startsWith(sqlStatement, "with(")
				|| Strings.CI.startsWith(sqlStatement, "with\t")
				|| Strings.CI.startsWith(sqlStatement, "with\n")
				|| Strings.CI.startsWith(sqlStatement, "with\r");
	}
	...
	public static boolean resultsetHasColumn(ResultSet resultSet, String columnNameToSearch) throws SQLException {
		...
		for (int i = 1; i < numberOfColumns + 1; i++) {
		    ...
		    if (Strings.CI.equals(columnNameToSearch, columnName)) {
		        return true;
		    }
		}
		return false;
	...
	public static boolean isTautologicWhereClause(String whereClause) {
		if (StringUtils.isBlank(whereClause)) {
            ...
            return "1=1".equals(whereClause.trim().replace(" ", ""));
		}
	}
	
	public static String escapeSinglesQuotes(String string) {
	...
	public static String joinColumnsNames(Collection<String> columns, boolean addCommaBefore) {
	...
	public static String joinColumnsNamesForUpdate(Collection<String> columns, boolean addCommaBefore) {
		if(CollectionUtils.isEmpty(columns)) {
			return StringUtils.EMPTY;
		}
	...
	public static long getTableStorageSize(DataSource dataSource, String tableName) {
		if (dataSource == null) {
			throw new IllegalArgumentException("Invalid empty dataSource for getTableStorageSize");
		} else if (StringUtils.isBlank(tableName)) {
			throw new IllegalArgumentException("Invalid empty tableName for getTableStorageSize");
		} else {
			try {
				try (final Connection connection = dataSource.getConnection()) {
					if (checkDbVendorIsOracle(connection)) {
						...
						try (final PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
							...
							try (final ResultSet resultSet = preparedStatement.executeQuery()) {
								if (resultSet.next()) {
									...
									throw new IllegalStateException("Invalid data for: " + tableName);
								}
							}
						}
		        	} else if (checkDbVendorIsPostgreSQL(dataSource)) {
						String query = "SELECT pg_total_relation_size(quote_ident(?))";
				...
				logger.error("Invalid size data for table: {}", tableName, e);
	...
	public static boolean isAllowedValueForDataType(boolean isOracle, DbColumnType dbColumnType, Object value) {
		switch (dbColumnType.getSimpleDataType()) {
			...
			case Numeric, Float:
				double numericValue;
				if (value instanceof String strNumber) {
					try {
						numericValue = Double.parseDouble(strNumber);
				...
				if (isOracle) {
					if ("integer".equalsIgnoreCase(dbColumnType.getTypeName())) {
						...
						return numericValue >= -2147483648;
					} else {
                        ...
                        return dbColumnType.getNumericPrecision() >= stringRepresentation.length();
					}
				} else {
					if ("smallint".equalsIgnoreCase(dbColumnType.getTypeName())) {
						...
						return numericValue >= -9223372036854775808L;
					} else {
						return true;
					}
				}
			...
			case Date, DateTime:
				if (value instanceof String strDate) {
					try {
						DateUtilities.detectSimpleFormat(strDate);
						return true;
					} catch (Exception e) {
						return false;
					}
	...
	public static String getCustomerBindingTableName(int companyId) {
		return "customer_%d_binding_tbl".formatted(companyId);
	}

	public static String getHstCustomerBindingTableName(int companyId) {
		return "hst_customer_%d_binding_tbl".formatted(companyId);
	}
}
