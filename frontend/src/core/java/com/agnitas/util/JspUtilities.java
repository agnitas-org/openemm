...
public class JspUtilities {
    ...
    public static String absUrlPrefix(String url) {
        url = Strings.CS.removeEnd(Strings.CS.removeStart(url, "/"), "/");
    ...
    public static String getJoditEditorPath(HttpServletRequest request) throws Exception {
        ...
        boolean shouldUseLatestVersion = ConfigService.getInstance().getBooleanValue(ConfigValue.UseLatestJoditEditor, companyID);
    ...
    public static String getWysiwygToolbarType(HttpServletRequest request, String defaultType) {
        if (AgnUtils.allowed(request, Permission.WYSIWYG_EXTENDED)) {
            return "Full";
        }

        if (AgnUtils.allowed(request, Permission.WYSIWYG_TRIMMED)) {
            return "Trimmed";
        }
