...
import java.io.File;
import com.agnitas.exception.RemoteCallException;
...
public class MailoutClient {
	 ...
	 */
	public void invoke (String command, String option) throws RemoteCallException {
		...
		if (syscfg.get ("mailout-dispatch-enabled", false)) {
			...
			if (! fail) {
				if (message != null) {
					...
					throw new RemoteCallException ("invokation failed: " + message);
				} else {
					log.out (Log.WARNING, "invoke", "Dispatch fails with null message");
					throw new RemoteCallException ("invokation failed with null message");
				}
			}
		...
		try {
			...
			throw new RemoteCallException("MailoutClient exception: " + e.toString (), e);
		}
	 ...
	 * @param statusId the status_id in maildrop_status_tbl to start
	 * @param custid the customer_id to start mailout for
	 * @throws RemoteCallException
	 */
	public void run(String statusId, String custid) throws RemoteCallException {
		String	option;
