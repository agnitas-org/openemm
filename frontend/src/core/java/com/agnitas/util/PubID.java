 ...
 */
public class PubID {
	private static final String	cl = "w5KMCHOXE_PTuLcfF6D1ZI3BydeplQaztVAnUj0bqos7k49YgWhxiS-RrGJm8N2v";
	private static final Pattern	srcInvalid = Pattern.compile ("[^0-9a-zA-Z_-]");
	...
	public void setSource (String nSource) {
		if (nSource == null) {
			...
			if (! m.find ()) {
				...
				StringBuilder scratch = new StringBuilder(nSource.length());
				int last;
				int pos;
	...
	public String createID () {
		String	src = mailingID + ";" + customerID + ";" +
			      (source == null ? "" : source) + (parm == null || parm.isEmpty() ? "" : ";" + parm);
		return encode (src);
	}
	...
	private String encode (String s) {
		byte[]		dump = s.getBytes (StandardCharsets.UTF_8);
		StringBuilder temp = new StringBuilder(((dump.length + 3) * 4) / 3);
			...
			if (check == checksum (s)) {
				...
				if (st) {
					while ((pos > 0) && (collect[pos - 1] == 0))
						--pos;
					rc = new String (collect, 0, pos, StandardCharsets.UTF_8);
				}
			}
		}
		return rc;
