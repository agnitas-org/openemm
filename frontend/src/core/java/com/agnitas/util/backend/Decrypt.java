...
public class Decrypt {
	 ...
	 */
	public Decrypt (String secretKey) {
		byte[]	input = secretKey.getBytes (StandardCharsets.UTF_8);
	 ...
	 */
	public String decrypt (String input, long customerID) throws DecryptionException {
		int	version = 0;
		...
		if (input.startsWith ("V") && (input.length () > 1)) {
			...
			version = switch (versionID) {
				case 'A' | 'a' -> 1;
				default -> throw new DecryptionException ("Unsupported version " + versionID + " found");
			};
			input = input.substring (2);
			compressed = Character.isLowerCase (versionID);
		}
		//
		byte[]	raw = switch (version) {
			case 0:
				try {
					yield Hex.decodeHex (input);
				} catch (DecoderException e) {
					throw new DecryptionException ("failed to decode input string \"" + input + "\" as hex: " + e.toString (), e);
				}
			case 1:
				yield Base64.decodeBase64 (input);
			default:
				yield null;
		};
		if (raw == null) {
			throw new DecryptionException ("Failed to decode input string " + input + " for version " + version);
		}
		//
		byte[]	iv = new byte[16];
		...
		try {
			...
			throw new DecryptionException ("failed to find required class for cipher: " + e.toString (), e);
		} catch (java.security.InvalidKeyException e) {
			throw new DecryptionException ("failed to handle invalid key: " + e.toString (), e);
		} catch (javax.crypto.IllegalBlockSizeException e) { 
			throw new DecryptionException ("failed to handle illegal block size: " + e.toString (), e);
		} catch (javax.crypto.NoSuchPaddingException | javax.crypto.BadPaddingException e) {
			throw new DecryptionException ("failed to handle padding: " + e.toString (), e);
		} catch (java.util.zip.DataFormatException e) {
			throw new DecryptionException ("failed to handle invalid date format: " + e.toString (), e);
		}
	}
	 ...
	 */
	public Map <String, Object> decryptAndDecode (String input, long customerID) throws DecryptionException {
		...
		if (input != null) try (JsonParser parser  = jsonFactory.createParser (decrypt (input, customerID))) {
			...
			throw new DecryptionException ("failed to parse input \"" + input + "\": " + e.toString (), e);
		}
		return rc;
	}
}
