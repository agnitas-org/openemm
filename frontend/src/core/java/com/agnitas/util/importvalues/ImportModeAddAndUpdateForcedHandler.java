...
public class ImportModeAddAndUpdateForcedHandler extends ImportModeAddAndUpdateHandler {
    ...
    private static final Logger logger = LogManager.getLogger(ImportModeAddAndUpdateForcedHandler.class);
	...
	public Map<MediaTypes, Map<Integer, Integer>> handlePostProcessing(EmmActionService emmActionService, ImportStatus status, ImportProfile importProfile, String temporaryImportTableName, int datasourceId, List<Integer> mailingListIdsToAssign, Set<MediaTypes> mediatypes) {
		if (mailingListIdsToAssign != null && !mailingListIdsToAssign.isEmpty()) {
			Map<MediaTypes, Map<Integer, Integer>> mailinglistAssignStatistics = new EnumMap<>(MediaTypes.class);
			if (importProfile.getActionForNewRecipients() > 0) {
		    	...
		    	for (int mailingListId : mailingListIdsToAssign) {
		    		for (MediaTypes mediatype : mediatypes) {
		    			...
		    			existingCustomerSubscribed += importRecipientsDao.changeStatusInMailingList(temporaryImportTableName, importProfile.getKeyColumns(), importProfile.getCompanyId(), mailingListId, mediatype, UserStatus.UserOut, UserStatus.Active, "RecipientImport");
		    			existingCustomerSubscribed += importRecipientsDao.changeStatusInMailingList(temporaryImportTableName, importProfile.getKeyColumns(), importProfile.getCompanyId(), mailingListId, mediatype, UserStatus.AdminOut, UserStatus.Active, "RecipientImport");
