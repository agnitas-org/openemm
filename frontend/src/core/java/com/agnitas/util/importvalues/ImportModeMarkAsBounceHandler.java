...
public class ImportModeMarkAsBounceHandler implements ImportModeHandler {
    
    private final ImportRecipientsDao importRecipientsDao;

    public ImportModeMarkAsBounceHandler(ImportRecipientsDao importRecipientsDao) {
		...
		if (mailingListIdsToAssign != null) {
	    	...
	    	for (int mailingListId : mailingListIdsToAssign) {
	    		for (MediaTypes mediatype : mediatypes) {
	    			int changed = importRecipientsDao.changeStatusInMailingList(temporaryImportTableName, importProfile.getKeyColumns(), importProfile.getCompanyId(), mailingListId, mediatype, UserStatus.Active, UserStatus.Bounce, "Mass Bounce by Admin");
