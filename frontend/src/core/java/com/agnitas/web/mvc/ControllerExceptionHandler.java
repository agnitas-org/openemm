...
public class ControllerExceptionHandler {
    ...
    @ExceptionHandler(FulltextSearchInvalidQueryException.class)
    public ModelAndView onFulltextSearchInvalidQueryException(FulltextSearchInvalidQueryException exception, Popups popups) {
        popups.alert(exception.getUiMessage());
    ...
    public ModelAndView onConstraintViolationException(ConstraintViolationException ex, Locale locale, Popups popups) {
        ex.getConstraintViolations().forEach(error -> popups.exactAlert(
            "%s: '%s'".formatted(I18nString.t(error.getMessage(), locale), error.getPropertyPath().toString())
        ));
        return new ModelAndView(MESSAGES_VIEW, HttpStatus.BAD_REQUEST);
    }
