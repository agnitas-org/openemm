...
public class EmbeddedClickStatDaoImpl extends BaseDaoImpl implements EmbeddedClickStatDao {
	...
	public EmbeddedClickStatDaoImpl(@Qualifier("dataSource") DataSource dataSource, @Autowired(required = false) JavaMailService javaMailService) {
	...
	public ClickStatInfo getClickStatInfo(int companyId, int mailingId, int mode, int deviceClass) {
		...
		if (EcsModeType.GROSS_CLICKS.getId() == mode) {
			...
			sqlClicksPerLink = "SELECT url_id, COALESCE(position, 1) AS position, COUNT(customer_id) AS clicks"
				+ " FROM rdirlog_" + companyId + "_tbl"
				+ " WHERE mailing_id = ?";
			...
			sqlClicksPerLink = "SELECT url_id, COALESCE(position, 1) AS position, COUNT(DISTINCT customer_id) AS clicks"
				+ " FROM rdirlog_" + companyId + "_tbl"
				+ " WHERE mailing_id = ?";
			optionsClicksPerLink.add(mailingId);
