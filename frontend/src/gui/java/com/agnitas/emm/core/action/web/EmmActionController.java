...
public class EmmActionController implements XssCheckAware {
	...
	public String view(
			@PathVariable int id,
			@ModelAttribute("form") EmmActionForm form,
		...
		model.addAttribute("form", conversionService.convert(emmAction, EmmActionForm.class))
		        .addAttribute("isActive", emmAction.getIsActive())
	...
	public String save(
			@ModelAttribute("form") EmmActionForm form,
			@RequestParam(required = false) String returnUrl,
			@RequestParam(required = false) Boolean redirectToReturnUrl,
		...
		try {
			...
			if (isValidAction(admin, form, parameters, popups)) {
				...
				action.setActionOperations(defaultIfNull(parameters, new ArrayList<>()));
                ...
                if (Boolean.TRUE.equals(redirectToReturnUrl)) {
					return "redirect:" + returnUrl;
				}
				return redirectToView(actionId) + (isBlank(returnUrl) ? "" : "?returnUrl=" + returnUrl);
            }
		} catch (Exception e) {
