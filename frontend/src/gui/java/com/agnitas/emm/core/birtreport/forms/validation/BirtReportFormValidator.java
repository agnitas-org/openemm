...
public class BirtReportFormValidator {
	...
	private boolean validateDateRangeType(ReportSettingsType type, Map<String, Object> settings) {
        return switch (type) {
            case COMPARISON -> BirtReportSettingsUtils.validateComparisonDateRange(settings);
            case MAILING -> BirtReportSettingsUtils.validateMailingsDateRange(settings);
            case RECIPIENT, TOP_DOMAIN -> BirtReportSettingsUtils.validateDateRangedSettings(settings);
        };
    }
