	...
	public String runJob() {
		try {
	        ...
	        if (currentlyRunningAutoExports < maximumRunningAutoExports) {
	        	...
	        	sendReports(reportsForSend);
	        }
		} catch (Exception e) {
			logger.error("Unknown error in BirtReportJobWorker.runJob(): {}", e.getMessage(), e);
		}
		
		return null;
	}

	protected void sendReports(List<BirtReport> reportsForSend) {
		if (reportsForSend != null && !reportsForSend.isEmpty()) {
			for (BirtReport report : reportsForSend) {
				logger.debug("Processing report '{}' (ID {})", report.getShortname(), report.getId());
