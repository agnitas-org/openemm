...
public class RecipientStatController implements XssCheckAware {
    ...
    private final RecipientStatsUiService recipientStatsUiService;

    public RecipientStatController(
            ...
            RecipientStatsUiService recipientStatsUiService
    ) {
        ...
        this.recipientStatsUiService = recipientStatsUiService;
    }
    ...
    public String view(Admin admin, @ModelAttribute(name="form") RecipientStatisticForm form, HttpSession session, Model model, Popups popups) {
        if (admin.permissionAllowed(Permission.RECIPIENT_STAT_MIGRATION)) {
        ...
        if (Strings.CS.equals(form.getReportName(), "remarks")) {
    ...
    @GetMapping("/progress.action")
    @ResponseBody
    public Map<String, RecipientProgress> getProgressStats(RecipientStatsForm form, Admin admin) {
        writeUAL(admin, "view recipient statistics", "progress");
        return recipientStatsUiService.getProgress(form, admin);
    }
