...
public class ImportProfileFormValidator {
    ...
    public boolean validate(ImportProfileForm form, Admin admin, Popups popups) {
        ...
        if ((form.getImportMode() == ImportMode.TO_BLACKLIST.getIntValue() || form.getImportMode() == ImportMode.BLACKLIST_EXCLUSIVE.getIntValue())
                && (form.getKeyColumns().size() > 1 || !RecipientStandardField.Email.getColumnName().equalsIgnoreCase(form.getKeyColumns().iterator().next()))) {
            popups.fieldError("keyColumns", "error.import.blacklist.column.email");
        }
