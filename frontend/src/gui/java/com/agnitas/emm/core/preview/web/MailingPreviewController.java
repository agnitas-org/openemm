...
public class MailingPreviewController implements XssCheckAware {
    ...
    protected final MailingWebPreviewService previewService;
    ...
    public String viewContent(PreviewForm form, Model model, Admin admin, Popups popups) {
        try {
            PreviewSettings settings = formToSettingsBuilder(form).build();
            Mailing mailing = mailingService.getMailing(admin.getCompanyID(), settings.getMailingId());
            PreviewResult previewResult = previewService.getPreview(mailing, settings, admin);

            addPreviewContentAttributes(mailing, previewResult, model);
    ...
    protected void addPreviewContentAttributes(Mailing mailing, PreviewResult previewResult, Model model) {
        model.addAttribute("previewContent", previewResult.getPreviewContent().orElse(""));
    }
    ...
    private ResponseEntity<byte[]> tryGetMailingHtml(PreviewForm previewForm, Admin admin) throws Exception {
        ...
        final PreviewResult previewResult = previewService.getPreview(settings, admin);
    ...
    public @ResponseBody
    FileSystemResource saveAsPdf(@ModelAttribute PreviewForm form, Admin admin, HttpServletResponse response) {
        PreviewSettings previewSettings = PreviewSettings.builder()
        ...
        File pdfFile = pdfService.generatePDF(admin, previewSettings, false, mailingName, "Mailing");
    ...
    private void loadPreviewHeaderData(Mailing mailing, PreviewForm form, Admin admin, Model model, Popups popups) {
        ...
        String fromParameter = mailing.getEmailParam().getFromAdr();

        try (TAGCheck tagCheck = tagCheckFactory.createTAGCheck(form.getMailingId(), admin.getLocale())) {
