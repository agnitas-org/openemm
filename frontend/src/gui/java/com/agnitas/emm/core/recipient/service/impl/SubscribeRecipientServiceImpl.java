...
public class SubscribeRecipientServiceImpl implements SubscribeRecipientService {
    ...
    public SimpleServiceResult subscribe(SubscribeWidgetForm form, SubscribeWidgetSettings settings) {
        Integer companyId = companyTokenService.findCompanyIdByToken(settings.getCompanyToken()).orElse(null);
        if (companyId == null) {
        ...
        try {
            ...
            logger.error("Error occurred when send DOI mailing: {}", settings.getDoiMailingId(), sme);
    ...
    private Optional<Recipient> saveRecipient(SubscribeWidgetForm form, SubscribeWidgetSettings settings, int companyId) {
