...
public class ServerStatusController implements XssCheckAware {
	...
	public String view(HttpServletRequest request, Admin admin, Model model, ServerStatusForm form) {
		...
		model.addAttribute("isVersionUpdateExists", serverStatusService.isVersionUpdateExists());
		model.addAttribute("isEOLReached", serverStatusService.isEOLReached());
		model.addAttribute("eolDate", serverStatusService.getEOLDate(admin));
	...
	public String saveConfig(Admin admin, ServerStatusForm form, Popups popups) {
		...
		boolean saved = serverStatusService.saveServerConfig(
				configForm.getCompanyId(),
				configForm.getName(),
				configForm.getValue(),
				configForm.getDescription(),
				admin
		);
	...
	public String viewConfig(Admin admin, RedirectAttributes model, ServerStatusForm form, Popups popups) {
		ServerConfigForm configForm = form.getConfigForm();
		if (!configFormValidator.validateName(configForm, popups) || !configFormValidator.validateCompanyId(configForm, popups)) {
			return MESSAGES_VIEW;
		}
	...
	@RequiredPermission("server.status")
	public Object downloadConfig() throws IOException {
		File configFile = serverStatusService.downloadConfigFile();
	...
	public String jobQueueView(Admin admin, Model model) {
        model.addAttribute("activeJobQueueList", jobQueueService.getForOverview());
	...
	@Anonymous
	@RequestMapping(value = "/jobqueuestatus.action", method = { RequestMethod.GET, RequestMethod.POST }, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<String> showDbStatus(HttpServletRequest request) {
		Admin admin = getAdmin(request);
		if (admin == null) {
		...
		for (VersionStatus versionItem : serverStatusService.getLatestDBVersionsAndErrors()) {
			if (Boolean.FALSE.equals(versionItem.getStatus())) {
	...
	public ResponseEntity<String> showOverallStatus(HttpServletRequest request) {
		Admin admin = getAdmin(request);
		if (admin == null) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("ACCESS DENIED");
		}
	...
	public String licenseFileUpload(@RequestParam("file") MultipartFile file, RedirectAttributes redirectAttributes, Admin admin) {
		if (file.isEmpty()) {
			throw new BadRequestException(Message.exact("Please select a file to upload"));
		}
		...
		try {
			...
			File licenseDataFile = new File(unzippedLicenseDataDirectory + File.separator + "emm.license.xml");
			File licenseSignatureDataFile = new File(unzippedLicenseDataDirectory + File.separator + "emm.license.xml.sig");

			if (!licenseDataFile.exists() || !licenseSignatureDataFile.exists()) {
				throw new BadRequestException(Message.exact("Unknown license data content"));
			}

			byte[] licenseDataArray = FileUtils.readFileToByteArray(licenseDataFile);
			if (licenseDataArray.length == 0) {
				throw new BadRequestException(Message.exact("Missing license data content"));
			}
			...
			if (licenseSignatureDataArray == null || licenseSignatureDataArray.length == 0) {
				throw new BadRequestException(Message.exact("Missing license data signature content"));
			}
			...
			if (!hasUnlimited && (!licenseVersionMatcher.find() || !majorMinorVersion.equals(licenseVersionMatcher.group(1))
				...
				throw new BadRequestException(Message.exact("License version does not match maximum version or application version"));
			}

			if (hasUnlimited && !(licenseContent.contains("<licenseID>1</licenseID>") || licenseContent.contains("<licenseID>1000</licenseID>"))) {
				throw new BadRequestException(Message.exact("For this license, maximum version 'Unlimited' is not allowed"));
			}

			if (!licenseContent.contains("<licenseID>" + configService.getLicenseID() + "</licenseID>")) {
				throw new BadRequestException(Message.exact("Wrong license id in license data content. Expected license id: " + configService.getLicenseID()));
			}
			...
			if (e instanceof UiMessageException uiMessageException) {
				throw uiMessageException;
			}

			logger.error("Cannot update license data: {}", e.getMessage(), e);
			throw new UiMessageException(Message.exact("Upload of license data was not successful for '%s': %s".formatted(file.getOriginalFilename(), e.getMessage())));
		} finally {
	...
	public String uploadDbSchemaSnapshot(@RequestParam MultipartFile file, @RequestParam(required = false) Boolean overwrite,
		...
		if (!Boolean.TRUE.equals(overwrite)) {
			DbSchemaSnapshot existingSnapshot = dbSchemaSnapshotService.getStoredSnapshot();

			if (existingSnapshot != null) {
				model.addAttribute("snapshotVersion", existingSnapshot.getVersionNumber());
				return "server_status_db_schema_overwrite";
			}
		}

		dbSchemaSnapshotService.overwriteSnapshot(file);
		popups.changesSaved();
	...
	private Admin getAdmin(HttpServletRequest request) {
		return Optional.ofNullable(AgnUtils.getAdmin(request))
				.orElseGet(() -> tryLoginByRequestParameters(request));
	}

	private Admin tryLoginByRequestParameters(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(username) && StringUtils.isNotEmpty(password)) {
            try {
				...
				logger.warn("Attempt to login failed using request parameters. Username: '{}', Password '{}'. Reason: {}",
