...
public final class WebhookBackendDataMessageGeneratorJobWorker extends JobWorkerBase {
	...
	private final void processEventType(final WebhookEventType eventType) {
		...
		for(final WebhookRegistryEntry activeWebhook : activeWebhooks) {
			...
			LOGGER.info("Active webhook '{}' for company ID {} found", activeWebhook.getEventType(), activeWebhook.getCompanyId());

			try {
				this.messageGenerator.generateWebhookMessages(activeWebhook.getCompanyId(), activeWebhook.getEventType());
			} catch(final Exception e) {
				LOGGER.error("Error generating webhook messages from backend data (company ID: {}, event type: {})",
						activeWebhook.getCompanyId(), activeWebhook.getEventType());
			}
		}
	}
