...
public final class WebhookAdministrationController implements XssCheckAware {
	...
	@ExceptionHandler(MalformedWebhookUrlException.class)
	public ModelAndView onMalformedWebhookUrlException(MalformedWebhookUrlException ex, Popups popups) {
		popups.alert("error.webhooks.url.malformed", ex.getWebhookUrl());
		return new ModelAndView(MESSAGES_VIEW, HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(NoHttpsWebhookUrlException.class)
	public ModelAndView onNoHttpsWebhookUrlException(NoHttpsWebhookUrlException ex, Popups popups) {
		popups.alert("error.webhooks.url.noHttps", ex.getWebhookUrl());
		return new ModelAndView(MESSAGES_VIEW, HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(WebhookUrlException.class)
	public ModelAndView onWebhookUrlException(WebhookUrlException ex, Popups popups) {
		popups.alert("error.webhooks.url.general", ex.getWebhookUrl());
		return new ModelAndView(MESSAGES_VIEW, HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(UnknownEventTypeException.class)
	public String onUnknownEventTypeException(Popups popups) {
		popups.alert("error.webhooks.general");
		return REDIRECT_TO_LIST;
	}

	@ExceptionHandler(WebhookRegistrationException.class)
	public String onWebhookRegistrationException(Popups popups) {
		popups.alert("error.webhooks.general");
	...
	public String view(Admin admin, @ModelAttribute("webhookConfigForm") WebhookConfigurationForm configForm, @PathVariable String eventTypeName, Model model) {
		WebhookEventType eventType = webhookEventTypeFromString(eventTypeName);
	...
	public String updateWebhookSettings(@ModelAttribute("webhookConfigForm") WebhookConfigurationForm configForm,
										@PathVariable String eventTypeName, Admin admin, Popups popups) {
		WebhookEventType eventType = webhookEventTypeFromString(eventTypeName);

		Set<String> includedProfileFields = configForm.getIncludedProfileFields();
	...
	private void includeWebhookConfigurationData(Admin admin, Model model) {
        List<WebhookListItem> listItems = webhookSettingsService.listWebhookSettings(admin.getCompanyID())
				.stream()
	...
	private static WebhookEventType webhookEventTypeFromString(String name) {
		try {
