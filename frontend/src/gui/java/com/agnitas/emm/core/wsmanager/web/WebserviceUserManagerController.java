...
public class WebserviceUserManagerController implements XssCheckAware {
    ...
    @ExceptionHandler(WebserviceUserCreateException.class)
    public ModelAndView onWebserviceUserCreateException(WebserviceUserCreateException ex, Popups popups) {
        logger.error("Cannot create webservice user.", ex);
        popups.alert("error.default.message");
        return new ModelAndView(MESSAGES_VIEW, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(WebserviceUserUpdateException.class)
    public ModelAndView onWebserviceUserUpdateException(WebserviceUserUpdateException ex, Popups popups) {
        logger.error("Cannot update webservice user", ex);
        popups.alert("error.default.message");
        return new ModelAndView(MESSAGES_VIEW, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(WebserviceUserAlreadyExistsException.class)
    public ModelAndView onWebserviceUserAlreadyExistsException(WebserviceUserAlreadyExistsException ex, Popups popups) {
        logger.error("Cannot create webservice user. {}", ex.getMessage());
    ...
    @ExceptionHandler(WebserviceUserNotFoundException.class)
    public String onWebserviceUserNotFoundException(WebserviceUserNotFoundException ex, Popups popups) {
    ...
    public String list(Admin admin, @ModelAttribute("filter") WebserviceUserOverviewFilter filter, WebserviceUserFormSearchParams searchParams,
                                 @RequestParam(required = false) Boolean restoreSort, WebserviceUserForm userForm, Model model) {
    ...
    public String create(Admin admin, @ModelAttribute("webserviceUserForm") WebserviceUserForm userForm, Popups popups) {
        ...
        saveWebserviceUser(admin, true, userForm, popups);
        return REDIRECT_TO_OVERVIEW;
    }
    ...
    public String update(Admin admin, @ModelAttribute("webserviceUserForm") WebserviceUserForm userForm, Popups popups) {
        ...
        saveWebserviceUser(admin, false, userForm, popups);
        return REDIRECT_TO_OVERVIEW;
    }
    ...
    private void saveWebserviceUser(Admin admin, boolean isNew, WebserviceUserForm userForm, Popups popups) {
    ...
    @RequiredPermission("webservice.user.change")
    public String view(Admin admin, @PathVariable String username, Model model) {
        WebserviceUserDto webserviceUser = webserviceUserService.getWebserviceUserByUserName(username);
        WebserviceUserForm userForm = conversionService.convert(webserviceUser, WebserviceUserForm.class);
        ...
        if (permissionsEnabled) {
            ...
            model.addAttribute("PERMISSIONS", webservicePermissionService.listAllPermissions());
            model.addAttribute("PERMISSION_GROUPS", webservicePermissionGroupService.listAllPermissionGroups());
        }
