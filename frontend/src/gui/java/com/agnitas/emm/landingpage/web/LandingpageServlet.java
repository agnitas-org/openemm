...
public final class LandingpageServlet extends HttpServlet {
	...
	private transient LandingpageService landingpageService;
	...
	private final void redirectToLandingPage(final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
		...
		if(settings.getHttpCode() == 0) {
			doHtmlMetaRedirect(settings, resp);
		} else {
			doHttpRedirect(settings, resp);
		}
	}
	
	private void doHtmlMetaRedirect(RedirectSettings settings, HttpServletResponse resp) throws IOException {
		resp.setContentType("text/html"); 
		final String html = "<html><head><meta http-equiv=\"refresh\" content=\"0; URL=%s\"></head></html>";
		
		LOGGER.info("Redirecting to landing page '{}' by HTML meta", settings.getRedirectUrl());

		resp.getWriter().println(String.format(html, settings.getRedirectUrl()));
	}
	
	private void doHttpRedirect(RedirectSettings settings, HttpServletResponse resp) {
		LOGGER.info("Redirecting to landing page '{}' by HTTP {}", settings.getRedirectUrl(), settings.getHttpCode());
