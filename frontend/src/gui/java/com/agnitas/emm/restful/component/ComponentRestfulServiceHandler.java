 ...
 */
@Component("RestfulServiceHandler_component")
public class ComponentRestfulServiceHandler implements RestfulServiceHandler {
    ...
    public ComponentRestfulServiceHandler(RestfulUserActivityLogDao userActivityLogDao, MailingDao mailingDao, MailingComponentDao mailingComponentDao, TargetDao targetDao, CompanyDao companyDao, ThumbnailService thumbnailService, CompanyTokenService companyTokenService) {
				...
				if (JsonDataType.OBJECT == jsonNode.getJsonDataType()) {
					...
					for (Entry<String, Object> entry : jsonObject.entrySet()) {
						if ("name".equals(entry.getKey())) {
							...
							if (entry.getValue() instanceof String typeName) {
								mailingComponent.setType(MailingComponentType.getMailingComponentTypeByName(typeName));
							} else if (entry.getValue() instanceof Integer typeCode) {
								mailingComponent.setType(MailingComponentType.getMailingComponentTypeByCode(typeCode));
							} else {
							...
							if (mimeTypePropertyUsed) {
							...
							} else if (entry.getValue() instanceof String emmBlock) {
								...
								if (jsonObject.get("mimetype") instanceof String mimeType) {
									mailingComponent.setEmmBlock(emmBlock, mimeType);
							...
							if (entry.getValue() instanceof Integer targetID) {
							...
							if (entry.getValue() instanceof String url) {
							...
							} else if (entry.getValue() instanceof String binBlock) {
								byte[] data;
								try {
									data = AgnUtils.decodeZippedBase64(binBlock);
								} catch (IOException e) {
									throw new RestfulClientException("Invalid value for 'bin_block'. String(Base64) expected");
								}
								if (data.length == 0 && !binBlock.isEmpty()) {
									...
									data = AgnUtils.decodeBase64(binBlock);
								}
								if (jsonObject.get("mimetype") instanceof String mimeType) {
									mailingComponent.setBinaryBlock(data, mimeType);
									mimeTypePropertyUsed = true;
								} else {
									throw new RestfulClientException("Invalid data type for 'mimetype'. String expected");
								}
	...
	public boolean isBodyPropertyExcludedForUnsafeHtmlTagCheck(Admin admin, String property, HttpServletRequest request) {
		return "emm_block".equals(property);
	}
