 ...
 */
@Component("RestfulServiceHandler_content")
public class ContentRestfulServiceHandler implements RestfulServiceHandler {
    ...
    public ContentRestfulServiceHandler(RestfulUserActivityLogDao userActivityLogDao, MailingDao mailingDao, DynamicTagDao dynamicTagDao, TargetDao targetDao, ThumbnailService thumbnailService, MailingContentService mailingContentService, MaildropService maildropService) {
				...
				if (JsonDataType.OBJECT == jsonNode.getJsonDataType()) {
					...
					for (Entry<String, Object> entry : jsonObject.entrySet()) {
						if ("name".equals(entry.getKey())) {
							if (entry.getValue() instanceof String dynName) {
							...
							if (entry.getValue() instanceof Boolean disableLinkExtension) {
								dynamicTag.setDisableLinkExtension(disableLinkExtension);
							} else {
							...
							if (entry.getValue() instanceof JsonArray contentArray && contentArray.size() > 0) {
								for (Object contentObject : contentArray) {
									if (contentObject instanceof JsonObject contentObj) {
										DynamicTagContent dynamicTagContent = new DynamicTagContentImpl();
										
										for (Entry<String, Object> contentItemEntry : contentObj.entrySet()) {
											if ("id".equals(contentItemEntry.getKey())) {
												if (contentItemEntry.getValue() instanceof Integer tagContentId) {
													dynamicTagContent.setId(tagContentId);
												} else {
												...
												if (contentItemEntry.getValue() instanceof Integer targetId) {
												...
												if (contentItemEntry.getValue() instanceof Integer dynOrder) {
												...
												if (contentItemEntry.getValue() instanceof String dynContent) {
										...
										if (!contentObj.containsPropertyKey("order")) {
	...
	public boolean isBodyPropertyExcludedForUnsafeHtmlTagCheck(Admin admin, String property, HttpServletRequest request) {
		return property.matches("^content\\[\\d+]\\.text$");
	}
