 ...
 */
@Component("RestfulServiceHandler_form")
public class FormRestfulServiceHandler implements RestfulServiceHandler {
	...
	private final UserFormExporter userFormExporter;
	private final CompanyTokenService companyTokenService;

    public FormRestfulServiceHandler(RestfulUserActivityLogDao userActivityLogDao, UserFormDao userFormDao, UserFormExporter userFormExporter, CompanyTokenService companyTokenService) {
	...
	private void fillUserformObject(Admin admin, UserForm userForm, JsonObject jsonObject) throws Exception {
		...
		for (Entry<String, Object> entry : jsonObject.entrySet()) {
			if ("formname".equals(entry.getKey()) || "name".equals(entry.getKey())) {
				...
				if (entry.getValue() instanceof String successTemplate) {
					userForm.setSuccessTemplate(successTemplate);
				} else {
				...
				if (entry.getValue() instanceof String successUrl) {
				...
				if (entry.getValue() instanceof Boolean successUseUrl) {
					userForm.setSuccessUseUrl(successUseUrl);
				} else {
				...
				if (entry.getValue() instanceof String successMimetype) {
					userForm.setSuccessMimetype(successMimetype);
				} else {
				...
				if (entry.getValue() instanceof String errorTemplate) {
					userForm.setErrorTemplate(errorTemplate);
				} else {
				...
				if (entry.getValue() instanceof String errorUrl) {
					userForm.setErrorUrl(errorUrl);
				} else {
				...
				if (entry.getValue() instanceof Boolean errorUseUrl) {
					userForm.setErrorUseUrl(errorUseUrl);
				} else {
				...
				if (entry.getValue() instanceof String errorMimetype) {
					userForm.setErrorMimetype(errorMimetype);
				} else {
					throw new RestfulClientException("Invalid data type for 'error_mimetype'. String expected");
				}
			} else if ("active".equals(entry.getKey())) {
				if (entry.getValue() instanceof Boolean active) {
	...
	public boolean isBodyPropertyExcludedForUnsafeHtmlTagCheck(Admin admin, String property, HttpServletRequest request) {
		return "success_template".equals(property) || "error_template".equals(property);
	}
