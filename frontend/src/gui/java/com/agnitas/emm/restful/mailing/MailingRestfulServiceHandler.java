 ...
 */
@Component("RestfulServiceHandler_mailing")
public class MailingRestfulServiceHandler implements RestfulServiceHandler {
    ...
    public MailingRestfulServiceHandler(RestfulUserActivityLogDao userActivityLogDao, MailingDao mailingDao, MailinglistDao mailinglistDao, MailingImporter mailingImporter, MailingExporter mailingExporter, CopyMailingService copyMailingService, ThumbnailService thumbnailService, MailingService mailingService, MailingBaseService mailingBaseService, CompanyTokenService companyTokenService, MaildropService maildropService) {
					...
					if (JsonDataType.OBJECT == jsonNode.getJsonDataType()) {
						...
						for (Entry<String, Object> entry : jsonObject.entrySet()) {
							if ("shortname".equals(entry.getKey())) {
								if (entry.getValue() instanceof String shortname) {
								...
								if (entry.getValue() instanceof String description) {
								...
								if (jsonObject.containsPropertyKey("mailinglist_shortname")) {
									...
									if (entry.getValue() instanceof Integer mailinglistId) {
								...
								if (entry.getValue() instanceof String mailingType) {
                                    mailing.setMailingType(MailingType.fromName(mailingType));
								} else {
								...
								if (entry.getValue() instanceof String contentType) {
                                    mailing.setMailingContentType(MailingContentType.getFromString(contentType));
								} else {
								...
								if (entry.getValue() instanceof String targetExpression) {
								...
								if (entry.getValue() instanceof Boolean isTemplate) {
								...
								if (entry.getValue() instanceof Integer openActionId) {
								...
								if (entry.getValue() instanceof Integer clickActionId) {
								...
								if (entry.getValue() instanceof Integer campaignId) {
								...
								if (entry.getValue() instanceof JsonArray paramsArray) {
									try {
										List<MailingParameter> parameters = new ArrayList<>();
										for (Object parameterObject : paramsArray) {
	...
	public boolean isBodyPropertyExcludedForUnsafeHtmlTagCheck(Admin admin, String property, HttpServletRequest request) {
		return true; // Checks are done manually in the right places here and in com.agnitas.service.impl.MailingImporterImpl
	}
