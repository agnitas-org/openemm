 ...
 */
@Component("RestfulServiceHandler_profilefield")
public class ProfilefieldRestfulServiceHandler implements RestfulServiceHandler {
	...
	private Collection<RecipientFieldDescription> sortProfileFields(Collection<RecipientFieldDescription> profileFields, String... keepItemsFirst) {
		...
		Collections.sort(list, (o1, o2) -> {
			...
			if (o1.getSortOrder() < 1000) {
				return -1;
			}

			if (o2.getSortOrder() < 1000) {
				return 1;
			}
	...
	private void validateProfileFieldData(int companyID, RecipientFieldDescription profileField) throws Exception {
		try {
    ...
    private boolean mayAddNewColumn(int companyID) {
		if (companyID <= 0) {
    		return false;
    	} else {
    		int maxFields;
			...
			return currentFieldCount < maxFields + configService.getIntegerValue(ConfigValue.System_License_MaximumNumberOfProfileFields_Graceful, companyID);
    	}
	}
	...
	private boolean isAllowedToUseHtml(Admin admin, String param) {
		return (Strings.CS.startsWith(param, "allowedValues[") || "defaultValue".equals(param))
