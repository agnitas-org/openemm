 ...
 */
@Component("RestfulServiceHandler_recipient")
public class RecipientRestfulServiceHandler implements RestfulServiceHandler {
	...
	public RecipientRestfulServiceHandler(RestfulUserActivityLogDao userActivityLogDao, RecipientDao recipientDao, BindingEntryDao bindingEntryDao, RecipientFieldService recipientFieldService,
                                          ...
                                          SubscriberLimitCheck subscriberLimitCheck, RecipientService recipientService) {
	...
	private Object getCustomerData(HttpServletRequest request, Admin admin) throws Exception {
		...
		if (restfulContext.length == 0) {
			...
			if (AgnUtils.isEmailValid(requestedRecipientKeyValue)) {
				...
				customerIDs = recipientDao.getRecipientIDs(admin.getCompanyID(), "customer_id", Integer.parseInt(requestedRecipientKeyValue));
			} else {
						...
						while ((readJsonToken = jsonReader.readNextToken()) != null) {
							if (JsonToken.JsonSimpleValue == readJsonToken) {
								if (customerIDs.size() >= 1000) {
									...
									if (recipientKeyValue instanceof String recipientEmail) {
									...
									} else if (recipientKeyValue instanceof Integer recipientId) {
										customerIDs.add(recipientId);
									} else {
			...
			try (Json5Reader jsonReader = new Json5Reader(inputStream)) {
				...
				if (JsonToken.JsonObject_Open == readJsonToken) {
					...
					while ((readJsonToken = jsonReader.readNextToken()) == JsonToken.JsonObject_PropertyKey) {
						...
						if (propertyValue instanceof String propertyValueStr) {
							// Check for unallowed html content
							try {
								HtmlChecker.checkForUnallowedHtmlTags(propertyValueStr, allowHtmlTags);
					...
					File importTempFileDirectory = new File(IMPORT_FILE_DIRECTORY + File.separator + admin.getCompanyID());
	...
	private void removeTripleDateEntries(Map<String, Object> custParameters) {
		custParameters.entrySet().removeIf(item -> Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY)
			|| Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH)
			|| Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR)
			|| Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR)
			|| Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE)
			|| Strings.CI.endsWith(item.getKey(), RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND));
	}
