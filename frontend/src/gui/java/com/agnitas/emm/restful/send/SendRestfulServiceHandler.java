...
public class SendRestfulServiceHandler implements RestfulServiceHandler {
	...
	public void doService(HttpServletRequest request, HttpServletResponse response, Admin admin, byte[] requestData, File requestDataFile, BaseRequestResponse restfulResponse, ServletContext context, RequestMethod requestMethod, boolean extendedLogging) throws Exception {
		if (requestMethod == RequestMethod.GET) {
			...
			((JsonRequestResponse) restfulResponse).setJsonResponseData(new JsonNode(deactivateMailing(request, admin)));
	...
	protected Object deactivateMailing(HttpServletRequest request, Admin admin) throws Exception {
	...
	protected Object sendMailing(HttpServletRequest request, byte[] requestData, File requestDataFile, Admin admin, @SuppressWarnings("unused") boolean extendedLogging) throws Exception {
		...
		List<String> keyColumns = new ArrayList<>(List.of("email"));
					...
					if (JsonDataType.OBJECT == jsonNode.getJsonDataType()) {
						...
						for (Entry<String, Object> entry : jsonObject.entrySet()) {
							if ("send_type".equals(entry.getKey())) {
								if (entry.getValue() instanceof String sendType && sendType.length() == 1) {
									try {
										maildropStatus = MaildropStatus.fromCode(sendType.charAt(0));
								...
								if (entry.getValue() instanceof String sendDateStr) {
									try {
										sendDate = DateUtilities.parseIso8601DateTimeString(sendDateStr);
									} catch (@SuppressWarnings("unused") Exception e) {
								...
								if (entry.getValue() instanceof Integer steppingValue) {
									stepping = steppingValue;
								} else {
									throw new RestfulClientException("Invalid data type for 'stepping'. Integer expected");
								}
							} else if ("block_size".equals(entry.getKey())) {
								if (entry.getValue() instanceof Integer blockSizeValue) {
								...
								if (entry.getValue() instanceof JsonObject profileDataObj) {
								...
								if (entry.getValue() instanceof String keyCols) {
									keyColumns = AgnUtils.splitAndTrimList(keyCols);
								} else if (entry.getValue() instanceof JsonArray keyCols) {
									keyColumns = new ArrayList<>();
									for (Object keyColumn : keyCols) {
										if (keyColumn instanceof String keyCol) {
								...
								if (entry.getValue() instanceof Boolean alwaysCreateRecipientValue) {
									alwaysCreateRecipient = alwaysCreateRecipientValue;
								} else {
								...
								if (entry.getValue() instanceof Boolean cleanupAdminAndTestActionsValue) {
									cleanupAdminAndTestActions = cleanupAdminAndTestActionsValue;
								} else {
		...
		if (maildropStatus == MaildropStatus.ACTION_BASED || maildropStatus == MaildropStatus.ON_DEMAND) {
			...
			if (!worldSend || !maildropService.isActiveMailing(mailing.getId(), mailing.getCompanyID())) {
				...
				if (startGen == 1 && maildropEntry.getStatus() != MaildropStatus.ACTION_BASED.getCode() && maildropEntry.getStatus() != MaildropStatus.DATE_BASED.getCode()) {
					classicTemplateGenerator.generate(mailingID, admin.getCompanyID(), true);
	...
	@Override
	public boolean isParameterExcludedForUnsafeHtmlTagCheck(Admin admin, String parameter, HttpServletRequest request) {
		return isAllowedToUseHtml(admin, parameter);
	}

	@Override
	public boolean isBodyPropertyExcludedForUnsafeHtmlTagCheck(Admin admin, String property, HttpServletRequest request) {
		return isAllowedToUseHtml(admin, property);
	}

	private boolean isAllowedToUseHtml(Admin admin, String columnName) {
