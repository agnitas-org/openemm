 ...
 *    (SELECT MAX(id) + 1, 'DBJobWorkerTest', CURRENT_TIMESTAMP, null, 0, 'OK', 0, 0, '***0;***5', CURRENT_TIMESTAMP, null, 'DbJobWorker', 0  FROM job_queue_tbl);
 * 
				...
				try {
					if (DbUtilities.isSelectStatement(sqlStatement)) {
						if (StringUtils.isEmpty(format) || "CSV".equalsIgnoreCase(format) || "CSV_ZIP".equalsIgnoreCase(format)) {
							...
							infoMailContent.append("Export data file '").append(sftpFileName).append("' transfered to SFTP server<br />\n");
						}
					}
				} catch (Exception e) {
					infoMailContent.append("Error occurred while executing statement with index ").append(statementIndex).append(": ").append(e.getMessage());
