 ...
 *    (SELECT MAX(id) + 1, 'InformationReport', CURRENT_TIMESTAMP, null, 0, 'OK', 0, 0, '***0;***5', CURRENT_TIMESTAMP, null, 'InformationReport', 0 FROM job_queue_tbl);
 */
				...
				for (int customerID : customerIdList) {
					try {
						...
						if (remoteFileHelper != null) {
							remoteFileHelper.put(new ByteArrayInputStream(informationReportText.getBytes(StandardCharsets.UTF_8)), fileName, true);
						} else {
							...
							FileUtils.writeStringToFile(informationReportFile, informationReportText, StandardCharsets.UTF_8);
						}
					} catch (Exception e) {
