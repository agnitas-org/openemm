...
public class CustomerMatchTargetTag extends TagSupport implements BodyTag {
	...
	private static final Logger logger = LogManager.getLogger(CustomerMatchTargetTag.class);
    ...
    public int doStartTag()	{
        if (targetID == 0) {
	        ...
	        final TargetService targetService = applicationContext.getBean("targetService", TargetService.class);
	        
	        try {
		        final RecipientTargetGroupMatcher matcher = targetService.createRecipientTargetGroupMatcher(customerID, getCompanyID());
	        	final Target target = targetService.getTargetGroup(targetID, getCompanyID());
