...
public class ShowWhenFeatureEnabledTag extends PermissionExceptionTagSupport {
	...
	private static final Logger LOGGER = LogManager.getLogger(ShowWhenFeatureEnabledTag.class);
	...
	public int doStartTag() throws JspException {
    	...
    	if(premiumFeature != null) {
			LOGGER.debug("Identified feature '{}' as '{}'", this.feature, premiumFeature);
	    	...
	    	try {
		    	final PremiumFeaturesService featureService = applicationContext.getBean(PremiumFeaturesService.class);
				final Admin admin = AgnUtils.getAdmin(pageContext);
				if (admin != null) {
					try {
						final boolean enabled = featureService.isFeatureRightsAvailable(premiumFeature.getName(), admin.getCompanyID());

						LOGGER.debug("Feature '{}' enabled: {}", premiumFeature, enabled);
