...
public class OnePixelCountServlet extends HttpServlet {
	...
	private transient CompanyDaoCache companyDaoCache;
	private transient ExtensibleUIDService uidService;
	private transient OpenTrackingService openTrackingService;
	...
	private transient EmmActionDao actionDao;
	...
	private AccessDataService getAccessDataService() {
		if (accessDataService == null) {
			...
			accessDataService = (AccessDataService) applicationContext.getBean("AccessDataService");
		}
	...
	private DeviceService getDeviceService() {
		if (deviceService == null) {
			ApplicationContext applicationContext = WebApplicationContextUtils.getWebApplicationContext(getServletContext());
			deviceService = (DeviceService) applicationContext.getBean("DeviceService");
		}
	...
	public void init() throws ServletException {
		...
		try {
			...
			logger.error("Cannot instantiate OnePixelCountServlet: {}", e.getMessage(), e);
		}
	}
	...
	public void service(HttpServletRequest request, HttpServletResponse res) throws IOException {
		...
		int deviceID = getDeviceService().getDeviceId(request.getHeader("User-Agent"));
		...
		if (agnUidString == null) {
			...
			try {
				...
				if (uid != null && uid.getCompanyID() > 0) {
					...
					if (company == null) {
						logger.error("OnepixelLog error: Company with ID: {} not found", uid.getCompanyID());
						...
						if (deviceID != DeviceService.DEVICE_BLACKLISTED_NO_COUNT) {
							if (!noCount) {
								...
								logger.info("OnepixelLog: cust: {} mi: {} ci: {}", uid.getCustomerID(), uid.getMailingID(), uid.getCompanyID());
							} else {
								logger.info("Counting tracking pixel disabled by request parameter (customer id: {}, mailing id: {}, company id: {})",
										uid.getCustomerID(), uid.getMailingID(), uid.getCompanyID());
							}
						}
	 ...
	 *            HTTP request
	 */
	protected void executeMailingOpenAction(ExtensibleUID uid, HttpServletRequest req) {
