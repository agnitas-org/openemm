...
public class RedirectServlet extends HttpServlet {
	...
	private transient AccessDataService accessDataService;
	private transient DeviceService deviceService;
	private transient ClientService clientService;
	private transient ExtensibleUIDService extensibleUIDService;
	private transient TrackableLinkDao trackableLinkDao;
	private transient EmmActionService emmActionService;
	private transient CompanyDao companyDao;
	private transient ConfigService configService;
	private transient MailingDao mailingDao;
	private transient SnowflakeMailingCache snowflakeMailingCache;
	private transient RecipientService recipientService;
	private transient SubstituteLinkRdirPostProcessor substituteLinkRdirPostProcessor;
	private transient ClickTrackingService clickTrackingService;
	private transient HoneypotLinkService honeypotLinkService;
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			...
			logger.info("Request method not allowed for RDIR requests: {}", e.getHttpRequestMethod(), e);
	...
	private final void doResolveLink(final HttpServletRequest request, final HttpServletResponse response) throws IOException, HttpMethodNotAllowedException {
        ...
        try {
			...
			final boolean cachingDisabled = Strings.CS.equals(uid.getPrefix(), "nc"); // StringUtils.equals() is null-safe
			...
			String fullUrl = getLinkService().personalizeLink(trackableLink, uid, agnUidString, uid.getCustomerID(), referenceTableRecordSelector, !NamedUidBit.isBitSet(uid.getBitField(), NamedUidBit.NO_LINK_EXTENSION), encryptedStaticValueMapOrNull);
			...
			if(showHoneypotLinkIntermediatePage(fullUrl, request, response, company, agnUidString, uid)) {
				...
				if(getConfigService().getBooleanValue(ConfigValue.RedirectMakeAgnDynMultiLinksTrackable, trackableLink.getCompanyID())) {
					if(this.getSubstituteLinkRdirPostProcessor() != null) {
						...
						logger.fatal("Config key '{}' is set to true, but no SubstituteLinkRdirPostProcessor is defined",
								ConfigValue.RedirectMakeAgnDynMultiLinksTrackable.getName());
					}
				}
			...
			logger.info("Error handling UID: {}", agnUidString, e);
	...
	private final boolean showHoneypotLinkIntermediatePage(final String fullUrl, final HttpServletRequest request, final HttpServletResponse response, final Company company, final String uidString, final ExtensibleUID uid) throws IOException {
		...
		if(show) {
			...
			LogManager.getLogger("com.agnitas.honeypot.web.HoneypotLinkConfirmController")
                    .info("Redirecting to target via honeypot page (company {}, mailing {}, customer {}, link {})", uid.getCompanyID(), uid.getMailingID(), uid.getCustomerID(), uid.getUrlID());
		}
	...
	private final String punycodeEncodeDomainInLink(final String url) {
		try {
			...
			logger.warn("Error Punycode-encoding url '{}'. Using unencoded URL", url, e);
			return url;
		}
	}

	private void executeLinkActions(ExtensibleUID uid, int deviceID, DeviceClass deviceClass, TrackableLink trackableLink, HttpServletRequest request) {
	...
	private void executeLinkAction(int actionID, int deviceID, ExtensibleUID uid, DeviceClass deviceClass, HttpServletRequest request) {
		if (actionID == 0) {
			return;
		}
	...
	private static String referenceTableRecordSelector(final HttpServletRequest request, final Company company, final long customerID) {
		...
		try {
            ...
            logger.error("Cannot decrypt 'ref' parameter (value: {})", encrypted, e);
			return "";
		}
	}
	...
	private TrackableLink loadTrackableLink(final ExtensibleUID uid, final boolean cachingDisabled) throws Exception {
		...
		if (trackableLink == null) {
			logger.error("service: trackable link not found: CID={} LID={}", uid.getCompanyID(), uid.getUrlID());
