...
public class ShowFormImageServlet extends HttpServlet {
	...
	private CachedImageData getImage(HttpServletRequest request, int licenseID, int companyID, int formID, String imageFileName, boolean noCache, boolean thumbnail) throws Exception {
		if (thumbnail) {
            ...
            return new CachedImageData(thumbnailFormComponent.getData(), thumbnailFormComponent.getMimeType());
		} else {
			...
			if (image != null && !noCache) {
				...
				if (formComponent == null || formComponent.getData() == null || formComponent.getData().length == 0) {
					...
					CachedImageData newImage = new CachedImageData(formComponent.getData(), formComponent.getMimeType());
	...
	private String generateCachingKey(int licenseID, int companyID, int formID, String name) {
        return "%d-%d-%d-%s".formatted(licenseID, companyID, formID, name);
	}

	/**
	 * The Class CachedImageData.
	 */
	private record CachedImageData(byte[] data, String mimeType) {
