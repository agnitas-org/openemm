...
public class ShowImageServlet extends HttpServlet {
     ...
     */
	protected static final DeliverableImage NOT_FOUND_IMAGE = new DeliverableImage(null, null, "");

	/** Serial version UID: */
	private static final long serialVersionUID = -595094416663851734L;

	protected transient MailingComponentDao componentDao;
	protected transient DeviceService deviceService;
	protected transient ImageCache imageCache;
	protected transient CdnCache cdnCache;
    protected transient RdirTrafficAmountDao rdirTrafficAmountDao;
    protected transient CompanyDaoCache companyDaoCache;
	protected transient ConfigService configService;
	protected transient HttpResponseHeaderApplier responseHeaderApplier;
			...
			if (noCache || cdnBaseLink == null) {
				...
				if (image != null) {
					if (getConfigService().getBooleanValue(ConfigValue.ImageTrafficMeasuring, companyID)) {
						getRdirTrafficAmountDao().save(companyID, StringUtils.isNotEmpty(mailingIdString) ? Integer.parseInt(mailingIdString) : 0, image.name(), image.data() == null ? 0 : image.data().length);
					}
				...
				if (cdnImage != null) {
					...
					if (getConfigService().getBooleanValue(ConfigValue.ImageTrafficMeasuring, companyID)) {
						getRdirTrafficAmountDao().save(companyID, Integer.parseInt(mailingIdString), cdnImage.name(), cdnImage.dataLength());
					}
				}
			}
		} catch (Exception e) {
	 ...
	 */
	private DeliverableImage getImageForMobileRequest(HttpServletRequest request, int companyID, int mailingID, String imageName, boolean noCache) {
		...
		if (mobileImage != null && !noCache) {
			...
			if (comp != null && checkValidityPeriod(comp) && comp.getBinaryBlock() != null) {
				...
				DeliverableImage newImage = new DeliverableImage(comp.getBinaryBlock(), comp.getMimeType(), comp.getComponentName());
	 ...
	 */
	private DeliverableImage getImageForStandardRequest(HttpServletRequest request, int companyID, int mailingID, String imageName, boolean noCache) {
		...
		if (image != null && !noCache) {
			...
			if (comp != null && checkValidityPeriod(comp) && comp.getBinaryBlock() != null) {
				// Valid imagedata found
				newImage = new DeliverableImage(comp.getBinaryBlock(), comp.getMimeType(), comp.getComponentName());
				if (checkImageCacheSize(newImage)) {
	...
	private DeliverableImage getDefaultImage(HttpServletRequest req) {
		...
		DeliverableImage newImage;
		if (defaultImageData == null) {
			// Default image data was not found so write some text
			newImage = new DeliverableImage("Defaultimage not found".getBytes(StandardCharsets.UTF_8), "text/html", "");
		} else {
			// Use the default image
			newImage = new DeliverableImage(defaultImageData, "image/png", "grid_expire_image.png");
		}
		return newImage;
	}
	...
	private String generateCachingKey(int companyID, int mailingID, String name) {
        return "%d-%d-%s".formatted(companyID, mailingID, name);
	}
	...
	protected boolean checkImageCacheSize(DeliverableImage image) {
		if (image != null) {
			...
			return image.data() != null && image.data().length <= maxSize;
		} else {
			return true;
		}
	}
