    ...
    <emm:ShowByPermission token="actions.change">
        <c:if test="${not ACTION_READONLY}">
	        <emm:instantiate var="element" type="java.util.LinkedHashMap">
                ...
                <c:set target="${element}" property="btnCls" value="${not empty returnUrl ? 'js-btn-dropdown' : ''}"/>
                <c:set target="${element}" property="cls" value="mobile-hidden" />
                ...
                <c:if test="${not empty returnUrl}">
                    ...
                    <emm:instantiate var="option" type="java.util.LinkedHashMap">
                        ...
                        <c:set target="${option}" property="extraAttributes" value="data-form-set='redirectToReturnUrl: true' data-form-target='#emm-action-view' data-form-submit-event"/>
