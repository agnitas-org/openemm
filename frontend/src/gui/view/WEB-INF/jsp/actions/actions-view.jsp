...
<c:if test="${not empty operationList}">
    <mvc:form id="emm-action-view" servletRelativeAction="/action/save.action?returnUrl=${returnUrl}" data-form="resource"
...
<template id="action-dependency-view-link">
...
<template id="action-dependency-type">
...
<template id="go-to-btn">
    <a href="
    {{ if ('${MAILING_DEPENDENCY_TYPE.name()}' === type) { }}
        {{- AGN.url('/mailing/' + id + '/settings.action') }}
    {{ } else if ('${FORM_DEPENDENCY_TYPE.name()}' === type) { }}
        {{- AGN.url('/webform/' + id + '/view.action') }}
    {{ } }}
