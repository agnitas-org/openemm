...
<%--@elvariable id="isVersionUpdateExists" type="java.lang.Boolean"--%>
<%--@elvariable id="isEOLReached" type="java.lang.Boolean"--%>
<%--@elvariable id="eolDate" type="java.lang.String"--%>

<div id="system-notifications">
	<%-- Loads with JS --%>
</div>

<div id="system-status-overview" class="tiles-container" data-controller="system-status" data-initializer="system-status" data-editable-view="${agnEditViewKey}">
		...
		<div class="tile-body vstack gap-3 js-scrollable">
			<mvc:form data-form="resource" servletRelativeAction="/serverstatus/updatecheck.action">
				<button type="button" class="btn btn-primary w-100" data-form-submit>
					<mvc:message code="server.status.check.version.db" />
				</button>
			</mvc:form>
			...
			<c:if test="${checkDbSchemaAllowed}">
				<a href="<c:url value="/serverstatus/schema/check.action" />" type="button" class="btn btn-primary" data-confirm>
					<mvc:message code="server.status.check.db.schema" />
				</a>
			</c:if>
			...
			<emm:ShowByPermission token="master.dbschema.snapshot.create">
				<a href="<c:url value="/serverstatus/db-schema/download.action" />" class="btn btn-primary" data-prevent-load>
					<mvc:message code="server.status.db.schema.download" />
				</a>
			...
			<mvc:form id="dbSchema-upload-form" servletRelativeAction="/serverstatus/db-schema/upload.action" data-form="resource" enctype="multipart/form-data" method="POST">
				<div class="tile tile--sm tile--highlighted">
					<div class="tile-header">
						<h3 class="tile-title"><mvc:message code="server.status.db.schema.upload" /></h3>
					</div>
			...
			<mvc:form data-form="resource" modelAttribute="serverStatusForm">
				<div class="tile tile--sm tile--highlighted">
					...
					<div class="tile-body">
						<div class="row g-3">
							<div class="col">
								<mvc:text path="configForm.companyId" cssClass="form-control js-inputmask" data-inputmask-options="mask: '9{1,20}'" placeholder="CompanyID"/>
							</div>
...
<c:choose>
	<c:when test="${isEOLReached}">
		<template id="system-eol-msg">
			<div class="panel panel--alert" data-system-version="${serverStatus.version}">
				<h2 class="d-flex gap-1 align-items-center w-100">
					<i class="icon icon-state-alert"></i>
					<span class="flex-grow-1">
						<mvc:message code="serverStatus.version.eol" arguments="${[serverStatus.shortVersion, eolDate]}" />
					</span>

					<button type="button" class="btn-close" data-action="close-system-update-msg">
						<span class="sr-only"><mvc:message code="close" /></span>
					</button>
				</h2>
			</div>
		</template>
	</c:when>
	<c:when test="${isVersionUpdateExists}">
		<template id="system-update-available-msg">
			<div class="panel panel--warning" data-system-version="${serverStatus.version}">
				<h2 class="d-flex gap-1 align-items-center w-100">
					<i class="icon icon-state-warning"></i>
					<span class="flex-grow-1">
						<mvc:message code="serverStatus.version.update" arguments="${[serverStatus.shortVersion, eolDate]}" />
					</span>
