...
<c:set var="skipUploadProgress" value="false" />
...
<c:choose>
    ...
    <c:when test="${type eq BUILDING_BLOCK}">
        ...
        <c:url var="formActionUrl" value="/import/buildingBlocks.action" />
        <c:set var="skipUploadProgress" value="true" />
    </c:when>
    <c:when test="${type eq TARGET_GROUP}">
        <mvc:message var="titleMsg" code="target.import" />
        <c:url var="formActionUrl" value="/target/import.action" />
    </c:when>
</c:choose>
...
<div class="modal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-fullscreen-lg-down" data-controller="import-modal">
        <mvc:form action="${formActionUrl}" enctype="multipart/form-data" modelAttribute="form" cssClass="modal-content"
                  data-form="${isStaticForm ? 'static' : 'resource'}" data-custom-loader="" data-initializer="upload"
                  data-skip-upload-progress="${skipUploadProgress}"
                  data-action="${isWorkflowDriven ? 'import-for-workflow' : ''}">
            <div class="modal-header">
                <h1 class="modal-title">
                    <span class="text-truncate">${titleMsg}</span>

                    <c:if test="${type eq MAILING or type eq TEMPLATE}">
            ...
            <c:if test="${type eq MAILING or type eq TEMPLATE}">
                <div class="modal-footer modal-footer--nav">
                    <c:choose>
                        <c:when test="${type eq MAILING}">
                            <c:url var="backUrl" value="/mailing/create.action?keepForward=${isWorkflowDriven}" />
                        </c:when>
                        <c:otherwise>
