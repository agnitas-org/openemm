<%@ page import="com.agnitas.emm.core.datasource.enums.GeneralDataSourceType" %>
...
<%--@elvariable id="datasources" type="java.util.List<com.agnitas.emm.core.datasource.bean.DataSourceDto>"--%>
...
<mvc:message var="recipientsMsg"   code="Recipients" />
        ...
        <div class="tile-body vstack gap-3 js-scrollable">
            ...
            <div>
                ...
                <select id="type-filter" class="form-control">
                    ...
                    <c:forEach var="type" items="${GeneralDataSourceType.values()}">
                        <option value="${type}"><mvc:message code="${type.messageKey}" /></option>
                    </c:forEach>
                </select>
            </div>
            <div>
                <label class="form-label" for="recipientsCount-min-filter">${recipientsMsg}</label>
                <div class="inline-input-range">
                    <input type="number" id="recipientsCount-min-filter" class="form-control" placeholder="${minMsg}" min="1" step="1" pattern="\d+"/>
                    <input type="number" id="recipientsCount-max-filter" class="form-control" placeholder="${maxMsg}" min="1" step="1" pattern="\d+"/>
                </div>
            </div>
        </div>
    </div>
