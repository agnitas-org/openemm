...
<%--@elvariable id="emailSettingsDisabled" type="java.lang.Boolean"--%>
...
<c:if test="${gridTemplateId <= 0}">
    <div class="${isPostMediatypeActive ? 'hidden' : ''}">
        ...
        <ul id="mediatypes-list" class="list-group d-flex flex-column gap-1">
            <c:forEach var="mediaType" items="${prioritizedMediatypes}" varStatus="status">
                ...
                <emm:ShowByPermission token="${mediaType.requiredPermission}">
                    <li class="mediatype-item" data-mediatype="${mediaTypeName}" data-action="change-mediatype">
                        <a href="#" class="btn btn-icon btn-secondary mediatype-order-handle">
                            <i class="icon icon-arrows-alt"></i>
                        </a>
                        <div class="input-group">
                            <span class="input-group-text disabled">${status.index + 1}</span>
                                ...
                                <div class="form-check form-switch" data-field="toggle-vis" data-field-vis-scope="#mailingSettingsForm">
                                    <c:choose>
                                        <c:when test="${mediaTypeName eq 'email'}">
                                            <c:set var="fieldsToToggle" value="#html-content-tile, #text-content-tile, #subject-field, #pre-header-field, #format-fields, #sender-fields, #reply-to-fields, #envelope-email-field, #mailing-bcc-recipients, #onepixel-field"/>
                                        </c:when>
                                        <c:when test="${mediaTypeName eq 'sms'}">
                                            <c:set var="fieldsToToggle" value="#sms-address-field, #sms-content-tile"/>
                                        </c:when>
                                        <c:when test="${mediaTypeName eq 'rcs'}">
                                            <c:set var="fieldsToToggle" value="#rcs-sender-icon-block, #rcs-content-tile"/>
                                        </c:when>
                                        <c:otherwise>
                                            <c:set var="fieldsToToggle" value=""/>
                                        </c:otherwise>
                                    </c:choose>
                                    <mvc:checkbox id="${mediaTypeName}-mediatype-switch" cssClass="form-check-input" path="${mediaTypeName}Mediatype.active" role="switch" value="true" disabled="${not MAILING_EDITABLE or mt.readonly or isSettingsReadonly}"
                                                  data-field-vis="" data-field-vis-nondisabled="" data-field-vis-show="${fieldsToToggle}" />
                                    <div class="hidden" data-field-vis-default="" data-field-vis-hide="${fieldsToToggle}" ></div>
...
<c:if test="${not isNewGridMailing || isCopying}">
    ...
    <div class="row g-3" id="format-fields">
        <div class="col-${param.isTabsView ? 12 : 6}">
    ...
    <emm:ShowByPermission token="settings.extended">
        <div id="onepixel-field">
            ...
            <mvc:select path="emailMediatype.onepixel" id="emailOnepixel" size="1"
                        cssClass="form-control" disabled="${emailSettingsDisabled or isSettingsReadonly}">
    ...
    <emm:ShowByPermission token="campaign.show">
        <div>
            ...
            <mvc:select path="archiveId" id="settings_general_campaign" cssClass="form-control" disabled="${workflowDriven or isSettingsReadonly}" data-workflow-driven="${workflowDriven}">
