...
<div class="filter-overview" data-editable-view="${agnEditViewKey}" data-controller="profile-field-list">
        ...
        <script type="application/json" data-initializer="web-storage-persist">
            {
                "profile-field-overview": {
                    ...
                    "fields": ${emm:toJson(profileForm.selectedFields)}
                }
            }
        </script>

        <div class="tile-body">
            <div class="table-wrapper" data-table-column-manager data-action="save-selected-columns">
                ...
                <div class="table-wrapper__body">
                    <emm:table var="field" modelAttribute="profileFields" cssClass="table table-hover table--borderless js-table"
                        ...
                        <emm:column titleKey="settings.FieldName" sortable="true" property="shortName" sortProperty="shortname" data-table-column=""/>

                        <emm:column titleKey="settings.FieldNameDB" sortable="true" sortProperty="column" property="columnName" data-table-column="" />
                        <emm:column titleKey="Description" sortable="true" property="description" data-table-column="" />

                        <emm:column titleKey="default.Type" sortable="true" sortProperty="dataType" data-table-column="">
                            <span><mvc:message code="${field.simpleDataType.messageKey}"/></span>
                        </emm:column>

                        <emm:column titleKey="profile.history.include" sortable="true" sortProperty="historized" data-table-column="">
                            <div class="flex-center">
                                <c:choose>
                                    <c:when test="${RecipientStandardField.isHistorized(field)}">
                                        <span class="icon-badge" style="background: var(--badge-green)">
                                            <i class="icon icon-toggle-on"></i>
                                        </span>
                                    </c:when>
                                    <c:otherwise>
                                       <span class="icon-badge" style="background: var(--badge-light-red)">
                        ...
                        <emm:column titleKey="settings.Length" sortable="true" sortProperty="dataTypeLength" data-table-column="length"
                                    headerClass="fit-content ${profileForm.isSelectedColumn('length') ? '' : 'hidden'}"
                                    cssClass="${profileForm.isSelectedColumn('length') ? '' : 'hidden'}">
                            <c:if test="${field.characterLength > 0}">
                                <span>${field.characterLength}</span>
                            </c:if>
                        </emm:column>

                        <emm:column titleKey="settings.Default_Value" sortable="true" sortProperty="defaultValue" data-table-column="">
                            <span>${fn:escapeXml(field.defaultValue)}</span>
                        </emm:column>

                        <emm:column titleKey="visibility" headerClass="fit-content" sortable="true" sortProperty="modeEdit" data-table-column="">
                            <span><mvc:message code="${field.defaultPermission.messageKey}"/></span>
                        </emm:column>

                        <emm:column titleKey="default.creationDate" sortable="true" property="creationDate"
                                    data-table-column="creation_date"
                                    headerClass="${profileForm.isSelectedColumn('creation_date') ? '' : 'hidden'}"
                                    cssClass="${profileForm.isSelectedColumn('creation_date') ? '' : 'hidden'}" />

                        <emm:column titleKey="default.changeDate" sortable="true" property="changeDate"
                                    data-table-column="change_date"
                                    headerClass="${profileForm.isSelectedColumn('change_date') ? '' : 'hidden'}"
                                    cssClass="${profileForm.isSelectedColumn('change_date') ? '' : 'hidden'}" />

                        <emm:column titleKey="settings.FieldFixedValue" data-table-column="fixed_values"
                                    headerClass="${profileForm.isSelectedColumn('fixed_values') ? '' : 'hidden'}"
                                    cssClass="${profileForm.isSelectedColumn('fixed_values') ? '' : 'hidden'}">
                            <span>
                               <c:forEach var="fixedValue" items="${field.allowedValues}" varStatus="loop_status">
                                   ${fixedValue}
                                   <c:if test="${loop_status.index + 1 lt fn:length(field.allowedValues)}">
                                       <br>
                                       <br>
                                   </c:if>
                               </c:forEach>
    ...
    <mvc:form id="filter-tile" cssClass="tile" method="GET" servletRelativeAction="/profiledb/search.action" modelAttribute="profileForm"
        ...
        <div class="tile-body form-column js-scrollable">
            ...
            <div>
                <label class="form-label" for="filter-historization"><mvc:message code="profile.history.include"/></label>
                <mvc:select id="filter-historization" path="historized" cssClass="form-control">
                    <mvc:option value=""><mvc:message code="default.All"/></mvc:option>
