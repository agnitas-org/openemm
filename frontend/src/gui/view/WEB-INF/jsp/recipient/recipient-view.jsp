...
<%--@elvariable id="dataSource" type="com.agnitas.beans.DatasourceDescription"--%>
<%--@elvariable id="latestDataSource" type="com.agnitas.beans.DatasourceDescription"--%>
    ...
    <div id="settings-tile" class="tile" data-editable-tile>
        ...
        <div class="tile-body vstack gap-3 js-scrollable">
            ...
            <div>
                ...
                <mvc:select path="mailtype" id="recipient-mailtype" cssClass="form-control" multiple="false">
        ...
        <div class="tile-body vstack gap-3 js-scrollable">
            <c:if test="${showProfileFieldsHint}">
                <div class="notification-simple notification-simple--info notification-simple--lg">
                    <span><mvc:message code="hint.recipient.fields.update" /></span>
                </div>
            </c:if>
    ...
    <div id="mailinglists-tile" class="tile" data-editable-tile>
        ...
        <div class="tile-body vstack gap-3 js-scrollable">
            ...
            <c:forEach items="${mailinglists}" var="mlist">
                ...
                <c:set var="mailinglistTileState" value="false"/>
                ...
                <c:forEach items="${availableMediaTypes}" var="mediaType">
                    <emm:ShowByPermission token="${mediaType.requiredPermission.tokenString}">
                        ...
                        <c:if test="${not empty binding.status and binding.status eq USER_STATUS_ACTIVE}">
                            <c:set var="mailinglistTileState" value="true"/>
                        </c:if>
                    </emm:ShowByPermission>
                </c:forEach>

                <div>
                    <div id="tile-recipient-mailinglist-${mlist.id}" class="tile tile--md" data-toggle-tile="${mailinglistTileState}" data-mailinglist-tile>
                        <div class="tile-header">
                            ...
                            <div class="d-flex gap-3">
                                <c:forEach items="${availableMediaTypes}" var="mediaType">
                                    <emm:ShowByPermission token="${mediaType.requiredPermission.tokenString}">
                                        ...
                                        <i class="icon icon-mediatype-${mediaType.mediaCode} fs-1 ${isBindingActive ? 'icon-mediatype--active' : ''}"></i>
                                    </emm:ShowByPermission>
                                </c:forEach>
                            </div>
                        </div>
                                            ...
                                            <div class="tile-body form-column">
                                                ...
                                                <div>
                                                    ...
                                                    <c:if test="${binding.status eq USER_STATUS_USER_OUT}">
                                                        <c:set var="statusRemark" value="${statusRemark}${NEWLINE}Opt-Out-Mailing: ${binding.exitMailingId}"/>
                                                    </c:if>
                                                    ...
                                                    <div class="row g-2">
                                                        <div class="col-8">
                                                            <c:choose>
                                                                <c:when test="${fn:contains(statusRemark, NEWLINE)}">
                                                                    <textarea class="form-control" readonly rows="1">${statusRemark}</textarea>
                                                                </c:when>
                                                                <c:otherwise>
...
<script id="duplicated-email-block" type="text/x-mustache-template">
    <div class="hstack justify-content-between w-100">
        ...
        <a href="{{- AGN.url('/recipient/' + recipientID + '/view.action') }}" class="btn btn-sm btn-secondary text-nowrap">
