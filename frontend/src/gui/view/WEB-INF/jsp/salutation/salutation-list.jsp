...
<%--@elvariable id="salutations" type="com.agnitas.beans.PaginatedList<com.agnitas.beans.Title>"--%>
...
<c:url var="bulkDeleteUrl" value="/salutation/delete.action"/>

<div class="filter-overview" data-editable-view="${agnEditViewKey}">
    <mvc:form id="table-tile" cssClass="tile" servletRelativeAction="/salutation/list.action" modelAttribute="salutationOverviewFilter" method="GET" data-editable-tile="main">
                    ...
                    <emm:table var="salutation" modelAttribute="salutations" cssClass="table table-hover table--borderless js-table">
                        ...
                        <c:if test="${deleteAllowed}">
                            <emm:column title="${checkboxSelectAll}" cssClass="mobile-hidden" headerClass="mobile-hidden">
                                <input class="form-check-input" type="checkbox" name="bulkIds" value="${salutation.id}" autocomplete="off" data-bulk-checkbox ${salutationDeleteAllowed ? '' : 'disabled'} />
                            </emm:column>
                        </c:if>
