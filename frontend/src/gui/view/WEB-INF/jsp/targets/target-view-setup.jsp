...
<emm:instantiate var="itemActionsSettings" type="java.util.LinkedHashMap" scope="request">
    ...
    <c:if test="${not hidden}">
        <emm:instantiate var="element" type="java.util.LinkedHashMap">
            ...
            <c:if test="${targetId > 0}">
                <emm:instantiate var="dropDownItem" type="java.util.LinkedHashMap">
                    <c:set target="${dropDownItems}" property="1" value="${dropDownItem}"/>
                    <c:set target="${dropDownItem}" property="url">
                        <c:url value="/target/${targetId}/export.action"/>
                    </c:set>
                    <c:set target="${dropDownItem}" property="name"><mvc:message code="Export"/></c:set>
                    <c:set target="${dropDownItem}" property="extraAttributes" value="data-prevent-load=''"/>
                </emm:instantiate>
            </c:if>
