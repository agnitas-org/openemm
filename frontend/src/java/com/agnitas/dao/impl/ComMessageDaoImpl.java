...
public class ComMessageDaoImpl extends BaseDaoImpl implements ComMessageDao {
	...
	/** The logger. */
	private static final transient Logger logger = LogManager.getLogger(ComMessageDaoImpl.class);
    ...
    public List<String> getAllMessageKeys(boolean isIncludeDeleted) {
        return select(logger, "SELECT message_key FROM messages_tbl" + (!isIncludeDeleted ? " WHERE deleted = 0" : "") + " ORDER BY deleted DESC, change_date", StringRowMapper.INSTANCE);
    }
