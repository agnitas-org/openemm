...
public class EmmLayoutBaseDaoImpl extends BaseDaoImpl implements EmmLayoutBaseDao {
	/** The logger. */
	private static final transient Logger logger = LogManager.getLogger(EmmLayoutBaseDaoImpl.class);
	...
	public EmmLayoutBase getEmmLayoutBase(int companyID, int emmLayoutBaseID) {
		...
		try {
			List<EmmLayoutBase> result = select(logger, "SELECT layout_base_id, base_url, shortname, menu_position, livepreview_position, company_id, theme_type FROM emm_layout_base_tbl WHERE layout_base_id = ? AND company_id IN(0, ?)", new EmmLayoutBaseRowMapper(), emmLayoutBaseID, companyID);
			if (result.size() > 0) {
				return result.get(0);
			} else {
	...
	public List<EmmLayoutBase> getEmmLayoutsBase(int companyID) {
		try {
			return select(logger, "SELECT layout_base_id, base_url, shortname, menu_position, livepreview_position, company_id, theme_type FROM emm_layout_base_tbl WHERE company_id = ? OR company_id = 0 ORDER BY company_id, menu_position", new EmmLayoutBaseRowMapper(), companyID);
		} catch (Exception e) {
	...
	protected class EmmLayoutBaseRowMapper implements RowMapper<EmmLayoutBase> {
		@Override
