...
public class MailingArchiveController implements XssCheckAware {
    ...
    public MailingArchiveController(WebStorage webStorage, CampaignService campaignService, ComMailingBaseService mailingBaseService, UserActivityLogService userActivityLogService) {
    ...
    public Object save(Admin admin, @ModelAttribute MailingArchiveForm form, Popups popups, HttpSession session, RedirectAttributes ra) {
        ...
        return isRedesignedUiUsed(admin) ? "redirect:/mailing/archive/list.action?restoreSort=true" : redirectToViewPage(campaignId);
    }
    ...
    private void writeArchiveSaveLog(Campaign oldArchive, Campaign newArchive, Admin admin) {
        if (oldArchive == null) {
            ...
            writeImportChangeLog(oldArchive, newArchive, admin);
        }
    }

    private void writeImportChangeLog(Campaign oldArchive, Campaign newArchive, Admin admin) {
