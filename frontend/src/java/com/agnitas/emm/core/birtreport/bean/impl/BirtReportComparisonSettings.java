...
import static com.agnitas.emm.core.birtreport.bean.impl.BirtReportMailingSettings.DATE_RANGE_WEEK;
...
public class BirtReportComparisonSettings extends BirtReportSettings {
    ...
    public String getReportName(String reportFormat) {
        if (StringUtils.equalsIgnoreCase(BirtReport.FORMAT_CSV, reportFormat)) {
    ...
    public Set<String> getMissingReportParameters(int intervalReportType) {
        ...
        if (missing.contains(BirtReportSettings.TARGET_GROUPS_KEY)) {
            String mailingType = getReportSettingAsString(BirtReportSettings.MAILING_TYPE_KEY);
            
            boolean isPredefined = BirtReportSettingsUtils.equalParameter(mailingType, BirtReportSettings.MAILINGS_PREDEFINED);
            ...
            if (isPredefined || isCustom) {
                missing.remove(BirtReportSettings.TARGET_GROUPS_KEY);
            }
        }
        return missing;
    }
