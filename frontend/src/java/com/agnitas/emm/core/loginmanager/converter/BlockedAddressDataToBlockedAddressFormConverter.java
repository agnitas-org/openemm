...
@Component
public class BlockedAddressDataToBlockedAddressFormConverter implements Converter<BlockedAddressData, BlockedAddressForm> {
	
	@Override
	public BlockedAddressForm convert(BlockedAddressData source) {
		BlockedAddressForm form = new BlockedAddressForm();
		form.setTrackingId(source.getTrackingId());
		
		if(source.getUsername().isPresent()) {
			form.setUsername(source.getUsername().get());
		}
