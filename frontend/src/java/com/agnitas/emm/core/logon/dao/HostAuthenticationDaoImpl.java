 ...
 * 
 *  If a database-specific query is required, create subclass of this implementation and use {@link DbSwitchingHostAuthenticationDaoImpl} for
 *  to switch code.
 * 
 * @see DbSwitchingHostAuthenticationDaoImpl
 */
 ...
 * 
 * Use this class (HostAuthenticationDaoImpl) as base class for common SQL statements and inherit from this class for Oracle, MySQL, ...
...
public class HostAuthenticationDaoImpl extends BaseDaoImpl implements HostAuthenticationDao {
	...
	private boolean renewHostAuthenticationData(Admin admin, String hostID, int expiresInDays) {
		...
		int result = update("UPDATE authenticated_hosts_tbl SET expire_date = ?, change_date = CURRENT_TIMESTAMP WHERE admin_id = ? AND host_id = ?", expireDate, adminID, hostID);
	...
	private boolean renewHostAuthenticationData(Supervisor supervisor, String hostID, int expiresInDays) {
		...
		// Update authentication data
		int result = update("UPDATE authenticated_hosts_sv_tbl SET expire_date = ?, change_date = CURRENT_TIMESTAMP WHERE supervisor_id = ? AND host_id = ?", expireDate, supervisorID, hostID);
	...
	private void writeNewHostAuthenticationData(Admin admin, String hostID, int expiresInDays) {
		...
		update("INSERT INTO authenticated_hosts_tbl (admin_id, host_id, expire_date, change_date) VALUES (?, ?, ?, CURRENT_TIMESTAMP)", adminID, hostID, expireDate);
	}
	...
	private void writeNewHostAuthenticationData(Supervisor supervisor, String hostID, int expiresInDays) {
		...
		// Write authentication data
		update("INSERT INTO authenticated_hosts_sv_tbl (supervisor_id, host_id, expire_date, change_date) VALUES (?, ?, ?, CURRENT_TIMESTAMP)", supervisorID, hostID, expireDate);
	}
	...
	public void removePendingSecurityCode(Admin admin, String hostID) {
		int adminID = admin.getAdminID();
		update("DELETE FROM hostauth_pending_tbl WHERE admin_id = ? AND host_id = ?", adminID, hostID);
	}
	...
	public void removePendingSecurityCode(Supervisor supervisor, String hostID) {
		int supervisorID = supervisor.getId();
		update("DELETE FROM hostauth_pending_sv_tbl WHERE supervisor_id = ? AND host_id = ?", supervisorID, hostID);
	}
