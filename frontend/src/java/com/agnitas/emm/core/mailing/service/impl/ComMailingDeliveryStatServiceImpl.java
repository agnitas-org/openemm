...
public class ComMailingDeliveryStatServiceImpl implements ComMailingDeliveryStatService {
    ...
    public DeliveryStat getDeliveryStats(int companyID, int mailingID, MailingType mailingType) {
        if (triggerdialogService != null && triggerdialogService.isPostMailing(mailingDao.getMailing(mailingID, companyID))) {
			...
			try {
				deliveryStatistic = triggerdialogService.getTriggerdialogDeliveryStatus(mailingID);
				...
				logger.error("Error in getTriggerdialogDeliveryStatus: " + e.getMessage(), e);
				deliveryStatistic = new DeliveryStat();
			}
    ...
    private final void updateCancelStateIfMailingStopped(final DeliveryStat deliveryStatistic, final int mailingID) {
        if(mailingStopService.isStopped(mailingID)) {
        	...
        	default:
        		logger.warn(String.format("Don't know how to handle delivery status %d of mailing %d", deliveryStatistic.getDeliveryStatus(), mailingID));
        	}
        }
