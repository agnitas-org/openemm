...
public class ComAccessDataDaoImpl extends BaseDaoImpl implements ComAccessDataDao {
	...
	public void writeData(ComAccessData accessData) {
		...
		try {
			update(logger, sql, userAgentForStorage, accessData.getXuid(), accessData.getIp(), refererForStorage, accessData.getAccessType().toString(), accessData.getMailingID(), accessData.getCustomerID(), accessData.getLinkID(), accessData.getDeviceID());
		} catch (Exception e) {
