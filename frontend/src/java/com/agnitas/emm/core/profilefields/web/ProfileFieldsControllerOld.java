...
public class ProfileFieldsControllerOld implements XssCheckAware {

    private static final Logger logger = LogManager.getLogger(ProfileFieldsControllerOld.class);
    ...
    public ProfileFieldsControllerOld(ProfileFieldService profileFieldService, WebStorage webStorage, ConversionService conversionService, ConfigService configService,
    ...
    public String list(@ModelAttribute("profileForm") ProfileFieldForm profileForm, Admin admin, Model model, Popups popups,
        ...
        if (profileFieldService.isWithinGracefulLimit(companyId)) {
            int currentField = profileFieldService.getCurrentSpecificFieldCount(companyId);
            int maximumField = profileFieldService.getMaximumCompanySpecificFieldCount(companyId);

            popups.warning("error.numberOfProfileFieldsExceeded.graceful", maximumField, currentField, ConfigValue.System_License_MaximumNumberOfProfileFields.getGracefulExtension());
        }

        return "settings_profile_field_list_old";
    }
    ...
    public String view(Admin admin, @PathVariable("fieldname") String fieldName, ModelMap model) {
        ...
        if (!model.containsAttribute("profileForm")) {
            if (field == null) {
                return "redirect:/profiledbold/new.action";
            }
        ...
        return "settings_profile_field_view_old";
    }
    ...
    public String dependents(Admin admin, @PathVariable("fieldname") String fieldName, Model model, Popups popups) {
        ...
        if (field == null) {
            ...
            return "redirect:/profiledbold/profiledb.action";
        }

        model.addAttribute("fieldname", fieldName);
        model.addAttribute("dependents", profileFieldService.getDependents(companyId, fieldName));

        return "settings_profile_field_dependents_list_old";
    }
        ...
        if (StringUtils.isBlank(profileForm.getFieldname())) {
            ...
            if (!popups.hasAlertPopups()) {
                if (isNewField ? createField(admin, profileForm) : updateField(admin, field, profileForm)) {
                    ...
                    return "redirect:/profiledbold/profiledb.action?syncSorting=true";
                } else {
                    popups.alert(ERROR_MSG);
                }
            }
    ...
    public String saveWizardField(Admin admin, @ModelAttribute("profileForm") ProfileFieldForm profileForm, RedirectAttributes model, Popups popups) {
        ...
        return "redirect:/profiledbold/newWizardField.action";
    }
