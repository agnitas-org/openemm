...
public class ReferencedItemsDaoImpl extends BaseDaoImpl implements ReferencedItemsDao {
	...
	public void removeAllReferencedObjects(final int companyID, final int targetID) {
		update("DELETE FROM target_ref_mailing_tbl WHERE company_ref=? AND target_ref=?", companyID, targetID);
		update("DELETE FROM target_ref_link_tbl WHERE company_ref=? AND target_ref=?", companyID, targetID);
		update("DELETE FROM target_ref_autoimport_tbl WHERE company_ref=? AND target_ref=?", companyID, targetID);
		update("DELETE FROM target_ref_profilefield_tbl WHERE company_ref=? AND target_ref=?", companyID, targetID);
	}
	...
	public final List<TargetLight> listTargetGroupsReferencingProfileField(final int companyID, final String visibleShortname) {
		...
		return select(sql, TargetLightRowMapper.INSTANCE, companyID, visibleShortname);
	}
	...
	public final List<TargetLight> listTargetGroupsReferencingMailing(final int companyID, final int mailingID) {
		...
		return select(sql, TargetLightRowMapper.INSTANCE, companyID, mailingID);
	}
	...
	public final List<TargetLight> listTargetGroupsReferencingAutoImport(final int companyID, final int autoImportID) {
		...
		return select(sql, TargetLightRowMapper.INSTANCE, companyID, autoImportID);
	}
}
