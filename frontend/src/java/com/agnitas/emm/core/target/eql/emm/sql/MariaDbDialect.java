...
/**
 * MariaDB-specific implementation of {@link SqlDialect}.
 */
public class MariaDbDialect implements SqlDialect {
	
	/** MariaDB's format symbol for "day, numeric, 2 digits". */
	private static final String MARIA_DB_DAY_SYMBOL = "%d";
	
	/** MariaDB's format symbol for "month, numeric, 2 digits". */
	private static final String MARIA_DB_MONTH_SYMBOL = "%m";
	
	/** MariaDB's format symbol for "year, numeric, 4 digits". */
	private static final String MARIA_DB_YEAR_SYMBOL = "%Y";
	...
	public String dateFormat(EqlDateFormat dateFormat) {
		...
		for (DateFragment fragment : dateFormat) {
			switch (fragment) {
				case DAY:
					builder.append(MARIA_DB_DAY_SYMBOL);
					break;

				case MONTH:
					builder.append(MARIA_DB_MONTH_SYMBOL);
					break;

				case YEAR:
					builder.append(MARIA_DB_YEAR_SYMBOL);
					break;
