...
/**
 * Utility class to convert a {@link LinkService.LinkScanResult} to {@link Message}s.
 */
public final class LinkScanResultToMessage {
	...
	/** Message key for list of links less than or equals to {@link #MAX_LINKS_PER_WARNING}. */
	public static final String MESSAGE_KEY_AGN_TAG_NON_TRACKABLE_LINK = "warning.mailing.links.insecureProtocol";

	/** Message key for list of links greater than {@link #MAX_LINKS_PER_WARNING}. */
	public static final String MESSAGE_KEY_AGN_TAG_NON_TRACKABLE_LINK_WITH_MORE = "warning.mailing.links.insecureProtocol.withMore";
	 ...
	 */
	public static final void linkWarningsToMessage(final LinkScanResult result, final List<Message> warnings) {
		insecureLinksToActionMessages(result, warnings);
		agnTagNonTrackableToActionMessage(result, warnings);
	 ...
	 */
	private static final void insecureLinksToActionMessages(final LinkScanResult result, final List<Message> warnings) {
		...
		if(!list.isEmpty()) {
			...
			if(numMoreLinks == 0) {
				warnings.add(Message.of(MESSAGE_KEY_INSECURE_LINKS, links));
			} else {
				warnings.add(Message.of(MESSAGE_KEY_INSECURE_LINKS_WITH_MORE, links, numMoreLinks));
			}
		}
	}
	/**
	 ...
	 * @param warnings container for warnings
	 */
	private static final void agnTagNonTrackableToActionMessage(final LinkScanResult result, final List<Message> warnings) {
		final List<LinkWarning> list = filterWarningsByType(result, WarningType.AGN_TAG_NON_TRACKABLE);
