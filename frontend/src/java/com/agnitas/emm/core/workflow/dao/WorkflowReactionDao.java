...
public interface WorkflowReactionDao {
	...
	List<WorkflowReaction> getReactionsToCheck(CompaniesConstraints constraints);

	/**
	 * Get all undone steps whose time has come (calculated step date is in the past).
	 *
	 * Notice that {@link WorkflowReaction#isActive()} doesn't enable/disable
    ...
    void saveReaction(WorkflowReaction reaction);
	...
	List<Integer> getClickedRecipients(WorkflowReaction reaction, boolean excludeLoggedReactions);

	List<Integer> getOpenedRecipients(WorkflowReaction reaction, boolean excludeLoggedReactions);

	List<Integer> getRecipientsWithChangedProfile(WorkflowReaction reaction, boolean excludeLoggedReactions);

	List<Integer> getRecipientsWithChangedBinding(WorkflowReaction reaction, boolean excludeLoggedReactions);
	 ...
	 */
	void trigger(WorkflowReaction reaction, List<Integer> recipients);
	 ...
	 *
	 * @param step the step to be marked as done.
	 * @param sqlTargetExpression and SQL code to be used to filter recipients (see {@link TargetDao#getTargetSQL(int, int)}).
	 */
	 ...
	 * recipients must have active binding (see {@link com.agnitas.beans.BindingEntry#USER_STATUS_ACTIVE}) to pass the filter.
	 * Use "now" as step timestamp for recipients.
	 *
