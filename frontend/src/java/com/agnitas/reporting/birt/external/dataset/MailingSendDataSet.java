...
public class MailingSendDataSet extends BIRTDataSet {
	/** The logger. */
	private static final transient Logger logger = LogManager.getLogger(MailingSendDataSet.class);
	...
	public Integer getTotalSend(Integer mailingID, @VelocityCheck Integer companyId, String selectedTargets) {
		...
		if (null == result) {
			...
			if (StringUtils.isEmpty(targetSql)) {
				result = selectInt(logger, "SELECT SUM(no_of_mailings) FROM mailing_account_tbl WHERE status_field in ('W', 'E', 'R') AND mailing_id = ?", mailingID);
			} else {
				String sql = "SELECT SUM(acc.no_of_mailings) FROM mailing_account_tbl acc"
					...
					+ " WHERE acc.mailing_id = ? AND status_field in ('W', 'E', 'R') "
					+ " AND " + targetSql;
