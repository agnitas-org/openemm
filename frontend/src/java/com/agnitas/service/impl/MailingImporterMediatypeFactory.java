...
public class MailingImporterMediatypeFactory {
	...
	public Mediatype createMediatypeFromJson(final JsonObject mediatypeJsonObject) {
		...
		case EMAIL: {
			...
			if (mediatypeJsonObject.containsPropertyKey("priority")) {
				emailType.setPriority((Integer) mediatypeJsonObject.get("priority"));
			}
			if (mediatypeJsonObject.containsPropertyKey("priority")) {
				emailType.setStatus(MediaTypeStatus.getMediaTypeStatusByName((String) mediatypeJsonObject.get("status")).getCode());
			} else {
				emailType.setStatus(MediaTypeStatus.Active.getCode());
			}
			return emailType;
		}
