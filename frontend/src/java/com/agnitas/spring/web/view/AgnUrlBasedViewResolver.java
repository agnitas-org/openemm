...
public class AgnUrlBasedViewResolver extends UrlBasedViewResolver {
    ...
    static {
        viewsPermissions.put("upload_list", Permission.UPLOAD_UI_MIGRATION);
        viewsPermissions.put("upload_view", Permission.UPLOAD_UI_MIGRATION);
        classicMediapoolCategoryViewNames.forEach(v -> viewsPermissions.put(v, Permission.MEDIAPOOL_CATEGORIES_UI_MIGRATION));
        classicMediapoolViewNames.forEach(v -> viewsPermissions.put(v, Permission.MEDIAPOOL_UI_MIGRATION));
    ...
    private boolean isRedesignedViewUsageAllowed(String viewName) {
        ...
        if (request.isEmpty() || !AgnUtils.allowed(request.get(), Permission.USE_REDESIGNED_UI)) {
            return false;
        }
