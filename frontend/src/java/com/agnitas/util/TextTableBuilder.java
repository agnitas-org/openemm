...
public class TextTableBuilder {
	...
	private static final Logger logger = LogManager.getLogger(TextTableBuilder.class);
	...
	public String getData(int lineIndex, int columnIndex) throws Exception {
		if (content.size() <= lineIndex || content.get(lineIndex).size() <= columnIndex) {
			throw new Exception("Invalid data query for row index " + lineIndex + " column index " + columnIndex);
		}
