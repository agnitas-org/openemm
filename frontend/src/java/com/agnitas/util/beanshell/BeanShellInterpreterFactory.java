...
package com.agnitas.util.beanshell;
...
public class BeanShellInterpreterFactory {
	...
	private Interpreter createBeanShellInterpreter(final int companyID, final PopulateFieldsConsumer populateFieldsConsumer) throws Exception {
		...
		try {
			...
			beanShellNameSpace.importClass("com.agnitas.util.AgnUtils");
	...
	private Date buildDate(final Map<String, Object> recipientData, String fieldName) {
		final int day = Integer.parseInt(recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY).toString()),
				month = Integer.parseInt(recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH).toString()),
				year = Integer.parseInt(recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR).toString());
		Object hourObj = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_HOUR);
		if(hourObj == null) {
			hourObj = "0";
		}
		Object minuteObj = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MINUTE);
		if(minuteObj == null) {
			minuteObj = "0";
		}
		Object secondObj = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_SECOND);
		if (secondObj == null) {
			secondObj = "0";
		}
	...
	private static boolean hasTripleDateParameter(final Map<String, Object> recipientData, final String fieldName) {
		final Object day = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_DAY),
				month = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_MONTH),
				year = recipientData.get(fieldName + RecipientDao.SUPPLEMENTAL_DATECOLUMN_SUFFIX_YEAR);
		return  day != null
