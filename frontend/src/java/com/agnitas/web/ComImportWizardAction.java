...
public final class ComImportWizardAction extends StrutsActionBase {
    ...
    private Map<String, Future<?>> futureHolder;
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest req, HttpServletResponse response) throws Exception {
                ...
                switch (aForm.getAction()) {
                    ...
                    case ACTION_MODE:
                        if (aForm.isUseCsvUpload()) {
                            aForm.setCsvFile(importWizardService.getFormFileByUploadId(aForm.getAttachmentCsvFileID(), "text/csv"));
                        }
                    ...
                    case ACTION_CHECK_FIELDS:
                        ...
                        if (isColumnDuplicate(mapParametersOnlyMap)) {
                    ...
                    case ACTION_WRITE:
                        ...
                        aForm.getStatus().getErrors().clear();
                        startImportWorker(req, errors, aForm);
        ...
        if (!errors.isEmpty() && destination != null) {
            ...
            while (messageIterator.hasNext()) {
                ...
                errorMessage.append(message.getKey()).append(": ").append(message.getValues());
            }
    ...
    private void loadResultPageFormData(HttpServletRequest req, ComImportWizardForm comImportWizardForm) {
        ...
        if (comImportWizardForm.getResultMailingListAdded() != null) {
        	for (MediaTypes mediaType : comImportWizardForm.getResultMailingListAdded().keySet()) {
	            for (String mailinglistID : comImportWizardForm.getResultMailingListAdded().get(mediaType).keySet()) {
        ...
        req.setAttribute("resultMLAdded", comImportWizardForm.getResultMailingListAdded().get(MediaTypes.EMAIL));
    }
    ...
    private void startImportWorker(HttpServletRequest req, ActionMessages errors, ComImportWizardForm comImportWizardForm) {
        try {
            String key = FUTURE_TASK + "@" + req.getSession(false).getId();

            if (!futureHolder.containsKey(key)) {
                // Create new import worker
                Future<?> writeContentFuture = getWriteContentFuture(req, comImportWizardForm);
    ...
    private ActionForward checkImportWorker(ActionMapping mapping, HttpServletRequest req, ActionMessages errors, ComImportWizardForm comImportWizardForm) {
        try {
            ...
            if (futureHolder.containsKey(key) && futureHolder.get(key) != null && futureHolder.get(key).isDone()) {
                Future<?> future = futureHolder.remove(key);
    ...
    private void createReportResult(Admin admin, ProfileImportWorker profileImportWorker, ComImportWizardForm comImportWizardForm, boolean isError) throws Exception {
        ...
        if (profileImportWorker.getMailinglistAssignStatistics() != null) {
        	for (MediaTypes mediaType : profileImportWorker.getMailinglistAssignStatistics().keySet()) {
	            for (Entry<Integer, Integer> entry : profileImportWorker.getMailinglistAssignStatistics().get(mediaType).entrySet()) {
        ...
        String filename = my_time.getTime() + ".csv";
        ...
        reportService.createAndSaveImportReport(admin.getCompanyID(), admin.getAdminID(), filename, comImportWizardForm.getDatasourceID(), new Date(), csvfile, -1, isError);
    }

    private String generateLocalizedImportCSVReport(Locale locale, Date my_time, ImportStatus status, int mode) {
        String csvfile = "";
    ...
    private Future<?> getWriteContentFuture(final HttpServletRequest request, final ComImportWizardForm aForm) throws Exception {
    ...
    private void checkProfileKeyColumnIndexed(ComImportWizardForm aForm, HttpServletRequest req, ActionMessages messages) {
        ...
        if (!DbUtilities.checkForIndex(dataSource, "customer_" + AgnUtils.getCompanyID(req) + "_tbl", Collections.singletonList(keyColumn))) {
    ...
    public ActionErrors formValidate(ComImportWizardForm form, HttpServletRequest request) throws Exception {
        ...
        switch (form.getAction()) {
                ...
                if (AgnUtils.parameterNotEmpty(request, "mode_back")) {
                    ...
                    try {
                        ...
                        for (Entry<String, ProfileField> profilefieldEntry : profileFieldDao.getProfileFieldsMap(admin.getCompanyID(), admin.getAdminID()).entrySet()) {
                        	if (profilefieldEntry.getValue().getModeEdit() == ProfileFieldMode.NotVisible) {
            ...
            case ComImportWizardAction.ACTION_MODE:
                form.getImportWizardHelper().getStatus().setErrors(new HashMap<>());
                ...
                SimpleServiceResult checkedResult = importWizardService.checkAndReadCsvFile(AgnUtils.getCompanyID(request), form);

                if (!checkedResult.isSuccess()) {
                    checkedResult.getErrorMessages().forEach(e -> {
                        errors.add("global", new ActionMessage(e.getCode(), e.getArguments()));
                    });
                }
            ...
            case ComImportWizardAction.ACTION_WRITE:
                if (AgnUtils.parameterNotEmpty(request, "mlists_back")) {
                    form.setAction(ComImportWizardAction.ACTION_PRESCAN);
                } else {
                    form.getImportWizardHelper().setMailingLists(getMailinglistsFromRequest(request));

                    if (form.getImportWizardHelper().getMailingLists().size() <= 0
                            && form.getMode() != ImportMode.UPDATE.getIntValue()
                            && !(form.getMode() == ImportMode.ADD.getIntValue() && admin.permissionAllowed(Permission.IMPORT_WITHOUT_MAILINGLIST))
                            && !(form.getMode() == ImportMode.ADD_AND_UPDATE.getIntValue() && admin.permissionAllowed(Permission.IMPORT_WITHOUT_MAILINGLIST))) {
                        // Error: at least one mailinglist must be selected
                        form.setAction(ComImportWizardAction.ACTION_MLISTS);
                    }
                }
                break;
     ...
     */
    private Vector<String> getMailinglistsFromRequest(HttpServletRequest req) {
        String aParam;
    ...
    private boolean isColumnDuplicate(Map<String, String> mapParametersOnlyMap) {
