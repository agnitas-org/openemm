...
public class Code {
	...
	private String[] resolveParameter (String element,
	...
	private void showMissing (String ... what) throws Exception {
		if (missing.size () > 0) {
			throw new Exception (
				"Missing parameter" + (missing.size () > 1 ? "s " : " ") +
				missing.stream ().map (e -> "\"" + e + "\"").reduce ((s, e) -> s + ", " + e).orElse ("") +
				" in " +
				Arrays.stream (what).reduce ((s, e) -> s + " " + e).orElse ("unspecified")
			);
		}
	}
}
