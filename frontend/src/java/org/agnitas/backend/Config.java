...
public class Config {
	...
	public boolean loadConfig (String nFilename) {
		if (! fileExists (nFilename)) {
			...
			if (filename == null) {
				filename = scanForConfig (Data.home, filesep, nFilename);
				if (filename != null) {
					logging (Log.VERBOSE, "Found config file " + filename + " while starting search in home directory " + Data.home);
				}
			}
		} else {
			filename = nFilename;
	...
	public void set (String name, Object value) {
		...
		config.setProperty (key, valueString);
		namespace.put (key, valueString);
	}
	/**
	 * Search for a value
	 *
	...
	public String cget (String key) {
		return fill (config.getProperty (key));
	}
	/**
	 * Search for a value
	 *
