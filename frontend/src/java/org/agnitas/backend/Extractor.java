...
public class Extractor implements ResultSetExtractor <Object> {
	...
	public Extractor (Map<String, EMMTag> tagNames, MediaMap mmap, Data nData, Custinfo nCinfo,
			  Blacklist nBlist, MailWriter nMailer,
			  boolean nNeedSamples, Set <Long> nSeen, boolean nMulti,
			  boolean nHasOverwriteData, boolean nHasVirtualData,
			  List <EMMTag> nEmailTags, int nEmailCount) {
		...
		if (! seen.contains (cid)) {
			...
			try {
				...
				for (@SuppressWarnings("unused") Target t : data.targetExpression.resolveByDatabase ()) {
					...
					cinfo.setTargetGroupValue (pos, c != null ? c.get ().equals ("1") : false);
					++pos;
				}
			} catch (Exception e) {
