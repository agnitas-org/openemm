...
abstract public class MailWriter {
	...
	public MailWriter (Data data, BlockCollection allBlocks) throws Exception {
		...
		// create boundaries
		innerBoundary = "-==" + data.mailing.boundary () + "INNERB%(sys$boundary)==";
		outerBoundary = "-==" + data.mailing.boundary () + "OUTER%(sys$boundary)==";
		attachBoundary = "-==" + data.mailing.boundary () + "ATTACH%(sys$boundary)==";
