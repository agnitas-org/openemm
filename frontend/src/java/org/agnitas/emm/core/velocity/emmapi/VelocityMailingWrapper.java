...
public final class VelocityMailingWrapper implements VelocityMailing {
	...
	/** The logger. */
	private static final transient Logger LOGGER = LogManager.getLogger(VelocityMailingWrapper.class);
	...
	public final void setMailingType(final int mailingTypeCode) {
		try {
			this.mailing.setMailingType(MailingType.fromCode(mailingTypeCode));
		} catch (Exception e) {
			throw new RuntimeException("Invalid MailingType code: " + mailingTypeCode);
		}
	}
