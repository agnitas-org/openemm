...
public class ProfileImportWorkerWithExcel extends ProfileImportWorker {
    private static final transient Logger logger = LogManager.getLogger(ProfileImportWorkerWithExcel.class);
	...
	@Override
	public ProfileImportWorkerWithExcel call() throws Exception {
		try {
			...
			if (error != null) {
				...
				try {
					...
					status.setInvalidRecipientsCsv(createInvalidRecipients(getImportFile().getLocalFile().getName() + "_invalid_recipients", importedDataFileAttributes, "CSV".equalsIgnoreCase(importProfile.getDatatype())));
					currentProgressStatus = ImportItemizedProgress.REPORTING_FIXED_RECIPIENTS;
					status.setFixedByUserRecipientsCsv(createFixedByUserRecipients(getImportFile().getLocalFile().getName() + "_fixed_recipients", importedDataFileAttributes, "CSV".equalsIgnoreCase(importProfile.getDatatype())));
	...
	private void validateAndSetInsertParameter(ImportModeHandler importModeHandler, PreparedStatement preparedStatement, int columnIndex, ColumnMapping columnMapping, Object dataValue, CaseInsensitiveMap<String, DbColumnType> columnDataTypes, List<Object> batchValueEntry) throws Exception {
		...
		if (simpleColumnDataType == SimpleDataType.Date) {
			...
			if (StringUtils.isBlank(dataValueString)) {
				...
				try {
					HtmlChecker.checkForUnallowedHtmlTags(dataValueString, configService.getBooleanValue(ConfigValue.AllowHtmlInProfileFields, importProfile.getCompanyId()));
