	...
	public Map<Integer, Integer> handlePostProcessing(EmmActionService emmActionService, CustomerImportStatus status, ImportProfile importProfile, String temporaryImportTableName, int datasourceId, List<Integer> mailingListIdsToAssign) {
		...
		if (mailingListIdsToAssign != null) {
	    	...
	    	for (int mailingListId : mailingListIdsToAssign) {
	    		int changed = importRecipientsDao.changeStatusInMailingList(temporaryImportTableName, importProfile.getKeyColumns(), importProfile.getCompanyId(), mailingListId, UserStatus.Active.getStatusCode(), UserStatus.Bounce.getStatusCode(), "Mass Bounce by Admin");
