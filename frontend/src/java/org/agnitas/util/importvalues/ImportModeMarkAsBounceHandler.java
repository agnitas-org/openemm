...
public class ImportModeMarkAsBounceHandler implements ImportModeHandler {
	...
	private static final transient Logger logger = Logger.getLogger(ImportModeMarkAsBounceHandler.class);
	...
	public Map<Integer, Integer> handlePostProcessing(EmmActionService emmActionService, CustomerImportStatus status, ImportProfile importProfile, String temporaryImportTableName, int datasourceId, List<Integer> mailingListIdsToAssign, MediaTypes mediatype) throws Exception {
		...
		if (mailingListIdsToAssign != null) {
	    	...
	    	for (int mailingListId : mailingListIdsToAssign) {
	    		int changed = importRecipientsDao.changeStatusInMailingList(temporaryImportTableName, importProfile.getKeyColumns(), importProfile.getCompanyId(), mailingListId, mediatype, UserStatus.Active.getStatusCode(), UserStatus.Bounce.getStatusCode(), "Mass Bounce by Admin");
