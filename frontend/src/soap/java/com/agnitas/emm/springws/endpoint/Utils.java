...
import com.agnitas.emm.springws.jaxb.MapItem;
...
public class Utils {
	...
	public static CaseInsensitiveMap<String, Object> toCaseInsensitiveMap(Map map,  boolean extractStringFromSubXml) {
	...
	public static CaseInsensitiveMap<String, Object> toCaseInsensitiveComMap(com.agnitas.emm.springws.jaxb.extended.Map map, final boolean extractStringFromSubXml) {
		if (map == null || map.getItem() == null) {
			return null;
		}
		CaseInsensitiveMap<String, Object> resultMap = new CaseInsensitiveMap<>(map.getItem().size());
		
		for (com.agnitas.emm.springws.jaxb.extended.MapItem item : map.getItem()) {
			final String key = (item.getKey() instanceof Element && extractStringFromSubXml) ? stringFromSubXml((Element) item.getKey()) : (String) item.getKey();
			final Object value = (item.getValue() instanceof Element && extractStringFromSubXml) ? stringFromSubXml((Element) item.getValue()) : item.getValue();
	...
	private static String stringFromSubXml(Element object) {
	...
	public static com.agnitas.emm.springws.jaxb.extended.Map toJaxbComMap(java.util.Map<String, Object> map) {
		if (map == null) {
			return null;
		}
		com.agnitas.emm.springws.jaxb.extended.Map resultMap = new com.agnitas.emm.springws.jaxb.extended.Map();
		for (Entry<String, Object> entry : map.entrySet()) {
			com.agnitas.emm.springws.jaxb.extended.MapItem mapItem = new com.agnitas.emm.springws.jaxb.extended.MapItem();
			mapItem.setKey(entry.getKey());
			mapItem.setValue(entry.getValue());
			resultMap.getItem().add(mapItem);
		}
	...
	public static void checkBulkSizeLimit(String username, String endpointName, WebserviceUserService userService, int size) {
	...
	private static OptionalInt readBulkSizeLimitForWebserviceUser(WebserviceUserService userService, String username) {
		return userService.findSettingsForWebserviceUser(username)
				.map(WebserviceUserSettings::getBulkSizeLimit)
