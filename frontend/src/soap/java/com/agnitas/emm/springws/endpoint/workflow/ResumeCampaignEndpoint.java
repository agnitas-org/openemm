...
public class ResumeCampaignEndpoint extends BaseEndpoint {
    ...
    public @ResponsePayload ResumeCampaignResponse resume(@RequestPayload ResumeCampaignRequest request) {
        int workflowId = request.getCampaignID();
        int companyId = securityContextAccess.getWebserviceUserCompanyId();

        Workflow workflow = workflowService.getWorkflowOrThrow(workflowId, companyId);

        if (workflow.getStatus().equals(WorkflowStatus.STATUS_ACTIVE)) {
            return new ResumeCampaignResponse();
        }

        if (!workflow.getStatus().equals(WorkflowStatus.STATUS_PAUSED)) {
        ...
        workflowActivationService.resume(workflowId, admin);

        workflowService.changeWorkflowStatus(workflowId, admin.getCompanyID(), WorkflowStatus.STATUS_ACTIVE);
        return new ResumeCampaignResponse();
    }
}
