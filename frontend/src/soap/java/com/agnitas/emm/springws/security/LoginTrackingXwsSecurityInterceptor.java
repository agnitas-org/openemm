	...
	private void validateMessageWithLoginTracking(SoapMessage message, MessageContext context) {
		try {
			...
			if(usernameOrNull != null) {
				LOGGER.info("Login for user '{}' failed", usernameOrNull, e);
			} else {
		...
		if(this.loginTrackService.isIpAddressLocked(ip, companyID)) {
			LOGGER.info("Login for user '{}' blocked by IP lock", usernameOrNull);
			...
			LOGGER.info("Login for user '{}' successful", usernameOrNull);
