		...
		<div class="tile-body">
			<div class="row g-3">
				<div class="col">
					...
					<mvc:select id="mailinglist" path="mailinglistId" size="1" cssClass="form-control">
				...
				<div class="col">
					...
					<mvc:select id="target-group" path="targetId" size="1" cssClass="form-control">
						<mvc:option value="0"><mvc:message code="statistic.all_subscribers"/></mvc:option>
						<c:forEach items="${targetGroups}" var="targetGroup">
							<c:set var="targetGroupNameLabel" value="${targetGroup.targetName} (${targetGroup.id})" />
							<mvc:option value="${targetGroup.id}">${fn:escapeXml(targetGroupNameLabel)}</mvc:option>
						</c:forEach>
					</mvc:select>
				</div>
