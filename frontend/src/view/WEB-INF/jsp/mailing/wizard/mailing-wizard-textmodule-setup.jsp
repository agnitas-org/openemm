...
<c:set var="aForm" value="${mailingWizardForm}" scope="request"/>
<c:set var="mailing" value="${mailingWizardForm.mailing}" scope="request"/>
<c:set var="len" value="0" scope="request"/>

<c:choose>
    <c:when test="${not empty mailingWizardForm.mailing}">
        <c:set var="shortname" value="${mailingWizardForm.mailing.shortname}"/>
        <c:set var="len" value="${fn:length(mailing.dynTags[mailingWizardForm.dynName].dynContent)}" scope="request"/>
    </c:when>
    <c:otherwise>
...
<c:set var="agnSubtitleValue" 		value="${shortname}" 		scope="request" />
