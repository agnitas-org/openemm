...
<emm:CheckLogon/>
<emm:Permission token="restfulUser.show"/>
...
<c:choose>
    <c:when test="${adminForm.adminID ne 0}">
        <c:set var="agnNavigationKey"	value="restfulUser" 				scope="request" />
        <c:set var="agnSubtitleKey" 	value="settings.RestfulUser" 		scope="request" />
        <c:set var="agnHighlightKey" 	value="settings.RestfulUser.edit" scope="request" />
    </c:when>
    <c:otherwise>
...
<emm:instantiate var="agnBreadcrumbs" type="java.util.LinkedHashMap" scope="request">
    <emm:instantiate var="agnBreadcrumb" type="java.util.LinkedHashMap">
        ...
        <c:set target="${agnBreadcrumb}" property="url">
            <c:url value="/restfulUser/list.action">
    ...
    <emm:instantiate var="agnBreadcrumb" type="java.util.LinkedHashMap">
        ...
        <c:choose>
            <c:when test="${adminForm.adminID eq 0}">
                <c:set target="${agnBreadcrumb}" property="textKey" value="settings.New_RestfulUser"/>
            </c:when>
            <c:otherwise>
    ...
    <c:choose>
        <c:when test="${adminForm.adminID eq 0}">
            <emm:ShowByPermission token="admin.new">
                <c:url var="createUrl" value="/restfulUser/saveNew.action"/>
        ...
        <c:otherwise>
            <emm:ShowByPermission token="admin.delete">
                <emm:instantiate var="element" type="java.util.LinkedHashMap">
                    <c:url var="deleteUrl" value="/restfulUser/${adminForm.adminID}/confirmDelete.action"/>
                    <c:set target="${itemActionsSettings}" property="0" value="${element}"/>
            ...
            <emm:ShowByPermission token="admin.change">
                <c:url var="saveUrl" value="/restfulUser/${adminForm.adminID}/save.action"/>
                <emm:instantiate var="element" type="java.util.LinkedHashMap">
                    <c:set target="${itemActionsSettings}" property="1" value="${element}"/>
                    <c:set target="${element}" property="btnCls" value="btn btn-regular btn-inverse"/>
                    <c:set target="${element}" property="extraAttributes" value="data-form-url='${saveUrl}' data-form-target='#adminForm' data-form-submit-static"/>
                    <c:set target="${element}" property="iconBefore" value="icon-save"/>
                    <c:set target="${element}" property="name">
