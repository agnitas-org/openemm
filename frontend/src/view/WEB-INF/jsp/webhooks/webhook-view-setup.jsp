...
<c:url var="switchDesignUrl" value="/webhooks/list.action" scope="request" />
...
<emm:instantiate var="agnBreadcrumbs" type="java.util.LinkedHashMap" scope="request">
    <emm:instantiate var="agnBreadcrumb" type="java.util.LinkedHashMap">
        ...
        <c:set target="${agnBreadcrumb}" property="url">
			<c:url value="/webhooks/list.action"/>
		</c:set>
    </emm:instantiate>
    <emm:instantiate var="agnBreadcrumb" type="java.util.LinkedHashMap">
        <c:set target="${agnBreadcrumbs}" property="1" value="${agnBreadcrumb}"/>
        <c:set target="${agnBreadcrumb}" property="textKey" value="webhooks.event.${WEBHOOK_EVENT_TYPE}.label" />
    </emm:instantiate>
</emm:instantiate>
...
<emm:instantiate var="itemActionsSettings" type="java.util.LinkedHashMap" scope="request">
    <emm:instantiate var="element" type="java.util.LinkedHashMap">
        ...
        <c:set target="${element}" property="btnCls" value="btn btn-regular btn-inverse"/>
   		<c:set target="${element}" property="extraAttributes" value="data-form-target='#webhookConfigForm' data-form-submit" />
