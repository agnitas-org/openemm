...
<emm:instantiate var="itemActionsSettings" type="java.util.LinkedHashMap" scope="request">
    ...
    <emm:instantiate var="element" type="java.util.LinkedHashMap">
        ...
        <c:set target="${element}" property="btnCls" value="btn btn-regular btn-inverse"/>
   		<c:set target="${element}" property="extraAttributes" value="data-form-set='method:save' data-form-target='#webhookListConfigForm' data-form-submit" />
        <c:set target="${element}" property="iconBefore" value="icon-save"/>
        <c:set target="${element}" property="name">
            <mvc:message code="button.Save"/>
        </c:set>
        <c:set target="${element}" property="url">
            <c:url value="/webhooks.action"/>
        </c:set>
    </emm:instantiate>
    --%>
</emm:instantiate>
