AGN.Lib.Controller.new('action-view-new', function() {
  var moduleList;
  var operationTypes = {};
  ...
  Module.toJson = function($module, index) {
    ...
    $module.find('[name^="modules[]"]').each(function (i, field) {
      ...
      moduleJson[fieldName] = $field.prop('type') == 'checkbox' ? $field.prop('checked') : $field.val();
    });
    return moduleJson;
  };
  ...
  this.addDomInitializer('action-view-new', function () {
    var $form = $(this.el);
  ...
  this.addAction({click: 'toggle-if-first'}, function() {
    ...
    if ($elModule.index() == 0) {
      if ($el.prop('checked')) {
        $elModule.find('[data-useTrack-trackingPointId]').show();
        $elModule.find('[data-useTrack-updateValue]').hide();
      } else {
        $elModule.find('[data-useTrack-trackingPointId]').hide();
        $elModule.find('[data-useTrack-updateValue]').show();
      }
    }
  });
});