AGN.Lib.Controller.new('ai-image-generation', function () {
  ...
  this.addDomInitializer('ai-image-generation', function () {
    ...
    $remainingGenerationsText = $('#remaining-generations-text')
  ...
  this.addAction({click: 'generate'}, function () {
    ...
    form.jqxhr().done(resp => {
      if (resp.success) {
        ...
        lastGeneratedImageData = resp.data.base64Data;
        displayImage(`data:image/png;base64,${lastGeneratedImageData}`);
  ...
  function updateRemainingGenerationsText(count) {
    if (count <= 0) {
      $remainingGenerationsText.text(t(`mediapool.imageGeneration.limitExceeded`));
      $generateBtn.parent().remove();
    } else {
      $remainingGenerationsText.text(t(`mediapool.imageGeneration.remainingGenerations`, count));
    }
  }

  this.addAction({click: 'save'}, function () {
    $.ajax({
      url: AGN.url('/mediapool/generated-image/save.action'),
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({ base64Data: lastGeneratedImageData }),
      success: resp => AGN.Lib.Page.render(resp)
    });
  });
  ...
  function setImageGenerationFiltersToForm(form) {
    ...
    _.forEach(groupedFilters, (filterValues, filterName) => {
