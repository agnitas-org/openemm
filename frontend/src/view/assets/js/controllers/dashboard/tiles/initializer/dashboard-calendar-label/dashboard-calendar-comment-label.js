  ...
  class CommentLabel extends DashboardCalendarLabel {
    ...
    constructor(data) {
      ...
      if (_.isNumber(data.plannedSendDate)) {
        data.plannedSendDate = moment(data.plannedSendDate).tz(window.agnTimeZoneId).format(`${window.adminDateFormat.toUpperCase()} ${Def.TIME_FORMAT}`);
      }
      super(data);
    }
    ...
    createPopover() {
      // created with truncated-text-popover.js
    }
    ...
    static createNewEmptyComment(date) {
      return {
        ...
        recipients: [],
        deadline: false,
        plannedSendDate: moment(date, Def.DATE_FORMAT).format(window.adminDateFormat.toUpperCase()) + ' 08:00'
      }
    }
