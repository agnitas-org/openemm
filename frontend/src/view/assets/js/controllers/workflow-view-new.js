AGN.Lib.Controller.new('workflow-view-new', function() {
    ...
    this.addAction({click: 'workflow-dry-run'}, function () {
        var newStatus = Def.constants.initialWorkflowStatus == Def.constants.statusTesting ?
          Def.constants.statusOpen : Def.constants.statusTesting;

        Dialogs.confirmTestingStartStop(newStatus)
          .done(function() {
    ...
    function submitWorkflowForm(validateNeeded) {
        ...
        if (inactivating || !Utils.checkActivation()) {
            if (activating) {
                activate(validateNeeded);
            } else if (inactivating) {
                deactivate(validateNeeded);
            } else {
    ...
    function activate(validateNeeded) {
        Dialogs.Activation(
            function() {
                saveWorkflowFormData(validateNeeded);
            },
    ...
    function deactivate(validateNeeded) {
