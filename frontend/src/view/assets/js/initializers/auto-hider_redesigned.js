...
;(function(){
  ...
  AGN.Lib.CoreInitializer.new('checkbox-hider', function($scope) {
    ...
    _.each($scope.find('[data-show-by-select]'), function(el) {
      ...
      const { 'showBySelectValues': hideForData = '' } = selector.data();
  ...
  function updateVisibleBySelect(selector, selectSelector, controlOptions, showIfChecked) {
    const selectVal = selectSelector.val();
    const valueArr = Array.isArray(selectVal) ? selectVal : [selectVal];
    const selected = valueArr.some(val => controlOptions.includes(val));

    const hidden = selected ? !showIfChecked : showIfChecked;
    selector
      .toggleClass("hidden", hidden)
      .trigger(`select:${hidden ? 'hide' : 'show'}`);
  }
})();
