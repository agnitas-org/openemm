(function () {
  ...
  AGN.Lib.DomInitializer.new('recipient-insights', function () {
    ...
    this.addAction({change: 'selectReactionProgressPeriod'}, function () {
      ...
      updateReactionInProgressChart(min, max, config);
    });

    this.addAction({change: 'period-select'}, function () {
      const periodDates = getSelectedPeriodDates();
      updateReactionInProgressChart(periodDates.min, periodDates.max, config);
    });
  });

  function updateReactionInProgressChart(min, max, config) {
    if (min && max) {
          ...
          tooltip: {
            ...
            callbacks: {
              label: function (context) {
                return t('daysWithReactions') + ': ' + context.formattedValue;
              }
            }
          }
        }
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function (context) {
                return t('reactionTimes') + ': ' + context.formattedValue;
              }
            }
          }
        }
  ...
  function showReactionsInProgressChart(data) {
    ...
    reactionInProgressChart = new Chart(ctx, options);
  }
