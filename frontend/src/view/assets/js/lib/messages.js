...
(() => {
  ...
  const JsonMessages = function (messages, removeDisplayedMessages = false) {
    ...
    messages.info?.forEach(message => Messages.infoText(message));
    messages.success?.forEach(message => Messages.successText(message));
    messages.warning?.forEach(message => Messages.warnText(message));
    messages.alert?.forEach(message => Messages.alertText(message));
    messages.fields?.forEach(fieldData => AGN.Lib.Form.showFieldError(fieldData.name, fieldData.message));
  };
