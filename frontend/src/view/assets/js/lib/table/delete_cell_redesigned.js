(function(){
  ...
  DeleteCellRenderer.prototype.init = function(params) {
    ...
    this.eGui.innerHTML = '<button type="button" class="btn btn-icon-sm btn-danger"><i class="icon icon-trash-alt"></i></button>';
      ...
      $.get(getValue(params)).done(function(resp) {
        ...
        if ($modal.length == 1) {
          Confirm.create(resp).done(function(positiveResp) {
            const $positiveResp = $(positiveResp);
            const tableApi = Table.get($(self)).api;
            tableApi.applyTransaction({ remove: [ params.data ] });
