  ...
  const MailingEditorHelper = function (data, submitWorkflowForm) {
    ...
    this.changeDecisionOptionsSet = function (scope, isChangeDecisionOptions) {
      if (isChangeDecisionOptions) {
        const selectedMailingId = this.mailingId;
        const $followUpSelect = $(scope.find(this.followUpContainer));
        const additionalOptions = this.advertisingAdditionalOptions;

        $followUpSelect.find('.advertisingOption').remove();
        if (selectedMailingId) {
          $.get(AGN.url(`/mailing/ajax/${selectedMailingId}/isAdvertisingContentType.action`), resp => {
            if (resp?.success) {
              $followUpSelect.append(AGN.Lib.Template.text(
    ...
    this.validateEditor = function (save) {
      ...
      if ($(`${this.containerId} .delivery-settings`).exists()) {
        const maxRecipients = $(`${this.containerId} [name="maxRecipients"]`).val();
      ...
      if ($(`${this.formNameJId} ${this.selectNameJId}`).val() <= 0) {
