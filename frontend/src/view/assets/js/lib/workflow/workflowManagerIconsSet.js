  ...
  var IconsSetLoader = function IconsSetLoader(campaignManager) {
    ...
    this.loadIconsSetAJAX = function(rootNode, useOriginal) {
      ...
      if (sample && !useOriginal) {
        params = {
          method: "getSampleWorkflowContent",
          type: rootNode.type
        };
      } else {
        params = {
          method: "getWorkflowContent",
          workflowId: rootNode.data.ownWorkflowId
        };
      }

      params["isWithContent"] = rootNode.data.copyContent;

      var campaignManagerNodes = campaignManager.getCMNodes();

      $.ajax({
        type: "POST",
        url: "workflow.do;jsessionid=" + campaignManager.getSessionId(),
        data: params,
        async: false
      }).done(function(data) {
        $.ajax({
