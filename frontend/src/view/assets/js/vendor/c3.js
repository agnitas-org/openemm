(function (window) {
    ...
    c3_chart_internal_fn.redraw = function (options, transitions) {
        ...
        if ((withUpdateXDomain || withUpdateXAxis) && targetsToShow.length) {
            if (config.axis_x_tick_culling && tickValues) {
                $$.svg.selectAll('.' + CLASS.axisX).each(function() {
                    var $axis = d3.select(this);
                    var axisWidth = $axis.select('path.domain').node().getTotalLength() - (config.axis_x_tick_outer ? 12 : 0);
                    var ticks = [];

                    $axis.selectAll('.tick text').each(function(idx) {
                        var index = tickValues.indexOf(idx);
                        if (index >= 0) {
                            ticks.push({e: this, index: index});
                        }
                    });
                    ...
                    if (!config.axis_x_tick_rotate) {
                        ticks.forEach(function(tick) {
                            var width2 = tick.e.getBoundingClientRect().width / 2;
                            var transform = d3.transform(d3.select(tick.e.parentNode).attr('transform'));

                            d3.select(tick.e).style('text-anchor', function() {
                                if (transform.translate[0] < width2) {
                                    return 'start';
                                }
                                if (transform.translate[0] + width2 > axisWidth) {
                                    return 'end';
                                }
                                return 'middle';
                            });
                    ...
                    if (ticks.length > config.axis_x_tick_culling_max) {
                        if (config.axis_x_tick_culling_max > 1) {
                            var ticksMax = config.axis_x_tick_culling_max;

                            var step = Math.floor(ticks.length / ticksMax);
                            while (ticks.length / step >= (ticksMax + 1)) {
                                ticksMax--;
                                step = Math.floor(ticks.length / ticksMax);
                            }

                            var offset = Math.floor((ticks.length - step * (ticksMax - 1) - 1) / 2);

                            ticks.forEach(function(tick, index) {
                                d3.select(tick.e).style('display', (index + step - offset) % step ? 'none' : 'block');
                            });
                        } else {
                            var middle = Math.round(ticks.length / 2);
                            ticks.forEach(function(tick, index) {
                                d3.select(tick.e).style('display', index == middle ? 'block' : 'none');
                            });
                        }
                    } else {
                        ticks.forEach(function(tick) {
                            d3.select(tick.e).style('display', 'block');
                        });
                    }
                });
            } else {
    ...
    c3_chart_internal_fn.getYDomain = function (targets, axisId, xDomain) {
        ...
        domain = $$.config.axis_rotated ? [yDomainMin, yDomainMax] : [yDomainMin - padding_bottom, yDomainMax + padding_top];
