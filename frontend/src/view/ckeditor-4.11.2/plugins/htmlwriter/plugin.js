...
CKEDITOR.htmlWriter = CKEDITOR.tools.createClass( {
	...
	proto: {
      ...
      this._.output.push(' ', attName, '="');

			if ( typeof attValue == 'string' ) {
        var useSimpleAmpersand = this.forceSimpleAmpersand;
        // Browsers don't always escape special character in attribute values. (#4683, #4719).
        function escape(str) {
          str = CKEDITOR.tools.htmlEncodeAttr(str);
          if (useSimpleAmpersand) {
            str = str.replace(/&amp;/g, '&');
          }
          return str;
        }

				// Heuristic optimization (do not execute regex processing unless there's a square bracket in attribute value)
				if (attValue.indexOf('[') >= 0) {
					var re = /\[agn[^\s\]\/]+(?:\s*(?:[^'"\]]|'.*?'|".*?")*)[\/]?\s*]/gi;
          var pos = 0;
          var match;

          while (match = re.exec(attValue)) {
          	if (match.index > pos) {
          		this._.output.push(escape(attValue.substring(pos, match.index)));
						}

            pos = match.index + match[0].length;
            this._.output.push(match[0]);
					}

					if (pos < attValue.length - 1) {
            this._.output.push(escape(attValue.substring(pos)));
          }
        } else {
          this._.output.push(escape(attValue));
        }
			} else {
