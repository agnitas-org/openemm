...
def configTableMenuAction(actionParameters):
	if DbConnector.checkTableExists("config_tbl"):
		...
		if Environment.getSystemUrl() == "Unknown":
			...
			try:
				...
				if ESTUtilities.isDebugMode():
					logging.exception("Cannot read birtKeys from db")
				birtKeysExist = None
			...
			print()
			if birtKeysExist is not None and not birtKeysExist and not os.path.isfile(ESTUtilities.getUserHomeDirectory(applicationUserName) + "/tomcat/conf/keys/birt_private.pem"):
				print("Generating Initial Statistics PPK keys")
				try:
					keyGenReturnCode = os.system("openssl genrsa -out \"" + ESTUtilities.getUserHomeDirectory(applicationUserName) + "/tomcat/conf/keys/birt_private.pem\" 2048; openssl rsa -in \"" + ESTUtilities.getUserHomeDirectory(applicationUserName) + "/tomcat/conf/keys/birt_private.pem\" -out \"" + ESTUtilities.getUserHomeDirectory(applicationUserName) + "/tomcat/conf/keys/birt_public.pem\" -pubout")
				except:
					keyGenReturnCode = -1
		...
		for configurationItem in currentConfigurationValues:
			...
			if wasChanged == True:
				if value == "<delete>":
					print(Colors.RED + " " + key + Colors.DEFAULT + ((Colors.GREEN + " (" + description + ")" + Colors.DEFAULT) if description is not None and len(description) > 0 else "") + " = " + Colors.RED + "<delete>" + Colors.DEFAULT)
				else:
					print(Colors.YELLOW + " " + key + Colors.DEFAULT + ((Colors.GREEN + " (" + description + ")" + Colors.DEFAULT) if description is not None and len(description) > 0 else "") + " = " + (valueColor if valueColor is not None else "") + (value if value is not None else "") + (Colors.DEFAULT if valueColor is not None else ""))
			elif (configurationItem["class"] + "." + configurationItem["name"]) in Environment.readonlyConfigProperties:
				print(Colors.RED + " " + key  + Colors.DEFAULT + ((Colors.GREEN + " (" + description + ")" + Colors.DEFAULT) if description is not None and len(description) > 0 else "") + " = " + (valueColor if valueColor is not None else "") + (value if value is not None else "") + (Colors.DEFAULT if valueColor is not None else ""))
			elif configurationItem["class"] == "birt" and configurationItem["name"].endswith("key") and value != "[to be defined]":
				print(" " + key + ((Colors.GREEN + " (" + description + ")" + Colors.DEFAULT) if description is not None and len(description) > 0 else "") + " = " + (valueColor if valueColor is not None else "") + ((value[0:5] + " ... " + value[-5:]) if value is not None else "") + (Colors.DEFAULT if valueColor is not None else ""))
			else:
				print(" " + key + ((Colors.GREEN + " (" + description + ")" + Colors.DEFAULT) if description is not None and len(description) > 0 else "") + " = " + (valueColor if valueColor is not None else "") + (value if value is not None else "") + (Colors.DEFAULT if valueColor is not None else ""))
...
def systemCfgMenuAction(actionParameters):
	...
	if os.access(Environment.systemCfgFilePath, os.W_OK):
		...
		if choice == "":
			...
			try:
				ESTUtilities.updatePropertiesFile(Environment.systemCfgFilePath, Environment.unsavedSystemCfgChanges)
			except:
				if ESTUtilities.isDebugMode():
...
def clientMenuAction(actionParameters):
	clientProperties = []
	result = DbConnector.select("SELECT company_id, shortname, description, rdir_domain, mailloop_domain, stat_admin, uid_version, auto_mailing_report_active, mails_per_day, max_recipients, salutation_extended, sector, business_field, enabled_uid_version, auto_deeptracking, default_datasource_id, priority_count, contact_tech, company_token FROM company_tbl WHERE status = 'active' ORDER BY company_id")
	if not result:
		print(Colors.RED + "Client/Account Configuration could not be queried. Perhaps the Database is in an incomplete State" + Colors.DEFAULT)
		input("> ")
		return
	...
	for row in result:
		...
		company["auto_deeptracking"] = row[14]
		company["default_datasource_id"] = row[15]
		company["priority_count"] = row[16]
		company["contact_tech"] = row[17]
		company["company_token"] = row[18]
...
def landingPageMenuAction(actionParameters):
	print("Default redirect homepage configuration")
	...
	if choice == "":
		...
		domain = choice
		print("Please enter new default redirect homepage url for domain '" + domain + "' (Blank => Cancel, 'Remove' => Remove domain):")
		choice = input(" > ")
		choice = choice.strip()
		if choice != "":
